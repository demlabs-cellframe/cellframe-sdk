...
void dap_enc_aes_key_new(struct dap_enc_key * a_key)
{
    ...
    memcpy(a_key->data,masterkey,AES_KEYSIZE);
...
void dap_enc_aes_key_new_from_seed(struct dap_enc_key * a_key, const void * seed, size_t a_in_size)
{
    ...
    a_key->last_used_timestamp = time(NULL);
    a_key->data = (unsigned char*)malloc(AES_KEYSIZE);
    memcpy(a_key->data,seed,AES_KEYSIZE);
    //a_key->data[16]='\0';
    a_key->data_size = AES_KEYSIZE;
    a_key->type=DAP_ENC_KEY_TYPE_AES;
    a_key->enc=dap_enc_aes256_cbc_encrypt;
...
void dap_enc_aes_key_generate_from_kex_and_seed(struct dap_enc_key * a_key, struct dap_enc_key * kex_key,
                                                const void * id_session, size_t id_size)
{
    a_key->last_used_timestamp = time(NULL);

    uint8_t * id_concat_kex = (uint8_t *) malloc(kex_key->data_size+id_size);
    a_key->data = (uint8_t *)malloc(AES_KEYSIZE);
    a_key->ivec = (uint8_t *)malloc(AES_BLOCKSIZE);

    memcpy(id_concat_kex,id_session, id_size);
    memcpy(id_concat_kex + id_size, kex_key->data, kex_key->data_size);
    shake256(a_key->data, AES_KEYSIZE, id_concat_kex, (kex_key->data_size + id_size));
    shake128(a_key->ivec, AES_BLOCKSIZE, id_session, id_size);

    a_key->data_size = AES_KEYSIZE;
    a_key->ivec_size = AES_BLOCKSIZE;
    a_key->type=DAP_ENC_KEY_TYPE_AES;
    a_key->enc=dap_enc_aes256_cbc_encrypt;
    a_key->dec=dap_enc_aes256_cbc_decrypt;
    a_key->delete_callback=dap_enc_aes_key_delete;
...
size_t dap_enc_aes256_cbc_decrypt(struct dap_enc_key * a_key, const void * a_in, size_t a_in_size, void ** a_out)
{
    ...
    IAES_256_CBC_decrypt(a_in, data, a_key->ivec, a_in_size, a_key->data);
...
size_t dap_enc_aes256_cbc_encrypt(struct dap_enc_key * a_key, const void * a_in, size_t a_in_size, void ** a_out)
{
    ...
    *a_out = (uint8_t *)malloc(length_data_new);
    ...
    IAES_256_CBC_encrypt(data_new, *a_out, a_key->ivec, length_data_new, a_key->data);
