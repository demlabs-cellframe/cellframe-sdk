...
typedef enum dap_enc_key_type {
    ...
    /* https://github.com/monero-project/monero/tree/master/src/crypto */
    ...
    /* http://www.ecrypt.eu.org/stream/salsa20pf.html//https://ianix.com/pub/salsa20-deployment.html */
    DAP_ENC_KEY_TYPE_SALSA2012,

    /* SEED Cipher in OFB mode */
    DAP_ENC_KEY_TYPE_SEED_OFB,

    /* "NewHope": key exchange from the ring learning with errors problem
    ...
    /* Key exchange from the supersingular isogeny Diffie-Hellman problem
    ...
    /* Microsoft Research implementation of Peikert's ring-LWE key exchange
    ...
    /* "McBits": key exchange from the error correcting codes,
    ...
    DAP_ENC_KEY_TYPE_CODE_MCBITS,

    /* NTRU: key transport using NTRU public key encryption
    ...
    DAP_ENC_KEY_TYPE_NTRU,

    /* Kyber: a CCA-secure module-lattice-based key exchange mechanism
    ...
    /* signature based on Ring_LWE problem with zero-knowledge proof as specified in
     * Post-Quantum Zero-Knowledge and Signatures from Symmetric-Key Primitives */
...
typedef struct dap_enc_key {
    ...
    void    *pub_key_data; /* can be null in symmetric algos */

    time_t  last_used_timestamp;
    dap_enc_key_type_t  type;
    dap_enc_callback_dataop_t       enc, dec;
    dap_enc_callback_dataop_na_t    enc_na, dec_na;
    dap_enc_callback_dataop_na_ext_t dec_na_ext;

    dap_enc_callback_sign_op_t      sign_get, sign_verify;
    ...
    void *_pvt;
    void *_inheritor; /* WARNING! Inheritor must have only serializable/deserializable data (copy) */
...
typedef struct dap_enc_key_serialize {
    size_t priv_key_data_size, pub_key_data_size, inheritor_size;
    time_t last_used_timestamp;
    dap_enc_key_type_t type;
    byte_t *priv_key_data, *pub_key_data, *inheritor;
...
typedef struct dap_enc_key_callbacks{
    ...
    dap_enc_callback_calc_out_size      enc_out_size, dec_out_size;
    dap_enc_gen_bob_shared_key          gen_bob_shared_key;
    dap_enc_gen_alice_shared_key        gen_alice_shared_key;
...
/* update after inserting foreign keys */
void dap_enc_key_update(dap_enc_key_t *a_key);

/* for asymmetric algos */
