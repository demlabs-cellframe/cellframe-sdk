...
size_t dap_stream_data_proc_read (dap_stream_t *a_stream)
{
    ...
    if(!a_stream->pkt_buf_in)
    {
        ...
        debug_if(s_dump_packet_headers, "dap_stream_data_proc_read() Receive previously unprocessed data %zu bytes + new %zu bytes", a_stream->pkt_buf_in_data_size, l_buf_in_size);
    ...
    if(l_buf_in_left >= sizeof(dap_stream_pkt_hdr_t)) {
        ...
        while(l_buf_in_left > 0 && (l_pkt = dap_stream_pkt_detect(l_buf_in, l_buf_in_left))) { // Packet signature detected
            ...
            if(l_buf_in_left >= l_pkt_size) {
                ...
                debug_if(s_dump_packet_headers, "Input: Not all stream packet in input (pkt_size=%zu buf_in_left=%zu)", l_pkt_size, l_buf_in_left);
                break;
            }
        }
    }
    ...
    if(l_buf_in_left > 0) {
        ...
        if(l_pkt) {
            ...
            debug_if(s_dump_packet_headers, "dap_stream_data_proc_read() left unprocessed data %zu bytes", l_buf_in_left);
        }
        else {
