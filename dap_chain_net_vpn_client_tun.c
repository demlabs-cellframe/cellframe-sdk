...
//                ch_vpn_pkt_t* pkt = ch_sf_tun_read();
//                if(pkt) {
//                    int write_ret = write(s_fd_tun, pkt->data, pkt->header.op_data.data_size);
//                    if(write_ret > 0) {
//                        log_it(L_DEBUG, "Wrote out %d bytes to the tun/tap interface", write_ret);
...
//            s_fd_tun = m_tun_server->tun_ctl_fd; // Looks yes, its so
...
//}
//
//void ch_sf_tun_delete(dap_chain_net_srv_vpn_t * ch_sf)
//{
 ...
 */
void ch_sf_tun_addr_leased(dap_chain_net_srv_vpn_t * a_sf, ch_vpn_pkt_t * a_pkt, size_t a_pkt_data_size)
{
...
//
//    //pthread_mutex_lock(&m_tun_server->clients_mutex);
//    //ch_sf_tun_client_t * n_client = DAP_NEW_Z(ch_sf_tun_client_t);
//    //n_client->ch = a_sf->ch;
//
 ...
 * @param a_pkt_data_size
 */
void ch_sf_tun_addr_request(dap_chain_net_srv_vpn_t * a_ch_sf, ch_vpn_pkt_t * a_pkt_request, size_t a_pkt_data_size)
{
...
//
//    l_pkt_out = DAP_NEW_Z_SIZE(ch_vpn_pkt_t, l_pkt_out_size);
//    l_pkt_out->header.sock_id = s_fd_tun;
...
/*
 * ch_sf_t -> dap_chain_net_srv_vpn_t
 * ch_sf_pkt_t -> dap_stream_ch_pkt_t -> ch_vpn_pkt_t
 *
 */
