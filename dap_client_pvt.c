...
static void s_stage_status_after(dap_client_pvt_t * a_client_pvt)
{
    switch(a_client_pvt->stage_status){
        case STAGE_STATUS_IN_PROGRESS:{
            switch( a_client_pvt->stage){
                case STAGE_ENC_INIT:{
                    ...
                    a_client_pvt->session_key_open = dap_enc_key_new_generate(DAP_ENC_KEY_TYPE_MSRLN,
                                                                              NULL,0,
                                                                              NULL,0,
                                                                              0);
                ...
                case STAGE_STREAM_CTL:{
                    ...
                    dap_client_pvt_request_enc(a_client_pvt,
                                                   DAP_UPLINK_PATH_STREAM_CTL,
                                                    "stream_ctl","type=tcp,maxconn=4",l_request,l_request_size,
        ...
        case STAGE_STATUS_DONE :{
            ...
            } else log_it(L_ERROR,"!! dap_CLIENT_STAGE_STATUS_DONE but not l_is_last_stage !!");
        }break;
                    ...
                    size_t a_request_size,  dap_client_callback_data_size_t a_response_proc, dap_client_callback_int_t a_response_error)
{
    ...
    if(a_path){
        ...
        snprintf(l_url,l_url_size_max,"http://%s:%u/%s",a_client_internal->uplink_addr, a_client_internal->uplink_port, a_path );
    }else{
...
{
    log_it(L_DEBUG,"Encrypted request: sub_url '%s' query '%s'",a_sub_url?a_sub_url:"NULL", a_query?a_query:"NULL" );
        ...
        if (l_arg_count <2 ){
            ...
            if(strlen(l_stream_id)<13){
                ...
                l_client_internal->stream_key =
                        dap_enc_key_new_generate(DAP_ENC_KEY_TYPE_IAES, l_stream_key, strlen(l_stream_key), NULL, 0, 0);
