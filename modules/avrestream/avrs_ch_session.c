...
// Session callbacks
static avrs_ch_pkt_session_callback_t * s_pkt_in_session_callbacks = NULL;
...
{
    s_pkt_in_session_callbacks_size++;
    if (s_pkt_in_session_callbacks)
        s_pkt_in_session_callbacks = DAP_REALLOC(s_pkt_in_session_callbacks, s_pkt_in_session_callbacks_size * sizeof(avrs_ch_pkt_session_callback_t));
    else
        s_pkt_in_session_callbacks = DAP_NEW_SIZE(avrs_ch_pkt_session_callback_t, s_pkt_in_session_callbacks_size * sizeof(avrs_ch_pkt_session_callback_t));

    s_pkt_in_session_callbacks[s_pkt_in_session_callbacks_size-1] = a_callback;
...
void avrs_ch_pkt_in_session(avrs_ch_t * a_avrs_ch,avrs_ch_pkt_session_t * a_pkt, size_t a_pkt_args_size)
{
    ...
    if(a_pkt->type == AVRS_CH_PKT_SESSION_TYPE_OPEN){
        if (a_avrs_ch->session){  // Already opened
            log_it(L_WARNING, "Session is already opened, close it first");
        ...
        if (s_parse_session_and_verify(a_avrs_ch, a_pkt, a_pkt_args_size, &l_session_id, NULL, NULL) != 0 ){
            log_it(L_WARNING, "Wrong session open request");
            return;
        }
...
static int s_parse_callback_content_add(avrs_ch_t *a_avrs_ch,dap_tsd_t* l_tsd, size_t l_tsd_offset,avrs_ch_pkt_session_t * a_pkt, size_t a_pkt_args_size, void * a_arg)
{
    ...
    switch(l_tsd->type){
        case AVRS_CH_PKT_SESSION_ARG_CLUSTER_UUID:
            l_content_req->cluster = avrs_cluster_find( dap_tsd_get_scalar(l_tsd, dap_guuid_t) );
            if( ! l_content_req->cluster){
                log_it(L_WARNING, "Can't find cluster uuid");
...
static int s_parse_callback_content_remove(avrs_ch_t *a_avrs_ch,dap_tsd_t* l_tsd, size_t l_tsd_offset,avrs_ch_pkt_session_t * a_pkt, size_t a_pkt_args_size, void * a_arg)
{
    ...
    assert(l_content_id);
    switch(l_tsd->type){
        case AVRS_CH_PKT_SESSION_ARG_CONTENT_ID: *l_content_id = dap_tsd_get_scalar(l_tsd, uint32_t); break;
        default: return 1; // Code of unknown TSD type
    }
    return 0;
}
