...
#endif
    if (ftruncate(fileno(l_cell->file_storage), l_pos) == -1)
...
void dap_chain_cell_close_all(dap_chain_t *a_chain) {
    ...
    dap_ht_foreach(a_chain->cells, l_cell, l_tmp) {
        s_cell_close(l_cell);
        dap_ht_del(a_chain->cells, l_cell);
        DAP_DELETE(l_cell);
    }
...
#endif
        if (ftruncate(fileno(a_cell->file_storage), l_pos) == -1)
            log_it(L_ERROR, "ftruncate() failed: %s", strerror(errno));
    }
...
#define m_ret_err(err, ...) return ({ if (l_cell->file_storage) fclose(l_cell->file_storage); \
    ...
    *l_cell = (dap_chain_cell_t) {
    ...
    case 'a': {
        ...
        if (ftruncate(fileno(l_cell->file_storage), 0) == -1)
            log_it(L_ERROR, "ftruncate() failed: %s", strerror(errno));
        *mode = 'w';
    }
    case 'w': {
    ...
    dap_ht_add(a_chain->cells, id, l_cell);
