...
ssize_t dap_chain_cell_file_append(dap_chain_cell_t *a_cell, const void *a_atom, size_t a_atom_size)
{
    ...
    if (!a_atom || !a_atom_size) {
        ...
        debug_if (s_debug_more && a_cell->chain->is_mapped,L_DEBUG, "After rewriting file %s, stream pos is %d and map pos is %zu",
                      a_cell->file_storage_path, ftello(a_cell->file_storage),
                      (size_t)(a_cell->map_pos - a_cell->map));
    } else {
        debug_if (s_debug_more && a_cell->chain->is_mapped,L_DEBUG, "Before appending an atom of size %zu, stream pos of %s is %d, map pos is %zu",
                      a_atom_size, a_cell->file_storage_path, ftello(a_cell->file_storage),
                      (size_t)(a_cell->map_pos - a_cell->map));
        ...
        debug_if (s_debug_more && a_cell->chain->is_mapped, L_DEBUG,"After appending an atom of size %zu, stream pos of %s is %d, map pos is %zu",
                                                a_atom_size, a_cell->file_storage_path, ftello(a_cell->file_storage),
