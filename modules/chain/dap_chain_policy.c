...
DAP_STATIC_INLINE struct net_policy_item *s_net_item_find(dap_chain_net_id_t a_net_id)
{
    ...
    dap_ht_find_hh(hh, s_net_policy_items, &a_net_id, sizeof(a_net_id), l_net_item);
    return l_net_item;
}
...
DAP_STATIC_INLINE dap_chain_policy_activate_t *s_policy_activate_find(dap_chain_net_id_t a_net_id, uint32_t a_num)
{
    ...
    dap_ht_find_by_hashvalue_hh(hh, l_net_item->activate, &a_num, sizeof(a_num), a_num, l_ret);
...
DAP_STATIC_INLINE void s_net_item_purge(struct net_policy_item *a_net_item)
{
    ...
    dap_ht_foreach_hh(hh, a_net_item->activate, l_current_activate, l_temp_activate) {
        dap_ht_del(a_net_item->activate, l_current_activate);
        DAP_DELETE(l_current_activate);
    }
    struct policy_deactivate_table
        *l_temp_deactivate = NULL,
        *l_current_deactivate = NULL;
    dap_ht_foreach_hh(hh, a_net_item->deactivate, l_current_deactivate, l_temp_deactivate) {
        dap_ht_del(a_net_item->deactivate, l_current_deactivate);
...
DAP_STATIC_INLINE struct policy_deactivate_table *s_policy_find_in_deactivate_table(struct net_policy_item *a_net_item, uint32_t a_num)
{
    ...
    dap_ht_find_by_hashvalue_hh(hh, a_net_item->deactivate, &a_num, sizeof(a_num), a_num, l_find);
    return l_find;
}
...
void dap_chain_policy_net_remove(dap_chain_net_id_t a_net_id)
{
    ...
    if (l_net_item)
        dap_ht_del(s_net_policy_items, l_net_item);
...
void dap_chain_policy_deinit()
{
    struct net_policy_item
        *l_temp = NULL,
        *l_current = NULL;
    dap_ht_foreach_hh(hh, s_net_policy_items, l_current, l_temp) {
        dap_ht_del(s_net_policy_items, l_current);
...
int dap_chain_policy_net_add(dap_chain_net_id_t a_net_id, dap_config_t *a_net_cfg)
{
    ...
    dap_ht_add_hh(hh, s_net_policy_items, net_id, l_new_item);
...
int dap_chain_policy_apply(dap_chain_policy_t *a_policy, dap_chain_net_id_t a_net_id)
{
    ...
    if (DAP_FLAG_CHECK(a_policy->flags, DAP_CHAIN_POLICY_FLAG_ACTIVATE)) {
        ...
        dap_ht_find_by_hashvalue_hh(hh, l_net_item->activate, &l_to_add->num, sizeof(l_to_add->num), l_to_add->num, l_item_to_add);
        if (l_item_to_add) {
        ...
        dap_ht_add_by_hashvalue_hh(hh, l_net_item->activate, policy->num, sizeof(l_item_to_add->policy->num), l_item_to_add->policy->num, l_item_to_add);
        ...
        if (l_item_to_del) {
            dap_ht_del(l_net_item->deactivate, l_item_to_del);
        ...
        for (uint32_t i = 0; i < l_to_deactivate->count; ++i) {
            ...
            dap_ht_find_by_hashvalue_hh(hh, l_net_item->deactivate, &l_to_deactivate->nums[i], sizeof(l_to_deactivate->nums[i]), l_to_deactivate->nums[i], l_item_to_add);
            if (l_item_to_add) {
            ...
            dap_ht_add_by_hashvalue_hh(hh, l_net_item->deactivate, num, sizeof(l_item_to_add->num), l_item_to_add->num, l_item_to_add);
            // remove from activate table
            struct policy_activate_table *l_item_to_del = NULL;
            dap_ht_find_by_hashvalue_hh(hh, l_net_item->activate, &l_to_deactivate->nums[i], sizeof(l_to_deactivate->nums[i]), l_to_deactivate->nums[i], l_item_to_del);
            if (l_item_to_del) {
                dap_ht_del(l_net_item->activate, l_item_to_del);
                DAP_DELETE(l_item_to_del);
            }
        }
    }
...
bool dap_chain_policy_is_activated(dap_chain_net_id_t a_net_id, uint32_t a_policy_num)
{
    ...
    dap_ht_find_by_hashvalue_hh(hh, l_net_item->activate, &a_policy_num, sizeof(a_policy_num), a_policy_num, l_policy_activate);
    ...
    if (!l_ret) {
        ...
        *l_temp = NULL,
        *l_current = NULL;
        dap_ht_foreach_hh(hh, l_net_item->activate, l_current, l_temp) {
...
dap_json_t *dap_chain_policy_list(dap_chain_net_id_t a_net_id, int a_version)
{
    ...
    struct policy_deactivate_table
        *l_temp_deactivate = NULL,
        *l_current_deactivate = NULL;
    dap_ht_foreach_hh(hh, l_net_item->deactivate, l_current_deactivate, l_temp_deactivate) {
...
dap_json_t *dap_chain_policy_activate_json_collect(dap_chain_net_id_t a_net_id, uint32_t a_num)
{
    ...
    if (l_policy_activate->ts_start) {
        ...
        dap_json_object_add_object(l_ret, "ts_start", dap_json_object_new_string(l_time));
    } else {
        dap_json_object_add_object(l_ret, "ts_start", dap_json_object_new_int(0));
    }
    dap_json_object_add_object(l_ret, "block_start", dap_json_object_new_uint64(l_policy_activate->block_start));
    if (l_policy_activate->block_start) {
        dap_chain_t *l_chain = dap_chain_find_by_id(a_net_id, l_policy_activate->chain_id);
        if (!l_chain) {
            dap_json_object_add_object(l_ret, "chain", dap_json_object_new_string("NULL"));
        } else {
            char l_chain_id[32] = { };
            snprintf(l_chain_id, sizeof(l_chain_id) - 1, "0x%016"DAP_UINT64_FORMAT_x, l_policy_activate->chain_id.uint64);
            dap_json_object_add_object(l_ret, "chain", dap_json_object_new_string(l_chain_id));
        }
    } else {
        dap_json_object_add_object(l_ret, "chain", dap_json_object_new_string(""));
    }
    dap_json_object_add_object(l_ret, "description", dap_json_object_new_string("WIKI"));
    return l_ret;
}
...
dap_json_t *dap_chain_policy_json_collect(dap_chain_policy_t *a_policy)
{
    ...
    dap_json_object_add_object(l_ret, "version", dap_json_object_new_uint64(a_policy->version));
    dap_json_object_add_object(l_ret, "type", dap_json_object_new_string(dap_chain_policy_to_str(a_policy)));
    if (DAP_FLAG_CHECK(a_policy->flags, DAP_CHAIN_POLICY_FLAG_ACTIVATE)) {
        dap_chain_policy_activate_t *l_policy_activate = (dap_chain_policy_activate_t *)a_policy->data;
        dap_json_object_add_object(l_ret, "num", dap_json_object_new_uint64(l_policy_activate->num));
        if (l_policy_activate->ts_start) {
            char l_time[DAP_TIME_STR_SIZE] = {};
            dap_time_to_str_rfc822(l_time, DAP_TIME_STR_SIZE - 1, l_policy_activate->ts_start);
            dap_json_object_add_object(l_ret, "ts_start", dap_json_object_new_string(l_time));
        ...
        if (l_policy_deactivate->count) {
            ...
            dap_json_object_add_object(l_ret, "deactivate", dap_json_object_new_string(l_nums_list->str));
            dap_string_free(l_nums_list, true);
        } else {
            dap_json_object_add_object(l_ret, "deactivate", dap_json_object_new_string(""));
        }
    }
    return l_ret;
}