...
void dap_chain_srv_deinit()
{
    ...
    assert(!err);
    dap_ht_foreach(s_srv_list, it, tmp) {
...
int dap_chain_srv_add(dap_chain_srv_uid_t a_uid, const char *a_name, dap_chain_static_srv_callbacks_t *a_static_callbacks)
{
    ...
    int err = pthread_rwlock_wrlock(&s_srv_list_lock);
    assert(!err);
    dap_ht_find(s_srv_list, &a_uid, sizeof(a_uid), l_service_item);
    if (l_service_item) {
    ...
    dap_ht_add(s_srv_list, uuid, l_service_item);
    pthread_rwlock_unlock(&s_srv_list_lock);
    return 0;
}
...
static struct service_list *s_service_find(dap_chain_srv_uid_t a_srv_uid)
{
    ...
    assert(!err);
    dap_ht_find(s_srv_list, &a_srv_uid, sizeof(dap_chain_srv_uid_t), l_service_item);
    pthread_rwlock_unlock(&s_srv_list_lock);
...
int dap_chain_srv_delete(dap_chain_srv_uid_t a_srv_uid)
{
    ...
    int err = pthread_rwlock_wrlock(&s_srv_list_lock);
    assert(!err);
    dap_ht_find(s_srv_list, &a_srv_uid, sizeof(a_srv_uid), l_service_item);
    if (!l_service_item) {
        pthread_rwlock_unlock(&s_srv_list_lock);
        return -1;
    }
