...
int dap_stream_ch_chain_init()
{
    ...
    s_skip_in_reactor_count = s_update_pack_size / 10;
...
{
    UNUSED(a_worker);
    dap_stream_ch_chain_t *l_stm_ch_chain = (dap_stream_ch_chain_t *)a_arg;

    debug_if(s_debug_more, L_DEBUG, "[stm_ch_chain:%p] --- delete chain:%p", l_stm_ch_chain, l_stm_ch_chain->_inheritor);

    s_ch_chain_go_idle(l_stm_ch_chain);
    s_free_log_list_gdb(l_stm_ch_chain);

    if (!l_stm_ch_chain->_inheritor) {
        if (l_stm_ch_chain->callback_notify_packet_out)
            l_stm_ch_chain->callback_notify_packet_out(l_stm_ch_chain, DAP_STREAM_CH_CHAIN_PKT_TYPE_DELETE, NULL, 0,
...
{
    if (!a_sync_request || !*a_sync_request)
        return;
...
static void s_sync_out_chains_first_worker_callback(dap_worker_t *a_worker, void *a_arg)
{
    ...
    dap_chain_node_addr_t l_node_addr = { .uint64 = dap_chain_net_get_cur_addr_int(l_net) };

    debug_if (s_debug_more, L_INFO,"[stm_ch_chain:%p] Out: DAP_STREAM_CH_CHAIN_PKT_TYPE_FIRST_CHAIN", l_ch_chain);
    ...
    DAP_DEL_Z(l_sync_request); /* preserve iterators */
...
static void s_sync_out_chains_last_worker_callback(dap_worker_t *a_worker, void *a_arg)
{
    ...
    DAP_DEL_Z(l_sync_request); /* iterators are already disposed */
}
/**
...
static bool s_sync_out_chains_proc_callback(dap_proc_thread_t *a_thread, void *a_arg)
{
    ...
    l_sync_request->chain.request_atom_iter = l_chain->callback_atom_iter_create(l_chain, l_sync_request->request_hdr.cell_id, true);
    debug_if(g_debug_reactor, L_INFO, "Arg %p -> worker %d", l_sync_request, l_sync_request->worker->id);
    size_t l_first_size = 0;
    dap_chain_atom_ptr_t l_atom_first = l_chain->callback_atom_iter_get_first(l_sync_request->chain.request_atom_iter, &l_first_size);
    if (l_atom_first && l_first_size) {
        // first packet
        ...
        //l_sync_request->chain.request_atom_iter = NULL; /* Wtf is this?... */
...
static void s_sync_out_gdb_first_worker_callback(dap_worker_t *a_worker, void *a_arg)
{
    ...
    dap_chain_node_addr_t l_node_addr = { .uint64 = dap_chain_net_get_cur_addr_int(l_net) };
    debug_if (s_debug_more,L_INFO,"[stm_ch_chain:%p] Out: DAP_STREAM_CH_CHAIN_PKT_TYPE_FIRST_GLOBAL_DB", l_ch_chain);
    ...
    if(l_ch_chain->callback_notify_packet_out)
        l_ch_chain->callback_notify_packet_out(l_ch_chain, DAP_STREAM_CH_CHAIN_PKT_TYPE_FIRST_GLOBAL_DB,
                                                NULL, 0, l_ch_chain->callback_notify_arg);
    if (a_worker)
        DAP_DEL_Z(l_sync_request); /* preserve log_list */
}
...
static bool s_sync_out_gdb_proc_callback(dap_proc_thread_t *a_thread, void *a_arg)
{
    ...
    l_sync_request->gdb.db_log = dap_db_log_list_start(l_net, l_sync_request->request.node_addr, l_flags);

    debug_if(g_debug_reactor, L_INFO, "Arg %p -> worker %d", l_sync_request, l_sync_request->worker->id);

    if (l_sync_request->gdb.db_log)
    {
        debug_if (s_debug_more, L_DEBUG, "[stm_ch_chain:%p] Sync out gdb proc, requested %"DAP_UINT64_FORMAT_U" transactions from address "NODE_ADDR_FP_STR,
            l_ch_chain, l_sync_request->gdb.db_log->items_number, NODE_ADDR_FP_ARGS_S(l_sync_request->request.node_addr));
...
static void s_sync_update_gdb_start_worker_callback(dap_worker_t *a_worker, void *a_arg)
{
    ...
    debug_if (s_debug_more, L_INFO, "[stm_ch:%p] Out: DAP_STREAM_CH_CHAIN_PKT_TYPE_UPDATE_GLOBAL_DB_START for net_id 0x%016"DAP_UINT64_FORMAT_x" "
                       "chain_id 0x%016"DAP_UINT64_FORMAT_x" cell_id 0x%016"DAP_UINT64_FORMAT_x"",
            l_ch, l_sync_request->request_hdr.net_id.uint64, l_sync_request->request_hdr.chain_id.uint64, l_sync_request->request_hdr.cell_id.uint64);
...
static bool s_sync_in_chains_callback(dap_proc_thread_t *a_thread, void *a_arg)
{
    ...
    case ATOM_ACCEPT:
        ...
        int l_res = dap_chain_atom_save(l_chain, l_atom, l_atom_size, l_sync_request->request_hdr.cell_id);
        if(l_res < 0) {
...
void s_stream_ch_packet_in(dap_stream_ch_t* a_ch, void* a_arg)
{
    ...
    int     l_acl_idx = dap_chain_net_get_acl_idx(l_net),
...
static bool s_ch_chain_get_idle(dap_stream_ch_chain_t *a_ch_chain)
{
    return a_ch_chain->state == CHAIN_STATE_IDLE;
}
    ...
    switch (l_ch_chain->state) {
        ...
        case CHAIN_STATE_UPDATE_GLOBAL_DB: {
            ...
            if (!l_obj) {
                ...
                debug_if(s_debug_more, L_INFO, "Out: DAP_STREAM_CH_CHAIN_PKT_TYPE_UPDATE_GLOBAL_DB_END %lu elements", i);
                l_go_idle = true;
            }


            /* For reading objs one by one */
            /*uint_fast16_t i;
        ...
        case CHAIN_STATE_SYNC_GLOBAL_DB: {
            ...
            /* TODO: A less heavy but not completely correct solution */
            size_t l_pkt_size = 0, i = 0, j = s_skip_in_reactor_count;
            dap_db_log_list_obj_t *l_obj = NULL;
            while (l_pkt_size < DAP_CHAIN_PKT_EXPECT_SIZE) {
                l_obj = dap_db_log_list_get_multiple(l_ch_chain->request_db_log, &j);
                if (j && l_obj && DAP_POINTER_TO_SIZE(l_obj) != 1) {
                    for (i = 0; i < j; ++i) {
                        dap_stream_ch_chain_hash_item_t *l_hash_item = NULL;
                        unsigned l_hash_item_hashv = 0;
                        HASH_VALUE(&(l_obj + i)->hash, sizeof(dap_chain_hash_fast_t), l_hash_item_hashv);
                        HASH_FIND_BYHASHVALUE(hh, l_ch_chain->remote_gdbs, &(l_obj + i)->hash, sizeof(dap_hash_fast_t),
                                              l_hash_item_hashv, l_hash_item);
                        if (l_hash_item) { // If found - skip it
                            //if (s_debug_more) {
                            //    char l_request_atom_hash_str[DAP_CHAIN_HASH_FAST_STR_SIZE];
                            //    dap_chain_hash_fast_to_str(&(l_obj + i)->hash, l_request_atom_hash_str, DAP_CHAIN_HASH_FAST_STR_SIZE);
                            //    log_it(L_DEBUG, "Out CHAIN: skip GDB hash %s because its already present in remote GDB hash table",
                            //                    l_request_atom_hash_str);
                            //}
                        } else {
                            l_hash_item = DAP_NEW_Z(dap_stream_ch_chain_hash_item_t);
                            l_hash_item->hash = (l_obj + i)->hash;
                            l_hash_item->size = (l_obj + i)->pkt->data_size;
                            HASH_ADD_BYHASHVALUE(hh, l_ch_chain->remote_gdbs, hash, sizeof(dap_chain_hash_fast_t),
                                                 l_hash_item_hashv, l_hash_item);
                            l_pkt = dap_store_packet_multiple(l_pkt, (l_obj + i)->pkt);
            ...
            if (l_pkt_size) {
                ...
                if (s_debug_more)
                    ...
                    debug_if(s_debug_more, L_DEBUG, "Send GDB packet, size = %zu (rest=%zu/%zu items)", l_pkt_size,
                             l_ch_chain->request_db_log->items_rest, l_ch_chain->request_db_log->items_number);
                ...
                l_was_sent_smth = true;
                log_it( L_INFO,"Syncronized database: items %"DAP_UINT64_FORMAT_U" of %zu",
