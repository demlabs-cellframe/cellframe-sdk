...
#ifdef DAP_CHAIN_TX_COMPOSE_TEST
    if (rand() % 2)
        return dap_chain_datum_tx_add_new_generic( a_tx, dap_chain_tx_out_std_t,  dap_chain_datum_tx_item_out_std_create(a_addr, a_value, a_token, dap_min(rand() % UINT64_MAX, dap_time_now()) ) );
    else
