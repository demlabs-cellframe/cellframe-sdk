...
{
    size_t size = 0;
    if ( !a_tx || !*a_tx || !(dap_chain_datum_item_tx_get_type_n_size(a_item, &size), size) )
        return -1;
...
#define dap_chain_datum_tx_add_new_generic(a_tx, type, a_item) ({               \
    type item = (type)(a_item);                                                 \
    item ? dap_chain_datum_tx_add_item(a_tx, item), DAP_DELETE(item), 1 : -1;   \
})
...
int dap_chain_datum_tx_verify_sign(dap_chain_datum_tx_t *a_tx)
{
    ...
    TX_ITEM_ITER_TX(l_item, l_size, a_tx) {
        if (l_item->type == TX_ITEM_TYPE_SIG) {
