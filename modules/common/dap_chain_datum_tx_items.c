 ...
 */
dap_chain_tx_token_t *dap_chain_datum_tx_item_token_create(dap_chain_id_t a_id, dap_chain_hash_fast_t *a_datum_token_hash, const char *a_ticker/*, dap_chain_tx_token_type_t token_type*/)
{
    if(!a_ticker)
        return NULL;
    dap_chain_tx_token_t *l_item = DAP_NEW_Z(dap_chain_tx_token_t);
    l_item->header.type = TX_ITEM_TYPE_TOKEN;
/*	l_item->header.token_type = token_type;*/
