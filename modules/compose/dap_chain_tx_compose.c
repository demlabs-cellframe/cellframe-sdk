                                                    ...
                                                    const char *a_tag_str)
{
    ...
    for (size_t i = 0; i < l_hashes_count_max; i++) {
        ...
        dap_strncpy(l_hash_str_buf, l_token_ptr, dap_min((size_t)DAP_HASH_FAST_STR_SIZE, l_hash_len + 1));
                                                 ...
                                                 const char *a_position_id_str, const char *a_fee_str)
{
    ...
    if (!l_config) {
        log_it(L_ERROR, "Can't create compose config");
        dap_json_t *l_json_obj_ret = dap_json_object_new();
        dap_json_compose_error_add(l_json_obj_ret, STAKE_EXT_LOCK_COMPOSE_ERR_CONFIG, "Can't create compose config");
        return l_json_obj_ret;
    }
    ...
    if (dap_chain_hash_fast_from_str(a_stake_ext_hash_str, &l_stake_ext_hash)) {
        ...
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_INVALID_HASH, "Invalid stake_ext hash format");
        return dap_chain_tx_compose_config_return_response_handler(l_config);
    }
    ...
    if (IS_ZERO_256(l_amount)) {
        ...
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_INVALID_AMOUNT, "Invalid amount format");
        return dap_chain_tx_compose_config_return_response_handler(l_config);
    }
    ...
    if (l_lock_months < 3 || l_lock_months > 24) {
        ...
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_INVALID_LOCK_PERIOD, 
    ...
    if (l_position_id == 0) {
        ...
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_INVALID_POSITION_ID, "Invalid position ID");
        return dap_chain_tx_compose_config_return_response_handler(l_config);
    }
    ...
    if (IS_ZERO_256(l_fee)) {
        ...
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_INVALID_FEE, "Invalid fee format");
        return dap_chain_tx_compose_config_return_response_handler(l_config);
    }

    dap_json_t *l_stake_ext_info = dap_request_command_to_rpc_with_params(l_config, "stake_ext", 
                                                                         "info;-net;%s;-stake_ext;%s",
                                                                         l_config->net_name, a_stake_ext_hash_str);
    if (!l_stake_ext_info) {
        log_it(L_ERROR, "Failed to get stake_ext info");
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_RPC_REQUEST, "Failed to get stake_ext info");
        return dap_chain_tx_compose_config_return_response_handler(l_config);
    }

    dap_json_t *l_first_item = dap_json_array_get_idx(l_stake_ext_info, 0);
    if (!l_first_item) {
        log_it(L_ERROR, "stake_ext not found");
        dap_json_object_free(l_stake_ext_info);
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_STAKE_EXT_NOT_FOUND, "stake_ext not found");
        return dap_chain_tx_compose_config_return_response_handler(l_config);
    }

    const char *l_status_str = dap_json_object_get_string(l_first_item, "stake_ext_status");
    if (!l_status_str || dap_strcmp(l_status_str, "active") != 0) {
        ...
        dap_json_object_free(l_stake_ext_info);
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_STAKE_EXT_NOT_ACTIVE, 
                                "stake_ext is not active");
        return dap_chain_tx_compose_config_return_response_handler(l_config);
    }

    dap_json_t *l_positions_json = dap_json_object_get_array(l_first_item, "positions");
    bool l_position_found = false;
    if (l_positions_json && dap_json_is_array(l_positions_json)) {
        size_t l_positions_count = dap_json_array_length(l_positions_json);
        for (size_t i = 0; i < l_positions_count; i++) {
            dap_json_t *l_pos = dap_json_array_get_idx(l_positions_json, i);
            uint64_t l_pos_id = dap_json_object_get_uint64(l_pos, "position_id");
            if (l_pos_id == l_position_id) {
    ...
    if (!l_position_found) {
        ...
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_POSITION_NOT_FOUND, 
    ...
    if (MULT_256_COIN(l_amount, l_emission_rate, &l_delegated_value) || IS_ZERO_256(l_delegated_value)) {
        ...
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_TX_CREATE, 
    ...
    if (!l_json_coins) {
        ...
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_RPC_REQUEST, "Can't get coins from ledger");
        return dap_chain_tx_compose_config_return_response_handler(l_config);
    }
    ...
    uint256_t l_balance = dap_chain_tx_compose_get_balance_from_json(l_json_outs, a_native_ticker);
    dap_json_object_free(l_json_outs);

    if (compare256(l_balance, l_amount) == -1) {
        log_it(L_ERROR, "Insufficient funds in wallet");
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_NOT_ENOUGH_FUNDS, 
                                                               ...
                                                               dap_chain_id_t a_chain_id, dap_chain_tx_compose_config_t *a_config)
{
    ...
    if (!l_outs_native) {
        ...
        dap_json_compose_error_add(a_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_NOT_ENOUGH_FUNDS, 
                                "Can't get wallet outputs");
        return NULL;
    }
    size_t l_out_native_count = dap_json_array_length(l_outs_native);
#else
    dap_json_t *l_outs_native = NULL;
    size_t l_out_native_count = 0;
#endif

    dap_list_t *l_list_used_out = dap_ledger_get_list_tx_outs_from_json(l_outs_native, l_out_native_count,
                                                                      l_value_need, &l_value_transfer, false);
    if (!l_list_used_out) {
    ...
    if (!l_tx) {
        log_it(L_ERROR, "Failed to create transaction");
        dap_json_compose_error_add(a_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_TX_CREATE, 
    ...
    if (!l_out_cond) {
        ...
        dap_json_compose_error_add(a_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_ADD_COND_OUT, 
    ...
    if (dap_chain_datum_tx_add_out_ext_item(&l_tx, a_wallet_addr, a_delegated_value, a_delegated_ticker_str) != 1) {
        ...
        dap_json_compose_error_add(a_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_ADD_MTOKEN_OUT, 
    ...
    if (l_net_fee_used) {
        if (dap_chain_datum_tx_add_out_ext_item(&l_tx, l_addr_fee, l_net_fee, l_native_ticker) != 1) {
            ...
            dap_json_compose_error_add(a_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_ADD_NET_FEE, 
    ...
    if (!IS_ZERO_256(a_value_fee)) {
        if (dap_chain_datum_tx_add_fee_item(&l_tx, a_value_fee) != 1) {
            ...
            dap_json_compose_error_add(a_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_ADD_VALIDATOR_FEE, 
    ...
    if (!IS_ZERO_256(l_value_back)) {
        if (dap_chain_datum_tx_add_out_ext_item(&l_tx, a_wallet_addr, l_value_back, l_native_ticker) != 1) {
            ...
            dap_json_compose_error_add(a_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_ADD_COIN_BACK, 
                                                   ...
                                                   const char *a_fee_str)
{
    ...
    if (!l_config) {
        log_it(L_ERROR, "Can't create compose config");
        dap_json_t *l_json_obj_ret = dap_json_object_new();
        dap_json_compose_error_add(l_json_obj_ret, STAKE_EXT_UNLOCK_COMPOSE_ERR_CONFIG, "Can't create compose config");
        return l_json_obj_ret;
    }
    ...
    if (dap_chain_hash_fast_from_str(a_lock_tx_hash_str, &l_lock_tx_hash)) {
        ...
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_INVALID_HASH, 
    ...
    if (IS_ZERO_256(l_fee)) {
        log_it(L_ERROR, "Invalid fee format '%s'", a_fee_str);
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_INVALID_FEE, "Invalid fee format");
        return dap_chain_tx_compose_config_return_response_handler(l_config);
    }

    dap_chain_datum_tx_t *l_lock_tx = dap_chain_tx_compose_get_datum_from_rpc(a_lock_tx_hash_str, l_config, 
    ...
    if (l_spent_by_hash) {
        ...
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_TX_ALREADY_SPENT, 
    ...
    if (!l_stake_ext_info) {
        log_it(L_ERROR, "Failed to get stake_ext info");
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_STAKE_EXT_NOT_FOUND, 
    ...
    dap_json_t *l_first_item = dap_json_array_get_idx(l_stake_ext_info, 0);
    const char *l_status_str = l_first_item ? dap_json_object_get_string(l_first_item, "stake_ext_status") : NULL;
    ...
    if (!l_unlock_allowed) {
        ...
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_UNLOCK_NOT_ALLOWED, 
    ...
    if (IS_ZERO_256(l_emission_rate) ||
        ...
        log_it(L_ERROR, "Failed to calculate delegated value");
        dap_json_compose_error_add(l_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_TX_CREATE, 
                                "Failed to calculate delegated value");
        DAP_DEL_MULTY(l_ticker_str, l_cond_tx);
        if (l_lock_tx) dap_chain_datum_tx_delete(l_lock_tx);
        return dap_chain_tx_compose_config_return_response_handler(l_config);
                                                                 ...
                                                                 dap_chain_tx_compose_config_t *a_config)
{
    ...
    if (!l_outs_native) {
        log_it(L_ERROR, "Can't get remote tx outs");
        DAP_DEL_Z(l_addr_fee);
        dap_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_NOT_ENOUGH_FUNDS,
                                "Can't get wallet outputs");
        return NULL;
    }
    ...
    if (!l_outs_delegated) {
        ...
        dap_json_object_free(l_outs_native);
        DAP_DEL_Z(l_addr_fee);
        dap_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_NOT_ENOUGH_DELEGATED,
                                "Can't get delegated token outputs");
        return NULL;
    }

    size_t l_out_native_count = dap_json_array_length(l_outs_native);
    size_t l_out_delegated_count = dap_json_array_length(l_outs_delegated);
#else
    dap_json_t *l_outs_native = NULL;
    dap_json_t *l_outs_delegated = NULL;
    size_t l_out_native_count = 0;
    size_t l_out_delegated_count = 0;
#endif
    ...
    if (!IS_ZERO_256(l_total_fee)) {
        if (!l_main_native) {
            l_list_fee_out = dap_ledger_get_list_tx_outs_from_json(l_outs_native, l_out_native_count,
                                                                 l_total_fee, &l_fee_transfer, false);
            if (!l_list_fee_out) {
                log_it(L_ERROR, "Not enough funds to pay fee");
                dap_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_NOT_ENOUGH_FUNDS,
    ...
    if (!IS_ZERO_256(a_delegated_value)) {
        l_list_used_out = dap_ledger_get_list_tx_outs_from_json(l_outs_delegated, l_out_delegated_count,
                                                              a_delegated_value, &l_value_transfer, false);
        if (!l_list_used_out) {
    ...
    if (!l_tx) {
        log_it(L_ERROR, "Failed to create transaction");
        dap_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_TX_CREATE,
                                "Failed to create transaction");
        dap_list_free_full(l_list_used_out, NULL);
        dap_list_free_full(l_list_fee_out, NULL);
        DAP_DEL_Z(l_addr_fee);
    ...
    if (l_net_fee_used) {
        if (dap_chain_datum_tx_add_out_ext_item(&l_tx, l_addr_fee, l_net_fee, l_native_ticker) != 1) {
            log_it(L_ERROR, "Failed to add network fee output");
            dap_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_NET_FEE,
                                    "Failed to add network fee output");
            dap_chain_datum_tx_delete(l_tx);
            DAP_DEL_Z(l_addr_fee);
            return NULL;
    ...
    if (!IS_ZERO_256(a_value_fee)) {
        if (dap_chain_datum_tx_add_fee_item(&l_tx, a_value_fee) != 1) {
            log_it(L_ERROR, "Failed to add validator fee");
            dap_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_VALIDATOR_FEE,
                                    "Failed to add validator fee");
            dap_chain_datum_tx_delete(l_tx);
            return NULL;
        }
    ...
    if (!IS_ZERO_256(l_value_back)) {
        if (dap_chain_datum_tx_add_out_ext_item(&l_tx, a_wallet_addr, l_value_back, a_main_ticker) != 1) {
            log_it(L_ERROR, "Failed to add coin back output");
            dap_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_COIN_BACK,
                                    "Failed to add coin back output");
            dap_chain_datum_tx_delete(l_tx);
            return NULL;
        }
    ...
    if (!IS_ZERO_256(a_delegated_value)) {
        if (dap_chain_datum_tx_add_out_ext_item(&l_tx, &c_dap_chain_addr_blank,
            ...
            dap_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_BURN,
