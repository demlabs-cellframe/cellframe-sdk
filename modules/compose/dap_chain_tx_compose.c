...
static json_object* s_request_command_to_rpc_with_params(compose_config_t *a_config, const char *a_method, const char *msg, ...) {
    ...
    if (!l_msg) {
        ...
        s_json_compose_error_add(a_config->response_handler, DAP_COMPOSE_ERROR_REQUEST_FAILED, "failed to create message");
        return NULL;
    }

    if (dap_strlen(a_method) * 2 + dap_strlen(l_msg) + 50 >= 512) {
        log_it(L_ERROR, "message is too long");
        s_json_compose_error_add(a_config->response_handler, DAP_COMPOSE_ERROR_REQUEST_FAILED, "message is too long");
        DAP_DELETE(l_msg);
        return NULL;
    }
    char data[512] = {0};
    ...
    if (l_ret < 0 || l_ret >= (int)sizeof(data)) {
        ...
        s_json_compose_error_add(a_config->response_handler, DAP_COMPOSE_ERROR_REQUEST_FAILED, "failed to create data");
        return NULL;
    }
...
static bool s_get_remote_net_fee_and_address(uint256_t *a_net_fee, dap_chain_addr_t **a_addr_fee, compose_config_t *a_config) {
    ...
    *a_addr_fee = NULL;
    ...
    if (!l_json_get_fee) {
        ...
        s_json_compose_error_add(a_config->response_handler, -5, "failed to get fee");
        return false;
    }
    ...
    if (!l_first_result || !json_object_is_type(l_first_result, json_type_object)) {
        ...
        s_json_compose_error_add(a_config->response_handler, -6, "failed to get first result");
    ...
    if (!json_object_object_get_ex(l_first_result, "fees", &l_fees) || 
        ...
        s_json_compose_error_add(a_config->response_handler, -7, "failed to get fees");
        json_object_put(l_json_get_fee);
        return false;
    }
    ...
    if (!json_object_object_get_ex(l_network, "balance", &l_balance) || 
        ...
        s_json_compose_error_add(a_config->response_handler, -8, "failed to get balance");
        json_object_put(l_json_get_fee);
        return false;
    }
    ...
    if (!l_balance_str) {
        ...
        s_json_compose_error_add(a_config->response_handler, -9, "failed to get balance string");
        json_object_put(l_json_get_fee);
        return false;
    }
    ...
    *a_net_fee = dap_chain_balance_scan(l_balance_str);
    ...
    if (!json_object_object_get_ex(l_network, "addr", &l_addr) || 
        ...
        s_json_compose_error_add(a_config->response_handler, -11, "failed to get addr");
        json_object_put(l_json_get_fee);
        return false;
    }
    ...
    if (!l_addr_str) {
        ...
        s_json_compose_error_add(a_config->response_handler, -12, "failed to get addr string");
        json_object_put(l_json_get_fee);
        return false;
    }
    ...
    *a_addr_fee = dap_chain_addr_from_str(l_addr_str);
    ...
    if (!*a_addr_fee) {
        log_it(L_ERROR, "failed to get addr");
        s_json_compose_error_add(a_config->response_handler, -13, "failed to get addr");
        return false;
    }
#endif
    return true;
...
static bool s_get_remote_wallet_outs_and_count(dap_chain_addr_t *a_addr_from, const char *a_token_ticker,
    ...
    if (!l_json_outs) {
        ...
        s_json_compose_error_add(a_config->response_handler, -14, "failed to get outs");
        return false;
    }

    if (!json_object_is_type(l_json_outs, json_type_array)) {
        log_it(L_ERROR, "outs is not an array");
        s_json_compose_error_add(a_config->response_handler, -15, "outs is not an array");
    ...
    if (json_object_array_length(l_json_outs) == 0) {
        ...
        s_json_compose_error_add(a_config->response_handler, -16, "outs is empty");
        json_object_put(l_json_outs);
        return false;
    }
    ...
    if (!l_first_array || !json_object_is_type(l_first_array, json_type_array)) {
        ...
        s_json_compose_error_add(a_config->response_handler, -17, "first array is not an array");
        json_object_put(l_json_outs);
        return false;
    }
    ...
    if (!l_first_item) {
        ...
        s_json_compose_error_add(a_config->response_handler, -18, "first item is NULL");
        json_object_put(l_json_outs);
        return false;
    }
    ...
    if (!json_object_object_get_ex(l_first_item, "outs", l_outs) ||
        ...
        log_it(L_ERROR, "outs is not an array");
        s_json_compose_error_add(a_config->response_handler, -19, "outs is not an array");
        json_object_put(l_json_outs);
        return false;
    }
...
json_object *dap_chain_tx_compose_tx_create(dap_chain_net_id_t a_net_id, const char *a_net_name, const char *a_native_ticker, const char *a_url_str,
    ...
    if (l_time_el_count && (l_time_el_count != l_value_el_count || l_time_el_count != l_addr_el_count)) {
        ...
        s_json_compose_error_add(l_config->response_handler, TX_CREATE_COMPOSE_INVALID_PARAMS, "num of '-to_addr', '-value' and  '-lock_before' should be equal");
        return s_compose_config_return_response_handler(l_config);
    }
    ...
    if (l_time_unlock_str) {
        ...
        if (!l_time_unlock_array) {
            ...
            s_json_compose_error_add(l_config->response_handler, TX_CREATE_COMPOSE_ADDR_ERROR, "Can't read '-lock_before' arg");
            return s_compose_config_return_response_handler(l_config);
        }
        for (size_t i = 0; i < l_value_el_count; ++i) {
            ...
            if (!l_time_unlock[i]) {
                ...
                s_json_compose_error_add(l_config->response_handler, TX_CREATE_COMPOSE_ADDR_ERROR, "Wrong time format. Parameter -lock_before must be in format \"Day Month Year HH:MM:SS Timezone\" e.g. \"19 August 2024 22:00:00 +0300\"");
        ...
        const char *a_token_ticker, uint256_t *a_value, dap_time_t *a_time_unlock, uint256_t a_value_fee, size_t a_tx_num, compose_config_t *a_config)
{
    ...
    if (!s_get_remote_wallet_outs_and_count(a_addr_from, a_token_ticker, &l_outs, &l_outputs_count, a_config)) {
        ...
        s_json_compose_error_add(a_config->response_handler, -21, "failed to get wallet outs and count");
    ...
    if (l_single_channel) {
        ...
        if (!s_get_remote_wallet_outs_and_count(a_addr_from, l_native_ticker, &l_native_outs, &l_native_outputs_count, a_config)) {
            ...
            s_json_compose_error_add(a_config->response_handler, -22, "failed to get native outs and count");
...
json_object *dap_chain_tx_compose_stake_lock_hold(dap_chain_net_id_t a_net_id, const char *a_net_name, const char *a_native_ticker, const char *a_url_str,
    ...
    if (!l_json_coins) {
        ...
        s_json_compose_error_add(l_config->response_handler, CLI_HOLD_COMPOSE_ERROR_INVALID_CONFIG, "Can't get coins");
        return s_compose_config_return_response_handler(l_config);
    }
...
typedef enum {
    VOTE_COMPOSE_OK = 0,
    VOTE_COMPOSE_INVALID_CONFIG = -1,
    VOTE_COMPOSE_INVALID_HASH = -2,
    VOTE_COMPOSE_CERT_NOT_FOUND = -3,
    VOTE_COMPOSE_INVALID_FEE = -4,
    VOTE_COMPOSE_WALLET_NOT_FOUND = -5
...
json_object *dap_chain_tx_compose_poll_vote(dap_chain_net_id_t a_net_id, const char *a_net_name, const char *a_native_ticker, const char *a_url_str,
    ...
    if (!l_config) {
        log_it(L_ERROR, "can't create compose config");
        json_object* l_json_obj_ret = json_object_new_object();
        s_json_compose_error_add(l_json_obj_ret, VOTE_COMPOSE_INVALID_CONFIG, "Can't create compose config");
        return l_json_obj_ret;
    }
    ...
    if (dap_chain_hash_fast_from_str(a_hash_str, &l_voting_hash)) {
        ...
        s_json_compose_error_add(l_config->response_handler, VOTE_COMPOSE_INVALID_HASH, "Hash string is not recognozed as hex of base58 hash\n");
        return s_compose_config_return_response_handler(l_config);
    }
    ...
    if (a_cert_name && !l_cert) {
        ...
        s_json_compose_error_add(l_config->response_handler, VOTE_COMPOSE_CERT_NOT_FOUND, "Can't find \"%s\" certificate\n", a_cert_name);
        return s_compose_config_return_response_handler(l_config);
    }
    uint256_t l_value_fee = dap_chain_balance_scan(a_fee_str);
    if (IS_ZERO_256(l_value_fee)) {
        log_it(L_ERROR, "invalid fee");
        s_json_compose_error_add(l_config->response_handler, VOTE_COMPOSE_INVALID_FEE, "command requires parameter '-fee' to be valid uint256\n");            
        return s_compose_config_return_response_handler(l_config);
    }
...
typedef enum {
    STAKE_INVALIDATE_OK = 0,
    STAKE_INVALIDATE_CERT_NOT_FOUND = -1,
    STAKE_INVALIDATE_PRIVATE_KEY_MISSING = -2,
    STAKE_INVALIDATE_WRONG_CERT = -3,
    STAKE_INVALIDATE_LEDGER_ERROR = -4,
    STAKE_INVALIDATE_INVALID_TX_HASH = -5,
    STAKE_INVALIDATE_NOT_DELEGATED = -6,
    STAKE_INVALIDATE_NO_DELEGATE_OUT = -7,
    STAKE_INVALIDATE_PREV_TX_NOT_FOUND = -8,
    STAKE_INVALIDATE_KEY_DELEGATED = -9,
    STAKE_INVALIDATE_WALLET_NOT_FOUND = -10,
    STAKE_INVALIDATE_COMPOSE_ERROR = -11,
    ...
    STAKE_INVALIDATE_FEE_ERROR = -29,
    STAKE_INVALIDATE_COMPOSE_ERR_RPC_RESPONSE = -30,
                                                  ...
                                                  const char *a_cert_str, const char *a_fee_str)
{
    ...
    if (IS_ZERO_256(l_fee)) {
        ...
        s_json_compose_error_add(l_config->response_handler, STAKE_INVALIDATE_FEE_ERROR, "Unrecognized number in '-fee' param");
        return s_compose_config_return_response_handler(l_config);
    }
    ...
    if (a_tx_hash_str) {
        ...
        if (a_cert_str) {
            ...
            if (!l_cert) {
                ...
                s_json_compose_error_add(l_config->response_handler, STAKE_INVALIDATE_CERT_NOT_FOUND, "Specified certificate not found");
                return s_compose_config_return_response_handler(l_config);
            }
            if (!l_cert->enc_key->priv_key_data || l_cert->enc_key->priv_key_data_size == 0) {
                log_it(L_ERROR, "private key missing in certificate");
                s_json_compose_error_add(l_config->response_handler, STAKE_INVALIDATE_PRIVATE_KEY_MISSING, "Private key missing in certificate");
                return s_compose_config_return_response_handler(l_config);
            }
            if (dap_chain_addr_fill_from_key(&l_signing_addr, l_cert->enc_key, l_config->net_id)) {
                log_it(L_ERROR, "wrong certificate");
                s_json_compose_error_add(l_config->response_handler, STAKE_INVALIDATE_WRONG_CERT, "Wrong certificate");
                return s_compose_config_return_response_handler(l_config);
            }
        }
        ...
        if (!l_json_coins) {
            ...
            s_json_compose_error_add(l_config->response_handler, STAKE_INVALIDATE_COMPOSE_ERR_RPC_RESPONSE, "Failed to get coins list");
            return s_compose_config_return_response_handler(l_config);
        }
        ...
        for (int i = 0; i < items_count; i++) {
            ...
            if (node_addr_str && !dap_strcmp(l_addr_str, node_addr_str)) {
                ...
                if (dap_chain_hash_fast_from_str(tx_hash_str, &l_tx_hash)) {
                    log_it(L_ERROR, "invalid transaction hash format");
                    json_object_put(l_json_coins);
                    s_json_compose_error_add(l_config->response_handler, STAKE_INVALIDATE_INVALID_TX_HASH, "Invalid transaction hash format");
                    return s_compose_config_return_response_handler(l_config);
                }
                found = true;
                break;
        ...
        if (!found) {
            ...
            s_json_compose_error_add(l_config->response_handler, STAKE_INVALIDATE_NOT_DELEGATED, "Specified certificate/pkey hash is not delegated");
            return s_compose_config_return_response_handler(l_config);
        }
    }
    ...
    if (a_tx_hash_str) {
        ...
        if (!l_json_answer) {
            ...
            s_json_compose_error_add(l_config->response_handler, STAKE_INVALIDATE_COMPOSE_ERR_RPC_RESPONSE, "Failed to get RPC answer");
            log_it(L_ERROR, "failed to get rpc answer");
            return s_compose_config_return_response_handler(l_config);
        }
        ...
        if (!l_json_coins) {
            ...
            s_json_compose_error_add(l_config->response_handler, STAKE_INVALIDATE_COMPOSE_ERR_RPC_RESPONSE, "Failed to get keys list");
            return s_compose_config_return_response_handler(l_config);
        }
        ...
        for (int i = 0; i < tx_count; i++) {
            ...
            if (tx_hash && strcmp(tx_hash, l_tx_hash_str_tmp) == 0) {
                ...
                s_json_compose_error_add(l_config->response_handler, STAKE_INVALIDATE_KEY_DELEGATED, "Transaction %s has active delegated key %s, need to revoke it first", tx_hash, l_pkey_hash_str);
                return s_compose_config_return_response_handler(l_config);
            }
        }
        json_object_put(l_json_answer);
...
static dap_chain_net_srv_order_t *s_get_remote_srv_order(const char* l_order_hash_str, compose_config_t *a_config){
    ...
    if(!dap_json_rpc_get_uint64(l_response, "data_size", &l_order_data_size) || !l_order_data_size) {
                                                    ...
                                                    const char *a_fee_str, const char *a_tx_in_hash_str)
{
    ...
    uint256_t l_fee = dap_chain_balance_scan(a_fee_str);
    if (IS_ZERO_256(l_fee)) {
        s_json_compose_error_add(l_config->response_handler, SHARED_FUNDS_REFILL_COMPOSE_ERR_VALUE, "Format -fee <256 bit integer> and not equal zero");
...
dap_chain_datum_tx_t *dap_chain_tx_compose_datum_wallet_shared_refill(dap_chain_addr_t *a_owner_addr, uint256_t a_value, uint256_t a_fee, dap_hash_fast_t *a_tx_in_hash, dap_list_t* a_tsd_items, compose_config_t *a_config)
{
    ...
    if (!l_json_ledger_info) {
        log_it(L_ERROR, "Can't get ledger info");
        s_json_compose_error_add(a_config->response_handler, SHARED_FUNDS_REFILL_COMPOSE_ERR_NETWORK, "Can't get ledger info");
        return NULL;
    }
    ...
    if (!l_tx_ticker) {
        ...
        s_json_compose_error_add(a_config->response_handler, SHARED_FUNDS_REFILL_COMPOSE_ERR_TX_MISMATCH, "Can't get token ticker");
        return NULL;
    }
#else
    ...
    if (!l_outs_native) {
        ...
        s_json_compose_error_add(a_config->response_handler, SHARED_FUNDS_REFILL_COMPOSE_ERR_FUNDS, "Can't get native outs");
        DAP_DELETE(l_tx_ticker);
        return NULL;
    }
    ...
    if (!l_json_shared_info) {
        ...
        s_json_compose_error_add(a_config->response_handler, SHARED_FUNDS_REFILL_COMPOSE_ERR_TX_MISMATCH, "Can't get shared info");
        DAP_DELETE(l_tx_ticker);
        return NULL;
    }
    ...
    if (!l_final_tx_hash_str) {
        ...
        s_json_compose_error_add(a_config->response_handler, SHARED_FUNDS_REFILL_COMPOSE_ERR_TX_MISMATCH, "Can't get final tx hash");
        DAP_DELETE(l_tx_ticker);
        return NULL;
    }
    ...
    if (dap_chain_hash_fast_from_str(l_final_tx_hash_str, &l_final_tx_hash)) {
        ...
        s_json_compose_error_add(a_config->response_handler, SHARED_FUNDS_REFILL_COMPOSE_ERR_TX_MISMATCH, "Can't recognize %s as a hex or base58 format hash", l_final_tx_hash_str);
...
typedef enum
{
    TX_COND_REFILL_COMPOSE_ERR_OK = 0,
    TX_COND_REFILL_COMPOSE_ERR_CONFIG,
    TX_COND_REFILL_COMPOSE_ERR_VALUE,
    TX_COND_REFILL_COMPOSE_ERR_NETWORK,
    TX_COND_REFILL_COMPOSE_ERR_OVERFLOW,
                                                 ...
                                                 const char *a_fee_str, const char *a_tx_cond_hash_str)
{
    ...
    {
        json_object *l_json_obj_ret = json_object_new_object();
        s_json_compose_error_add(l_json_obj_ret, TX_COND_REFILL_COMPOSE_ERR_CONFIG, "Can't create compose config");
        log_it(L_ERROR, "Can't create compose config");
        return l_json_obj_ret;
    }
    ...
    if (IS_ZERO_256(l_value))
    {
        s_json_compose_error_add(l_config->response_handler, TX_COND_REFILL_COMPOSE_ERR_VALUE, "Invalid value");
        log_it(L_ERROR, "Invalid value");
        return s_compose_config_return_response_handler(l_config);
    }
    ...
    if (IS_ZERO_256(l_fee))
    {
        s_json_compose_error_add(l_config->response_handler, TX_COND_REFILL_COMPOSE_ERR_VALUE, "Invalid fee");
        log_it(L_ERROR, "Invalid fee");
        return s_compose_config_return_response_handler(l_config);
    }

    dap_hash_fast_t l_tx_cond_hash = {};
    if (dap_chain_hash_fast_from_str(a_tx_cond_hash_str, &l_tx_cond_hash))
    {
        s_json_compose_error_add(l_config->response_handler, TX_COND_REFILL_COMPOSE_ERR_TX_MISMATCH, "Invalid TX hash");
        log_it(L_ERROR, "Invalid TX hash");
        return s_compose_config_return_response_handler(l_config);
    }

    dap_chain_datum_tx_t *l_tx = dap_chain_tx_compose_datum_tx_cond_refill(a_owner_addr, l_value, l_fee, &l_tx_cond_hash, l_config);
    if (l_tx)
    {
        dap_chain_net_tx_to_json(l_tx, l_config->response_handler);
        dap_chain_datum_tx_delete(l_tx);
                                                                ...
                                                                compose_config_t *a_config)
{
    ...
    const char *l_native_ticker = a_config->native_ticker;
    bool l_refill_native = !dap_strcmp(l_native_ticker, l_tx_ticker);
    uint256_t l_value = a_value;
    uint256_t l_value_transfer = {};
    uint256_t l_fee_transfer = {};
    uint256_t l_net_fee = {};
    uint256_t l_fee_total = a_fee;
    ...
    bool l_net_fee_used = s_get_remote_net_fee_and_address(&l_net_fee, &l_net_fee_addr, a_config);
    if ((l_net_fee_used && SUM_256_256(l_net_fee, a_fee, &l_fee_total)) ||
        (l_refill_native && SUM_256_256(l_value, l_fee_total, &l_value)))
    {
#ifndef DAP_CHAIN_TX_COMPOSE_TEST
    ...
    json_object *l_outs_native = dap_get_remote_tx_outs(l_native_ticker, a_owner_addr, a_config);
    if (!l_outs_native)
    {
        log_it(L_ERROR, "Can't get native outs");
    ...
    if (!l_list_used_out)
    {
        s_json_compose_error_add(a_config->response_handler, TX_COND_REFILL_COMPOSE_ERR_FUNDS, "Not enough funds to transfer");
#ifndef DAP_CHAIN_TX_COMPOSE_TEST
        json_object_put(l_outs_native);
        if (!l_refill_native)
            json_object_put(l_outs_main);
    ...
    if (!EQUAL_256(l_value_to_items, l_value_transfer))
    {
        s_json_compose_error_add(a_config->response_handler, TX_COND_REFILL_COMPOSE_ERR_COMPOSE, "Can't compose transaction input");
        json_object_put(l_outs_native);
        if (!l_refill_native)
            json_object_put(l_outs_main);
    ...
    // Add fee inputs if not native token
    if (!l_refill_native)
    {
        dap_list_t *l_list_fee_out = s_ledger_get_list_tx_outs_from_json(l_outs_native, l_out_native_count, l_fee_total, &l_fee_transfer);
        if (!l_list_fee_out)
        {
            s_json_compose_error_add(a_config->response_handler, TX_COND_REFILL_COMPOSE_ERR_FUNDS, "Not enough funds to pay fee");
            json_object_put(l_outs_native);
            json_object_put(l_outs_main);
            log_it(L_ERROR, "Not enough funds to pay fee");
            dap_chain_datum_tx_delete(l_tx);
        ...
        if (!EQUAL_256(l_value_fee_items, l_fee_transfer))
        {
            s_json_compose_error_add(a_config->response_handler, TX_COND_REFILL_COMPOSE_ERR_COMPOSE, "Can't compose fee transaction input");
            json_object_put(l_outs_native);
            json_object_put(l_outs_main);
    ...
    if (!IS_ZERO_256(l_value_back))
    {
        const char *l_back_ticker = l_refill_native ? l_native_ticker : l_tx_ticker;
        if (dap_chain_datum_tx_add_out_ext_item(&l_tx, a_owner_addr, l_value_back, l_back_ticker) != 1)
        {
    ...
    // Add fee coin back if not native token
    if (!l_refill_native)
    {
        uint256_t l_fee_back = {};
        SUBTRACT_256_256(l_fee_transfer, l_fee_total, &l_fee_back);
        if (!IS_ZERO_256(l_fee_back))
        {
            if (dap_chain_datum_tx_add_out_ext_item(&l_tx, a_owner_addr, l_fee_back, l_native_ticker) != 1)
            {
...
typedef enum
{
    TX_COND_REMOVE_COMPOSE_ERR_OK = 0,
    TX_COND_REMOVE_COMPOSE_ERR_CONFIG,
    TX_COND_REMOVE_COMPOSE_ERR_VALUE,
    TX_COND_REMOVE_COMPOSE_ERR_NETWORK,
    TX_COND_REMOVE_COMPOSE_ERR_OVERFLOW,
    TX_COND_REMOVE_COMPOSE_ERR_FUNDS,
    TX_COND_REMOVE_COMPOSE_ERR_COMPOSE,
...
static dap_list_t *s_parse_hashes_str(const char *a_hashes_str)
{
    if (!a_hashes_str)
        return NULL;
    dap_list_t *l_ret = NULL;
    char *l_dup = dap_strdup(a_hashes_str);
    char *l_ptr = NULL;
    char *l_hash_str = strtok_r(l_dup, ",", &l_ptr);
    while (l_hash_str)
    {
        dap_hash_fast_t *l_hash = DAP_NEW_Z(dap_hash_fast_t);
        if (l_hash && !dap_chain_hash_fast_from_str(l_hash_str, l_hash))
            l_ret = dap_list_append(l_ret, l_hash);
        else
            DAP_DEL_Z(l_hash);
        l_hash_str = strtok_r(NULL, ",", &l_ptr);
    }
    DAP_DELETE(l_dup);
    return l_ret;
}

json_object *dap_chain_tx_compose_tx_cond_remove(dap_chain_net_id_t a_net_id, const char *a_net_name,
                                                 const char *a_native_ticker, const char *a_url_str,
                                                 uint16_t a_port, const char *a_enc_cert_path,
                                                 ...
                                                 const char *a_fee_str, const char *a_srv_uid_str)
{
    ...
    {
        json_object *l_json_obj_ret = json_object_new_object();
        s_json_compose_error_add(l_json_obj_ret, TX_COND_REMOVE_COMPOSE_ERR_CONFIG, "Can't create compose config");
        log_it(L_ERROR, "Can't create compose config");
        return l_json_obj_ret;
    }
    ...
    if (IS_ZERO_256(l_fee))
    {
        s_json_compose_error_add(l_config->response_handler, TX_COND_REMOVE_COMPOSE_ERR_VALUE, "Invalid fee");
        log_it(L_ERROR, "Invalid fee");
        return s_compose_config_return_response_handler(l_config);
    }
    ...
    if (!l_srv_uid.uint64)
    {
        s_json_compose_error_add(l_config->response_handler, TX_COND_REMOVE_COMPOSE_ERR_SRV_UID, "Invalid srv_uid");
        log_it(L_ERROR, "Invalid srv_uid");
        return s_compose_config_return_response_handler(l_config);
    }

    dap_list_t *l_hashes = s_parse_hashes_str(a_hashes_str);
    if (!l_hashes)
    {
        s_json_compose_error_add(l_config->response_handler, TX_COND_REMOVE_COMPOSE_ERR_NO_HASHES, "No valid hashes provided");
        log_it(L_ERROR, "No valid hashes provided");
        return s_compose_config_return_response_handler(l_config);
    }
...
dap_chain_datum_tx_t *dap_chain_tx_compose_datum_tx_cond_remove(dap_chain_addr_t *a_owner_addr, dap_list_t *a_tx_hashes,
                                                                uint256_t a_fee, dap_chain_net_srv_uid_t a_srv_uid,
                                                                compose_config_t *a_config)
{
    if (!a_config || !a_tx_hashes || IS_ZERO_256(a_fee) || !a_srv_uid.uint64)
        return NULL;
    ...
    if (!l_tx)
    {
        s_json_compose_error_add(a_config->response_handler, TX_COND_REMOVE_COMPOSE_ERR_MEMORY, "Can't create TX");
        return NULL;
    }
    ...
    if (l_added_count == 0 || IS_ZERO_256(l_cond_value_sum))
    {
        s_json_compose_error_add(a_config->response_handler, TX_COND_REMOVE_COMPOSE_ERR_TX_MISMATCH,
                                 "No valid conditional transactions found");
        log_it(L_ERROR, "No valid conditional transactions found");
        dap_chain_datum_tx_delete(l_tx);
        return NULL;
    }
    ...
    {
        s_json_compose_error_add(a_config->response_handler, TX_COND_REMOVE_COMPOSE_ERR_FUNDS,
                                 "Sum of conditional outputs must be greater than fees sum");
        log_it(L_ERROR, "Sum of conditional outputs must be greater than fees sum");
        dap_chain_datum_tx_delete(l_tx);
        return NULL;
    }
    ...
    // Add output to owner
    if (dap_chain_datum_tx_add_out_ext_item(&l_tx, a_owner_addr, l_coin_back, l_native_ticker) != 1)
    {
        s_json_compose_error_add(a_config->response_handler, TX_COND_REMOVE_COMPOSE_ERR_COMPOSE,
                                 "Can't add return output");
        log_it(L_ERROR, "Can't add return output");
        dap_chain_datum_tx_delete(l_tx);
        return NULL;
    }
    ...
    {
        s_json_compose_error_add(a_config->response_handler, TX_COND_REMOVE_COMPOSE_ERR_COMPOSE,
                                 "Can't add validator fee output");
        log_it(L_ERROR, "Can't add validator fee output");
        dap_chain_datum_tx_delete(l_tx);
        return NULL;
    }
...
// ============== STAKE EXT LOCK/UNLOCK ==============

typedef enum {
    STAKE_EXT_LOCK_COMPOSE_OK = 0,
    STAKE_EXT_LOCK_COMPOSE_ERR_CONFIG = -1,
    STAKE_EXT_LOCK_COMPOSE_ERR_INVALID_HASH = -2,
    STAKE_EXT_LOCK_COMPOSE_ERR_INVALID_AMOUNT = -3,
    STAKE_EXT_LOCK_COMPOSE_ERR_INVALID_LOCK_PERIOD = -4,
    STAKE_EXT_LOCK_COMPOSE_ERR_INVALID_POSITION_ID = -5,
    STAKE_EXT_LOCK_COMPOSE_ERR_INVALID_FEE = -6,
    STAKE_EXT_LOCK_COMPOSE_ERR_STAKE_EXT_NOT_FOUND = -7,
    STAKE_EXT_LOCK_COMPOSE_ERR_STAKE_EXT_NOT_ACTIVE = -8,
    STAKE_EXT_LOCK_COMPOSE_ERR_POSITION_NOT_FOUND = -9,
    STAKE_EXT_LOCK_COMPOSE_ERR_NOT_ENOUGH_FUNDS = -10,
    STAKE_EXT_LOCK_COMPOSE_ERR_TX_CREATE = -11,
    STAKE_EXT_LOCK_COMPOSE_ERR_ADD_IN = -12,
    STAKE_EXT_LOCK_COMPOSE_ERR_ADD_COND_OUT = -13,
    STAKE_EXT_LOCK_COMPOSE_ERR_ADD_MTOKEN_OUT = -14,
    STAKE_EXT_LOCK_COMPOSE_ERR_ADD_NET_FEE = -15,
    STAKE_EXT_LOCK_COMPOSE_ERR_ADD_VALIDATOR_FEE = -16,
    STAKE_EXT_LOCK_COMPOSE_ERR_ADD_COIN_BACK = -17,
    STAKE_EXT_LOCK_COMPOSE_ERR_RPC_REQUEST = -18
} dap_stake_ext_lock_compose_error_t;

typedef enum {
    STAKE_EXT_UNLOCK_COMPOSE_OK = 0,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_CONFIG = -1,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_INVALID_HASH = -2,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_INVALID_FEE = -3,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_TX_NOT_FOUND = -4,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_COND_NOT_FOUND = -5,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_TX_ALREADY_SPENT = -6,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_STAKE_EXT_NOT_FOUND = -7,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_UNLOCK_NOT_ALLOWED = -8,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_TOKEN_NOT_FOUND = -9,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_DELEGATED_TOKEN_NOT_FOUND = -10,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_NOT_ENOUGH_DELEGATED = -11,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_NOT_ENOUGH_FUNDS = -12,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_TX_CREATE = -13,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_IN_COND = -14,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_IN = -15,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_NET_FEE = -16,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_VALIDATOR_FEE = -17,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_COIN_BACK = -18,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_BURN = -19,
    STAKE_EXT_UNLOCK_COMPOSE_ERR_RPC_REQUEST = -20
} dap_stake_ext_unlock_compose_error_t;
                                                 ...
                                                 const char *a_position_id_str, const char *a_fee_str)
{
    ...
    if (!l_config) {
        log_it(L_ERROR, "Can't create compose config");
        json_object *l_json_obj_ret = json_object_new_object();
        s_json_compose_error_add(l_json_obj_ret, STAKE_EXT_LOCK_COMPOSE_ERR_CONFIG, "Can't create compose config");
        return l_json_obj_ret;
    }
    ...
    uint256_t l_amount = {};
    uint256_t l_fee = {};
    uint32_t l_position_id = 0;
    uint8_t l_lock_months = 0;
    char l_delegated_ticker_str[DAP_CHAIN_TICKER_SIZE_MAX] = {};

    if (dap_chain_hash_fast_from_str(a_stake_ext_hash_str, &l_stake_ext_hash)) {
        log_it(L_ERROR, "Invalid stake_ext hash format");
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_INVALID_HASH, "Invalid stake_ext hash format");
        return s_compose_config_return_response_handler(l_config);
    }
    ...
    l_lock_months = (uint8_t)atoi(a_lock_period_str);
    if (l_lock_months < 3 || l_lock_months > 24) {
        log_it(L_ERROR, "Lock period must be between 3 and 24 months");
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_INVALID_LOCK_PERIOD, 
                                "Lock period must be between 3 and 24 months");
        return s_compose_config_return_response_handler(l_config);
    }

    l_position_id = (uint32_t)atoi(a_position_id_str);
    if (l_position_id == 0) {
        log_it(L_ERROR, "Invalid position ID");
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_INVALID_POSITION_ID, "Invalid position ID");
        return s_compose_config_return_response_handler(l_config);
    }

    l_fee = dap_chain_balance_scan(a_fee_str);
    if (IS_ZERO_256(l_fee)) {
        log_it(L_ERROR, "Invalid fee format '%s'", a_fee_str);
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_INVALID_FEE, "Invalid fee format");
        return s_compose_config_return_response_handler(l_config);
    }

    json_object *l_stake_ext_info = s_request_command_to_rpc_with_params(l_config, "stake_ext", 
                                                                         "info;-net;%s;-stake_ext;%s",
                                                                         l_config->net_name, a_stake_ext_hash_str);
    if (!l_stake_ext_info) {
        log_it(L_ERROR, "Failed to get stake_ext info");
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_RPC_REQUEST, "Failed to get stake_ext info");
        return s_compose_config_return_response_handler(l_config);
    }

    json_object *l_first_item = json_object_array_get_idx(l_stake_ext_info, 0);
    if (!l_first_item) {
        log_it(L_ERROR, "stake_ext not found");
        json_object_put(l_stake_ext_info);
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_STAKE_EXT_NOT_FOUND, "stake_ext not found");
        return s_compose_config_return_response_handler(l_config);
    }

    json_object *l_status_json = json_object_object_get(l_first_item, "stake_ext_status");
    const char *l_status_str = l_status_json ? json_object_get_string(l_status_json) : NULL;
    if (!l_status_str || dap_strcmp(l_status_str, "active") != 0) {
        log_it(L_ERROR, "stake_ext is not active (status: %s)", l_status_str ? l_status_str : "unknown");
        json_object_put(l_stake_ext_info);
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_STAKE_EXT_NOT_ACTIVE, 
                                "stake_ext is not active");
        return s_compose_config_return_response_handler(l_config);
    }

    json_object *l_positions_json = json_object_object_get(l_first_item, "positions");
    bool l_position_found = false;
    if (l_positions_json && json_object_is_type(l_positions_json, json_type_array)) {
        int l_positions_count = json_object_array_length(l_positions_json);
        for (int i = 0; i < l_positions_count; i++) {
            json_object *l_pos = json_object_array_get_idx(l_positions_json, i);
            json_object *l_pos_id_json = json_object_object_get(l_pos, "position_id");
            if (l_pos_id_json && json_object_get_uint64(l_pos_id_json) == l_position_id) {
    ...
    if (!l_position_found) {
        log_it(L_ERROR, "Position ID %u not found in stake_ext", l_position_id);
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_POSITION_NOT_FOUND, 
                                "Position ID %u not found in stake_ext", l_position_id);
        return s_compose_config_return_response_handler(l_config);
    }

    dap_chain_datum_token_get_delegated_ticker(l_delegated_ticker_str, a_native_ticker);
    uint256_t l_emission_rate = dap_chain_coins_to_balance("0.001");
    uint256_t l_delegated_value = {};

    if (MULT_256_COIN(l_amount, l_emission_rate, &l_delegated_value) || IS_ZERO_256(l_delegated_value)) {
        log_it(L_ERROR, "Failed to calculate delegated value");
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_TX_CREATE, 
                                "Failed to calculate delegated value");
        return s_compose_config_return_response_handler(l_config);
    }

    dap_time_t l_lock_time = (dap_time_t)l_lock_months * 30 * DAP_SEC_PER_DAY;

    json_object *l_json_coins = s_request_command_to_rpc_with_params(l_config, "ledger", "list;coins;-net;%s", l_config->net_name);
    if (!l_json_coins) {
        log_it(L_ERROR, "Can't get coins from ledger");
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_RPC_REQUEST, "Can't get coins from ledger");
        return s_compose_config_return_response_handler(l_config);
    }

    if (!check_token_in_ledger(l_json_coins, l_delegated_ticker_str)) {
        log_it(L_ERROR, "Delegated token '%s' not found in ledger", l_delegated_ticker_str);
        json_object_put(l_json_coins);
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_TX_CREATE, 
                                "Delegated token not found");
        return s_compose_config_return_response_handler(l_config);
    }
    json_object_put(l_json_coins);
    ...
    json_object *l_json_outs = s_request_command_to_rpc_with_params(l_config, "wallet", "info;-addr;%s;-net;%s",
                                                                   dap_chain_addr_to_str(a_wallet_addr), l_config->net_name);
    uint256_t l_balance = s_get_balance_from_json(l_json_outs, a_native_ticker);
    json_object_put(l_json_outs);

    if (compare256(l_balance, l_amount) == -1) {
        log_it(L_ERROR, "Insufficient funds in wallet");
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_NOT_ENOUGH_FUNDS, 
                                "Insufficient funds in wallet");
        return s_compose_config_return_response_handler(l_config);
    }

    dap_chain_id_t l_chain_id = {};
    dap_chain_datum_tx_t *l_tx = dap_chain_tx_compose_datum_stake_ext_lock(a_wallet_addr, &l_stake_ext_hash,
                                                                          l_amount, l_fee, l_lock_time, l_position_id,
                                                                          l_delegated_ticker_str, l_delegated_value,
                                                                          l_chain_id, l_config);
...
dap_chain_datum_tx_t *dap_chain_tx_compose_datum_stake_ext_lock(dap_chain_addr_t *a_wallet_addr,
                                                               ...
                                                               dap_chain_id_t a_chain_id, compose_config_t *a_config)
{
    dap_return_val_if_pass(!a_config || !a_wallet_addr || !a_stake_ext_hash || IS_ZERO_256(a_value), NULL);
    ...
    if (!l_outs_native) {
        log_it(L_ERROR, "Can't get remote tx outs");
        DAP_DEL_Z(l_addr_fee);
        s_json_compose_error_add(a_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_NOT_ENOUGH_FUNDS, 
                                "Can't get wallet outputs");
        return NULL;
    }
    int l_out_native_count = json_object_array_length(l_outs_native);
#else
    ...
    dap_list_t *l_list_used_out = s_ledger_get_list_tx_outs_from_json(l_outs_native, l_out_native_count,
                                                                      l_value_need, &l_value_transfer);
    if (!l_list_used_out) {
        log_it(L_ERROR, "Not enough funds to create lock");
        s_json_compose_error_add(a_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_NOT_ENOUGH_FUNDS, 
                                "Not enough funds to create lock");
#ifndef DAP_CHAIN_TX_COMPOSE_TEST
        json_object_put(l_outs_native);
#endif
        DAP_DEL_Z(l_addr_fee);
    ...
    if (!l_tx) {
        log_it(L_ERROR, "Failed to create transaction");
        s_json_compose_error_add(a_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_TX_CREATE, 
    ...
    dap_chain_tx_out_cond_t *l_out_cond = dap_chain_datum_tx_item_out_cond_create_srv_stake_ext_lock(
        l_srv_uid, a_value, a_stake_ext_hash, a_lock_time, a_position_id, NULL, 0);
    if (!l_out_cond) {
        log_it(L_ERROR, "Failed to create conditional output");
        s_json_compose_error_add(a_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_ADD_COND_OUT, 
                                "Failed to create conditional output");
        dap_chain_datum_tx_delete(l_tx);
        DAP_DEL_Z(l_addr_fee);
        return NULL;
    }
    ...
    if (dap_chain_datum_tx_add_out_ext_item(&l_tx, a_wallet_addr, a_delegated_value, a_delegated_ticker_str) != 1) {
        ...
        s_json_compose_error_add(a_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_ADD_MTOKEN_OUT, 
                                "Failed to add m-tokens output");
        dap_chain_datum_tx_delete(l_tx);
        DAP_DEL_Z(l_addr_fee);
        return NULL;
    }
    ...
    if (l_net_fee_used) {
        if (dap_chain_datum_tx_add_out_ext_item(&l_tx, l_addr_fee, l_net_fee, l_native_ticker) != 1) {
            log_it(L_ERROR, "Failed to add network fee output");
            s_json_compose_error_add(a_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_ADD_NET_FEE, 
    ...
    if (!IS_ZERO_256(a_value_fee)) {
        if (dap_chain_datum_tx_add_fee_item(&l_tx, a_value_fee) != 1) {
            ...
            s_json_compose_error_add(a_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_ADD_VALIDATOR_FEE, 
    ...
    if (!IS_ZERO_256(l_value_back)) {
        if (dap_chain_datum_tx_add_out_ext_item(&l_tx, a_wallet_addr, l_value_back, l_native_ticker) != 1) {
            log_it(L_ERROR, "Failed to add coin back output");
            s_json_compose_error_add(a_config->response_handler, STAKE_EXT_LOCK_COMPOSE_ERR_ADD_COIN_BACK, 
                                    "Failed to add coin back output");
            dap_chain_datum_tx_delete(l_tx);
            return NULL;
        }
    }
                                                   ...
                                                   const char *a_fee_str)
{
    ...
    if (!l_config) {
        log_it(L_ERROR, "Can't create compose config");
        json_object *l_json_obj_ret = json_object_new_object();
        s_json_compose_error_add(l_json_obj_ret, STAKE_EXT_UNLOCK_COMPOSE_ERR_CONFIG, "Can't create compose config");
        return l_json_obj_ret;
    }
    ...
    if (dap_chain_hash_fast_from_str(a_lock_tx_hash_str, &l_lock_tx_hash)) {
        log_it(L_ERROR, "Invalid lock transaction hash format");
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_INVALID_HASH, 
                                "Invalid lock transaction hash format");
        return s_compose_config_return_response_handler(l_config);
    }
    ...
    if (IS_ZERO_256(l_fee)) {
        log_it(L_ERROR, "Invalid fee format '%s'", a_fee_str);
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_INVALID_FEE, "Invalid fee format");
        return s_compose_config_return_response_handler(l_config);
    }

    dap_chain_datum_tx_t *l_lock_tx = s_get_datum_info_from_rpc(a_lock_tx_hash_str, l_config, 
                                                                DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_STAKE_EXT_LOCK,
                                                                &l_cond_tx, &l_spent_by_hash, &l_ticker_str, 
                                                                &l_prev_cond_idx, true);
    if (!l_cond_tx) {
        log_it(L_ERROR, "Failed to get lock transaction info");
        DAP_DEL_Z(l_ticker_str);
        DAP_DEL_Z(l_spent_by_hash);
        return s_compose_config_return_response_handler(l_config);
    }

    if (l_spent_by_hash) {
        log_it(L_ERROR, "Lock transaction already spent by %s", l_spent_by_hash);
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_TX_ALREADY_SPENT, 
                                "Lock transaction already spent");
        DAP_DEL_MULTY(l_ticker_str, l_spent_by_hash, l_cond_tx);
        if (l_lock_tx) dap_chain_datum_tx_delete(l_lock_tx);
        return s_compose_config_return_response_handler(l_config);
    }

    dap_hash_fast_t l_stake_ext_hash = l_cond_tx->subtype.srv_stake_ext_lock.stake_ext_hash;
    ...
    json_object *l_stake_ext_info = s_request_command_to_rpc_with_params(l_config, "stake_ext",
                                                                         "info;-net;%s;-stake_ext;%s",
                                                                         l_config->net_name, l_stake_ext_hash_str);
    if (!l_stake_ext_info) {
        log_it(L_ERROR, "Failed to get stake_ext info");
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_STAKE_EXT_NOT_FOUND, 
                                "Failed to get stake_ext info");
        DAP_DEL_MULTY(l_ticker_str, l_cond_tx);
        if (l_lock_tx) dap_chain_datum_tx_delete(l_lock_tx);
        return s_compose_config_return_response_handler(l_config);
    }
    ...
    json_object *l_first_item = json_object_array_get_idx(l_stake_ext_info, 0);
    json_object *l_status_json = l_first_item ? json_object_object_get(l_first_item, "stake_ext_status") : NULL;
    const char *l_status_str = l_status_json ? json_object_get_string(l_status_json) : NULL;
    
    bool l_unlock_allowed = false;
    if (l_status_str) {
        if (dap_strcmp(l_status_str, "ended") == 0 || dap_strcmp(l_status_str, "cancelled") == 0) {
            l_unlock_allowed = true;
        }
    }
    json_object_put(l_stake_ext_info);

    if (!l_unlock_allowed) {
        log_it(L_ERROR, "Unlock not allowed: stake_ext status is %s", l_status_str ? l_status_str : "unknown");
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_UNLOCK_NOT_ALLOWED, 
                                "Unlock not allowed: stake_ext is still active or status unknown");
        DAP_DEL_MULTY(l_ticker_str, l_cond_tx);
        if (l_lock_tx) dap_chain_datum_tx_delete(l_lock_tx);
        return s_compose_config_return_response_handler(l_config);
    }

    dap_chain_datum_token_get_delegated_ticker(l_delegated_ticker_str, l_ticker_str ? l_ticker_str : a_native_ticker);
    uint256_t l_emission_rate = dap_chain_coins_to_balance("0.001");
    uint256_t l_delegated_value = {};

    if (IS_ZERO_256(l_emission_rate) ||
        MULT_256_COIN(l_cond_tx->header.value, l_emission_rate, &l_delegated_value) ||
        IS_ZERO_256(l_delegated_value)) {
        log_it(L_ERROR, "Failed to calculate delegated value");
        s_json_compose_error_add(l_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_TX_CREATE, 
                                "Failed to calculate delegated value");
        DAP_DEL_MULTY(l_ticker_str, l_cond_tx);
        if (l_lock_tx) dap_chain_datum_tx_delete(l_lock_tx);
        return s_compose_config_return_response_handler(l_config);
    ...
    if (l_lock_tx)
        dap_chain_datum_tx_delete(l_lock_tx);

    dap_chain_datum_tx_t *l_tx = dap_chain_tx_compose_datum_stake_ext_unlock(a_wallet_addr, &l_lock_tx_hash, l_prev_cond_idx,
                                                                             l_ticker_str ? l_ticker_str : a_native_ticker,
                                                                             l_cond_tx->header.value, l_fee,
                                                                             l_delegated_ticker_str, l_delegated_value, l_config);
    ...
    if (l_tx) {
        ...
        log_it(L_ERROR, "Failed to create unlock transaction");
    }
...
dap_chain_datum_tx_t *dap_chain_tx_compose_datum_stake_ext_unlock(dap_chain_addr_t *a_wallet_addr,
                                                                 ...
                                                                 compose_config_t *a_config)
{
    dap_return_val_if_pass(!a_config || !a_wallet_addr || !a_lock_tx_hash || dap_hash_fast_is_blank(a_lock_tx_hash), NULL);
    ...
    if (!l_outs_native) {
        log_it(L_ERROR, "Can't get remote tx outs");
        DAP_DEL_Z(l_addr_fee);
        s_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_NOT_ENOUGH_FUNDS,
                                "Can't get wallet outputs");
        return NULL;
    }
    ...
    json_object *l_outs_delegated = dap_get_remote_tx_outs(a_delegated_ticker_str, a_wallet_addr, a_config);
    if (!l_outs_delegated) {
        log_it(L_ERROR, "Can't get delegated token outs");
        json_object_put(l_outs_native);
        DAP_DEL_Z(l_addr_fee);
        s_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_NOT_ENOUGH_DELEGATED,
                                "Can't get delegated token outputs");
        return NULL;
    }
    ...
    if (!IS_ZERO_256(l_total_fee)) {
        if (!l_main_native) {
            ...
            log_it(L_ERROR, "Total fee more than locked value");
            s_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_NOT_ENOUGH_FUNDS,
                                    "Total fee more than locked value");
            json_object_put(l_outs_native);
            json_object_put(l_outs_delegated);
            DAP_DEL_Z(l_addr_fee);
            return NULL;
    ...
    if (!IS_ZERO_256(a_delegated_value)) {
        l_list_used_out = s_ledger_get_list_tx_outs_from_json(l_outs_delegated, l_out_delegated_count,
                                                              a_delegated_value, &l_value_transfer);
        if (!l_list_used_out) {
            log_it(L_ERROR, "Not enough delegated tokens");
            s_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_NOT_ENOUGH_DELEGATED,
    ...
    if (!l_tx) {
        log_it(L_ERROR, "Failed to create transaction");
        s_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_TX_CREATE,
                                "Failed to create transaction");
        dap_list_free_full(l_list_used_out, NULL);
        dap_list_free_full(l_list_fee_out, NULL);
        DAP_DEL_Z(l_addr_fee);
    ...
    dap_chain_datum_tx_add_in_cond_item(&l_tx, a_lock_tx_hash, a_prev_cond_idx, 0);
    ...
    uint256_t l_value_pack = l_main_native ? a_value : uint256_0;
    uint256_t l_native_pack = {};

    if (l_net_fee_used) {
        if (dap_chain_datum_tx_add_out_ext_item(&l_tx, l_addr_fee, l_net_fee, l_native_ticker) != 1) {
            log_it(L_ERROR, "Failed to add network fee output");
            s_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_NET_FEE,
                                    "Failed to add network fee output");
            dap_chain_datum_tx_delete(l_tx);
            DAP_DEL_Z(l_addr_fee);
            return NULL;
    ...
    if (!IS_ZERO_256(a_value_fee)) {
        if (dap_chain_datum_tx_add_fee_item(&l_tx, a_value_fee) != 1) {
            log_it(L_ERROR, "Failed to add validator fee");
            s_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_VALIDATOR_FEE,
                                    "Failed to add validator fee");
            dap_chain_datum_tx_delete(l_tx);
            return NULL;
        }
    ...
    uint256_t l_value_back = {};
    if (l_main_native) {
        SUBTRACT_256_256(l_value_pack, l_native_pack, &l_value_back);
    } else {
        l_value_back = a_value;
    }
    
    if (!IS_ZERO_256(l_value_back)) {
        if (dap_chain_datum_tx_add_out_ext_item(&l_tx, a_wallet_addr, l_value_back, a_main_ticker) != 1) {
            log_it(L_ERROR, "Failed to add coin back output");
            s_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_COIN_BACK,
                                    "Failed to add coin back output");
            dap_chain_datum_tx_delete(l_tx);
            return NULL;
        }
    ...
    if (!l_main_native && !IS_ZERO_256(l_fee_transfer)) {
        SUBTRACT_256_256(l_fee_transfer, l_native_pack, &l_value_back);
        if (!IS_ZERO_256(l_value_back)) {
            if (dap_chain_datum_tx_add_out_ext_item(&l_tx, a_wallet_addr, l_value_back, l_native_ticker) != 1) {
                log_it(L_ERROR, "Failed to add fee coin back output");
                s_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_COIN_BACK,
                                        "Failed to add fee coin back output");
                dap_chain_datum_tx_delete(l_tx);
                return NULL;
            }
        }
    ...
    if (!IS_ZERO_256(a_delegated_value)) {
        if (dap_chain_datum_tx_add_out_ext_item(&l_tx, &c_dap_chain_addr_blank,
            ...
            s_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_BURN,
                                    "Failed to add delegated token burn output");
            dap_chain_datum_tx_delete(l_tx);
            return NULL;
        }
        ...
        if (!IS_ZERO_256(l_value_back)) {
            if (dap_chain_datum_tx_add_out_ext_item(&l_tx, a_wallet_addr, l_value_back, a_delegated_ticker_str) != 1) {
                ...
                s_json_compose_error_add(a_config->response_handler, STAKE_EXT_UNLOCK_COMPOSE_ERR_ADD_COIN_BACK,
                                        "Failed to add delegated token coin back output");
                dap_chain_datum_tx_delete(l_tx);
                return NULL;
            }
        }
