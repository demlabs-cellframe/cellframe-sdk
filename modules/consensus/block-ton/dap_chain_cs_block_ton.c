...
static uint8_t *s_message_data_sign(dap_chain_cs_block_ton_message_t *a_message, uint8_t *a_data, size_t *a_sign_size);
...
static void s_message_chain_add(dap_chain_cs_block_ton_session_t * a_session,
...
static void s_session_send_startsync(dap_chain_cs_block_ton_session_t *a_session);
...
static bool s_validator_check(dap_chain_addr_t *a_addr, dap_list_t *a_validators);
...
static int s_callback_new(dap_chain_t *a_chain, dap_config_t *a_chain_cfg)
{
    ...
    l_ton_pvt->debug = dap_config_get_item_bool_default(a_chain_cfg, "block-ton", "consensus_debug", false);

    l_ton_pvt->validators_list_by_stake = dap_config_get_item_bool_default(a_chain_cfg, "block-ton", "validators_list_by_stake", true);
    ...
    int l_ret = 0;
    l_ton_pvt->min_validators_count = dap_config_get_item_uint16(a_chain_cfg, "block-ton", "min_validators_count");
    if (!l_ton_pvt->min_validators_count) {
        l_ret = -1;
        goto lb_err;
    }
    if ( !l_ton_pvt->validators_list_by_stake ) { // auth by cert for PoA mode
        const char *l_auth_certs_prefix = dap_config_get_item_str(a_chain_cfg, "block-ton", "auth_certs_prefix");
        ...
        if (l_auth_certs_count < l_ton_pvt->min_validators_count) {
            l_ret = -2;
            goto lb_err;
        }
        ...
        for (size_t i = 0; i < l_auth_certs_count; i++) {
            dap_chain_cs_block_ton_validator_t *l_validator = DAP_NEW(dap_chain_cs_block_ton_validator_t);
            ...
            if (dap_sscanf(l_addrs[i], NODE_ADDR_FP_STR, NODE_ADDR_FPS_ARGS_S(l_validator->node_addr) ) != 4) {
                log_it(L_ERROR,"TON: Wrong address format,  should be like 0123::4567::89AB::CDEF");
                DAP_DELETE(l_validator);
                l_ret = -4;
                goto lb_err;
            }
            log_it(L_MSG, "TON: add validator addr:"NODE_ADDR_FP_STR"", NODE_ADDR_FP_ARGS_S(l_validator->node_addr));

            l_validator->weight = uint256_1;
            l_ton_pvt->poa_validators = dap_list_append(l_ton_pvt->poa_validators, l_validator);
        }
    }
...
lb_err:
    dap_list_free_full(l_ton_pvt->poa_validators, NULL);
...
{
    return DAP_DUP((dap_chain_cs_block_ton_validator_t *)a_validator);
}
...
static dap_list_t *s_get_validators_list(dap_chain_cs_block_ton_session_t *a_session, dap_chain_hash_fast_t *a_last_block_hash)
{
    ...
    if (l_ton_pvt->validators_list_by_stake) {
        ...
        size_t l_need_vld_cnt = MIN(l_validators_count, (size_t)l_ton_pvt->min_validators_count * 2);
        ...
        l_ret = dap_list_copy_deep(l_ton_pvt->poa_validators, s_callback_list_copy, NULL);
...
static int s_callback_created(dap_chain_t *a_chain, dap_config_t *a_chain_net_cfg) {
    ...
    dap_chain_addr_fill_from_key(&l_my_signing_addr, l_ton_pvt->blocks_sign_key, a_chain->net_id);
    if (l_ton_pvt->validators_list_by_stake) {
        ...
        if (s_validator_check(&l_my_signing_addr, l_ton_pvt->poa_validators)) {
...
static void s_session_send_startsync(dap_chain_cs_block_ton_session_t *a_session)
{
    ...
    l_startsync->round_id = a_session->cur_round.id;
    s_get_last_block_hash(a_session->chain, &l_startsync->last_block_hash);
                                                                                     ...
                                                                                     dap_global_db_obj_t *a_values, void *a_arg)
{
    ...
    if (a_values_count) {
        for (size_t i = 0; i < a_values_count; i++) {
            ...
            if ( l_store->hdr.round_id != l_session->cur_round.id )
                continue;
...
static void s_session_proc_state( dap_chain_cs_block_ton_session_t * a_session)
{
    ...
    switch (a_session->state) {
        case DAP_STREAM_CH_CHAIN_SESSION_STATE_IDLE: {
            if (l_time > a_session->ts_round_finish
            ...
            if (PVT(a_session->ton)->debug)
                log_it(L_MSG, "TON: net:%s, chain:%s, round:%"DAP_UINT64_FORMAT_U", attempt:%hu Start syncing validators ",
                        a_session->chain->net_name, a_session->chain->name,
                            a_session->cur_round.id, a_session->attempt_current_number);
            } else
                break;
        }
        case DAP_STREAM_CH_CHAIN_SESSION_STATE_WAIT_START: {
            ...
            if (a_session->cur_round.validators_count * 3 >=  dap_list_length(a_session->cur_round.validators_list) * 2) {
        ...
        case DAP_STREAM_CH_CHAIN_SESSION_STATE_CS_PROC: {
            ...
            // TODO realize pseudo-random weighted submit logic
            if ( !a_session->cur_round.submit && a_session->attempt_current_number == 1 ) {
                dap_list_t *l_validators_list = a_session->cur_round.validators_list;
            ...
            if ( (l_time - a_session->ts_round_start) >=
                ...
                uint16_t l_validators_count = a_session->cur_round.validators_count;
                uint64_t l_mod = 0;
                if (!PVT(a_session->ton)->validators_list_by_stake) {
                    // rotate validatir list in non-stake mode
                    l_mod = a_session->cur_round.id;
                }
                ...
                a_session->attempt_coordinator = (dap_chain_node_addr_t *)
                                        (dap_list_nth(a_session->cur_round.validators_list,
                            ...
                            dap_chain_block_t *a_candidate, size_t a_candidate_size)
{
    ...
    HASH_ITER(hh, a_session->cur_round.messages_items, l_message_item, l_message_tmp) {
        ...
        if ( l_message_type == DAP_STREAM_CH_CHAIN_MESSAGE_TYPE_COMMIT_SIGN ) {
            l_candidate_hash = &((dap_chain_cs_block_ton_message_commitsign_t *)
                                   l_message_item->message->msg_n_sign)->candidate_hash;
            if ( memcmp(l_candidate_hash, a_candidate_hash, sizeof(dap_chain_hash_fast_t)) == 0) {
                l_commitsign_list = dap_list_append(l_commitsign_list, l_message_item);
            }
        }
    }
    ...
    dap_list_t *l_submit_temp = l_commitsign_list;
    while(l_submit_temp) {
        dap_chain_cs_block_ton_message_item_t *l_message = (dap_chain_cs_block_ton_message_item_t *)l_submit_temp->data;
        dap_chain_cs_block_ton_message_commitsign_t *l_commitsign =
                    (dap_chain_cs_block_ton_message_commitsign_t *)l_message->message->msg_n_sign;
        if( l_message->is_verified) {
        ...
        switch (l_res) {
            case ATOM_ACCEPT: {
                ...
                if (dap_chain_atom_save(a_session->chain, (uint8_t *)l_candidate, a_candidate_size, a_session->chain->cells->id) < 0) {
                    ...
                    log_it(L_INFO, "TON: candidate %s added in chain successfully", l_candidate_hash_str);
...
static void s_session_candidate_submit(dap_chain_cs_block_ton_session_t *a_session)
{
    ...
    l_submit->round_id = a_session->cur_round.id;
    ...
    if ( a_session->my_candidate ) {
        ...
        if (PVT(a_session->ton)->debug) {
            ...
            log_it(L_MSG, "TON: net:%s, chain:%s, round:%"DAP_UINT64_FORMAT_U" Submit my candidate:%s",
                    a_session->chain->net_name, a_session->chain->name,
                        a_session->cur_round.id, l_hash_str);
            DAP_DELETE(l_hash_str);
        }
    ...
    s_message_send(a_session, DAP_STREAM_CH_CHAIN_MESSAGE_TYPE_SUBMIT,
                    (uint8_t*)l_submit, l_submit_size, a_session->cur_round.validators_list);
    DAP_DELETE(l_submit);
}
...
static void s_session_round_clear(dap_chain_cs_block_ton_session_t *a_session)
{
    ...
    uint64_t l_round_id = a_session->cur_round.id;
    ...
    log_it(L_MSG, "All data for round %"DAP_UINT64_FORMAT_U" cleared", l_round_id);
}
...
{
    return dap_chain_addr_compare(&((dap_chain_cs_block_ton_validator_t *)a_list_data)->signing_addr,
                                        ...
                                        dap_chain_hash_fast_t *a_candidate_hash, uint16_t *a_attempt_number)
{
    ...
    HASH_ITER(hh, l_messages_items, l_chain_message, l_chain_message_tmp) {
        dap_chain_cs_block_ton_message_getinfo_t *l_getinfo =
                (dap_chain_cs_block_ton_message_getinfo_t *)l_chain_message->message->msg_n_sign;
        if (l_chain_message->message->hdr.type == a_type &&
            ...
            switch(a_type) {
                ...
                case DAP_STREAM_CH_CHAIN_MESSAGE_TYPE_COMMIT_SIGN: {
                    if (l_chain_message->is_verified){
                        l_message_count++;
                    }
                } break;
                default:
                                                             ...
                                                             dap_global_db_obj_t *a_values, void *a_arg)
{
    ...
    if (a_values_count) {
        ...
        if (l_found_candidate) {
            ...
            l_vote->round_id = l_session->cur_round.id;
            ...
            s_message_send(l_session, DAP_STREAM_CH_CHAIN_MESSAGE_TYPE_VOTE, (uint8_t*)l_vote,
                            sizeof(dap_chain_cs_block_ton_message_vote_t), l_session->cur_round.validators_list);
            if (PVT(l_session->ton)->debug) {
...
static void s_callback_get_candidate_block_and_commit_sign(dap_global_db_context_t * a_global_db_context,int a_rc, const char * a_group, const char * a_key, const void * a_value, const size_t a_value_size, dap_nanotime_t a_value_ts, bool a_is_pinned, void * a_arg)
{
    ...
    uint64_t l_round_id = l_args->round_id;
    ...
    if (l_store) {
        ...
        if ( (l_sign_verified=dap_sign_verify(l_candidate_sign,
            ...
            s_message_chain_add(l_session, l_message, l_message_size, NULL);
            if (dap_global_db_set_unsafe(a_global_db_context, l_session->gdb_group_store, l_candidate_hash_str, l_store,
                ...
                if (l_commitsign_count * 3 >= l_round->validators_count * 2) {
                    ...
                    if (l_round_id == l_session->cur_round.id)
    ...
    if (l_store) {
        if (PVT(l_session->ton)->blocks_sign_key) {
            ...
            if (dap_global_db_set_unsafe(a_global_db_context, l_session->gdb_group_store,l_candidate_hash_str, l_store,
                ...
                l_commitsign->round_id = l_session->cur_round.id;
//                memcpy(&l_commitsign->candidate_hash, &l_candidate_hash, sizeof(dap_chain_hash_fast_t));
                ...
                s_message_send(l_session, DAP_STREAM_CH_CHAIN_MESSAGE_TYPE_COMMIT_SIGN, (uint8_t*)l_commitsign,
                                    l_commitsign_size, l_session->cur_round.validators_list);
    ...
    if (l_store) {
        ...
        if (dap_global_db_set_unsafe(a_global_db_context, l_session->gdb_group_store, l_candidate_hash_str,
            ...
            dap_chain_cs_block_ton_message_precommit_t l_precommit = {
                .round_id = l_session->cur_round.id,
            ...
            s_message_send(l_session, DAP_STREAM_CH_CHAIN_MESSAGE_TYPE_PRE_COMMIT, (uint8_t*)&l_precommit,
                            sizeof(dap_chain_cs_block_ton_message_precommit_t), l_session->cur_round.validators_list);
            if (PVT(l_session->ton)->debug)
...
static void s_callback_check_and_save_candidate_block  (dap_global_db_context_t * a_global_db_context,int a_rc, const char * a_group, const char * a_key, const void * a_value, const size_t a_value_size, dap_nanotime_t a_value_ts, bool a_is_pinned, void * a_arg)
{
    ...
    l_store->hdr.round_id = l_session->cur_round.id;
    ...
    if (dap_global_db_set_unsafe(a_global_db_context, l_session->gdb_group_store, l_candidate_hash_str, l_store,
        ...
        if ( !s_session_atom_validation(l_blocks, l_candidate, l_candidate_size) ) {
            ...
            if (PVT(l_session->ton)->blocks_sign_key) {
                ...
                l_approve->round_id = l_session->cur_round.id;
                ...
                s_message_send(l_session, DAP_STREAM_CH_CHAIN_MESSAGE_TYPE_APPROVE,
                                        (uint8_t*)l_approve, l_approve_size, l_session->cur_round.validators_list);
            ...
            l_reject->round_id = l_session->cur_round.id;
            ...
            s_message_send(l_session, DAP_STREAM_CH_CHAIN_MESSAGE_TYPE_REJECT, (uint8_t*)l_reject,
                        sizeof(dap_chain_cs_block_ton_message_reject_t), l_session->cur_round.validators_list);
            DAP_DELETE(l_reject);
            if (PVT(l_session->ton)->debug)
                                ...
                                dap_chain_hash_fast_t *a_data_hash, uint8_t *a_data, size_t a_data_size)
{
    ...
    size_t l_data_size = l_message_data_size;
    dap_sign_t *l_sign = (dap_sign_t *)(l_message->msg_n_sign + l_data_size);
    ...
    uint8_t *l_data = s_message_data_sign(l_message, l_message_data, &l_data_size);
    bool l_verify_passed = dap_sign_verify(l_sign, l_data, l_data_size) == 1;
    DAP_DELETE(l_data);
    if (!l_verify_passed) {
        if (PVT(l_session->ton)->debug)
        ...
        if (PVT(l_session->ton)->debug)
            log_it(L_MSG, "TON: net:%s, chain:%s, round:%"DAP_UINT64_FORMAT_U", attempt:%hu Message rejected: message hash does not match",
                    l_session->chain->net_name, l_session->chain->name,
                        l_session->cur_round.id, l_session->attempt_current_number);
        return;
    }
    ...
    if ( l_message->hdr.type == DAP_STREAM_CH_CHAIN_MESSAGE_TYPE_START_SYNC ) {
        // check time offset
        dap_chain_cs_block_ton_message_startsync_t *l_startsync =
                            (dap_chain_cs_block_ton_message_startsync_t *)l_message->msg_n_sign;
        ...
        if (
            ...
            if (PVT(l_session->ton)->debug)
                log_it(L_MSG, "TON: net:%s, chain:%s, round:%"DAP_UINT64_FORMAT_U", attempt:%hu Message rejected: too much time difference: my time:%"DAP_UINT64_FORMAT_U" sender time:%"DAP_UINT64_FORMAT_U"",
                        l_session->chain->net_name, l_session->chain->name,
                            l_session->cur_round.id, l_session->attempt_current_number, l_time, l_startsync->ts);
            return;
        }

        //sync round_id
        if ( l_session->cur_round.id < l_startsync->round_id ) {
            l_session->cur_round.id = l_startsync->round_id;
        }
    ...
    // round check
    uint64_t l_round_id = ((dap_chain_cs_block_ton_message_getinfo_t *)l_message->msg_n_sign)->round_id;
    ...
    if ( l_message->hdr.type == DAP_STREAM_CH_CHAIN_MESSAGE_TYPE_SUBMIT ) {
        dap_list_t *l_validators_list = dap_list_first(l_session->cur_round.validators_list);
    ...
    uint32_t l_vote_count = 0, l_precommit_count = 0;
    // check messages chain
    dap_chain_cs_block_ton_message_item_t *l_chain_message=NULL, *l_chain_message_tmp=NULL;
    pthread_rwlock_rdlock(&l_round->messages_rwlock);
    HASH_ITER(hh, l_round->messages_items, l_chain_message, l_chain_message_tmp) {
        if (dap_chain_addr_compare(&l_chain_message->signing_addr, &l_signing_addr)) {
            dap_chain_hash_fast_t *l_candidate_hash_cur =
                    &((dap_chain_cs_block_ton_message_getinfo_t *)l_message->msg_n_sign)->candidate_hash;

            dap_chain_hash_fast_t *l_candidate_hash =
                &((dap_chain_cs_block_ton_message_getinfo_t *)l_chain_message->message->msg_n_sign)->candidate_hash;
            ...
            switch (l_chain_msg_type) {
                ...
                case DAP_STREAM_CH_CHAIN_MESSAGE_TYPE_VOTE: {
                    dap_chain_cs_block_ton_message_vote_t *l_vote =
                                    (dap_chain_cs_block_ton_message_vote_t *)l_message->msg_n_sign;
                    dap_chain_cs_block_ton_message_vote_t *l_vote_item =
                                    (dap_chain_cs_block_ton_message_vote_t *)l_chain_message->message->msg_n_sign;
                ...
                case DAP_STREAM_CH_CHAIN_MESSAGE_TYPE_SUBMIT:{
                    if ( l_chain_msg_type == l_message->hdr.type ){
                        if (PVT(l_session->ton)->debug)
                            log_it(L_MSG, "TON: net:%s, chain:%s, round:%"DAP_UINT64_FORMAT_U", attempt:%hu Message rejected: duplicate message PRE_COMMIT for one candidate for one attempt",
                                    l_session->chain->net_name, l_session->chain->name,
                                        l_session->cur_round.id, l_session->attempt_current_number);
                        return;
                    }
    ...
    switch (l_message->hdr.type) {
        case DAP_STREAM_CH_CHAIN_MESSAGE_TYPE_SUBMIT: {
            ...
            s_message_chain_add(l_session, l_message, a_data_size, NULL);
            ...
            if (dap_hash_fast_is_blank(l_candidate_hash)) {
                if (PVT(l_session->ton)->debug)
                    log_it(L_MSG, "TON: net:%s, chain:%s, round:%"DAP_UINT64_FORMAT_U", attempt:%hu Receive COMMIT_SIGN: candidate: NULL",
                            l_session->chain->net_name, l_session->chain->name,
                                l_round->id, l_session->attempt_current_number);
                pthread_rwlock_unlock(&l_session->rwlock);
...
static uint8_t *s_message_data_sign(dap_chain_cs_block_ton_message_t *a_message, uint8_t *a_data, size_t *a_sign_size)
{
    ...
    if (a_data && a_sign_size) {
        memcpy(l_data->data, a_data, *a_sign_size);
                                    ...
                                    uint8_t *a_data, size_t a_data_size, dap_list_t *a_validators)
{
    ...
    l_message->hdr.id = HASH_COUNT(a_session->cur_round.messages_items);
    ...
    size_t l_data_size = a_data_size;
    uint8_t *l_data = s_message_data_sign(l_message, a_data, &l_data_size);
                                    ...
                                    size_t a_message_size, dap_chain_hash_fast_t *a_message_hash)
{
    ...
    l_message_item->is_genesis = dap_hash_fast_is_blank(&l_round->last_message_hash);
    if (!l_message_item->is_genesis)
        l_message_item->prev_message_hash = l_round->last_message_hash;
    ...
    if (a_message->hdr.type == DAP_STREAM_CH_CHAIN_MESSAGE_TYPE_VOTE_FOR ||
        ...
        l_message_item->is_verified = true;
    ...
    if ( l_ton_pvt->flag_sign_verify && !l_ton_pvt->validators_list_by_stake ) { // PoA mode
        ...
        for (size_t i=0; i< l_signs_count; i++) {
            ...
            if (l_ton_pvt->validators_list_by_stake) {
                ...
                if (!s_validator_check(&l_signing_addr, l_ton_pvt->poa_validators))
                    continue;
            }
