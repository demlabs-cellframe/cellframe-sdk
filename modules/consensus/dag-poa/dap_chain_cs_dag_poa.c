...
static void s_round_event_cs_done(dap_chain_type_dag_poa_round_item_t *a_event_item, uint32_t a_timeout_s)
{
    ...
    pthread_rwlock_wrlock(&l_poa_pvt->rounds_rwlock);
    dap_ht_find_hh(hh, l_poa_pvt->event_items, &a_event_item->datum_hash.hash_key, DAP_HASH_SHA3_256_SIZE, l_event_item);
    if (!l_event_item) {
        ...
        dap_ht_add_hh(hh, l_poa_pvt->event_items, datum_hash.hash_key, l_event_item);
