...
#include "dap_chain_cs_dag_poc.h"
#include "dap_chain_net_srv_stake.h"
#include "dap_chain_cell.h"

#include "dap_cert.h"

#define LOG_TAG "dap_chain_cs_dag_poc"
...
#define PVT(a) ((dap_chain_cs_dag_poc_pvt_t *) a->_pvt )

typedef struct dap_chain_cs_dag_poc_callback_timer_arg {
    dap_chain_cs_dag_t * dag;
    char * l_event_hash_hex_str;
    dap_chain_cs_dag_event_round_cfg_t event_round_cfg;
} dap_chain_cs_dag_poc_callback_timer_arg_t;
...
static bool s_callback_round_event_to_chain(dap_chain_cs_dag_poc_callback_timer_arg_t * a_callback_arg);
...
// CLI commands
static int s_cli_dag_poc(int argc, char ** argv, char **str_reply);

static bool s_seed_mode = false;

/**
 * @brief
 * init consensus dag_poc
 * read parameters from config and register dag_poc commands to cellframe-node-cli
 * @return
 */
int dap_chain_cs_dag_poc_init(void)
{
    // Add consensus constructor
    dap_chain_cs_add ("dag_poc", s_callback_new );
    s_seed_mode = dap_config_get_item_bool_default(g_config,"general","seed_mode",false);
    dap_chain_node_cli_cmd_item_create ("dag_poc", s_cli_dag_poc, "DAG PoC commands",
        "dag_poc -net <chain net name> -chain <chain name> event sign -event <event hash> [-H hex|base58(default)]\n"
...
/**
 * @brief dap_chain_cs_dag_poc_deinit
 */
...
/**
 * @brief
 * parse and execute cellframe-node-cli dag-poc commands
 ...
 * @return
 */
static int s_cli_dag_poc(int argc, char ** argv, char **a_str_reply)
{
    int ret = -666;
    int arg_index = 1;
    ...
    //dap_chain_cs_dag_poc_t * l_poc = DAP_CHAIN_CS_DAG_poc( l_dag ) ;
    dap_chain_cs_dag_poc_pvt_t * l_poc_pvt = PVT ( DAP_CHAIN_CS_DAG_POC( l_dag ) );

    const char * l_event_cmd_str = NULL;
    const char * l_event_hash_str = NULL;
    if ( l_poc_pvt->events_sign_cert == NULL) {
    ...
    if (!l_event_hash_str) {
        dap_chain_node_cli_set_reply_text(a_str_reply, "Command dag_poc requires parameter '-event' <event hash>");
        return -4;
    }
        ...
        if ( strcmp(l_event_cmd_str,"sign") == 0) { // Sign event command
            ...
            if ( (l_event = dap_chain_cs_dag_event_gdb_get( l_event_hash_hex_str, &l_event_size,
                ...
                dap_chain_cs_dag_event_t *l_event_new = dap_chain_cs_dag_event_copy_with_sign_add(l_event, l_event_size, 
                                                        &l_event_size_new,
                                                        l_chain_net, l_poc_pvt->events_sign_cert->enc_key);
                if ( l_event_new ) {
                    ...
                    if (dap_chain_cs_dag_event_gdb_set(l_event_new_hash_hex_str, l_event_new,
                        ...
                        if ( !dap_chain_global_db_gr_del(dap_strdup(l_event_hash_hex_str), l_gdb_group_events) ) { // Delete old event
                            ...
                            dap_chain_node_cli_set_reply_text(a_str_reply, "Added new sign with cert \"%s\", event %s placed back in round.new\n"
                                                                           ...
                                                                           l_poc_pvt->events_sign_cert->name ,l_event_new_hash_hex_str, l_event_hash_str);
                        }

                        if(!dap_strcmp(l_hash_out_type, "hex")) {
                            dap_chain_node_cli_set_reply_text(a_str_reply,
                                    "Added new sign with cert \"%s\", event %s placed back in round.new\n",
                                    l_poc_pvt->events_sign_cert->name, l_event_new_hash_hex_str);
                        }
                        else {
                            dap_chain_node_cli_set_reply_text(a_str_reply,
                                    "Added new sign with cert \"%s\", event %s placed back in round.new\n",
                                    l_poc_pvt->events_sign_cert->name, l_event_new_hash_base58_str);
                        }
                        ret = 0;
                        // dap_chain_net_sync_gdb(l_chain_net); // Propagate changes in pool
                        if (l_event_is_ready && l_poc_pvt->auto_round_complete) { // cs done (minimum signs & verify passed)
            ...
            dap_chain_node_cli_set_reply_text(a_str_reply, "Command dag_poc requires subcommand 'sign'");
        }
    } else {
        dap_chain_node_cli_set_reply_text(a_str_reply, "Command dag_poc requires subcommand 'event'");
    }
    return ret;
}
 ...
 * get dag-poc consensus parameters from config
...
static int s_callback_new(dap_chain_t * a_chain, dap_config_t * a_chain_cfg)
{
    ...
    dap_chain_cs_dag_poc_t * l_poc = DAP_NEW_Z ( dap_chain_cs_dag_poc_t);
    l_dag->_inheritor = l_poc;
    ...
    l_poc->_pvt = DAP_NEW_Z ( dap_chain_cs_dag_poc_pvt_t );

    dap_chain_cs_dag_update(l_dag);

    dap_chain_cs_dag_poc_pvt_t * l_poc_pvt = PVT ( l_poc );
    if (dap_config_get_item_str(a_chain_cfg,"dag-poc","auth_certs_prefix") ) {
        l_poc_pvt->auth_certs_count = dap_config_get_item_uint16_default(a_chain_cfg,"dag-poc","auth_certs_number",0);
        l_poc_pvt->auth_certs_count_verify = dap_config_get_item_uint16_default(a_chain_cfg,"dag-poc","auth_certs_number_verify",0);
        l_poc_pvt->confirmations_timeout = dap_config_get_item_uint32_default(a_chain_cfg,"dag-poc","confirmations_timeout",600);
        l_poc_pvt->auto_confirmation = dap_config_get_item_bool_default(a_chain_cfg,"dag-poc","auto_confirmation",true);
        l_poc_pvt->auto_round_complete = dap_config_get_item_bool_default(a_chain_cfg,"dag-poc","auto_round_complete",true);
        l_poc_pvt->wait_sync_before_complete = dap_config_get_item_uint32_default(a_chain_cfg,"dag-poc","wait_sync_before_complete",180);
        l_poc_pvt->auth_certs_prefix = strdup ( dap_config_get_item_str(a_chain_cfg,"dag-poc","auth_certs_prefix") );
        if (l_poc_pvt->auth_certs_count && l_poc_pvt->auth_certs_count_verify ) {
            l_poc_pvt->auth_certs = DAP_NEW_Z_SIZE ( dap_cert_t *, l_poc_pvt->auth_certs_count * sizeof(dap_cert_t));
            char l_cert_name[512];
            for (size_t i = 0; i < l_poc_pvt->auth_certs_count ; i++ ){
                dap_snprintf(l_cert_name,sizeof(l_cert_name),"%s.%zu",l_poc_pvt->auth_certs_prefix, i);
                if ((l_poc_pvt->auth_certs[i] = dap_cert_find_by_name( l_cert_name)) == NULL) {
                    dap_snprintf(l_cert_name,sizeof(l_cert_name),"%s.%zu.pub",l_poc_pvt->auth_certs_prefix, i);
                    if ((l_poc_pvt->auth_certs[i] = dap_cert_find_by_name( l_cert_name)) == NULL) {
    ...
    log_it(L_NOTICE,"Initialized DAG-PoC consensus with %u/%u minimum consensus",l_poc_pvt->auth_certs_count,l_poc_pvt->auth_certs_count_verify);
    // Save old callback if present and set the call of its own (chain callbacks)
    l_poc_pvt->prev_callback_created = l_dag->chain->callback_created;
...
static void s_round_event_cs_done(dap_chain_cs_dag_t * a_dag, dap_chain_cs_dag_event_t * a_event,
                                    char * a_event_hash_hex_str, dap_chain_cs_dag_event_round_cfg_t * a_event_round_cfg) {
    dap_chain_cs_dag_poc_t * l_poc = DAP_CHAIN_CS_DAG_POC( a_dag );
    dap_chain_cs_dag_poc_callback_timer_arg_t * l_callback_arg = DAP_NEW_Z(dap_chain_cs_dag_poc_callback_timer_arg_t);
    ...
    if ( a_event->header.signs_count >= PVT(l_poc)->auth_certs_count) {
        // placement in chain now if max signs
        if (dap_timerfd_start(PVT(l_poc)->wait_sync_before_complete*1000, 
                            (dap_timerfd_callback_t)s_callback_round_event_to_chain, 
                            l_callback_arg) == NULL) {
            log_it(L_ERROR,"Can't run timer for Event %s", a_event_hash_hex_str);
        } else {
            log_it(L_NOTICE,"Run timer %dsec. for Event %s", PVT(l_poc)->wait_sync_before_complete, a_event_hash_hex_str);
        }
    }
        ...
        l_timeout += PVT(l_poc)->wait_sync_before_complete;
...
static void s_callback_get_round_cfg(dap_chain_cs_dag_t * a_dag, dap_chain_cs_dag_event_round_cfg_t * a_event_round_cfg) {
    dap_chain_cs_dag_poc_t * l_poc = DAP_CHAIN_CS_DAG_POC(a_dag);
    dap_chain_cs_dag_poc_pvt_t * l_poc_pvt = PVT (l_poc);
    a_event_round_cfg->confirmations_minimum = l_poc_pvt->auth_certs_count_verify;
    a_event_round_cfg->confirmations_timeout = l_poc_pvt->confirmations_timeout;
 ...
 * path to certificate iw written to chain config file in dag_poc section
...
static int s_callback_created(dap_chain_t * a_chain, dap_config_t *a_chain_net_cfg)
{
    ...
    if (PVT(l_poc)->prev_callback_created )
        PVT(l_poc)->prev_callback_created(a_chain,a_chain_net_cfg);

    const char * l_events_sign_cert = NULL;
    if ( ( l_events_sign_cert = dap_config_get_item_str(a_chain_net_cfg,"dag-poc","events-sign-cert") ) != NULL ) {

        if ( ( PVT(l_poc)->events_sign_cert = dap_cert_find_by_name(l_events_sign_cert)) == NULL ){
            log_it(L_ERROR,"Can't load events sign certificate, name \"%s\" is wrong",l_events_sign_cert);
        }else
            log_it(L_NOTICE,"Loaded \"%s\" certificate to sign poc event", l_events_sign_cert);
...
/**
 * @brief 
 * delete dap_chain_cs_dag_poc_pvt_t callback
...
static void s_callback_delete(dap_chain_cs_dag_t * a_dag)
{
    ...
    if ( l_poc->_pvt ) {
        ...
        if ( l_poc_pvt->auth_certs )
            DAP_DELETE ( l_poc_pvt->auth_certs);

        if ( l_poc_pvt->auth_certs_prefix )
            free ( l_poc_pvt->auth_certs_prefix );

        DAP_DELETE ( l_poc->_pvt);
    }

    if ( l_poc->_inheritor ) {
       DAP_DELETE ( l_poc->_inheritor );
    }
}
...
static dap_chain_cs_dag_event_t * s_callback_event_create(dap_chain_cs_dag_t * a_dag, dap_chain_datum_t * a_datum,
                                                          dap_chain_hash_fast_t * a_hashes, size_t a_hashes_count, size_t* a_event_size)
{
    dap_return_val_if_fail(a_dag && a_dag->chain && DAP_CHAIN_CS_DAG_POC(a_dag), NULL);
    dap_chain_net_t * l_net = dap_chain_net_by_name( a_dag->chain->net_name );
    dap_chain_cs_dag_poc_t * l_poc = DAP_CHAIN_CS_DAG_POC(a_dag);
    if ( PVT(l_poc)->events_sign_cert == NULL){
        log_it(L_ERROR, "Can't sign event with events_sign_cert in [dag-poc] section");
        return  NULL;
    }
    if ( s_seed_mode || (a_hashes && a_hashes_count) ){
        dap_chain_cs_dag_event_t * l_event = dap_chain_cs_dag_event_new( a_dag->chain->id, l_net->pub.cell_id, a_datum,
                                                         PVT(l_poc)->events_sign_cert->enc_key, a_hashes, a_hashes_count,a_event_size);
        return l_event;
    }else
        return NULL;
}
                                        ...
                                        const char *a_key, const void *a_value, const size_t a_value_size)
{
    ...
    dap_chain_cs_dag_poc_t * l_poc = DAP_CHAIN_CS_DAG_POC(a_dag);
    if ( !PVT(l_poc)->auto_confirmation ) {
        return 0;
    }
    ...
    dap_chain_cs_dag_event_t *l_event_new = dap_chain_cs_dag_event_copy_with_sign_add(l_event, l_event_size, 
                                            ...
                                            PVT(l_poc)->events_sign_cert->enc_key);
    ...
    if ( l_event_new ) {
        ...
        if (dap_chain_cs_dag_event_gdb_set(l_event_new_hash_hex_str, l_event_new,
            ...
            if (l_event_is_ready && PVT(l_poc)->auto_round_complete) { // cs done (minimum signs & verify passed)
        ...
        // if (PVT(l_poc)->auto_round_complete) {
...
static int s_callback_event_verify(dap_chain_cs_dag_t * a_dag, dap_chain_cs_dag_event_t * a_dag_event, size_t a_dag_event_size)
{
    dap_chain_cs_dag_poc_pvt_t * l_poc_pvt = PVT ( DAP_CHAIN_CS_DAG_POC( a_dag ) );
    ...
    uint16_t l_certs_count_verify = a_dag->use_event_round_cfg ? a_dag->event_round_cfg.confirmations_minimum
                                                                : l_poc_pvt->auth_certs_count_verify;
    ...
    // if ( a_dag_event->header.signs_count >= l_poc_pvt->auth_certs_count_verify ){
    if ( a_dag_event->header.signs_count >= l_certs_count_verify ){
        ...
        for ( uint16_t i = 0; i < a_dag_event->header.signs_count; i++ ) {
            ...
            for (uint16_t j = 0; j < l_poc_pvt->auth_certs_count; j++) {
                if (dap_cert_compare_with_sign ( l_poc_pvt->auth_certs[j], l_sign) == 0)
                    l_verified++;
            }
        }
        //return l_verified >= l_poc_pvt->auth_certs_count_verify ? 0 : -1;
...
dap_cert_t **dap_chain_cs_dag_poc_get_auth_certs(dap_chain_t *a_chain, size_t *a_auth_certs_count)
{
    ...
    if (strcmp(l_chain_pvt->cs_name, "dag_poc"))
        return NULL;
    dap_chain_cs_dag_poc_pvt_t *l_poc_pvt = PVT(DAP_CHAIN_CS_DAG_POC(DAP_CHAIN_CS_DAG(a_chain)));
    if (a_auth_certs_count)
        *a_auth_certs_count = l_poc_pvt->auth_certs_count;
    return l_poc_pvt->auth_certs;
}
