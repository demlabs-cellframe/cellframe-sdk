...
/**
 * @brief dap_chain_cs_dag_poc_init
 * @return
 */
...
static int s_callback_new(dap_chain_t * a_chain, dap_config_t * a_chain_cfg)
{
    ...
    l_tokens_hold = dap_config_get_array_str( a_chain_cfg,"dag-poc","tokens_hold",&l_tokens_hold_size);
    l_tokens_hold_value_str = dap_config_get_array_str( a_chain_cfg,"dag-poc","tokens_hold_value",&l_tokens_hold_value_size);

    if ( l_tokens_hold_size != l_tokens_hold_value_size ){
        log_it(L_CRITICAL, "tokens_hold and tokens_hold_value are different size!");
        goto lb_err;
    }
    l_poc_pvt->confirmations_minimum = dap_config_get_item_uint16_default( a_chain_cfg,"dag-poc","confirmations_minimum",1);
    l_poc_pvt->tokens_hold_size = l_tokens_hold_size;
    l_poc_pvt->tokens_hold = DAP_NEW_Z_SIZE( char*, sizeof(char*) *
                                             l_tokens_hold_size );

    l_poc_pvt->tokens_hold_value = DAP_NEW_Z_SIZE(uint64_t,
                                                  (l_tokens_hold_value_size +1) *sizeof (uint64_t));

    for (size_t i = 0; i < l_tokens_hold_value_size; i++){
        l_poc_pvt->tokens_hold[i] = dap_strdup( l_tokens_hold[i] );
        if ( ( l_poc_pvt->tokens_hold_value[i] =
               strtoull(l_tokens_hold_value_str[i],NULL,10) ) == 0 ) {
             log_it(L_CRITICAL, "Token %s has inproper hold value %"DAP_UINT64_FORMAT_U, l_poc_pvt->tokens_hold[i],
                    l_poc_pvt->tokens_hold_value[i] );
             goto lb_err;
        }
    }
...
lb_err:
    ...
    DAP_DELETE( l_poc_pvt->tokens_hold_value);
    DAP_DELETE( l_poc_pvt);
    DAP_DELETE(l_poc );
...
static int s_callback_created(dap_chain_t * a_chain, dap_config_t *a_chain_net_cfg)
{
    ...
    if ((l_sign_cert_str = dap_config_get_item_str(a_chain_net_cfg, "dag-poc", "events-sign-cert")) != NULL) {
        ...
        if (l_sign_cert == NULL) {
            ...
            PVT(l_poc)->events_sign_key = l_sign_cert->enc_key;
            log_it(L_NOTICE, "Loaded \"%s\" certificate to sign PoC events", l_sign_cert_str);
        } else {
...
{
    dap_return_val_if_fail(a_dag && a_dag->chain && DAP_CHAIN_CS_DAG_POC(a_dag) && a_datum, NULL);
    dap_chain_net_t * l_net = dap_chain_net_by_name( a_dag->chain->net_name );
    dap_chain_cs_dag_poc_t * l_poc = DAP_CHAIN_CS_DAG_POC(a_dag);

    if( PVT(l_poc)->events_sign_key == NULL) {
        log_it(L_ERROR, "Can't sign event with events-sign-wallet in [dag-poc] section");
        return NULL;
    }
    if(a_datum || (a_hashes && a_hashes_count)) {
        dap_chain_cs_dag_event_t * l_event = dap_chain_cs_dag_event_new(a_dag->chain->id, l_net->pub.cell_id, a_datum,
                                                                        PVT(l_poc)->events_sign_key, a_hashes, a_hashes_count, a_dag_event_size);
        return l_event;
    } else
        return NULL;
}
...
static int s_callback_event_verify(dap_chain_cs_dag_t * a_dag, dap_chain_cs_dag_event_t * a_dag_event, size_t a_dag_event_size)
{
    ...
    if ( a_dag_event->header.signs_count >= l_poc_pvt->confirmations_minimum ){
        ...
        for ( size_t l_sig_pos=0; l_sig_pos < a_dag_event->header.signs_count; l_sig_pos++ ){
            ...
            if ( l_sign == NULL){
                log_it(L_WARNING, "Event is NOT signed with anything: sig poc %zu, event size %zu", l_sig_pos, a_dag_event_size);
                return -4;
            }
        ...
        if (dap_file_test("D://Distr//concensrus.txt"))
        {
        ...
        // Check number
        if ( l_verified_num >= l_poc_pvt->confirmations_minimum ){
            // Passed all checks
            return 0;
        }else{
            log_it(L_WARNING, "Wrong event: only %hu/%hu signs are valid", l_verified_num, l_poc_pvt->confirmations_minimum );
            return -2;
        }
    }else{
