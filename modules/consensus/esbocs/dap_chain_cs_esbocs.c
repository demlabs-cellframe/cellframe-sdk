...
void dap_chain_esbocs_stop_timer(dap_chain_net_id_t a_net_id)
{
    ...
    dap_chain_net_t *l_net = dap_chain_net_api_by_id(a_net_id);
    dap_dl_foreach(s_session_items, l_session) {
        if (l_session->chain->net_id.uint64 == a_net_id.uint64 && l_session->cs_timer) {
            log_it(L_INFO, "Stop consensus timer for net: %s, chain: %s",
                   l_net ? l_net->pub.name : "UNKNOWN", l_session->chain->name);
...
void dap_chain_esbocs_start_timer(dap_chain_net_id_t a_net_id)
{
    ...
    dap_dl_foreach(s_session_items, l_session) {
        if (l_session->chain->net_id.uint64 == a_net_id.uint64) {
            log_it(L_INFO, "Start consensus timer for net: %s, chain: %s",
                   l_net ? l_net->pub.name : "UNKNOWN", l_session->chain->name);
...
static void s_callback_delete(dap_chain_type_blocks_t *a_blocks)
{
    ...
    dap_ht_foreach_hh(hh, l_session->sync_items, l_sync_item, l_sync_tmp) {
    ...
    dap_ht_foreach_hh(hh, l_session->penalty, l_pen_item, l_pen_tmp) {
...
static void s_session_update_penalty(dap_chain_esbocs_session_t *a_session)
{
    for (dap_list_t *it = a_session->cur_round.all_validators; it; it = it->next) {
        ...
        dap_ht_find_hh(hh, a_session->penalty, l_signing_addr, sizeof(*l_signing_addr), l_item);
        if (!l_item) {
            l_item = DAP_NEW_Z_RET_IF_FAIL(dap_chain_esbocs_penalty_item_t);
            l_item->signing_addr = *l_signing_addr;
            dap_ht_add_hh(hh, a_session->penalty, signing_addr, l_item);
        }
...
static void s_session_round_clear(dap_chain_esbocs_session_t *a_session)
{
    ...
    dap_ht_foreach_hh(hh, a_session->cur_round.message_items, l_message_item, l_message_tmp) {
    ...
    dap_ht_foreach_hh(hh, a_session->cur_round.store_items, l_store_item, l_store_tmp) {
...
static bool s_session_round_new(void *a_arg)
{
    ...
    dap_ht_find_hh(hh, a_session->sync_items, &a_session->cur_round.last_block_hash, sizeof(dap_hash_sha3_256_t), l_item);
    if (l_item) {
    ...
    dap_ht_foreach_hh(hh, a_session->sync_items, l_item, l_tmp) {
    ...
    if (!a_session->cur_round.sync_sent) {
        ...
        if (!l_round_already_started && a_session->sync_failed) {
            ...
            if (l_time_delta >= 0 && l_time_delta < PVT(a_session->esbocs)->new_round_delay && a_session->esbocs->last_accepted_block_timestamp) {
                uint16_t l_time_delta_u16 = (uint16_t)l_time_delta;
                l_sync_send_delay = PVT(a_session->esbocs)->new_round_delay - l_time_delta_u16;
            } else {
...
static bool s_session_directive_ready(dap_chain_esbocs_session_t *a_session, bool *a_kick, dap_chain_addr_t *a_signing_addr)
{
    ...
    dap_ht_foreach_hh(hh, a_session->penalty, l_item, l_tmp) {
...
static void s_session_state_change(dap_chain_esbocs_session_t *a_session, enum s_esbocs_session_state a_new_state, dap_time_t a_time)
{
    ...
    case DAP_CHAIN_ESBOCS_SESSION_STATE_WAIT_FINISH: {
        ...
        dap_ht_find_hh(hh, a_session->cur_round.store_items, &a_session->cur_round.attempt_candidate_hash, sizeof(dap_hash_sha3_256_t), l_store);
        if (!l_store) {
        ...
        dap_ht_foreach_hh(hh, a_session->cur_round.message_items, l_chain_message, l_chain_message_tmp) {
...
static void s_session_proc_state(void *a_arg)
{
    ...
    case DAP_CHAIN_ESBOCS_SESSION_STATE_WAIT_SIGNS:
        if (l_time - l_session->ts_stage_entry >= PVT(l_session->esbocs)->round_attempt_timeout) {
            dap_chain_esbocs_store_t *l_store = NULL;
            dap_ht_find_hh(hh, l_session->cur_round.store_items, &l_session->cur_round.attempt_candidate_hash, sizeof(dap_hash_sha3_256_t), l_store);
            if (!l_store) {
                                ...
                                dap_chain_addr_t *a_signing_addr)
{
    ...
    dap_ht_add_hh(hh, l_round->message_items, message_hash, l_message_item);
}
...
static void s_session_candidate_precommit(dap_chain_esbocs_session_t *a_session, dap_chain_esbocs_message_t *a_message)
{
    ...
    dap_ht_find_hh(hh, a_session->cur_round.store_items, l_candidate_hash, sizeof(dap_hash_sha3_256_t), l_store);
    if (!l_store) {
        log_it(L_MSG, "net:%s, chain:%s, round:%"DAP_UINT64_FORMAT_U", attempt:%hhu."
...
void s_session_sync_queue_add(dap_chain_esbocs_session_t *a_session, dap_chain_esbocs_message_t *a_message, size_t a_message_size)
{
    ...
    dap_ht_find_hh(hh, a_session->sync_items, &a_message->hdr.candidate_hash, sizeof(dap_hash_sha3_256_t), l_sync_item);
    if (!l_sync_item) {
        ...
        dap_ht_add_hh(hh, a_session->sync_items, last_block_hash, l_sync_item);
    }
...
void s_session_validator_mark_online(dap_chain_esbocs_session_t *a_session, dap_chain_addr_t *a_signing_addr)
{
    ...
    dap_chain_esbocs_penalty_item_t *l_item = NULL;
    dap_ht_find_hh(hh, a_session->penalty, a_signing_addr, sizeof(*a_signing_addr), l_item);
...
static void s_session_directive_process(dap_chain_esbocs_session_t *a_session, dap_chain_esbocs_directive_t *a_directive, dap_hash_sha3_256_t *a_directive_hash)
{
    ...
    case DAP_CHAIN_ESBOCS_DIRECTIVE_LIFT: {
        ...
        dap_chain_esbocs_penalty_item_t *l_item = NULL;
        dap_ht_find_hh(hh, a_session->penalty, l_voting_addr, sizeof(*l_voting_addr), l_item);
...
static void s_session_packet_in(dap_chain_esbocs_session_t *a_session, dap_chain_node_addr_t *a_sender_node_addr, uint8_t *a_data, size_t a_data_size)
{
    ...
    if (a_sender_node_addr) { //Process network messages only
        ...
        dap_ht_find_hh(hh, l_round->message_items, &l_data_hash, sizeof(dap_hash_sha3_256_t), l_message_item_temp);
        ...
        dap_chain_esbocs_message_item_t *l_chain_message, *l_chain_message_tmp;
        dap_ht_foreach_hh(hh, l_round->message_items, l_chain_message, l_chain_message_tmp) {
    ...
    case DAP_CHAIN_ESBOCS_MSG_TYPE_SUBMIT: {
        ...
        if (!dap_chain_addr_compare(&l_signing_addr, &l_session->cur_round.attempt_submit_validator)) {
            ...
            dap_ht_find_hh(hh, l_session->cur_round.message_items, &l_data_hash, sizeof(dap_hash_sha3_256_t), l_rejected_item);
            if (l_rejected_item) {
        ...
        dap_chain_esbocs_store_t *l_store = NULL;
        dap_ht_find_hh(hh, l_session->cur_round.store_items, l_candidate_hash, sizeof(dap_hash_sha3_256_t), l_store);
        if (l_store) {
            const char *l_candidate_hash_str = dap_hash_sha3_256_to_str_static(l_candidate_hash);
        ...
        dap_ht_add_hh(hh, l_session->cur_round.store_items, candidate_hash, l_store);
    ...
    case DAP_CHAIN_ESBOCS_MSG_TYPE_REJECT: {
        ...
        if (!l_store) {
            ...
            dap_ht_find_hh(hh, l_round->message_items, &l_data_hash, sizeof(dap_hash_sha3_256_t), l_unprocessed_item);
