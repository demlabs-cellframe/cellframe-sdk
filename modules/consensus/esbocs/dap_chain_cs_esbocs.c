...
//patch for tests
    ...
    for (size_t j = 0; j < l_auth_certs_count; ++j) {
        uint16_t i = j;
        if (i >= l_auth_certs_count) {
            log_it(L_ERROR, "HOW??? i_count = %u l_auth_certs_count = %llu i_count < l_auth_certs_count %d net %s  chain %s ", i, l_auth_certs_count, i < l_auth_certs_count, a_chain->net_name, a_chain->name);
        }
