            ...
            if(l_tsd->size >= sizeof(dap_pkey_t)) {
                    ...
                    if (!dap_pkey_get_hash(l_pkey, &l_hf)) {
                        ...
                        if (!dap_strcmp(a_hash_out_type, "hex") || !dap_strcmp(a_hash_out_type, "content_hash"))
                            l_hash_str = dap_hash_sha3_256_to_str_new(&l_hf);
                        else
        ...
        case DAP_CHAIN_DATUM_TOKEN_TSD_TYPE_TOTAL_PKEYS_REMOVE: {
            ...
            if(l_tsd->size == sizeof(dap_hash_sha3_256_t) ){
                    char *l_hash_str = (!dap_strcmp(a_hash_out_type,"hex")|| !dap_strcmp(a_hash_out_type, "content_hash"))
                                           ...
                                           : dap_enc_base58_encode_hash_to_str((dap_hash_sha3_256_t*) l_tsd->data);
                             ...
                             int a_version)
{
    ...
    if (l_in_item && dap_hash_sha3_256_is_blank(&l_in_item->header.tx_prev_hash))
        l_is_first = true;
    char l_tmp_buf[DAP_TIME_STR_SIZE];
    dap_hash_sha3_256_str_t l_hash_buf = dap_hash_sha3_256_to_str_static_ex(a_tx_hash, a_hash_out_type);
    ...
    TX_ITEM_ITER_TX(item, l_size, a_datum) {
        ...
        case TX_ITEM_TYPE_IN:
            ...
            if (dap_hash_sha3_256_is_blank(&l_hash_tmp)) {
                l_hash_str = "BLANK";
            } else {
                l_hash_buf = dap_hash_sha3_256_to_str_static_ex(&l_hash_tmp, a_hash_out_type);
        ...
        case TX_ITEM_TYPE_OUT: { // 256
            ...
            dap_chain_addr_str_t l_addr_buf = dap_chain_addr_to_str_static_(&((dap_chain_tx_out_t*)item)->addr);
            if (a_version == 1)
            ...
            dap_json_object_add_object(json_obj_item, a_version == 1 ? "Value": "value", dap_json_object_new_string(l_value_str));
            dap_json_object_add_object(json_obj_item, a_version == 1 ? "Address" : "addr", dap_json_object_new_string(l_addr_buf.s));
        } break;
        ...
        case TX_ITEM_TYPE_PKEY: {
            ...
            dap_hash_sha3_256(l_pkey->pkey, l_pkey->header.size, &l_pkey_hash);
            l_hash_buf = dap_hash_sha3_256_to_str_static_ex(&l_pkey_hash, a_hash_out_type);
            l_hash_str = l_hash_buf.s;
            if (a_version == 1)
        ...
        case TX_ITEM_TYPE_TSD: {
            ...
            dap_json_object_add_object(json_obj_item, a_version == 1 ? "type" : "data_type", dap_json_object_new_uint64(l_tsd->type));
            dap_json_object_add_object(json_obj_item,"size", dap_json_object_new_uint64(l_tsd->size));
        } break;
        ...
        case TX_ITEM_TYPE_OUT_COND: {
            ...
            switch (((dap_chain_tx_out_cond_t*)item)->header.subtype) {
                ...
                case DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_STAKE_EXT_LOCK: {
                    ...
                    dap_hash_sha3_256_str_t l_stake_ext_hash_buf = dap_hash_sha3_256_to_str_static_ex(&l_hash_tmp, a_hash_out_type);
                    dap_json_object_add_string(json_obj_item, "stake_ext_hash", l_stake_ext_hash_buf.s);
                    // Position ID
        ...
        case TX_ITEM_TYPE_VOTE:{
            ...
            dap_hash_sha3_256_str_t l_vote_hash_buf = dap_hash_sha3_256_to_str_struct(&l_vote_item->voting_hash);
            if (a_version == 1)
                dap_json_object_add_object(json_obj_item, "item type", dap_json_object_new_string("VOTE"));
            dap_json_object_add_object(json_obj_item, a_version == 1 ? "Voting hash" : "voting_hash", dap_json_object_new_string(l_vote_hash_buf.s));
