...
/** Wire size of @ref dap_chain_datum_decree_t fixed part before @c data_n_signs (packed). */
#define DAP_CHAIN_DATUM_DECREE_FIXED_WIRE_SIZE sizeof(dap_chain_datum_decree_t)
_Static_assert(DAP_CHAIN_DATUM_DECREE_FIXED_WIRE_SIZE == offsetof(dap_chain_datum_decree_t, data_n_signs),
               "dap_chain_datum_decree_t fixed wire size");

#define DAP_CHAIN_DATUM_DECREE_FIXED_SERIALIZE_MAGIC 0xCF5FF001U

/**
 * @brief Naturally compact in-memory view of @ref dap_chain_datum_decree_t fixed header (matches packed wire).
 */
typedef struct dap_chain_datum_decree_fixed_mem {
    uint16_t decree_version;
    uint8_t ts_created_wire[sizeof(dap_time_t)];
    uint16_t header_type;
    uint8_t srv_or_common_wire[24];
    uint16_t sub_type;
    uint8_t data_size_wire[sizeof(uint32_t)];
    uint8_t signs_size_wire[sizeof(uint32_t)];
} dap_chain_datum_decree_fixed_mem_t;

_Static_assert(sizeof(dap_chain_datum_decree_fixed_mem_t) == DAP_CHAIN_DATUM_DECREE_FIXED_WIRE_SIZE,
               "dap_chain_datum_decree_fixed_mem_t wire size");

extern const dap_serialize_field_t g_dap_chain_datum_decree_fixed_fields[];
extern const size_t g_dap_chain_datum_decree_fixed_field_count;
extern const dap_serialize_schema_t g_dap_chain_datum_decree_fixed_schema;

static inline int dap_chain_datum_decree_fixed_pack(const dap_chain_datum_decree_fixed_mem_t *a_mem, uint8_t *a_wire,
                                                    size_t a_wire_size)
{
    if (!a_mem || !a_wire || a_wire_size < DAP_CHAIN_DATUM_DECREE_FIXED_WIRE_SIZE)
        return -1;
    dap_serialize_result_t l_r =
        dap_serialize_to_buffer_raw(&g_dap_chain_datum_decree_fixed_schema, a_mem, a_wire, a_wire_size, NULL);
    return l_r.error_code;
}

static inline int dap_chain_datum_decree_fixed_unpack(const uint8_t *a_wire, size_t a_wire_size,
                                                      dap_chain_datum_decree_fixed_mem_t *a_mem)
{
    if (!a_wire || !a_mem || a_wire_size < DAP_CHAIN_DATUM_DECREE_FIXED_WIRE_SIZE)
        return -1;
    dap_deserialize_result_t l_r =
        dap_deserialize_from_buffer_raw(&g_dap_chain_datum_decree_fixed_schema, a_wire, a_wire_size, a_mem, NULL);
    return l_r.error_code;
}
 ...
 * @param a_hash_out_type
 */
void dap_chain_datum_decree_dump_json(dap_json_t *a_obj_out, const void *a_decree, size_t a_decree_size, const char *a_hash_out_type, int a_version);
