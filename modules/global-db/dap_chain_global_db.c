...
dap_store_obj_t* dap_chain_global_db_cond_load(const char *a_group, uint64_t a_first_id, size_t *a_objs_count)
{
    ...
    unlock();
    if (*a_objs_count > 1)
        qsort(l_store_obj, *a_objs_count, sizeof(dap_store_obj_t), dap_chain_global_db_compare_by_ts);
    return l_store_obj;
}
...
dap_global_db_obj_t* dap_chain_global_db_gr_load(const char *a_group, size_t *a_records_count_out)
{
    ...
    if (l_count > 1)
        qsort(l_store_obj, l_count, sizeof(dap_store_obj_t), dap_chain_global_db_compare_by_ts);
...
int dap_chain_global_db_compare_by_ts(const void *a_obj1, const void *a_obj2) {
