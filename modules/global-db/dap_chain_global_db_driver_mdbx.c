...
typedef struct __db_ctx__ {
        ...
        char name[DAP$SZ_MAXGROUPNAME + 1];                                 /* Group's name */
...
dap_db_ctx_t *l_db_ctx, *l_db_ctx2;
    ...
    if ( (l_rc = strlen(a_group)) > DAP$SZ_MAXGROUPNAME )                   /* Check length of the group name */
        return  log_it(L_ERROR, "Group name '%s' is too long (%d>%d)", a_group, l_rc, DAP$SZ_MAXGROUPNAME), NULL;
...
DIR *dir;
    ...
    log_it(L_NOTICE, "Set maximum number of local groups: %d", DAP$K_MAXGROUPS);
    assert ( !mdbx_env_set_maxdbs (s_mdbx_env, DAP$K_MAXGROUPS) );          /* Set maximum number of the file-tables (MDBX subDB)
...
struct  __record_suffix__   *l_suff;
    ...
    if (a_store_obj->type == DAP_DB$K_OPTYPE_ADD ) {
        ...
        l_key.iov_len =  a_store_obj->key_len ? a_store_obj->key_len : strnlen(a_store_obj->key, DAP$SZ_MAXKEY);
