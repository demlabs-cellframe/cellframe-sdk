...
static void *s_list_thread_proc(void *arg)
{
    ...
    for (dap_list_t *l_groups = l_dap_db_log_list->groups; l_groups; l_groups = dap_list_next(l_groups)) {
        ...
        while (l_group_cur->count && l_dap_db_log_list->is_process) { // Number of records to be synchronized
            ...
            for (size_t i = 0; i < l_item_count; i++) {
                ...
                l_objs_total_size += sizeof(dap_db_log_list_obj_t) + sizeof(dap_store_obj_pkt_t) + l_list_obj->pkt->data_size;
    ...
    pthread_mutex_lock(&l_dap_db_log_list->list_mutex);
    log_it(L_DEBUG, "! l_dap_db_log_list->is_process = false");
...
dap_db_log_list_t* dap_db_log_list_start(dap_chain_net_t *a_net, dap_chain_node_addr_t a_addr, int a_flags)
{
    ...
    if(a_net && a_net->pub.name && a_net->pub.name[0]!='\0') {
        l_net_name = a_net->pub.name;
    }
    ...
    log_it(L_DEBUG, "! l_dap_db_log_list->is_process = true");
...
{
char l_node_addr_str [512];
size_t l_id_len = 0;
uint8_t *l_id;
uint64_t l_ret_id = 0;

    dap_snprintf(l_node_addr_str, sizeof(l_node_addr_str) - 1, "%ju%s", a_node_addr, a_group);

    if ( (l_id = dap_chain_global_db_gr_get( l_node_addr_str, &l_id_len, GROUP_LOCAL_NODE_LAST_ID)) )
    {
        if (l_id_len == sizeof(uint64_t))
            l_ret_id = *((uint64_t *) l_id);
...
{
char l_node_chain_str[512];
size_t l_hash_len = 0;

    dap_snprintf(l_node_chain_str, sizeof(l_node_chain_str) - 1, "%ju%s%s", a_node_addr, a_chain->net_name, a_chain->name);

    return  (dap_chain_hash_fast_t *) dap_chain_global_db_gr_get((const char*)l_node_chain_str, &l_hash_len, GROUP_LOCAL_NODE_LAST_ID);
}
 ...
 */
static inline size_t dap_db_get_size_pdap_store_obj_t(pdap_store_obj_t store_obj)
{
