...
bool dap_ledger_datum_is_enforced(dap_ledger_t *a_ledger, dap_hash_sha3_256_t *a_hash, bool a_accept) {
    ...
    dap_ht_find_hh(hh, a_accept ? PVT(a_ledger)->hal_items : PVT(a_ledger)->hrl_items, a_hash->raw, DAP_HASH_SHA3_256_SIZE, l_wanted);
...
dap_json_t *dap_ledger_token_tx_item_list(dap_ledger_t * a_ledger, dap_chain_addr_t *a_addr, const char *a_hash_out_type, bool a_unspent_only)
{
    ...
    dap_ht_foreach_hh(hh, l_ledger_pvt->ledger_items, l_tx_item, l_tx_tmp) {
...
dap_json_t *dap_ledger_threshold_info(dap_ledger_t *a_ledger, size_t a_limit, size_t a_offset, dap_hash_sha3_256_t *a_threshold_hash, bool a_head, int a_version)
{
    ...
    if (a_threshold_hash) {
        ...
        dap_ht_find_hh(hh, l_ledger_pvt->threshold_txs, a_threshold_hash, sizeof(dap_hash_sha3_256_t), l_tx_item);
        if (l_tx_item) {
        ...
        size_t i_tmp = 0;
        if (a_head)
        dap_ht_foreach_hh(hh, l_ledger_pvt->threshold_txs, l_tx_item, l_tx_tmp) {
...
dap_json_t *dap_ledger_balance_info(dap_ledger_t *a_ledger, size_t a_limit, size_t a_offset, bool a_head, int a_version)
{
    ...
    size_t i_tmp = 0;
    if (a_head)
        dap_ht_foreach_hh(hh, l_ledger_pvt->balance_accounts, l_balance_item, l_balance_tmp) {
            if (i_tmp < l_arr_start) {
                i_tmp++;
                continue;
            }
...
int dap_ledger_pvt_threshold_txs_add(dap_ledger_t *a_ledger, dap_chain_datum_tx_t *a_tx, dap_hash_sha3_256_t *a_tx_hash)
{
    ...
    dap_ht_find_by_hashvalue_hh(hh, l_ledger_pvt->threshold_txs, a_tx_hash, sizeof(*a_tx_hash), l_hash_value, l_item);
    ...
    if (!l_item) {
        ...
        dap_ht_add_by_hashvalue_hh(hh, l_ledger_pvt->threshold_txs, tx_hash_fast, sizeof(dap_hash_sha3_256_t), l_hash_value, l_item);
...
static void s_threshold_txs_free(dap_ledger_t *a_ledger)
{
    ...
    dap_ht_foreach_hh(hh, l_pvt->threshold_txs, l_current, l_tmp) {
                                                      ...
                                                      dap_global_db_obj_t *a_values, void *a_arg)
{
    ...
    for (size_t i = 0; i < a_values_count; i++) {
        ...
        dap_ht_add_keyptr_hh(hh, l_ledger_pvt->balance_accounts, l_balance_item->key,
...
static dap_ledger_t *s_ledger_registry = NULL; // Using dap_ht
...
dap_ledger_t *dap_ledger_find_by_net_id(dap_chain_net_id_t a_net_id)
{
    ...
    dap_ht_foreach_hh(hh, s_ledger_registry, l_ledger, l_tmp) {
...
void dap_ledger_set_hal_hrl(dap_ledger_t *a_ledger, dap_chain_t *a_chain, const char **a_whitelist, uint16_t a_whitelist_size, const char **a_blacklist, uint16_t a_blacklist_size)
{
    ...
    for (uint16_t i = 0; i < a_blacklist_size; ++i) {
        ...
        dap_ht_add_hh(hh, l_ledger_pvt->hrl_items, hash_field.hash_key, l_item);
    }
    ...
    for (uint16_t i = 0; i < a_whitelist_size; ++i) {
        ...
        dap_ht_add_hh(hh, l_ledger_pvt->hal_items, hash_field.hash_key, l_item);
    }
...
bool dap_ledger_deduct_tx_tag(dap_ledger_t *a_ledger, dap_chain_datum_tx_t *a_tx, char **a_service_name, dap_chain_srv_uid_t *a_tag, dap_chain_tx_tag_action_type_t *a_action)
{
    ...
    pthread_rwlock_rdlock(&s_services_rwlock);
    dap_ht_foreach_hh(hh, s_services , l_sinfo_current, l_sinfo_tmp) {
...
void dap_ledger_tx_purge(dap_ledger_t *a_ledger, bool a_preserve_db)
{
    ...
    char *l_gdb_group;
    dap_ht_foreach_hh(hh, l_ledger_pvt->ledger_items , l_item_current, l_item_tmp) {
    ...
    dap_ht_foreach_hh(hh, l_ledger_pvt->balance_accounts, l_balance_current, l_balance_tmp) {
    ...
    dap_ht_foreach_hh(hh, l_ledger_pvt->emissions_for_stake_lock, l_stake_item_current, l_stake_item_tmp) {
    ...
    dap_ht_foreach_hh(hh, l_ledger_pvt->threshold_txs, l_item_current, l_item_tmp) {
...
void dap_ledger_token_purge(dap_ledger_t *a_ledger, bool a_preserve_db)
{
    ...
    dap_ht_foreach_hh(hh, l_ledger_pvt->tokens, l_token_current, l_token_tmp) {
        dap_ht_del(l_ledger_pvt->tokens, l_token_current);
        pthread_rwlock_wrlock(&l_token_current->token_emissions_rwlock);
        dap_ht_foreach_hh(hh, l_token_current->token_emissions, l_emission_current, l_emission_tmp) {
                                ...
                                uint16_t a_chain_type, void *a_chain_ptr)
{
    ...
    dap_ht_add_hh(hh, a_ledger->chains_registry, chain_id, l_info);
...
void dap_ledger_unregister_chain(dap_ledger_t *a_ledger, dap_chain_id_t a_chain_id)
{
    ...
    dap_ht_find_hh(hh, a_ledger->chains_registry, &a_chain_id, sizeof(dap_chain_id_t), l_info);
    if (l_info) {
...
dap_chain_info_t* dap_ledger_get_chain_info_by_name(dap_ledger_t *a_ledger, const char *a_chain_name)
{
    ...
    dap_ht_foreach_hh(hh, a_ledger->chains_registry, l_info, l_tmp) {
