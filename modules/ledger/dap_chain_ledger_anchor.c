...
int dap_ledger_anchor_load(dap_ledger_t *a_ledger, dap_chain_datum_anchor_t *a_anchor, dap_chain_id_t a_chain_id, dap_hash_sha3_256_t *a_anchor_hash)
{
    ...
    dap_ht_find_by_hashvalue(l_ledger_pvt->anchors, a_anchor_hash, sizeof(dap_hash_sha3_256_t), l_hash_value, l_new_anchor);
    if (!l_new_anchor) {
        ...
        dap_ht_add_by_hashvalue(l_ledger_pvt->anchors, anchor_hash, sizeof(dap_hash_sha3_256_t), l_hash_value, l_new_anchor);
    }
    pthread_rwlock_unlock(&l_ledger_pvt->decrees_rwlock);
    return ret_val;
}
...
int dap_ledger_anchor_purge(dap_ledger_t *a_ledger, dap_chain_id_t a_chain_id)
{
    ...
    dap_ledger_private_t *l_ledger_pvt = PVT(a_ledger);
    pthread_rwlock_wrlock(&l_ledger_pvt->decrees_rwlock);
    dap_ht_foreach(l_ledger_pvt->anchors, it, tmp) {
        ...
        if (!dap_chain_datum_anchor_get_hash_from_data(it->anchor, &l_decree_hash)) {
            ...
            dap_ht_find(l_ledger_pvt->decrees, &l_decree_hash, sizeof(dap_hash_sha3_256_t), l_decree);
            if (l_decree) {
...
static dap_ledger_anchor_item_t *s_find_anchor(dap_ledger_t *a_ledger, dap_hash_sha3_256_t *a_anchor_hash)
{
    ...
    dap_ht_find(l_ledger_pvt->anchors, a_anchor_hash, sizeof(dap_hash_sha3_256_t), l_anchor);
    pthread_rwlock_unlock(&l_ledger_pvt->decrees_rwlock);
    return l_anchor;
}
