...
static void s_tx_hash_processed_ht_free(dap_chain_tx_hash_processed_ht_t **a_hash_processed)
{
    ...
    dap_ht_foreach_hh(hh, *a_hash_processed, l_current, l_tmp) {
                              ...
                              int a_ret_code, dap_chain_tx_tag_action_type_t a_action, dap_chain_srv_uid_t a_uid)
{
    ...
    dap_ht_find_hh(hh, *a_tx_data_ht, a_tx_hash, sizeof(*a_tx_hash), l_tx_data);
    if (l_tx_data)  // this tx already present in ledger (double)
        ...
        dap_ht_add_hh(hh, *a_tx_data_ht, hash, l_tx_data);
...
static size_t dap_db_net_history_token_list(dap_json_t *a_json_arr_reply, dap_ledger_t *a_ledger, const char *a_token_name, const char *a_hash_out_type, dap_json_t *a_obj_out, int a_version) {
    ...
    dap_ht_foreach_hh(hh, a_ledger->chains_registry, l_chain_info, l_tmp) {
...
int com_tx_create_json(int a_argc, char ** a_argv, dap_json_t *a_json_arr_reply, int a_version)
{
    ...
    dap_chain_t *l_chain = NULL;
    if (l_chain_name) {
        dap_chain_info_t *l_chain_info = dap_ledger_get_chain_info_by_name(l_ledger, l_chain_name);
        if (l_chain_info)
            l_chain = (dap_chain_t *)l_chain_info->chain_ptr;
    }
    if(!l_chain) {
...
int com_tx_cond_create(int a_argc, char ** a_argv, dap_json_t *a_json_arr_reply, int a_version)
{
    ...
    if (l_cert_str) {
        ...
        dap_cert_get_pkey_hash(l_cert_cond, DAP_HASH_TYPE_SHA3_256, (byte_t *)&l_pkey_cond_hash, sizeof(l_pkey_cond_hash));
    } else {
