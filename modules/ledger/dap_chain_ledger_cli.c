                                          ...
                                          dap_chain_hash_fast_t *a_tx_hash)
{
    ...
    TX_ITEM_ITER_TX_TYPE(l_item, TX_ITEM_TYPE_OUT_ALL, l_size, i, a_datum) {
        ...
        for (dap_list_t *it = l_trackers; it; it = it->next) {
            ...
            dap_json_t *l_json_obj_tracker = json_object_new_object();
            ...
            dap_json_t *l_json_arr_tracker_items = json_object_new_array();
            ...
            for (dap_ledger_tracker_item_t *l_item = l_tracker->items; l_item; l_item = l_item->next) {
                dap_json_t *l_json_obj_tracker_item = json_object_new_object();
...
{
    dap_json_t* json_obj_datum = json_object_new_array();
    if (!json_obj_datum){
        log_it(L_CRITICAL, "%s", c_error_memory_alloc);
    ...
    while (l_datum || l_cur_tx_cache){
        ...
        dap_json_t * j_arr_data = json_object_new_array();
        dap_json_t * j_obj_tx = json_object_new_object();
        ...
        for (dap_list_t *it = l_list_out_items; it; it = it->next) {
            ...
            if (l_dst_addr && dap_chain_addr_compare(l_dst_addr, a_addr)) {  
                ...
                dap_json_t * j_obj_data = json_object_new_object();
                if (!j_obj_data) {
                    dap_json_rpc_allocation_error(a_json_arr_reply);
                    json_object_put(j_obj_tx);
                    json_object_put(j_arr_data);
...
next_step:
    ...
    if (json_object_array_length(json_obj_datum) == 2) {
        dap_json_t * json_empty_tx = json_object_new_object();
        if (!json_empty_tx) {
            dap_json_rpc_allocation_error(a_json_arr_reply);
            json_object_put(json_obj_datum);
            return NULL;
 ...
 * @return char*
 */
static dap_json_t* dap_db_chain_history_token_list(dap_json_t* a_json_arr_reply, dap_chain_t * a_chain, const char *a_token_name, const char *a_hash_out_type, size_t *a_token_num)
{
    dap_json_t *l_jobj_tickers = json_object_new_object();
    if (!a_chain->callback_datum_iter_create) {
    ...
    for (dap_chain_datum_t *l_datum = a_chain->callback_datum_iter_get_first(l_datum_iter);
        ...
        if (!l_jobj_ticker) {
            ...
            dap_ledger_t *l_ledger = dap_chain_net_by_id(a_chain->net_id)->pub.ledger;
            dap_json_t *l_current_state = dap_ledger_token_info_by_name(l_ledger, l_token->ticker);
        ...
        dap_json_t* json_history_token = json_object_new_object();
...
static size_t dap_db_net_history_token_list(dap_json_t* a_json_arr_reply, dap_chain_net_t * l_net, const char *a_token_name, const char *a_hash_out_type, dap_json_t* a_obj_out) {
    size_t l_token_num_total = 0;
    dap_chain_t *l_chain_cur;
    dap_json_t* json_arr_obj_tx = json_object_new_array();    
...
int com_ledger(int a_argc, char ** a_argv, void **reply)
{
    ...
    if(l_cmd == CMD_LIST){
        ...
        if (l_sub_cmd == SUB_CMD_LIST_LEDGER_THRESHOLD) {
            dap_json_t* json_obj_out = dap_ledger_threshold_info(l_ledger, l_limit, l_offset, NULL, l_head);
            if (json_obj_out){
        ...
        if (l_sub_cmd == SUB_CMD_LIST_LEDGER_THRESHOLD_WITH_HASH) {
            dap_json_t *json_obj_out = dap_ledger_threshold_info(l_ledger, 0, 0, &l_tx_threshold_hash, l_head);
            if (json_obj_out){
                json_object_array_add(*a_json_arr_reply, json_obj_out);
            }
            return 0;
        ...
        if (l_sub_cmd == SUB_CMD_LIST_LEDGER_BALANCE) {
            dap_json_t *json_obj_out = dap_ledger_balance_info(l_ledger, l_limit, l_offset, l_head);
            if (json_obj_out){
                json_object_array_add(*a_json_arr_reply, json_obj_out);
            }
            return 0;
        }
        dap_json_t *json_obj_datum = dap_ledger_token_info(l_ledger, l_limit, l_offset);
        ...
        dap_json_t* json_datum = json_object_new_object();
...
int com_tx_create_json(int a_argc, char ** a_argv, void **reply)
{
    ...
    dap_json_t *l_jobj_errors = json_object_new_array();
    ...
    if ((rc = dap_ledger_tx_add_check(l_net->pub.ledger, (dap_chain_datum_tx_t*)l_datum_tx->data, l_tx_size, &l_hf_tx))) {
        ...
        dap_json_t *l_jobj_total_items = json_object_new_uint64(l_items_count);
        dap_json_t *l_jobj_ledger_ret_code = json_object_new_object();
...
//    const char *value_str = NULL;
    ...
    if (l_emission_hash_str) {
        ...
        dap_json_t *l_jobj_emission = json_object_new_object();
    ...
    dap_json_t *l_jobj_result = json_object_new_object();
...
int com_tx_history(int a_argc, char ** a_argv, void **a_str_reply)
{
    ...
    if (l_tx_hash_str) {
        ...
        dap_json_t * json_obj_summary = json_object_new_object();
        if (!json_obj_summary) {
        ...
        dap_json_t * json_count_obj= json_object_new_object();
...
//    dap_string_t *l_reply_str = dap_string_new("");
    dap_json_t *l_jobj_tx_list_cond_outs = json_object_new_array();
    ...
    for (dap_list_t *it = l_tx_list; it; it = it->next) {
        ...
        dap_json_t *l_jobj_hash = json_object_new_string(l_hash_str);
        dap_json_t *l_jobj_remain = json_object_new_object();
        ...
        dap_json_t *l_jobj_tx = json_object_new_object();
    ...
    dap_json_t *l_jobj_total = json_object_new_object();
