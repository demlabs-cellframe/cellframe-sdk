                      ...
                      dap_hash_sha3_256_t *a_token_update_hash)
{
    ...
    for (uint16_t l_signs_passed = 0; l_signs_passed < l_token->signs_total; l_signs_passed++) {
        ...
        if (l_signs_offset + l_signs_size + sizeof(dap_sign_t) > l_token_size ||
            ...
            log_it(L_WARNING, "Incorrect size %"DAP_UINT64_FORMAT_U" of datum token, expected at least %"DAP_UINT64_FORMAT_U, l_token_size,
    ...
    if (l_token_size != l_signs_offset + l_signs_size) {
        log_it(L_WARNING, "Incorrect size %"DAP_UINT64_FORMAT_U" of datum token, expected %"DAP_UINT64_FORMAT_U, l_token_size, l_signs_offset + l_signs_size);
        DAP_DELETE(l_token);
        return DAP_LEDGER_CHECK_INVALID_SIZE;
    }
