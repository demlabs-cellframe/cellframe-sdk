        ...
        dap_hash_fast_t *a_tx_cond_hash, uint256_t a_value, uint256_t a_value_fee, const char *a_hash_out_type)
{
    ...
    if (!l_tx_ticker) {
        log_it(L_ERROR, "Can't get token ticker from TX (TX may still be in mempool, wait for it to be included in a block)");
        return NULL;
    }
    ...
    if (dap_chain_wallet_cache_tx_find_outs_with_val(l_ledger->net, l_tx_ticker, &l_addr_from, &l_list_used_out, l_value_need, &l_value_transfer) == -101)
        l_list_used_out = dap_chain_wallet_get_list_tx_outs_with_val(l_ledger, l_tx_ticker, &l_addr_from, l_value_need, &l_value_transfer);
    if (!l_list_used_out) {
    ...
    if (!l_refill_native) {
        if (dap_chain_wallet_cache_tx_find_outs_with_val(l_ledger->net, a_net->pub.native_ticker, &l_addr_from, &l_list_fee_out, l_fee_total, &l_fee_transfer) == -101)
            l_list_fee_out = dap_chain_wallet_get_list_tx_outs_with_val(l_ledger, a_net->pub.native_ticker, &l_addr_from, l_fee_total, &l_fee_transfer);
        if (!l_list_fee_out) {
            dap_list_free_full(l_list_used_out, NULL);
