...
const dap_chain_net_srv_order_t *dap_chain_net_srv_order_check(const char *a_order_hash_str, const byte_t *a_order, size_t a_order_size)
{
    ...
    dap_return_val_if_fail(l_order && a_order_size >= sizeof(dap_chain_net_srv_order_t) + sizeof(dap_sign_t) &&
                           l_order->version == ORDER_CURRENT_VERSION && l_order->direction <= SERV_DIR_SELL, NULL);
...
* @param a_str_out
*/
void dap_chain_net_srv_order_dump_to_json(const dap_chain_net_srv_order_t *a_order, json_object *a_jobj_out,
                                            const char *a_hash_out_type, const char *a_native_ticker)
{
    if (a_order && a_jobj_out ){
        dap_chain_hash_fast_t l_hash;
        dap_hash_fast(a_order, dap_chain_net_srv_order_get_size(a_order), &l_hash);
        const char *l_hash_str = dap_strcmp(a_hash_out_type,"hex")
                                 ? dap_enc_base58_encode_hash_to_str_static(&l_hash)
        ...
        json_object_object_add(a_jobj_out, "hash", json_object_new_string(l_hash_str));
        json_object_object_add(a_jobj_out, "version", json_object_new_uint64(a_order->version));

        switch ( a_order->direction) {
            case SERV_DIR_UNDEFINED:
                json_object_object_add(a_jobj_out, "direction", json_object_new_string("SERV_DIR_UNDEFINED"));
                break;
            case SERV_DIR_SELL:
                json_object_object_add(a_jobj_out, "direction", json_object_new_string("SSERV_DIR_SELL"));
                break;
            case SERV_DIR_BUY:
                json_object_object_add(a_jobj_out,"direction", json_object_new_string("SERV_DIR_BUY"));
                break;
        }
        char buf_time[DAP_TIME_STR_SIZE];
        dap_time_to_str_rfc822(buf_time, DAP_TIME_STR_SIZE, a_order->ts_created);
        json_object_object_add(a_jobj_out, "created", json_object_new_string(buf_time));
        char *l_f_srv_uid = dap_strdup_printf("0x%016"DAP_UINT64_FORMAT_X, a_order->srv_uid.uint64);
        json_object_object_add(a_jobj_out, "srv_uid", json_object_new_string(l_f_srv_uid));
        DAP_DELETE(l_f_srv_uid);

        const char *l_balance_coins, *l_balance = dap_uint256_to_char(a_order->price, &l_balance_coins);
        json_object *l_jobj_price = json_object_new_object();
        json_object_object_add(l_jobj_price, "coins", json_object_new_string(l_balance_coins));
        json_object_object_add(l_jobj_price, "value", json_object_new_string(l_balance));
        json_object_object_add(a_jobj_out,"price", l_jobj_price);
        json_object_object_add(a_jobj_out, "price_token", json_object_new_string((*a_order->price_ticker) ? a_order->price_ticker: a_native_ticker));
        json_object_object_add(a_jobj_out, "units", json_object_new_uint64(a_order->units));
        if( a_order->price_unit.uint32 )
            json_object_object_add(a_jobj_out, "price_unit", json_object_new_string(dap_chain_net_srv_price_unit_uid_to_str(a_order->price_unit)));
        if ( a_order->node_addr.uint64) {
            char *l_f_node_addr = dap_strdup_printf(NODE_ADDR_FP_STR, NODE_ADDR_FP_ARGS_S(a_order->node_addr));
            json_object_object_add(a_jobj_out, "node_addr", json_object_new_string(l_f_node_addr));
            DAP_DELETE(l_f_node_addr);
        }
        ...
        if(dap_chain_net_srv_order_get_continent_region(a_order, &l_continent_num, &l_region))
            l_continent_str = dap_chain_net_srv_order_continent_to_str(l_continent_num);
        json_object *l_jobj_location = json_object_new_object();
        json_object_object_add(l_jobj_location, "continent", json_object_new_string(l_continent_str ? l_continent_str : "None"));
        json_object_object_add(l_jobj_location, "region", json_object_new_string(l_region ? l_region : "None"));
        json_object_object_add(a_jobj_out, "node_location", l_jobj_location);
        DAP_DELETE(l_region);

        l_hash_str = dap_strcmp(a_hash_out_type, "hex")
                     ? dap_enc_base58_encode_hash_to_str_static(&a_order->tx_cond_hash)
                     : dap_chain_hash_fast_to_str_static(&a_order->tx_cond_hash);
        json_object_object_add(a_jobj_out, "tx_cond_hash", json_object_new_string(l_hash_str));
        char *l_ext_out = a_order->ext_size ? DAP_NEW_Z_SIZE(char, a_order->ext_size * 2 + 1) : NULL;
        if(l_ext_out) {
            dap_bin2hex(l_ext_out, a_order->ext_n_sign, a_order->ext_size);
            char *l_ext_str = dap_strdup_printf("0x%s", l_ext_out);
            json_object_object_add(a_jobj_out, "ext", json_object_new_string(l_ext_str));
            DAP_DELETE(l_ext_str);
        }
        else
            json_object_object_add(a_jobj_out, "ext", json_object_new_string("0x0"));
        dap_sign_t *l_sign = (dap_sign_t*)((byte_t*)a_order->ext_n_sign + a_order->ext_size);
        dap_hash_fast_t l_sign_pkey = {0};
        dap_sign_get_pkey_hash(l_sign, &l_sign_pkey);
        const char *l_sign_pkey_hash_str = dap_hash_fast_to_str_static(&l_sign_pkey);
        json_object_object_add(a_jobj_out, "pkey", json_object_new_string(l_sign_pkey_hash_str));
        DAP_DELETE(l_ext_out);
    }
}
