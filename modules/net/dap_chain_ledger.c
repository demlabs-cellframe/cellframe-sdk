    ...
    while ( (l_dir_entry = readdir(l_chains_dir) )!= NULL ){
        ...
        if (strlen(l_entry_name) > 4) {
            if ( strncmp (l_entry_name + strlen(l_entry_name)-4,".cfg",4) == 0 ) { // its .cfg file
                ...
                if (l_whitelist_size + l_blacklist_size) {
                    dap_ledger_hal_item_t **l_uncond_list = DAP_NEW_Z_COUNT(dap_ledger_hal_item_t*, l_whitelist_size + l_blacklist_size);
                    for (i = 0; i < l_whitelist_size; ++i) {
                        dap_chain_hash_fast_from_str(l_whitelist[i], &l_uncond_list[i]->hash);
                        HASH_ADD(hh, l_ledger_pvt->hal_items, hash, sizeof(dap_hash_fast_t), l_uncond_list[i]);
                    }
                    for (i = 0; i < l_blacklist_size; ++i) {
                        dap_chain_hash_fast_from_str(l_blacklist[i], &l_uncond_list[i + l_whitelist_size]->hash);
                        HASH_ADD(hh, l_ledger_pvt->hrl_items, hash, sizeof(dap_hash_fast_t), l_uncond_list[i + l_whitelist_size]);
                    }
                }
                dap_config_close(l_cfg);
                log_it(L_DEBUG, "Chain %s has %d datums in HAL and %d datums in HRL", l_entry_name, l_whitelist_size, l_blacklist_size);
            }
        }
