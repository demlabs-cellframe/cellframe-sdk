...
typedef struct dap_ledger_private {
    ...
    // Wallets cache
    dap_ledger_wallet_cache_t *wallets_cache;
    pthread_rwlock_t wallet_cache_rwlock;
} dap_ledger_private_t;

#define PVT(a) ( (dap_ledger_private_t *) a->_internal )

static  dap_chain_datum_tx_t* tx_item_find_by_addr(dap_ledger_t *a_ledger,
...
static dap_ledger_wallets_cache_type_t s_wallets_cache_type = DAP_LEDGER_WALLET_CACHE_TYPE_DISABLED;
...
int dap_ledger_init()
{
    ...
    if (l_walet_cache_type_str){
        if (dap_strcmp(l_walet_cache_type_str, "disable")){
            s_wallets_cache_type = DAP_LEDGER_WALLET_CACHE_TYPE_DISABLED;
        } else if (dap_strcmp(l_walet_cache_type_str, "local")){
            s_wallets_cache_type = DAP_LEDGER_WALLET_CACHE_TYPE_LOCAL;
        } else if (dap_strcmp(l_walet_cache_type_str, "all")){
            s_wallets_cache_type = DAP_LEDGER_WALLET_CACHE_TYPE_ALL;
        }
    }
...
static dap_ledger_t *dap_ledger_handle_new(void)
{
    ...
    pthread_rwlock_init(&l_ledger_pvt->wallet_cache_rwlock, NULL);
...
static bool s_check_wallet_is_local (dap_chain_addr_t *a_addr){
    return dap_chain_wallet_addr_cache_get_name(a_addr) != NULL ? true : false;
}


static int s_save_declined_tx_into_wallet_cache(dap_ledger_t* a_ledger, dap_chain_datum_tx_t *a_tx, 
                                                ...
                                                dap_chain_net_srv_uid_t a_tag, dap_chain_tx_tag_action_type_t a_action)
{
    ...
    if (s_wallets_cache_type == DAP_LEDGER_WALLET_CACHE_TYPE_DISABLED){
        return -101;
    }
    ...
    for (dap_list_t *it=l_out_list; it; it=it->next, l_out_idx++){
        ...
        if(!dap_chain_addr_is_blank(&l_addr)){
            if (s_wallets_cache_type == DAP_LEDGER_WALLET_CACHE_TYPE_LOCAL &&
                !s_check_wallet_is_local(&l_addr))
                continue;

            pthread_rwlock_wrlock(&l_ledger_pvt->wallet_cache_rwlock);
            dap_ledger_wallet_cache_t *l_wallet_item = NULL;
            HASH_FIND(hh, l_ledger_pvt->wallets_cache, &l_addr, sizeof(dap_chain_addr_t), l_wallet_item);
            if (!l_wallet_item){
                l_wallet_item = DAP_NEW_Z(dap_ledger_wallet_cache_t);
                l_wallet_item->wallet_addr = l_addr;
                HASH_ADD(hh, l_ledger_pvt->wallets_cache, wallet_addr, sizeof(dap_chain_addr_t), l_wallet_item);
            }
    ...
    for (dap_list_t *it=l_out_list; it; it=it->next ){
        ...
        if(l_cond_type == TX_ITEM_TYPE_IN){
            ...
            if(!dap_chain_addr_is_blank(&l_addr_from)){
                if (s_wallets_cache_type == DAP_LEDGER_WALLET_CACHE_TYPE_LOCAL &&
                    !s_check_wallet_is_local(&l_addr_from))
                    continue;

                pthread_rwlock_wrlock(&l_ledger_pvt->wallet_cache_rwlock);
                dap_ledger_wallet_cache_t *l_wallet_item = NULL;
                HASH_FIND(hh, l_ledger_pvt->wallets_cache, &l_addr_from, sizeof(dap_chain_addr_t), l_wallet_item);
                if (!l_wallet_item){
                    l_wallet_item = DAP_NEW_Z(dap_ledger_wallet_cache_t);
...
int dap_ledger_tx_add(dap_ledger_t *a_ledger, dap_chain_datum_tx_t *a_tx, dap_hash_fast_t *a_tx_hash, bool a_from_threshold, dap_ledger_datum_iter_data_t *a_datum_index_data)
{
    ...
    if( (l_ret_check = s_tx_cache_check(a_ledger, a_tx, a_tx_hash, a_from_threshold,
        ...
        s_save_declined_tx_into_wallet_cache(a_ledger, a_tx, a_tx_hash, l_ret_check, l_main_token_ticker, l_tag, l_action);
    ...
    for (dap_list_t *it = l_list_bound_items; it; it = it->next) {
        ...
        case TX_ITEM_TYPE_IN: {
            ...
            // Save input into appropriate wallet cache
            if(!dap_chain_addr_is_blank(&l_bound_item->in.addr_from) && 
                ((s_wallets_cache_type == DAP_LEDGER_WALLET_CACHE_TYPE_LOCAL &&
                    s_check_wallet_is_local(&l_bound_item->in.addr_from)) || s_wallets_cache_type == DAP_LEDGER_WALLET_CACHE_TYPE_ALL))
            {
                pthread_rwlock_wrlock(&l_ledger_pvt->wallet_cache_rwlock);
                dap_ledger_wallet_cache_t *l_wallet_item = NULL;
                HASH_FIND(hh, l_ledger_pvt->wallets_cache, &l_bound_item->in.addr_from, sizeof(dap_chain_addr_t), l_wallet_item);
                if (!l_wallet_item){
                    l_wallet_item = DAP_NEW_Z(dap_ledger_wallet_cache_t);
                    l_wallet_item->wallet_addr = l_bound_item->in.addr_from;
                    HASH_ADD(hh, l_ledger_pvt->wallets_cache, wallet_addr, sizeof(dap_chain_addr_t), l_wallet_item);
                }
                dap_ledger_wallet_tx_cache_t *l_wallet_tx_item = NULL;
                HASH_FIND(hh, l_wallet_item->wallet_txs, a_tx_hash, sizeof(dap_hash_fast_t), l_wallet_tx_item);
                if (!l_wallet_tx_item){
                    ...
                    l_wallet_tx_item->tx_hash = *a_tx_hash;
                    l_wallet_tx_item->tx = a_tx;
                    dap_strncpy(l_wallet_tx_item->token_ticker, l_main_token_ticker, DAP_CHAIN_TICKER_SIZE_MAX);
                    l_wallet_tx_item->multichannel = false;
                    l_wallet_tx_item->ret_code = l_ret;
                    l_wallet_tx_item->tag = l_tag;
                    l_wallet_tx_item->action = l_action;
                    HASH_ADD(hh, l_wallet_item->wallet_txs, tx_hash, sizeof(dap_hash_fast_t), l_wallet_tx_item);
                }
                dap_ledger_wallet_tx_cache_input_t *l_tx_in = DAP_NEW_Z(dap_ledger_wallet_tx_cache_input_t);
                l_tx_in->tx_prev_hash = l_bound_item->prev_item->tx_hash_fast;
                l_tx_in->tx_out_prev_idx = l_bound_item->prev_out_idx;
                l_tx_in->value = l_bound_item->value;
                l_wallet_tx_item->tx_wallet_inputs = dap_list_append(l_wallet_tx_item->tx_wallet_inputs, l_tx_in);
                pthread_rwlock_unlock(&l_ledger_pvt->wallet_cache_rwlock);
            }
    ...
    int l_out_idx = 0;
    for (dap_list_t *l_tx_out = l_list_tx_out; l_tx_out; l_tx_out = l_tx_out->next, l_outs_count++, l_out_idx++) {
        if (!l_tx_out->data) {
        ...
        // Save output into appropriate wallet cache
        if(!dap_chain_addr_is_blank(l_addr) && 
                ((s_wallets_cache_type == DAP_LEDGER_WALLET_CACHE_TYPE_LOCAL &&
                s_check_wallet_is_local(l_addr)) || s_wallets_cache_type == DAP_LEDGER_WALLET_CACHE_TYPE_ALL)){
            ...
            if(!dap_chain_addr_is_blank(&l_bound_item->in.addr_from) && 
                ...
                if (l_wallet_item){
                    ...
                    if (l_wallet_tx_item){
                        ...
                        if(l_wallet_tx_item->tx_wallet_outputs)
                            dap_list_free_full(l_wallet_tx_item->tx_wallet_outputs, NULL);
                        if (l_wallet_tx_item->tx_wallet_inputs)
                            dap_list_free_full(l_wallet_tx_item->tx_wallet_inputs, NULL);
                        
                        DAP_DEL_Z(l_wallet_tx_item);
                    }
                }
                if (l_wallet_item->wallet_txs->hh.tbl->num_items == 0)
                    HASH_DEL(l_ledger_pvt->wallets_cache, l_wallet_item);
                pthread_rwlock_unlock(&l_ledger_pvt->wallet_cache_rwlock);
            }
 ...
 */
static dap_chain_datum_tx_t *tx_item_find_by_addr(dap_ledger_t *a_ledger, const dap_chain_addr_t *a_addr,
                                                        const char * a_token, dap_chain_hash_fast_t *a_tx_first_hash)
{
    ...
    dap_chain_datum_tx_t *l_ioutput_item = NULL;

    if ((s_wallets_cache_type == DAP_LEDGER_WALLET_CACHE_TYPE_LOCAL &&
                s_check_wallet_is_local((dap_chain_addr_t*)a_addr)) || s_wallets_cache_type == DAP_LEDGER_WALLET_CACHE_TYPE_ALL){

        dap_ledger_wallet_cache_t *l_wallet_item = NULL;
        pthread_rwlock_rdlock(&l_ledger_pvt->wallet_cache_rwlock);
        ...
        dap_ledger_wallet_tx_cache_t *l_current_wallet_tx_iter = NULL, *l_tmp = NULL;
        HASH_ITER(hh, l_current_wallet_tx, l_current_wallet_tx_iter, l_tmp) {
            ...
            if (a_token){
                    ...
                    !l_current_wallet_tx_iter->multichannel)
                {
                    ...
                    for (dap_list_t *l_temp = l_current_wallet_tx_iter->tx_wallet_outputs; l_temp; l_temp=l_temp->next){
                        dap_ledger_wallet_tx_cache_output_t *l_cur_out_cache = (dap_ledger_wallet_tx_cache_output_t*)l_temp->data;
            ...
            // Now work with it
            dap_chain_hash_fast_t *l_tx_hash = &l_current_wallet_tx_iter->tx_hash;

            *a_tx_first_hash = l_current_wallet_tx_iter->tx_hash;
            l_ioutput_item = l_current_wallet_tx_iter->tx;
            is_tx_found = true;
            break;
        }
        pthread_rwlock_unlock(&l_ledger_pvt->wallet_cache_rwlock);
        ...
        pthread_rwlock_unlock(&l_ledger_pvt->ledger_rwlock);
        l_ioutput_item = is_tx_found ? l_iter_current->tx : NULL;
    }

    return is_tx_found? l_ioutput_item : NULL;
}
...
{
    dap_chain_datum_tx_t* l_tx_item = tx_item_find_by_addr(a_ledger, a_addr, a_token, a_tx_first_hash);
    return l_tx_item;
}
         ...
         dap_chain_net_srv_uid_t *a_tag, dap_chain_tx_tag_action_type_t *a_action, int *a_ret_code)
{
    ...
    HASH_ITER(hh, l_current_wallet_tx, l_current_wallet_tx_iter, l_tmp) {
        ...
        *a_tx_first_hash = l_current_wallet_tx_iter->tx_hash;
        if(a_tag)
            *a_tag = l_current_wallet_tx_iter->tag;
        
        if(a_action)
            *a_action = l_current_wallet_tx_iter->action;
        
        if(a_ret_code)
            *a_ret_code = l_current_wallet_tx_iter->ret_code;

        if(a_token)
            *a_token = l_current_wallet_tx_iter->token_ticker;
                                                       ...
                                                       uint256_t a_value_need, uint256_t *a_value_transfer)
{
    ...
    if(!a_addr_from || dap_chain_addr_is_blank(a_addr_from)){
    ...
    HASH_ITER(hh, l_current_wallet_tx, l_current_wallet_tx_iter, l_tmp) {
        ...
        bool skip = true;
        for (dap_list_t *l_temp = l_current_wallet_tx_iter->tx_wallet_outputs; l_temp; l_temp=l_temp->next){
            dap_ledger_wallet_tx_cache_output_t *l_out_cur = (dap_ledger_wallet_tx_cache_output_t*)l_temp->data;
            ...
            // Check whether used 'out' items
            if ( !dap_ledger_tx_hash_is_used_out_item (a_ledger, &l_current_wallet_tx_iter->tx_hash, l_out_cur->tx_out_idx, NULL) ) {
