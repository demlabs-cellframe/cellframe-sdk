...
static bool s_pack_ledger_threshold_info_json (json_object *a_json_arr_out, dap_ledger_tx_item_t *a_tx_item, int a_version)
{
    ...
    json_object_object_add(json_obj_tx, a_version == 1 ? "Ledger thresholded tx_hash_fast" : "thresholded_hash", json_object_new_string(l_tx_prev_hash_str));
...
json_object *dap_ledger_threshold_info(dap_ledger_t *a_ledger, size_t a_limit, size_t a_offset, dap_chain_hash_fast_t *a_threshold_hash, bool a_head, int a_version)
{
    ...
    if (a_threshold_hash) {
        ...
        if (l_tx_item) {
            json_object_object_add(json_obj_tx, a_version == 1 ? "Hash was found in ledger tx threshold" : "found_hash", json_object_new_string(dap_hash_fast_to_str_static(a_threshold_hash)));
            json_object_array_add(json_arr_out, json_obj_tx);
        } else {
            json_object_object_add(json_obj_tx, a_version == 1 ? "Hash wasn't found in ledger" : "found_hash", json_object_new_string("empty"));
            json_object_array_add(json_arr_out, json_obj_tx);
        }
    } else {
        size_t i_tmp = 0;
...
json_object *s_token_item_to_json(dap_ledger_token_item_t *a_token_item, int a_version)
{
    ...
    json_object_object_add(json_obj_datum, a_version == 1 ? "Auth signs valid" : "auth_signs_valid", json_object_new_int(a_token_item->auth_signs_valid));
    json_object_object_add(json_obj_datum, a_version == 1 ? "Auth signs total" : "auth_signs_total", json_object_new_int(a_token_item->auth_signs_total));
    ...
    json_object_object_add(json_obj_datum, a_version == 1 ? "Signatures public keys" : "signs_public_keys", l_json_arr_pkeys);
