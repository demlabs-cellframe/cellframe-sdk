...
void s_chain_net_states_to_json(dap_chain_net_t *a_net, dap_json_t *a_json_out, int a_version) {
    ...
    dap_json_object_add_object(a_json_out, a_version == 1 ? "networkState" : "network_state",
                           dap_json_object_new_string(dap_chain_net_state_to_str_user(a_net)));
...
static void s_set_reply_text_node_status_json(dap_chain_net_t *a_net, dap_json_t *a_json_out, int a_version) {
    ...
    dap_json_t *l_jobj_target_states = dap_json_object_new_string(dap_chain_net_state_to_str(PVT(a_net)->state_target));
...
static int s_chains_init_all(dap_chain_net_t *a_net, const char *a_path, uint16_t *a_ledger_flags)
{
    ...
    while (( l_dir_entry = readdir(l_chains_dir) )) {
        ...
        if ( l_len > 4 && !dap_strncmp(l_dir_entry->d_name + l_len - 4, ".cfg", 4) ) {
            *(l_dir_entry->d_name + l_len - 4) = '\0';
            ...
            dap_ht_add_keyptr(l_all_chain_configs, l_chain_config->path, strlen(l_chain_config->path), l_chain_config);
        }
    }
    ...
    dap_ht_foreach(l_all_chain_configs, l_chain_config, l_tmp_cfg) {
...
int s_chain_net_preload(dap_chain_net_t *a_net)
{
    ...
    if (!l_res) {
        ...
        /* Read ledger type from config: [ledger] type = "open"|"anon" */
...
void dap_chain_add_mempool_notify_callback(dap_chain_t *a_chain, dap_global_db_store_obj_callback_notify_t a_callback, void *a_cb_arg)
{
...
void dap_chain_net_add_nodelist_notify_callback(dap_chain_net_t *a_net, dap_global_db_store_obj_callback_notify_t a_callback, void *a_cb_arg)
{
    dap_global_db_cluster_add_notify_callback(PVT(a_net)->nodes_cluster, a_callback, a_cb_arg);
}

void dap_chain_net_srv_order_add_notify_callback(dap_chain_net_t *a_net, dap_global_db_store_obj_callback_notify_t a_callback, void *a_cb_arg)
{
