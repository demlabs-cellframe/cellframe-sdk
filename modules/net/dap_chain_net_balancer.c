...
void dap_chain_net_balancer_deinit()
{
    ...
    dap_ht_foreach(s_request_info_items, l_item, l_tmp)
...
// period request check
    ...
    dap_ht_find(s_request_info_items, &l_arg->net->pub.id, sizeof(l_arg->net->pub.id), l_item);
    if (!l_item) {
        l_item = DAP_NEW_Z_RET_IF_FAIL(dap_balancer_request_info_t);
        l_item->net_id = l_arg->net->pub.id;
        dap_ht_add(s_request_info_items, net_id, l_item);
    }
