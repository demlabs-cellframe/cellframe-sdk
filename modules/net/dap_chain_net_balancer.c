...
struct json_object *s_balancer_states_json_collect(dap_chain_net_t *a_net, const char* a_host_addr, uint16_t a_host_port)
{
    ...
    json_object_object_add(l_json, l_version == 1 ? "network_name" : "networkName", json_object_new_string((const char*)a_net->pub.name));
    json_object_object_add(l_json, l_version == 1 ? "host_address" : "hostAddress", json_object_new_string(a_host_addr ? a_host_addr : "localhost"));
    if (a_host_addr)
        json_object_object_add(l_json, l_version == 1 ? "host_port" : "hostPort", json_object_new_int(a_host_port));
    return l_json;
}
