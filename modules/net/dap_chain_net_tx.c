...
void dap_chain_net_get_tx_all(dap_chain_net_t * a_net, dap_chain_net_tx_search_type_t a_search_type ,dap_chain_net_tx_hash_callback_t a_tx_callback, void * a_arg)
{
    ...
    switch (a_search_type) {
        ...
        case TX_SEARCH_TYPE_LOCAL:{
            dap_ledger_datum_iter_t *l_iter = dap_ledger_datum_iter_create(a_net->pub.ledger);
...
{
    const char *l_unit_str = dap_json_object_get_string(a_json, a_key);
    if(!l_unit_str || !a_out)
        return false;
...
{
    const char *l_uint256_str = dap_json_object_get_string(a_json, a_key);
...
static bool s_json_get_srv_uid(dap_json_t *a_json, const char *a_key_service_id, const char *a_key_service, uint64_t *a_out)
{
    ...
    if(!a_out)
        return false;
    // Read service id
    const char *l_id = dap_json_object_get_string(a_json, a_key_service_id);
    ...
    if(l_id && sscanf(l_id,"0x%016"DAP_UINT64_FORMAT_x, &l_srv_id) == 1) {
        *a_out = l_srv_id;
        ...
        const char *l_service = dap_json_object_get_string(a_json, a_key_service);
        if(l_service) {
...
{
    return dap_chain_wallet_open(dap_json_object_get_string(a_json, a_key), dap_chain_wallet_get_path(g_config), NULL);
}

static const dap_cert_t* s_json_get_cert(dap_json_t *a_json, const char *a_key)
{
    return dap_cert_find_by_name(dap_json_object_get_string(a_json, a_key));
}
    ...
    for(size_t i = 0; i < a_items_count; ++i) {
        ...
        switch (l_item_type) {
            ...
            case TX_ITEM_TYPE_IN_EMS: {
                const char *l_emission_hash_str = dap_json_object_get_string(l_json_item_obj, "emission_hash");
...
static uint8_t *s_dap_chain_net_tx_create_in_item (dap_json_t *a_json_item_obj, dap_json_t *a_jobj_arr_errors, dap_list_t **a_in_list) {
...
static uint8_t *s_dap_chain_net_tx_create_in_ems_item (dap_json_t *a_json_item_obj, dap_json_t *a_jobj_arr_errors) {
    ...
    if (l_json_item_token && l_is_chain_id){
        ...
        const char *l_json_item_token_ems_hash = dap_json_object_get_string(a_json_item_obj, "token_ems_hash");
...
static uint8_t *s_dap_chain_net_tx_create_in_reward_item (dap_json_t *a_json_item_obj, dap_json_t *a_jobj_arr_errors) {
    const char *l_block_hash_str = dap_json_object_get_string(a_json_item_obj, "block_hash");
...
static uint8_t *s_dap_chain_net_tx_create_out_item (dap_json_t *a_json_item_obj, dap_json_t *a_jobj_errors, uint256_t *a_value_need) {
    // Read address and value
    uint256_t l_value = { };
    const char *l_json_item_addr_str = dap_json_object_get_string(a_json_item_obj, "addr");
...
static uint8_t *s_dap_chain_net_tx_create_out_ext_item (dap_json_t *a_json_item_obj, dap_json_t *a_jobj_errors, int a_type_tx, uint256_t *a_value_need) {
    // Read address and value
    uint256_t l_value = { };
    const char *l_json_item_addr_str = dap_json_object_get_string(a_json_item_obj, "addr");
    bool l_is_value = s_json_get_uint256(a_json_item_obj, "value", &l_value);
...
static uint8_t *s_dap_chain_net_tx_create_out_std_item (dap_json_t *a_json_item_obj, dap_json_t *a_jobj_errors, int a_type_tx, uint256_t *a_value_need) {
    // Read address and value
    uint256_t l_value = { };
    const char *l_json_item_addr_str = dap_json_object_get_string(a_json_item_obj, "addr");
    bool l_is_value = s_json_get_uint256(a_json_item_obj, "value", &l_value);
    ...
    const char* l_time_unlock_str = dap_json_object_get_string(a_json_item_obj, "time_unlock");
...
static uint8_t *s_dap_chain_net_tx_create_out_cond_item (dap_json_t *a_json_item_obj, dap_json_t *a_jobj_arr_errors, int a_type_tx,
                uint256_t *a_value_need, uint256_t *a_value_need_fee, dap_chain_addr_t *a_seller_addr, size_t i, dap_chain_net_t *a_net)
{
    // Read subtype of item
    const char *l_subtype_str = dap_json_object_get_string(a_json_item_obj, "subtype");
    ...
    switch (l_subtype) {
        case DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_PAY:{
            ...
            const char *l_pkey_hash_str = dap_json_object_get_string(a_json_item_obj, "pkey_hash");
        ...
        case DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_XCHANGE: {
            ...
            if(dap_chain_net_id_parse(dap_json_object_get_string(a_json_item_obj, "buy_net_id"), &l_buy_net_id)) {
            ...
            if(dap_chain_net_id_parse(dap_json_object_get_string(a_json_item_obj, "sell_net_id"), &l_sell_net_id)) {
            ...
            const char *l_token_buy = dap_json_object_get_string(a_json_item_obj, "buy_token");
            if(!l_token_buy) {
        ...
        case DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_STAKE_LOCK:{
            ...
            const char* l_time_staking_str = dap_json_object_get_string(a_json_item_obj, "time_staking");
            ...
            if((l_reinvest_percent_str = dap_json_object_get_string(a_json_item_obj, "reinvest_percent"))!=NULL) {
                l_reinvest_percent = dap_chain_balance_scan(l_reinvest_percent_str);
                if (compare256(l_reinvest_percent, dap_chain_balance_scan("100.0")) == 1){
        ...
        case DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_STAKE_POS_DELEGATE:{
            ...
            const char *l_node_addr_str = dap_json_object_get_string(a_json_item_obj, "signer_node_addr");
            ...
            dap_chain_tx_out_cond_t *l_out_cond_item = dap_chain_datum_tx_item_out_cond_create_srv_stake_delegate((dap_chain_srv_uid_t){.uint64 = l_srv_uid}, l_value, l_signing_addr,
        ...
        case DAP_CHAIN_TX_OUT_COND_SUBTYPE_FEE: {
            ...
            if(!IS_ZERO_256(l_value) && compare256(l_value, l_min) >= 0) {
                ...
                char *l_fee_value_str = dap_chain_balance_datoshi_print(l_value);
                char *l_fee_min_str = dap_chain_balance_datoshi_print(l_min);
...
static uint8_t *s_dap_chain_net_tx_create_receipt_item(dap_json_t *a_json_item_obj, dap_json_t *a_jobj_arr_errors, dap_chain_datum_tx_t *a_tx, dap_list_t *a_sign_list, size_t i)
{
    ...
    if((l_prev_tx_hash_str = dap_json_object_get_string(a_json_item_obj, "prev_tx")) == NULL) {
...
static uint8_t *s_dap_chain_net_tx_create_tsd_item(dap_json_t *a_json_item_obj, dap_json_t *a_jobj_arr_errors, dap_chain_datum_tx_t *a_tx, dap_list_t *a_sign_list)
{
    ...
    const char *l_tsd_data_str = dap_json_object_get_string(a_json_item_obj, "data");
    if (!l_tsd_data_str) {
    ...
    for(dap_list_t *l_list = a_in_list; l_list; l_list = dap_list_next(l_list)) {
        ...
        if(l_json_item_addr_str) {
            ...
            if (!l_addr_from) {
                ...
                dap_json_rpc_error_add(a_jobj_errors, -6, "Invalid element 'to', unable to convert string representation "
            ...
            dap_json_rpc_error_add(a_jobj_errors, -5, "Invalid 'in' item, incorrect addr_from: '%s'",
                                        l_json_item_addr_str ? l_json_item_addr_str : "[null]");
            continue;
        }
        if(!l_json_item_token) {
            log_it(L_WARNING, "Invalid 'in' item, not found token name");
            dap_json_rpc_error_add(a_jobj_errors, -4, "Invalid 'in' item, not found token name");
            continue;
        }
        ...
        if (l_list_used_out_fee) {
            ...
            if (!IS_ZERO_256(l_value_back)) {
                dap_chain_datum_tx_add_out_ext_item(a_tx, l_addr_from, l_value_back, a_net->pub.native_ticker);
                l_ret++;
            }
        } else {
        ...
        if(!IS_ZERO_256(l_value_got)) {
            ...
            uint256_t l_value_back;
            SUBTRACT_256_256(l_value_got, a_value_need, &l_value_back);
            if(!IS_ZERO_256(l_value_back)) {
                dap_chain_datum_tx_add_out_ext_item(a_tx, l_addr_from, l_value_back, l_json_item_token);
                l_ret++;
            }
        }   
    }
        ...
        dap_chain_datum_tx_t** a_out_tx, size_t* a_items_count, size_t *a_items_ready)
{
    ...
    for(size_t i = 0; i < l_items_count; ++i) {
        ...
        switch (l_item_type) {
            case TX_ITEM_TYPE_IN: {                
                l_item = s_dap_chain_net_tx_create_in_item(l_json_item_obj, a_jobj_arr_errors, &l_in_list);
                if (!l_item && l_in_list)
                    continue;
            }break;
            case TX_ITEM_TYPE_IN_COND: {
            ...
            case TX_ITEM_TYPE_OUT: {
                l_item = s_dap_chain_net_tx_create_out_item(l_json_item_obj, a_jobj_arr_errors, &l_value_need_fee);
            }break;
            case TX_ITEM_TYPE_OUT_EXT: {
                l_item = s_dap_chain_net_tx_create_out_ext_item(l_json_item_obj, a_jobj_arr_errors,l_type_tx, &l_value_need);
            }break;
            case TX_ITEM_TYPE_OUT_STD: {
                l_item = s_dap_chain_net_tx_create_out_std_item(l_json_item_obj, a_jobj_arr_errors,l_type_tx, &l_value_need);
            }break;
            case TX_ITEM_TYPE_OUT_COND: {
                l_item = s_dap_chain_net_tx_create_out_cond_item(l_json_item_obj, a_jobj_arr_errors, l_type_tx, &l_value_need, &l_value_need_fee, &l_seller_addr, i, l_net);
            }break;
        ...
        if (!l_item) {
            ...
            const char *l_hash_str = dap_json_object_get_string(l_json_item_obj, "item_hash");
            if (l_hash_str) {
            ...
            switch (((dap_chain_tx_out_cond_t*)item)->header.subtype) {
                ...
                case DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_STAKE_LOCK: {
                 ...
                 case DAP_CHAIN_TX_OUT_COND_SUBTYPE_WALLET_SHARED: {
                    ...
                    dap_json_object_add_object(json_obj_item, "owner_pkey_hashes", l_jobj_pkey_hashes);
                    if (l_tags_count > 0) {
                        dap_json_object_add_object(json_obj_item, "tags", l_jobj_tags);
                    }
                } break;
                default: break;
            }
