...
static bool s_json_get_unit(json_object *a_json, const char *a_key, dap_chain_net_srv_price_unit_uid_t *a_out)
{
...
static bool s_json_get_uint256(json_object *a_json, const char *a_key, uint256_t *a_out)
{
...
static bool s_json_get_srv_uid(json_object *a_json, const char *a_key_service_id, const char *a_key_service, uint64_t *a_out)
{
    uint64_t l_srv_id;
    if(!a_out)
        return false;
...
static dap_chain_wallet_t* s_json_get_wallet(json_object *a_json, const char *a_key)
{
    return dap_chain_wallet_open(dap_json_rpc_get_text(a_json, a_key), dap_chain_wallet_get_path(g_config), NULL);
}

static const dap_cert_t* s_json_get_cert(json_object *a_json, const char *a_key)
{
    return dap_cert_find_by_name(dap_json_rpc_get_text(a_json, a_key));
}

// Read pkey from wallet or cert
static dap_pkey_t* s_json_get_pkey(json_object *a_json)
{
...
static int s_dap_chain_net_tx_json_check(size_t a_items_count, json_object *a_json_item_objs, json_object *a_jobj_arr_errors, dap_chain_net_t * a_net) {
    ...
    for(size_t i = 0; i < a_items_count; ++i) {
        json_object *l_json_item_obj = json_object_array_get_idx(a_json_item_objs, i);
...
static uint8_t *s_dap_chain_net_tx_create_in_item (json_object *a_json_item_obj, json_object *a_jobj_arr_errors, dap_list_t **a_in_list) {
...
static uint8_t *s_dap_chain_net_tx_create_out_item (json_object *a_json_item_obj, json_object *a_jobj_errors, uint256_t *a_value_need) {
...
static uint8_t *s_dap_chain_net_tx_create_out_ext_item (json_object *a_json_item_obj, json_object *a_jobj_errors, int a_type_tx, uint256_t *a_value_need) {
    // Read address and value
    uint256_t l_value = { };
    const char *l_json_item_addr_str = dap_json_rpc_get_text(a_json_item_obj, "addr");
    bool l_is_value = s_json_get_uint256(a_json_item_obj, "value", &l_value);
...
static uint8_t *s_dap_chain_net_tx_create_out_std_item (json_object *a_json_item_obj, json_object *a_jobj_errors, int a_type_tx, uint256_t *a_value_need) {
    // Read address and value
    uint256_t l_value = { };
    const char *l_json_item_addr_str = dap_json_rpc_get_text(a_json_item_obj, "addr");
    bool l_is_value = s_json_get_uint256(a_json_item_obj, "value", &l_value);
                ...
                uint256_t *a_value_need, uint256_t *a_value_need_fee, dap_chain_addr_t *a_seller_addr, size_t i, dap_chain_net_t *a_net)
{
    ...
    switch (l_subtype) {
        ...
        case DAP_CHAIN_TX_OUT_COND_SUBTYPE_WALLET_SHARED: {
            ...
            json_object *l_json_pkey_hashes = json_object_object_get(a_json_item_obj, "owner_pkey_hashes");
            ...
            for(size_t j = 0; j < l_pkey_hashes_count; j++) {
                json_object *l_json_hash = json_object_array_get_idx(l_json_pkey_hashes, j);
            ...
            char *l_tag_str = NULL;
            json_object *l_json_tags = json_object_object_get(a_json_item_obj, "tags");
            if(l_json_tags && json_object_is_type(l_json_tags, json_type_array)) {
                ...
                if(l_tags_count > 0) {
                    ...
                    for(size_t j = 0; j < l_tags_count; j++) {
                        json_object *l_json_tag = json_object_array_get_idx(l_json_tags, j);
        ...
        dap_chain_datum_tx_t** a_out_tx, size_t* a_items_count, size_t *a_items_ready)
{
    ...
    json_object *l_json_items = json_object_object_get(a_tx_json, "items");
    json_object *l_json_net = json_object_object_get(a_tx_json, "net");
    size_t l_items_count;
    if(!l_json_items || !json_object_is_type(l_json_items, json_type_array) || !(l_items_count = json_object_array_length(l_json_items))) {
        return DAP_CHAIN_NET_TX_CREATE_JSON_NOT_FOUNT_ARRAY_ITEMS;
    } 
    ...
    json_object *l_json_timestamp = json_object_object_get(a_tx_json, "ts_created");
    if (l_json_timestamp)
