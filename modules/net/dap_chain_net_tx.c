            ...
            case TX_ITEM_TYPE_IN: {
                ...
                if(l_prev_hash_str && l_is_out_prev_idx){
                    ...
                    if(!dap_chain_hash_fast_from_str(l_prev_hash_str, &l_tx_prev_hash)) {
                        ...
                        byte_t *l_prev_item = l_prev_tx ? dap_chain_datum_tx_item_get_nth(l_prev_tx, TX_ITEM_TYPE_OUT_ALL, (int)l_out_prev_idx) : NULL;
                        if (l_prev_item){
    ...
    if(l_prev_hash_str && l_is_out_prev_idx){
        ...
        if(!dap_chain_hash_fast_from_str(l_prev_hash_str, &l_tx_prev_hash)) {
            if (!a_net) {
                ...
                return (uint8_t *)dap_chain_datum_tx_item_in_cond_create(&l_tx_prev_hash, (uint32_t)l_out_prev_idx, (uint32_t)l_receipt_idx);
            }
            //check out token
            dap_chain_datum_tx_t *l_prev_tx = dap_ledger_tx_find_by_hash(a_net->pub.ledger, &l_tx_prev_hash);
            byte_t *l_item; size_t l_tx_item_size;
                ...
                TX_ITEM_ITER_TX(l_item, l_tx_item_size, l_prev_tx) {
                    if (*l_item == TX_ITEM_TYPE_OUT_COND) {
                        ...
                        if (l_tx_out_cond && l_tx_out_cond->header.subtype == DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_STAKE_LOCK) {
                            ...
                            if (l_prev_item){
                                if (*l_prev_item == TX_ITEM_TYPE_OUT_COND){
                                    dap_chain_tx_in_cond_t * l_in_cond = dap_chain_datum_tx_item_in_cond_create(&l_tx_prev_hash, (uint32_t)l_out_prev_idx, 0);
                        ...
                        if (l_tx_out_cond && (l_tx_out_cond->header.subtype == DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_XCHANGE ||
                            ...
                            dap_chain_tx_in_cond_t * l_in_cond = dap_chain_datum_tx_item_in_cond_create(&l_tx_prev_hash, (uint32_t)l_out_prev_idx, (uint32_t)l_receipt_idx);
                            return (uint8_t *)l_in_cond;
                        }  
                    }
                }                 
    ...
    switch (l_subtype) {
        ...
        case DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_XCHANGE: {
            ...
            dap_chain_tx_out_cond_t *l_out_cond_item = dap_chain_datum_tx_item_out_cond_create_srv_xchange(l_srv_uid, l_sell_net_id,
                                                                                                            ...
                                                                                                            l_seller_addr,
                                                                                                            l_params, (uint32_t)l_params_size);
        ...
        case DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_DEX: {
            ...
            const char *l_token_buy = dap_json_rpc_get_text(a_json_item_obj, "buy_token");
            if(!l_token_buy) {
                dap_json_rpc_error_add(a_jobj_arr_errors, -1, "Json TX: bad buy_token in OUT_COND_SUBTYPE_SRV_DEX");
                log_it(L_ERROR, "Json TX: bad buy_token in OUT_COND_SUBTYPE_SRV_DEX");
                return NULL;
            }
            uint256_t l_value = { };
            if(!s_json_get_uint256(a_json_item_obj, "value", &l_value) || IS_ZERO_256(l_value)) {
                dap_json_rpc_error_add(a_jobj_arr_errors, -1, "Json TX: bad value in OUT_COND_SUBTYPE_SRV_DEX");
                log_it(L_ERROR, "Json TX: bad value in OUT_COND_SUBTYPE_SRV_DEX");
                return NULL;
            }
            uint256_t l_value_rate = { };
            if(!s_json_get_uint256(a_json_item_obj, "rate", &l_value_rate) || IS_ZERO_256(l_value_rate)) {
                dap_json_rpc_error_add(a_jobj_arr_errors, -1, "Json TX: bad value rate in OUT_COND_SUBTYPE_SRV_DEX");
            ...
            const char *l_params_str = dap_json_rpc_get_text(a_json_item_obj, "params");
            uint8_t *l_params = NULL; size_t l_params_size = 0;
            if (l_params_str) {
                l_params_size = DAP_ENC_BASE58_DECODE_SIZE(dap_strlen(l_params_str));
                l_params = DAP_NEW_Z_SIZE(uint8_t, l_params_size);
                l_params_size = dap_enc_base58_decode(l_params_str, l_params);
            ...
            dap_chain_tx_out_cond_t *l_out_cond_item = dap_chain_datum_tx_item_out_cond_create_srv_dex(
                ...
                l_order_root_hash_ptr, l_min_fill_combined, l_version, l_flags, l_type, l_params, (uint32_t)l_params_size);
            DAP_DELETE(l_params);
        ...
        case DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_STAKE_LOCK:{
            ...
            if(l_out_cond_item) {
                ...
                if (dap_json_rpc_get_uint64(a_json_item_obj, "flags", &l_flags)) {
                    l_out_cond_item->subtype.srv_stake_lock.flags = (uint32_t)l_flags;
                }
                return (uint8_t *)l_out_cond_item;
            } else {
                dap_json_rpc_error_add(a_jobj_arr_errors, -1, "Unable to create conditional out for transaction "
        ...
        case DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_STAKE_POS_DELEGATE:{
            ...
            if(l_out_cond_item) {
                ...
                uint64_t l_flags = 0;
                if (dap_json_rpc_get_uint64(a_json_item_obj, "flags", &l_flags)) {
                    l_out_cond_item->subtype.srv_stake_pos_delegate.flags = (uint32_t)l_flags;
                }
                return (uint8_t *)l_out_cond_item;
            } else {
                dap_json_rpc_error_add(a_jobj_arr_errors, -1, "Unable to create conditional out for transaction "
...
static uint8_t *s_dap_chain_net_tx_create_sig_item(json_object *a_json_item_obj, json_object *a_jobj_arr_errors, dap_chain_datum_tx_t *a_tx, dap_list_t **a_sign_list)
{
    ...
    l_tx_sig->header.sig_size = (uint32_t)dap_enc_base64_decode(l_sign_b64_str, l_sign_b64_strlen, l_tx_sig->sig, DAP_ENC_DATA_TYPE_B64_URLSAFE);
        ...
        case TX_ITEM_TYPE_SIG: {
            ...
            *l_tx_sig = (dap_chain_tx_sig_t) {
                .header = {
                    ...
                    .sig_size = (uint32_t)dap_enc_base64_decode(l_sign_b64_str, l_sign_b64_strlen, l_tx_sig->sig, DAP_ENC_DATA_TYPE_B64_URLSAFE)
                }
            };
            ...
            if ( dap_sign_verify_all((dap_sign_t*)l_tx_sig->sig, l_tx_sig->header.sig_size, (byte_t*)l_tx, l_tx_size) ) {
                ...
                l_tx->header.tx_items_size = (uint32_t)l_tx_items_size;
        ...
        dap_chain_datum_tx_t** a_out_tx, size_t* a_items_count, size_t *a_items_ready)
{
    ...
    for(size_t i = 0; i < l_items_count; ++i) {
        ...
        if (!l_item) {
            ...
            if (l_hash_str) {
                size_t l_item_sz = dap_chain_datum_item_tx_get_size(l_item, 0);
                char *l_hash_str_current = dap_hash_fast_str_new(l_item, l_item_sz);
                if (l_hash_str_current && strcmp(l_hash_str, l_hash_str_current)) {
                    log_it(L_ERROR, "Item %zu type '%s' item_hash: JSON has '%s', import rebuild '%s', size %zu — "
                                    "mismatch means to_json vs from_json produce different item bytes (SDK skew, import bug, or hex case)",
                             i + 1, l_item_type_str, l_hash_str, l_hash_str_current, l_item_sz);
                    dap_json_rpc_error_add(a_jobj_arr_errors, DAP_CHAIN_NET_TX_CREATE_JSON_CANT_CREATED_ITEM_ERR,
                        "Item %zu item_hash mismatch: expected '%s' recomputed '%s' size %zu", i, l_hash_str, l_hash_str_current, l_item_sz);
...
int dap_chain_net_tx_to_json(dap_chain_datum_tx_t *a_tx, json_object *a_out_json)
{
    ...
    TX_ITEM_ITER_TX(item, l_size, a_tx) {
        ...
        case TX_ITEM_TYPE_IN:
            ...
            json_object_object_add(json_obj_item,"prev_hash", json_object_new_string(dap_hash_fast_to_str_static(&l_hash_tmp)));
        ...
        case TX_ITEM_TYPE_OUT_COND: {
            ...
            switch (((dap_chain_tx_out_cond_t*)item)->header.subtype) {
                ...
                case DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_PAY: {
                    ...
                    json_object_object_add(json_obj_item,"pkey_hash", json_object_new_string(dap_hash_fast_to_str_static(&l_hash_tmp)));
        ...
        case TX_ITEM_TYPE_VOTE:{
            ...
            json_object_object_add(json_obj_item,"voting_hash", json_object_new_string(dap_chain_hash_fast_to_str_static(&l_vote_item->voting_hash)));
            json_object_object_add(json_obj_item,"answer_idx", json_object_new_uint64(l_vote_item->answer_idx));
        } break;
        case TX_ITEM_TYPE_IN_REWARD:{
            json_object_object_add(json_obj_item,"block_hash", json_object_new_string(dap_chain_hash_fast_to_str_static(&((dap_chain_tx_in_reward_t *)item)->block_hash)));
        } break;
        default:
