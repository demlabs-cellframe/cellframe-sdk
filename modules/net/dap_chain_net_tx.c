    ...
    for(size_t i = 0; i < l_items_count; ++i) {
        ...
        case TX_ITEM_TYPE_OUT_COND: {
            ...
            case DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_STAKE_POS_DELEGATE:{
                ...
                if (l_pkey) {
                    l_out_cond_item->subtype.srv_stake_pos_delegate.flags = DAP_SIGN_ADD_PKEY_HASHING_FLAG(l_out_cond_item->subtype.srv_stake_pos_delegate.flags);
                }
...
{
    if(!a_tx || !a_items_array)
        return -1;
    byte_t *item;
    size_t l_size;
    TX_ITEM_ITER_TX(item, l_size, a_tx) {
        json_object* json_obj_item = json_object_new_object();
        json_object_object_add(json_obj_item,"type", json_object_new_string(dap_chain_datum_tx_item_type_to_str(*item)));
        switch (*item) {
            case TX_ITEM_TYPE_IN: {
                dap_hash_fast_t l_hash_tmp = ((dap_chain_tx_in_t*)item)->header.tx_prev_hash;
                char* l_hash_str = dap_hash_fast_to_str_static(&l_hash_tmp);
            ...
            case TX_ITEM_TYPE_IN_COND: {
                dap_hash_fast_t l_hash_tmp = ((dap_chain_tx_in_cond_t*)item)->header.tx_prev_hash;
                char* l_hash_str = dap_hash_fast_to_str_static(&l_hash_tmp);
            ...
            case TX_ITEM_TYPE_VOTE: {
                ...
                const char *l_hash_str_vote = dap_chain_hash_fast_to_str_static(&l_vote_item->voting_hash);
                json_object_object_add(json_obj_item,"voting_hash", json_object_new_string(l_hash_str_vote));
                json_object_object_add(json_obj_item,"answer_idx", json_object_new_uint64(l_vote_item->answer_idx));
                break;
            }
            case TX_ITEM_TYPE_IN_REWARD: {
                const char *l_hash_str_reward = dap_chain_hash_fast_to_str_static(&((dap_chain_tx_in_reward_t *)item)->block_hash);
                json_object_object_add(json_obj_item,"block_hash", json_object_new_string(l_hash_str_reward));
                break;
            }
            default:
                break;
        }
        json_object_array_add(a_items_array, json_obj_item);
    }
    return 0;
}
...
int dap_chain_net_tx_to_json(dap_chain_datum_tx_t *a_tx, json_object *a_out_json)
{
    ...
    json_object_object_add(a_out_json, "datum_hash", json_object_new_string(l_tx_hash_str));
    DAP_DELETE(l_tx_hash_str);
    json_object_object_add(a_out_json, "ts_created", json_object_new_int64(a_tx->header.ts_created));
    json_object_object_add(a_out_json, "datum_type", json_object_new_string("tx"));

    json_object* json_arr_items = json_object_new_array();

    dap_chain_tx_items_to_json(a_tx, json_arr_items);

    json_object_object_add(a_out_json, "items", json_arr_items);
