...
int dap_chain_node_cli_init(dap_config_t * g_config)
{
    ...
    dap_cli_server_cmd_add("global_db", com_global_db, "Work with global database", 0,
            "global_db flush\n"
...
//                    "global_db wallet_info set -addr <wallet address> -cell <cell id> \n\n"
    ...
    dap_cli_server_cmd_add("mempool", com_signer, "Sign operations", 0,
    ...
    dap_cli_server_cmd_add("node", com_node, "Work with node", 0,
    ...
    dap_cli_server_cmd_add ("version", com_version, "Return software version", 0,
                                        "version\n"
                                        "\tReturn version number\n"
                                        );

    dap_cli_server_cmd_add ("help", com_help, "Description of command parameters", 0,
                                        "help [<command>]\n"
                                        "\tObtain help for <command> or get the total list of the commands\n"
                                        );
    dap_cli_server_cmd_add ("?", com_help, "Synonym for \"help\"", 0,
                                        "? [<command>]\n"
                                        "\tObtain help for <command> or get the total list of the commands\n"
                                        );
    dap_cli_server_cmd_add ("token_update", com_token_update, "Token update", 0,
    ...
    dap_cli_server_cmd_add ("wallet", com_tx_wallet, "Wallet operations", 0,
                            "wallet list\n"
    ...
    // Token commands
    dap_cli_server_cmd_add ("token_decl", com_token_decl, "Token declaration", 0,
    ...
    dap_cli_server_cmd_add("token_update_sign", com_token_decl_sign, "Token update add sign to datum", 0,
    ...
    dap_cli_server_cmd_add ("token_decl_sign", com_token_decl_sign, "Token declaration add sign", 0,
    ...
    dap_cli_server_cmd_add ("token_emit", com_token_emit, "Token emission", 0,
    ...
    dap_cli_cmd_t *l_cmd_mempool = dap_cli_server_cmd_add("mempool", com_mempool, "Command for working with mempool", 0,
    ...
    dap_cli_server_cmd_add ("chain_ca_pub", com_chain_ca_pub,
                                        "Add pubic certificate into the mempool to prepare its way to chains", 0,
            "chain_ca_pub -net <net_name> [-chain <chain_name>] -ca_name <priv_cert_name>\n");

    // Transaction commands
    dap_cli_server_cmd_add ("tx_create", com_tx_create, "Make transaction", 0,
    ...
    dap_cli_server_cmd_add ("tx_create_json", com_tx_create_json, "Make transaction", 0,
                "tx_create_json -net <net_name> [-chain <chain_name>] -json <json_file_path>\n" );
    dap_cli_server_cmd_add ("tx_cond_create", com_tx_cond_create, "Make cond transaction", 0,
        ...
        dap_cli_server_cmd_add ("tx_cond_remove", com_tx_cond_remove, "Remove cond transactions and return funds from condition outputs to wallet", 0,
                                        "tx_cond_remove -net <net_name> -hashes <hash1,hash2...> -w <wallet_name>"
                                        " -fee <value> -srv_uid <numeric_uid>\n" );
        dap_cli_server_cmd_add ("tx_cond_unspent_find", com_tx_cond_unspent_find, "Find cond transactions by wallet", 0,
                                        "tx_cond_unspent_find -net <net_name> -srv_uid <numeric_uid> -w <wallet_name> \n" );

    dap_cli_server_cmd_add ("tx_verify", com_tx_verify, "Verifing transaction in mempool", 0,
            "tx_verify -net <net_name> [-chain <chain_name>] -tx <tx_hash>\n" );

    // Transaction history
    dap_cli_server_cmd_add("tx_history", com_tx_history, "Transaction history (for address or by hash)", 0,
	...
	// Ledger info
    dap_cli_server_cmd_add("ledger", com_ledger, "Ledger information", 0,
    ...
    // Token info
    dap_cli_server_cmd_add("token", com_token, "Token info", 0,
            "token list -net <net_name>\n"
            "token info -net <net_name> -name <token_ticker>\n");

    // Log
    dap_cli_server_cmd_add ("print_log", com_print_log, "Print log info", 0,
                "print_log [ts_after <timestamp>] [limit <line_numbers>]\n" );

    // Statisticss
    dap_cli_server_cmd_add("stats", com_stats, "Print statistics", 0,
                "stats cpu");

    // Export GDB to JSON
    dap_cli_server_cmd_add("gdb_export", cmd_gdb_export, "Export gdb to JSON", 0,
                                        "gdb_export filename <filename without extension> [-groups <group names list>]");

    //Import GDB from JSON
    dap_cli_server_cmd_add("gdb_import", cmd_gdb_import, "Import gdb from JSON", 0,
                                        "gdb_import filename <filename_without_extension>");

    dap_cli_server_cmd_add ("remove", cmd_remove, "Delete chain files or global database", 0,
           "remove -gdb\n"
    ...
    dap_cli_server_cmd_add ("decree", cmd_decree, "Work with decree", 0,
    ...
    dap_cli_server_cmd_add ("exec_cmd", com_exec_cmd, "Execute command on remote node", 0,
            "exec_cmd -net <net_name> -addr <node_addr> -cmd <command,and,all,args,separated,by,commas>\n" );

    //Find command
    dap_cli_server_cmd_add("find", cmd_find, "The command searches for the specified elements by the specified attributes", 0,
    ...
    dap_cli_server_cmd_add ("policy", com_policy, "Policy commands", 0,
                ...
                "\t-net <net_name>\n");
    // Exit - always last!
    dap_cli_server_cmd_add ("exit", com_exit, "Stop application and exit", 0,
                "exit\n" );
