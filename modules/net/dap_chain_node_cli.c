...
int dap_chain_node_cli_init(dap_config_t * g_config)
{
    ...
    dap_cli_server_cmd_add("global_db", com_global_db, "Work with global database", DAP_CHAIN_NODE_CLI_CMD_ID_GLOBAL_DB,
            "global_db flush\n"
...
//                    "global_db wallet_info set -addr <wallet address> -cell <cell id> \n\n"
    ...
    dap_cli_server_cmd_add("mempool", com_signer, "Sign operations", DAP_CHAIN_NODE_CLI_CMD_ID_MEMPOOL,
    ...
    dap_cli_server_cmd_add ("version", com_version, "Return software version", DAP_CHAIN_NODE_CLI_CMD_ID_VERSION,
                                        "version\n"
                                        "\tReturn version number\n"
                                        );

    dap_cli_server_cmd_add ("help", com_help, "Description of command parameters", DAP_CHAIN_NODE_CLI_CMD_ID_HELP,
                                        "help [<command>]\n"
                                        "\tObtain help for <command> or get the total list of the commands\n"
                                        );
    dap_cli_server_cmd_add ("?", com_help, "Synonym for \"help\"", DAP_CHAIN_NODE_CLI_CMD_ID_HELP,
                                        "? [<command>]\n"
                                        "\tObtain help for <command> or get the total list of the commands\n"
                                        );
    dap_cli_server_cmd_add ("token_update", com_token_update, "Token update", DAP_CHAIN_NODE_CLI_CMD_ID_TOKEN_UPDATE,
    ...
    dap_cli_server_cmd_add ("wallet", com_tx_wallet, "Wallet operations", DAP_CHAIN_NODE_CLI_CMD_ID_WALLET,
                            "wallet list\n"
    ...
    // Token commands
    dap_cli_server_cmd_add ("token_decl", com_token_decl, "Token declaration", DAP_CHAIN_NODE_CLI_CMD_ID_TOKEN_DECL,
    ...
    dap_cli_server_cmd_add("token_update_sign", com_token_decl_sign, "Token update add sign to datum", DAP_CHAIN_NODE_CLI_CMD_ID_TOKEN_UPDATE_SIGN,
    ...
    dap_cli_server_cmd_add ("token_decl_sign", com_token_decl_sign, "Token declaration add sign", DAP_CHAIN_NODE_CLI_CMD_ID_TOKEN_DECL_SIGN,
    ...
    dap_cli_server_cmd_add ("token_emit", com_token_emit, "Token emission", DAP_CHAIN_NODE_CLI_CMD_ID_TOKEN_EMIT,
    ...
    dap_cli_cmd_t *l_cmd_mempool = dap_cli_server_cmd_add("mempool", com_mempool, "Command for working with mempool", DAP_CHAIN_NODE_CLI_CMD_ID_MEMPOOL,
    ...
    dap_cli_server_cmd_add ("chain_ca_pub", com_chain_ca_pub,
                                        "Add pubic certificate into the mempool to prepare its way to chains", DAP_CHAIN_NODE_CLI_CMD_ID_CHAIN_CA_PUB,
            "chain_ca_pub -net <net_name> [-chain <chain_name>] -ca_name <priv_cert_name>\n");

    // Transaction commands
    dap_cli_server_cmd_add ("tx_create", com_tx_create, "Make transaction", DAP_CHAIN_NODE_CLI_CMD_ID_TX_CREATE,
    ...
    dap_cli_server_cmd_add ("tx_create_json", com_tx_create_json, "Make transaction", DAP_CHAIN_NODE_CLI_CMD_ID_TX_CREATE_JSON,
                "tx_create_json -net <net_name> [-chain <chain_name>] -json <json_file_path>\n" );
    dap_cli_server_cmd_add ("tx_cond_create", com_tx_cond_create, "Make cond transaction", DAP_CHAIN_NODE_CLI_CMD_ID_TX_COND_CREATE,
        ...
        dap_cli_server_cmd_add ("tx_cond_remove", com_tx_cond_remove, "Remove cond transactions and return funds from condition outputs to wallet", DAP_CHAIN_NODE_CLI_CMD_ID_TX_COND_REMOVE,
                                        "tx_cond_remove -net <net_name> -hashes <hash1,hash2...> -w <wallet_name>"
                                        " -fee <value> -srv_uid <numeric_uid>\n" );
        dap_cli_server_cmd_add ("tx_cond_unspent_find", com_tx_cond_unspent_find, "Find cond transactions by wallet", DAP_CHAIN_NODE_CLI_CMD_ID_TX_COND_UNSPENT_FIND,
                                        "tx_cond_unspent_find -net <net_name> -srv_uid <numeric_uid> -w <wallet_name> \n" );

    dap_cli_server_cmd_add ("tx_verify", com_tx_verify, "Verifing transaction in mempool", DAP_CHAIN_NODE_CLI_CMD_ID_TX_VERIFY,
            "tx_verify -net <net_name> [-chain <chain_name>] -tx <tx_hash>\n" );

    // Transaction history
    dap_cli_server_cmd_add("tx_history", com_tx_history, "Transaction history (for address or by hash)", DAP_CHAIN_NODE_CLI_CMD_ID_TX_HISTORY,
	...
	// Ledger info
    dap_cli_server_cmd_add("ledger", com_ledger, "Ledger information", DAP_CHAIN_NODE_CLI_CMD_ID_LEDGER,
    ...
    // Token info
    dap_cli_server_cmd_add("token", com_token, "Token info", DAP_CHAIN_NODE_CLI_CMD_ID_TOKEN,
            "token list -net <net_name>\n"
            "token info -net <net_name> -name <token_ticker>\n");

    // Log
    dap_cli_server_cmd_add ("print_log", com_print_log, "Print log info", DAP_CHAIN_NODE_CLI_CMD_ID_PRINT_LOG,
                "print_log [ts_after <timestamp>] [limit <line_numbers>]\n" );

    // Statisticss
    dap_cli_server_cmd_add("stats", com_stats, "Print statistics", DAP_CHAIN_NODE_CLI_CMD_ID_STATS,
                "stats cpu");

    // Export GDB to JSON
    dap_cli_server_cmd_add("gdb_export", cmd_gdb_export, "Export gdb to JSON", DAP_CHAIN_NODE_CLI_CMD_ID_GDB_EXPORT,
                                        "gdb_export filename <filename without extension> [-groups <group names list>]");

    //Import GDB from JSON
    dap_cli_server_cmd_add("gdb_import", cmd_gdb_import, "Import gdb from JSON", DAP_CHAIN_NODE_CLI_CMD_ID_GDB_IMPORT,
                                        "gdb_import filename <filename_without_extension>");

    dap_cli_server_cmd_add ("remove", cmd_remove, "Delete chain files or global database", DAP_CHAIN_NODE_CLI_CMD_ID_REMOVE,
           "remove -gdb\n"
    ...
    dap_cli_server_cmd_add ("decree", cmd_decree, "Work with decree", DAP_CHAIN_NODE_CLI_CMD_ID_DECREE,
    ...
    dap_cli_server_cmd_add ("exec_cmd", com_exec_cmd, "Execute command on remote node", DAP_CHAIN_NODE_CLI_CMD_ID_EXEC_CMD,
            "exec_cmd -net <net_name> -addr <node_addr> -cmd <command,and,all,args,separated,by,commas>\n" );

    //Find command
    dap_cli_server_cmd_add("find", cmd_find, "The command searches for the specified elements by the specified attributes", DAP_CHAIN_NODE_CLI_CMD_ID_FIND,
    ...
    dap_cli_server_cmd_add ("policy", com_policy, "Policy commands", DAP_CHAIN_NODE_CLI_CMD_ID_POLICY,
                ...
                "\t-net <net_name>\n");
    // Exit - always last!
    dap_cli_server_cmd_add ("exit", com_exit, "Stop application and exit", DAP_CHAIN_NODE_CLI_CMD_ID_EXIT,
                "exit\n" );
