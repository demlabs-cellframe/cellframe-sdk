...
int l_arg_index = 1, l_rc, cmd_num = CMD_NONE;
    ...
    switch (cmd_num) {
        ...
        case CMD_WALLET_INFO: {
            ...
            json_object_object_add(json_obj_wall, "pkey_hash", json_object_new_string(dap_hash_fast_to_str_static(&l_pkey_hash)));
                                 ...
                                 const char *a_hash_out_type, json_object **a_json_arr_reply, int a_version, bool a_tx_to_json)
{
    ...
    json_object *l_jobj_datum = json_object_new_object();
    if (a_tx_to_json) {
        dap_chain_net_tx_to_json(l_datum, l_jobj_datum);
    } else {
