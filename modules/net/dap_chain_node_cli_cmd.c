    ...
    switch (cmd_num) {
        ...
        case CMD_WALLET_OUTPUTS: {
            ...
            if (l_addr->addr_type == DAP_CHAIN_ADDR_TYPE_SHARED) {
                dap_hash_fast_t l_hold_hash = l_addr->data.hash_fast;
                const char *l_shared_ticker = dap_ledger_tx_get_token_ticker_by_hash(l_net->pub.ledger, &l_hold_hash);
                if (!l_shared_ticker || dap_strcmp(l_shared_ticker, l_token_tiker)) {
                    dap_json_rpc_error_add(*a_json_arr_reply, DAP_CHAIN_NODE_CLI_COM_TX_WALLET_PARAM_ERR,
                                           "Token '%s' does not match shared wallet token '%s'",
                                           l_token_tiker, l_shared_ticker ? l_shared_ticker : "unknown");
                    json_object_put(json_obj_wall);
                ...
                dap_hash_fast_t l_final_hash = dap_ledger_get_final_chain_tx_hash(
                    l_net->pub.ledger, DAP_CHAIN_TX_OUT_COND_SUBTYPE_WALLET_SHARED, &l_hold_hash, true);
                if (!dap_hash_fast_is_blank(&l_final_hash)) {
                    ...
                    if (l_final_tx) {
                        ...
                        dap_chain_tx_out_cond_t *l_cond = dap_chain_datum_tx_out_cond_get(
                            l_final_tx, DAP_CHAIN_TX_OUT_COND_SUBTYPE_WALLET_SHARED, &l_cond_idx);
                        if (l_cond) {
                            l_value_sum = l_cond->header.value;
                            const char *l_coins_str, *l_datoshi_str = dap_uint256_to_char(l_cond->header.value, &l_coins_str);
                            json_object *json_obj_item = json_object_new_object();
                            json_object_object_add(json_obj_item, "item_type", json_object_new_string("unspent_cond_out"));
                            json_object_object_add(json_obj_item, "value_coins", json_object_new_string(l_coins_str));
                            json_object_object_add(json_obj_item, a_version == 1 ? "value_datosi" : "value_datoshi",
                                                   json_object_new_string(l_datoshi_str));
                            json_object_object_add(json_obj_item, "prev_hash",
                                                   json_object_new_string(dap_hash_fast_to_str_static(&l_final_hash)));
                            json_object_object_add(json_obj_item, "out_prev_idx", json_object_new_int64(l_cond_idx));
                                        ...
                                        json_object *a_json_obj, const char *a_hash_out_type, bool a_fast, size_t a_limit, size_t a_offset, int a_version)
{
    ...
    // Create main object for chain and immediately add it to a_json_obj
    ...
    // Add datums array to chain object
    ...
    for (size_t i = l_arr_start; i < l_arr_end; i++) {
        ...
        // Create object for current datum
        ...
        // Fill in creation time and hash information
        ...
        // Add basic datum information
        // Create JSON objects for type, hash and creation time
        ...
        // Check hash and key correspondence
        ...
        switch (l_datum->header.type_id) {
            case DAP_CHAIN_DATUM_TX: {
                ...
                // Add service and action information
                dap_chain_net_srv_uid_t uid;
                char *service_name;
                dap_chain_tx_tag_action_type_t action;
                int l_rc = dap_ledger_deduct_tx_tag(a_net->pub.ledger, l_tx, &service_name, &uid, &action);
                json_object *l_jobj_service = json_object_new_string(l_rc ? service_name : "UNKNOWN");
                if (!l_jobj_service) {
                    json_object_put(l_obj_chain);
                    dap_global_db_objs_delete(l_objs, l_objs_count);
                    dap_json_rpc_allocation_error(a_json_arr_reply);
                ...
                json_object *l_jobj_action = json_object_new_string(l_rc ? dap_ledger_tx_action_str(action) : "UNKNOWN");
                json_object_object_add(l_jobj_datum, "service", l_jobj_service);
                if (!l_jobj_action) {
                    json_object_put(l_obj_chain);
                    dap_global_db_objs_delete(l_objs, l_objs_count);
                    dap_json_rpc_allocation_error(a_json_arr_reply);
                ...
                if (!l_sig) {
                    // Handle situation when signature is not found
                ...
                json_object_object_add(l_jobj_datum, "to_from_emi", l_jobj_to_from_emi);
                ...
                json_object_object_add(l_jobj_datum, "srv_xchange", l_jobj_xchange_list);
                ...
                json_object *l_jobj_wallet_shared_list = json_object_new_array();
                if (!l_jobj_wallet_shared_list) {
                    json_object_put(l_obj_chain);
                    dap_global_db_objs_delete(l_objs, l_objs_count);
                    dap_json_rpc_allocation_error(a_json_arr_reply);
                    return;
                ...
                json_object_object_add(l_jobj_datum, "srv_pay", l_jobj_pay_list);
                    ...
                    if (l_dist_addr) {
                        ...
                        switch (l_out_cond_subtype) {
                            ...
                            case OUT_COND_TYPE_WALLET_SHARED: {
                                json_object_array_add(l_jobj_wallet_shared_list, l_jobj_money);
                ...
                json_object_object_add(l_jobj_datum, "voting", l_jobj_tx_voting);
                ...
                json_object *l_jobj_tx_event = json_object_new_array();
                if (!l_jobj_tx_event) {
                    json_object_put(l_obj_chain);
                    dap_global_db_objs_delete(l_objs, l_objs_count);
                    dap_json_rpc_allocation_error(a_json_arr_reply);
                ...
                json_object_object_add(l_jobj_datum, "event", l_jobj_tx_event);
                ...
                for (dap_list_t *it = l_event_list; it; it = it->next) {
                    json_object *l_jobj_event = json_object_new_object();
                    if (!l_jobj_event) {
                        json_object_put(l_obj_chain);
                        dap_global_db_objs_delete(l_objs, l_objs_count);
                        dap_json_rpc_allocation_error(a_json_arr_reply);
                    ...
                    if (l_ret == 0) {
                        json_object_array_add(l_jobj_tx_event, l_jobj_event);
                    } else {
                        json_object_put(l_jobj_event);
                    }
                }
                ...
                if (!json_object_array_length(l_jobj_pay_list))
                    json_object_object_del(l_jobj_datum, "srv_pay");
                if (!json_object_array_length(l_jobj_xchange_list))
                    json_object_object_del(l_jobj_datum, "srv_xchange");
                if (!json_object_array_length(l_jobj_stake_lock_list))
                    json_object_object_del(l_jobj_datum, "srv_stake_lock");
                if (!json_object_array_length(l_jobj_stake_pos_delegate_list))
                    json_object_object_del(l_jobj_datum, "srv_stake_pos_delegate");
                if (!json_object_array_length(l_jobj_wallet_shared_list))
                    json_object_object_del(l_jobj_datum, "srv_wallet_shared");
                if (!json_object_array_length(l_jobj_to_from_emi))
                    json_object_object_del(l_jobj_datum, "from_emission");
                if (!json_object_array_length(l_jobj_tx_vote))
                    json_object_object_del(l_jobj_datum, "vote");
                if (!json_object_array_length(l_jobj_tx_voting))
                    json_object_object_del(l_jobj_datum, "voting");
                if (!json_object_array_length(l_jobj_tx_event))
                    json_object_object_del(l_jobj_datum, "event");
            } break;
...
return_obj_chain:
    ...
    // Add total to chain object
    json_object_object_add(l_obj_chain, "total", l_object_total);
    
    // Add chain to general JSON objects array
...
//    const char *value_str = NULL;
    ...
    // Check for arbitrage unlock (from_arbitrage)
    const char *l_from_arbitrage_str = NULL;
    bool l_is_from_arbitrage = dap_cli_server_cmd_find_option_val(a_argv, arg_index, a_argc, "-from_arbitrage", &l_from_arbitrage_str);
    ...
    if((!l_from_wallet_name && !l_emission_hash_str && !l_from_arbitrage_str)||(l_from_wallet_name && l_emission_hash_str)) {
        dap_json_rpc_error_add(*a_json_arr_reply, DAP_CHAIN_NODE_CLI_COM_TX_CREATE_REQUIRE_PARAMETER_FROM_WALLET_OR_FROM_EMISSION, "tx_create requires one of parameters '-from_wallet', '-from_emission', or '-from_arbitrage'");
    ...
    if(l_tx_num){
        ...
        // If arbitrage unlock (-from_arbitrage) is set
        if (l_is_from_arbitrage && l_from_arbitrage_str) {
            // Parse -from_arbitrage <tx_hash>:<out_idx>
            char *l_sep = strchr(l_from_arbitrage_str, ':');
            if (!l_sep) {
                dap_json_rpc_error_add(*a_json_arr_reply, DAP_CHAIN_NODE_CLI_COM_TX_CREATE_CAN_NOT_CREATE_TRANSACTION,
                                       "Invalid -from_arbitrage format. Expected: <tx_hash>:<out_idx>");
                goto cleanup;
            }
            char l_tx_hash_buf[128] = {0};
            size_t l_hash_len = l_sep - l_from_arbitrage_str;
            if (l_hash_len >= sizeof(l_tx_hash_buf)) {
                dap_json_rpc_error_add(*a_json_arr_reply, DAP_CHAIN_NODE_CLI_COM_TX_CREATE_CAN_NOT_CREATE_TRANSACTION,
                                       "Transaction hash too long");
                goto cleanup;
            }
            strncpy(l_tx_hash_buf, l_from_arbitrage_str, l_hash_len);
            l_tx_hash_buf[l_hash_len] = '\0';
            size_t l_out_idx = strtoul(l_sep + 1, NULL, 10);
            
            // Get the wallet address
            dap_chain_wallet_t *l_wallet_from = NULL;
            if (l_from_wallet_name) {
                l_wallet_from = dap_chain_wallet_open(l_from_wallet_name, c_wallets_path, NULL);
            }
            const dap_chain_addr_t *l_addr_from_arb = l_wallet_from ? 
                (const dap_chain_addr_t *)dap_chain_wallet_get_addr(l_wallet_from, l_net->pub.id) : l_addr_from;
            
            json_object **l_json_arr_reply = (json_object **)a_json_arr_reply;
            l_tx_hash_str = dap_chain_arbitrage_cli_unlock_tx(
                l_chain, l_net, l_wallet, l_priv_key, l_addr_from_arb, l_addr_to[0],
                l_token_ticker, l_value[0], l_value_fee, l_hash_out_type,
                l_time_unlock ? l_time_unlock[0] : 0, l_certs_str,
                l_tx_hash_buf, l_out_idx, l_json_arr_reply, l_jobj_result);
            
            if (l_wallet_from && l_wallet_from != l_wallet)
                dap_chain_wallet_close(l_wallet_from);
        }
        // If arbitrage flag is set, use arbitrage module
        else if (l_is_arbitrage) {
            json_object **l_json_arr_reply = (json_object **)a_json_arr_reply;
