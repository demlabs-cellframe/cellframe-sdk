...
int com_global_db(int a_argc, char ** a_argv, char **a_str_reply)
{
    ...
    if(dap_chain_node_cli_find_option_val(a_argv, arg_index, min(a_argc, arg_index + 1), "cells", NULL))
        ...
        cmd_name = CMD_FLUSH;
            ...
            cmd_name = CMD_DELETE;
    else if(dap_chain_node_cli_find_option_val(a_argv, arg_index, min(a_argc, arg_index + 1), "drop_table", NULL))
            cmd_name = CMD_DROP;
    ...
    for(size_t i = 0; i < l_objs_size; i++) {
        ...
        if (l_datum->header.type_id == DAP_CHAIN_DATUM_TX) {    // TODO rewrite it for support of multivhannel & conditional transactions
            ...
            if (obj_token) {
                ...
                bool l_found_main_ticker = false;
                dap_chain_datum_tx_t *l_current_tx = (dap_chain_datum_tx_t*)l_datum->data;
                for (size_t l_item_counter = 0; l_item_counter < l_current_tx->header.tx_items_size;) {
                    void *l_item = l_current_tx->tx_items + l_item_counter;
                    size_t l_item_size = dap_chain_datum_item_tx_get_size(l_item);
                    l_item_counter += l_item_size;
                    switch (dap_chain_datum_tx_item_get_type(l_item)) {
                        case TX_ITEM_TYPE_OUT_EXT: {
                            dap_chain_tx_out_ext_t* l_out_ext = (dap_chain_tx_out_ext_t*)l_item;
                            l_tickers = dap_list_append(l_tickers, dap_strdup(l_out_ext->token));
                            l_found_main_ticker = true;
                            break;
                        }
                        case TX_ITEM_TYPE_OUT_COND: {
                            dap_chain_tx_out_cond_t *l_cond = (dap_chain_tx_out_cond_t*)l_item;
                            if (l_cond->header.subtype == DAP_CHAIN_TX_OUT_COND_SUBTYPE_FEE) {
                                l_tickers = dap_list_append(l_tickers, dap_strdup(a_net->pub.native_ticker));
                            }
                            break;
                        }
                    }
                }
                if (!l_found_main_ticker) {
                    for (size_t l_item_counter = 0; l_item_counter < l_current_tx->header.tx_items_size;) {
                        //
                        dap_hash_fast_t l_tx_parent_hash = {0};
                        ...
                        switch (dap_chain_datum_tx_item_get_type(l_item)) {
                            case TX_ITEM_TYPE_IN: {
                                l_tx_parent_hash = ((dap_chain_tx_in_t *) l_item)->header.tx_prev_hash;
                                l_parrent_tx_out_idx = ((dap_chain_tx_in_t *) l_item)->header.tx_out_prev_idx;
                                l_tx_parent = dap_chain_ledger_tx_find_by_hash(a_net->pub.ledger, &l_tx_parent_hash);
                                break;
                            }
                        ...
                        if (l_out_unknown) {
                            switch (dap_chain_datum_tx_item_get_type(l_out_unknown)) {
                                case TX_ITEM_TYPE_OUT:
                                    l_current_token = dap_chain_ledger_tx_get_token_ticker_by_hash(a_net->pub.ledger,
                                                                                                   &l_tx_parent_hash);
                                    l_tickers = dap_list_append(l_tickers, dap_strdup(l_current_token));
                                    break;
                                case TX_ITEM_TYPE_OUT_EXT:
                                    ...
                                    break;
                                case TX_ITEM_TYPE_OUT_COND:
                                    if (((dap_chain_tx_out_cond_t *) l_out_unknown)->header.subtype !=
                                        DAP_CHAIN_TX_OUT_COND_SUBTYPE_FEE &&
                                        ((dap_chain_tx_out_cond_t *) l_out_unknown)->header.subtype !=
                                        DAP_CHAIN_TX_OUT_COND_SUBTYPE_FEE_STAKE) {
                                        l_current_token = dap_chain_ledger_tx_get_token_ticker_by_hash(
                                                a_net->pub.ledger, &l_tx_parent_hash);
                                        l_tickers = dap_list_append(l_tickers, dap_strdup(l_current_token));
                                    }
                                    break;
                            }
                        } else {
                            char *l_tx_prev_hash_str = dap_hash_fast_to_str_new(&l_tx_parent_hash);
                            dap_string_append_printf(a_str_tmp,
                                                     ": Transaction %s refers to an unchained transaction %s. \n",
                                                     l_objs[i].key, l_tx_prev_hash_str);
