...
static int s_cli_net_srv( int argc, char **argv, dap_json_t *a_json_arr_reply, int a_version)
{
    ...
    int l_ret = dap_chain_net_parse_net_chain(a_json_arr_reply, &arg_index, argc, argv, NULL, &l_net, CHAIN_TYPE_INVALID);
...
dap_chain_net_srv_price_t *dap_chain_net_srv_get_price_from_order(dap_chain_net_srv_t *a_service, dap_chain_net_srv_order_t *a_order)
{
    ...
    dap_cert_get_pkey_hash(a_service->receipt_sign_cert, DAP_HASH_TYPE_SHA3_256, (byte_t *)&l_price_pkey_hash, sizeof(l_price_pkey_hash));
...
// grace table clean
    ...
    dap_ht_foreach_hh(hh, a_srv->grace_hash_tab, l_gdata, l_gdata_tmp) {
