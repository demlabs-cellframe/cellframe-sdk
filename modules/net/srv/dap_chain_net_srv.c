...
static int s_cli_net_srv( int argc, char **argv, void **reply)
{
    ...
    int l_ret = dap_chain_node_cli_cmd_values_parse_net_chain( &arg_index, argc, argv, a_str_reply, NULL, &l_net, CHAIN_TYPE_INVALID);
    if ( l_net ) {
            ...
            // Update order
                ...
                if (l_order_hash_str) {
                    ...
                    if (!l_ret)
                        ...
                        l_ret = -9;
                        dap_string_append_printf(l_string_ret, "Cert is wrong!");
                    }
                } else {
                    l_ret = -9 ;
