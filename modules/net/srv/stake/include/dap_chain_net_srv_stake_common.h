...
typedef struct dap_chain_net_srv_stake_item {
    ...
    union {
        ...
        uint8_t hash_key[DAP_HASH_SHA3_256_SIZE];  // Aligned key for dap_ht
    } tx_hash;
    union {
        dap_hash_sha3_256_t hash;     // Decree hash (packed)
        uint8_t hash_key[DAP_HASH_SHA3_256_SIZE];  // Aligned key for dap_ht
    } decree_hash;
    ...
    dap_pkey_t *pkey;
    dap_ht_handle_t hh, ht;  // hh for signing_addr hash, ht for tx_hash
