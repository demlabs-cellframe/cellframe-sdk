...
/* Legacy Base58 decoder uses input-sized VLAs and appends a NUL byte.
 * Bound JSON fields before entering it; leading '1' bytes each decode to a byte. */
#define TX_JSON_ENCODED_FIELD_MAX (64U * 1024U)
static bool s_tx_json_base58(const char *a_text, void **a_out, size_t *a_size)
{
    size_t l_len = strnlen(a_text, TX_JSON_ENCODED_FIELD_MAX + 1);
    if (!l_len || l_len > TX_JSON_ENCODED_FIELD_MAX) return false;
    size_t l_capacity = l_len + 1;
    void *l_data = DAP_NEW_Z_SIZE(uint8_t, l_capacity);
    if (!l_data) return false;
    size_t l_size = dap_enc_base58_decode(a_text, l_data);
    if (!l_size || l_size >= l_capacity) { DAP_DELETE(l_data); return false; }
    *a_out = l_data;
    *a_size = l_size;
    return true;
}
...
static void s_tx_cond_all_with_spends_by_srv_uid_callback(dap_chain_net_t* a_net, dap_chain_datum_tx_t *a_tx, dap_hash_sha3_256_t *a_tx_hash, void *a_arg)
{
    ...
    TX_ITEM_ITER_TX(l_item, l_size, a_tx) {
        ...
        case TX_ITEM_TYPE_IN_COND: {
            ...
            dap_ht_find(l_ret->tx_outs, &l_prev_hash, sizeof(l_prev_hash), l_spends);
            if (l_spends) {
                ...
                *l_in = (dap_chain_datum_tx_spends_item_t) {
                ...
                dap_ht_add(l_ret->tx_ins, tx_hash, l_in);
        ...
        case TX_ITEM_TYPE_OUT_COND: {
            ...
            if (l_tx_out_cond->header.srv_uid.uint64 == l_arg->srv_uid.uint64) {
                ...
                *l_out = (dap_chain_datum_tx_spends_item_t) {
                ...
                dap_ht_add(l_ret->tx_outs, tx_hash, l_out);
                // ??? TODO?
            }
        } break;
        default:
...
void dap_chain_datum_tx_spends_item_free(dap_chain_datum_tx_spends_item_t * a_items)
{
    ...
    dap_ht_foreach(a_items,l_item,l_tmp){
        DAP_DELETE(l_item->tx);
        dap_ht_del(a_items, l_item);
        DAP_DELETE(l_item);
    }
}
...
//                dap_ht_foreach(l_chain->cells,l_cell, l_cell_tmp){
...
{
    dap_list_t * ret;
    dap_hash_sha3_256_t * tx_begin_hash;
    dap_chain_datum_tx_t * tx_last;
    dap_hash_sha3_256_t tx_last_hash;
 ...
 * @param a_arg
 */
static void s_get_tx_cond_chain_callback(dap_chain_net_t* a_net, dap_chain_datum_tx_t *a_tx, dap_hash_sha3_256_t *a_tx_hash, void *a_arg)
{
    ...
    if( l_args->ret ){
        ...
        while ((l_tx_item = dap_chain_datum_tx_item_get(a_tx, &l_item_idx, NULL, TX_ITEM_TYPE_IN_COND , NULL)) != NULL){
            ...
            if(dap_hash_sha3_256_compare(&l_in_cond->header.tx_prev_hash, &l_args->tx_last_hash) &&
        ...
        if (dap_hash_sha3_256_compare(l_tx_hash,l_args->tx_begin_hash)) {
            // Found condition
            int l_item_idx = 0;
 ...
 * @param a_arg
 */
static void s_get_tx_cond_all_for_addr_callback(dap_chain_net_t* a_net, dap_chain_datum_tx_t *a_datum_tx, dap_hash_sha3_256_t *a_hash, void *a_arg)
{
    UNUSED(a_net);
    UNUSED(a_hash);
...
static void s_tx_cond_all_by_srv_uid_callback(UNUSED_ARG dap_chain_net_t* a_net, dap_chain_datum_tx_t *a_tx, dap_hash_sha3_256_t *a_tx_hash, void *a_arg)
{
        ...
        switch (l_item_type) {
            case TX_ITEM_TYPE_IN: {
                ...
                if(l_prev_hash_str && dap_json_object_get_uint64_ext(l_json_item_obj, "out_prev_idx", &l_out_prev_idx)){
                    ...
                    if(!dap_hash_sha3_256_from_str(l_prev_hash_str, &l_tx_prev_hash)) {
                        //check out token
...
static uint8_t *s_dap_chain_net_tx_create_in_ems_item (dap_json_t *a_json_item_obj, dap_json_t *a_jobj_arr_errors) {
    ...
    if (l_json_item_token && l_is_chain_id){
        dap_hash_sha3_256_t l_token_ems_hash = {};
        const char *l_json_item_token_ems_hash = dap_json_object_get_string(a_json_item_obj, "token_ems_hash");
        if(l_json_item_token_ems_hash && dap_hash_sha3_256_from_str(l_json_item_token_ems_hash, &l_token_ems_hash)) {
...
static uint8_t *s_dap_chain_net_tx_create_in_reward_item (dap_json_t *a_json_item_obj, dap_json_t *a_jobj_arr_errors) {
    ...
    dap_hash_sha3_256_t l_block_hash;
    if(l_block_hash_str && !dap_hash_sha3_256_from_str(l_block_hash_str, &l_block_hash)) {             
                ...
                uint256_t *a_value_need, uint256_t *a_value_need_fee, dap_chain_addr_t *a_seller_addr, size_t i, dap_chain_net_t *a_net)
{
    ...
    switch (l_subtype) {
        case DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_PAY:{
            ...
            if (l_params_str) {
                if (!s_tx_json_base58(l_params_str, (void **)&l_params, &l_params_size)) return NULL;
            }
            ...
            if(l_pkey) {
                ...
                DAP_DELETE(l_pkey);
            } else if (l_pkey_hash_str && !dap_hash_sha3_256_from_str(l_pkey_hash_str, &l_pkey_hash)) {
        ...
        case DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_XCHANGE: {
            ...
            dap_chain_addr_t *l_seller_addr = l_seller_addr_str ? dap_chain_addr_from_str(l_seller_addr_str) : NULL;
            if (!l_seller_addr) return NULL;
            ...
            if (l_out_cond_item && l_subtype == DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_DEX) {
                if (!l_seller_addr) { DAP_DELETE(l_out_cond_item); l_out_cond_item = NULL; }
                else {
                    memset(&l_out_cond_item->subtype, 0, sizeof(l_out_cond_item->subtype));
                    l_out_cond_item->header.subtype = DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_DEX;
                    l_out_cond_item->header.srv_uid.uint64 = 0xA;
                    s_json_get_srv_uid(a_json_item_obj, "service_id", "service", &l_out_cond_item->header.srv_uid.uint64);
                    l_out_cond_item->subtype.srv_dex.buy_net_id = l_buy_net_id;
                    l_out_cond_item->subtype.srv_dex.sell_net_id = l_sell_net_id;
                    dap_strncpy(l_out_cond_item->subtype.srv_dex.buy_token, l_token_buy, DAP_CHAIN_TICKER_SIZE_MAX);
                    l_out_cond_item->subtype.srv_dex.seller_addr = *l_seller_addr;
                    l_out_cond_item->subtype.srv_dex.rate = l_value_rate;
                    const char *root = dap_json_object_get_string(a_json_item_obj, "order_root_hash");
                    int64_t pct = 0, version = 0, flags = 0, tx_type = 1;
                    dap_json_object_get_int64_ext(a_json_item_obj, "min_fill_pct", &pct);
                    dap_json_object_get_int64_ext(a_json_item_obj, "version", &version);
                    dap_json_object_get_int64_ext(a_json_item_obj, "flags", &flags);
                    dap_json_object_get_int64_ext(a_json_item_obj, "tx_type", &tx_type);
                    if (strlen(l_token_buy) >= DAP_CHAIN_TICKER_SIZE_MAX || pct < 0 || pct > 100 || version < 0 || version > 255 || flags < 0 || flags > UINT32_MAX || tx_type < 0 || tx_type > 255 ||
                        (root && dap_chain_hash_fast_from_str(root, &l_out_cond_item->subtype.srv_dex.order_root_hash))) {
                        DAP_DELETE(l_out_cond_item); l_out_cond_item = NULL;
                    } else {
                        l_out_cond_item->subtype.srv_dex.min_fill = pct | (dap_json_object_get_bool(a_json_item_obj, "min_from_origin") ? 0x80 : 0);
                        l_out_cond_item->subtype.srv_dex.version = version;
                        l_out_cond_item->subtype.srv_dex.flags = flags;
                        l_out_cond_item->subtype.srv_dex.tx_type = tx_type;
                        const char *expires = dap_json_object_get_string(a_json_item_obj, "ts_expires");
                        if (expires && *expires && dap_strcmp(expires, "never") && sscanf(expires, "%"DAP_UINT64_FORMAT_U, &l_out_cond_item->header.ts_expires) != 1)
                            l_out_cond_item->header.ts_expires = dap_time_from_str_rfc822(expires);
                    }
                }
            }
            DAP_DELETE(l_params);
        ...
        case DAP_CHAIN_TX_OUT_COND_SUBTYPE_WALLET_SHARED: {
            ...
            for(size_t j = 0; j < l_pkey_hashes_count; j++) {
                ...
                if(dap_hash_sha3_256_from_str(l_hash_str, l_pkey_hashes + j)) {
...
static uint8_t *s_dap_chain_net_tx_create_receipt_item(dap_json_t *a_json_item_obj, dap_json_t *a_jobj_arr_errors, dap_chain_datum_tx_t *a_tx, dap_list_t *a_sign_list, size_t i)
{
    ...
    dap_hash_sha3_256_from_str(l_prev_tx_hash_str, &l_prev_tx_hash);
...
static uint8_t *s_dap_chain_net_tx_create_tsd_item(dap_json_t *a_json_item_obj, dap_json_t *a_jobj_arr_errors, dap_chain_datum_tx_t *a_tx, dap_list_t *a_sign_list)
{
    ...
    if (l_tsd_type < 0 || l_tsd_type > UINT16_MAX ||
        l_tsd_data_size > TX_JSON_ENCODED_FIELD_MAX ||
        l_tsd_data_size > UINT32_MAX - sizeof(dap_tsd_t) - sizeof(dap_chain_tx_tsd_t)) return NULL;
    uint8_t *l_tsd_data = NULL;
    size_t l_tsd_data_size_decoded = 0;
    if (!s_tx_json_base58(l_tsd_data_str, (void **)&l_tsd_data, &l_tsd_data_size_decoded)) return NULL;
    if (l_tsd_data_size_decoded != l_tsd_data_size) {
        ...
        dap_json_rpc_error_add(a_jobj_arr_errors, -1, "Json TX: data size in tsd section - %zu, expected - %"DAP_UINT64_FORMAT_U, l_tsd_data_size_decoded, l_tsd_data_size);
...
uint8_t *s_dap_chain_net_tx_create_sig_item(dap_json_t *a_json_item_obj, dap_json_t *a_jobj_arr_errors, dap_chain_datum_tx_t *a_tx, dap_list_t **a_sign_list)
{
    ...
    uint64_t
        l_sign_size = 0,
        l_sign_b64_strlen = strnlen(l_sign_b64_str, UINT32_MAX),
    ...
    if (l_sign_b64_strlen >= UINT32_MAX ||
        l_sign_decoded_size > UINT32_MAX - sizeof(dap_chain_tx_sig_t) ||
        l_sign_decoded_size > SIZE_MAX - sizeof(dap_chain_tx_sig_t)) return NULL;
    ...
    if ( l_tx_sig->header.sig_size < sizeof(dap_sign_t) ||
         ...
         (uint64_t)sizeof(dap_sign_t) + ((dap_sign_t *)l_tx_sig->sig)->header.sign_size +
             ((dap_sign_t *)l_tx_sig->sig)->header.sign_pkey_size != l_sign_size ) {
...
uint8_t *s_dap_chain_net_tx_create_vote_item(dap_json_t *a_json_item_obj, dap_json_t *a_jobj_arr_errors)
{
    ...
    if(l_voting_hash_str ) {
        ...
        if(dap_json_object_get_uint64_ext(a_json_item_obj, "answer_idx", &l_value_idx) && !dap_hash_sha3_256_from_str(l_voting_hash_str, &l_voting_hash)) {                             
        ...
        dap_chain_datum_tx_t** a_out_tx, size_t* a_items_count, size_t *a_items_ready)
{
    ...
    for(size_t i = 0; i < l_items_count; ++i) {
        ...
        if (!l_item) {
            ...
            if (l_hash_str) {
                char *l_hash_str_current = dap_hash_sha3_256_str_new(l_item, dap_chain_datum_item_tx_get_size(l_item, 0));
...
int dap_chain_net_tx_to_json(dap_chain_datum_tx_t *a_tx, dap_json_t *a_out_json)
{
    ...
    char *l_tx_hash_str = dap_hash_sha3_256_str_new(a_tx, dap_chain_datum_tx_get_size(a_tx));
...
/**
 * @brief Initialize net-tx module
 * @details Registers token CLI commands
 * @return 0 on success
 */
int dap_chain_net_tx_init(void)
{
    // Register token CLI commands
    int l_cli_res = dap_chain_token_cli_init();
    if (l_cli_res != 0) {
        log_it(L_WARNING, "Failed to initialize token CLI commands: %d", l_cli_res);
    }
    
    log_it(L_INFO, "Net-TX module initialized");
    return 0;
}

/**
 * @brief Cleanup net-tx module
 */
void dap_chain_net_tx_deinit(void)
{
    dap_chain_token_cli_deinit();
    log_it(L_INFO, "Net-TX module deinitialized");
}
