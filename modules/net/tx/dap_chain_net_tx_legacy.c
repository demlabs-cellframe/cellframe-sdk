        ...
        size_t a_tx_num)
{
    ...
    if(!a_chain | !a_key_from || !a_addr_from || !a_key_from->priv_key_data || !a_key_from->priv_key_data_size ||
        ...
        log_it(L_ERROR, "Wrong parameters in dap_chain_net_tx_create_massive() call");
        return -1;

    }
    const char *l_native_ticker = dap_chain_net_by_id(a_chain->net_id)->pub.native_ticker;
    if (dap_strcmp(a_token_ticker, l_native_ticker)) {
        log_it(L_WARNING, "dap_chain_net_tx_create_massive() should only be used with native token");
        return -2;
    }
    ...
    dap_ledger_t *l_ledger = dap_chain_net_by_id(a_chain->net_id)->pub.ledger;
    dap_list_t *l_list_used_out = dap_ledger_get_utxo_for_value(l_ledger, a_token_ticker,
                                                                             a_addr_from, l_value_need, &l_value_transfer);
    if (!l_list_used_out) {
        log_it(L_WARNING,"Not enough funds to transfer");
        DAP_DELETE(l_objs);
                                       ...
                                       const char *a_hash_out_type, uint256_t a_value_fee)
{
    ...
    if (not_native && !IS_ZERO_256(l_total_fee))
    {
        ...
        dap_ledger_t *l_ledger = dap_chain_net_by_id(a_chain->net_id)->pub.ledger;
        // list of transaction with 'out' items
        dap_list_t *l_list_used_out = dap_ledger_get_utxo_for_value(l_ledger, l_native_ticker,
