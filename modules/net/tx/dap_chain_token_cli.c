...
#include "dap_cert.h"
#include "dap_global_db.h"  // For GDB read (dap_global_db_store_obj_t)
...
#include "dap_strfuncs.h"
#include "dap_hash.h"
#include "dap_chain_net_types.h"  // For dap_chain_net_t
#include "dap_chain_net_core.h"   // For dap_chain_net_by_name, dap_chain_net_parse_net_chain
// REMOVED dap_chain_net.h - creates cycle! Use net/core for registry
#include "dap_time.h"
 ...
 */
static int s_parse_net_chain_args(dap_json_t *a_json_arr_reply, int *a_arg_index,
...
int com_token_decl_sign(int a_argc, char **a_argv, dap_json_t *a_json_arr_reply, UNUSED_ARG int a_version)
{
    ...
    if (!dap_strncmp(l_datum_hash_str, "0x", 2) || !dap_strncmp(l_datum_hash_str, "0X", 2)) {
        if (dap_hash_sha3_256_from_hex_str(l_datum_hash_str, &l_datum_hash)) {
        ...
        if (dap_hash_sha3_256_from_base58_str(l_datum_hash_str, &l_datum_hash)) {
            dap_json_rpc_error_add(a_json_arr_reply, DAP_CHAIN_NODE_CLI_COM_TOKEN_DECL_SIGN_HEX_PARAM_ERR,
                "token_decl_sign <datum hash> argument is incorrect");
            return DAP_CHAIN_NODE_CLI_COM_TOKEN_DECL_SIGN_HEX_PARAM_ERR;
        }
    ...
    s_parse_net_chain_args(a_json_arr_reply, &arg_index, a_argc, a_argv, &l_chain, &l_net, CHAIN_TYPE_TOKEN);
    if (!l_net)
...
static int s_parse_common_token_decl_arg(int a_argc, char ** a_argv, dap_json_t *a_json_arr_reply, dap_sdk_cli_params *a_params, bool a_update_token)
{
    ...
    int l_arg_index = 0;
    int l_res = s_parse_net_chain_args(a_json_arr_reply, &l_arg_index, a_argc, a_argv,
...
static int s_parse_additional_token_decl_arg(int a_argc, char ** a_argv, dap_json_t *a_json_arr_reply, dap_sdk_cli_params *a_params, bool a_update_token)
{
    ...
    if (l_remove_signs) {
        ...
        for (; l_remove_signs_str; l_remove_signs_str = strtok_r(NULL, ",", &l_remove_signs_ptrs)) {
            ...
            if (dap_hash_sha3_256_from_str(l_remove_signs_str, &l_hf) == 0) {
                dap_tsd_t *l_hf_tsd = dap_tsd_create(DAP_CHAIN_DATUM_TOKEN_TSD_TYPE_TOTAL_PKEYS_REMOVE, &l_hf, sizeof(dap_hash_sha3_256_t));
...
//				l_tsd_section.token_from = dap_hash_sha3_256();
                        ...
                        if(l_tsd->size >= sizeof(dap_pkey_t)){
                            ...
                            if (!dap_pkey_get_hash(l_pkey, &l_hf)) {
                                ...
                                log_it(L_DEBUG, "== TOTAL_PKEYS_ADD: %s",
                                    dap_hash_sha3_256_to_str_static(&l_hf));
                            }
                        } else
...
int com_token_emit(int a_argc, char **a_argv, dap_json_t *a_json_arr_reply, UNUSED_ARG int a_version)
{
    ...
    if(dap_strcmp(l_hash_out_type,"hex") && dap_strcmp(l_hash_out_type,"base58")) {
        dap_json_rpc_error_add(a_json_arr_reply, DAP_CHAIN_NODE_CLI_COM_TOKEN_EMIT_H_PARAM_ERR,
                                   "invalid parameter -H, valid values: -H <hex | base58>");
        l_ret = -DAP_CHAIN_NODE_CLI_COM_TOKEN_EMIT_H_PARAM_ERR;
        goto RET_CLEANUP;
    }

    s_parse_net_chain_args(a_json_arr_reply, &arg_index,a_argc,a_argv,NULL, &l_net, CHAIN_TYPE_INVALID);
    ...
    if(!l_certs_str) {
        ...
        l_ret = -DAP_CHAIN_NODE_CLI_COM_TOKEN_EMIT_REQUIRES_PARAMETER_CERTS;
        goto RET_CLEANUP;
    }
    ...
    if (!l_add_sign) {      //Create the emission
        ...
        if (IS_ZERO_256(l_emission_value)) {
            ...
            l_ret = -DAP_CHAIN_NODE_CLI_COM_TOKEN_EMIT_REQUIRES_PARAMETER_EMISSION_VAL;
            goto RET_CLEANUP;
        }
        ...
        if(!l_addr_str) {
            ...
            l_ret = -DAP_CHAIN_NODE_CLI_COM_TOKEN_EMIT_REQUIRES_PARAMETER_ADDR;
            goto RET_CLEANUP;
        }
        ...
        if(!l_ticker) {
            ...
            l_ret = -DAP_CHAIN_NODE_CLI_COM_TOKEN_EMIT_REQUIRES_PARAMETER_TOKEN;
            goto RET_CLEANUP;
        }
        ...
        if(!l_addr) {
            ...
            l_ret = -DAP_CHAIN_NODE_CLI_COM_TOKEN_EMIT_ADDR_INVALID_ERR;
            goto RET_CLEANUP;
        }
        ...
        if (l_chain_emission == NULL) { // Can't find such chain
            ...
            l_ret = -DAP_CHAIN_NODE_CLI_COM_TOKEN_EMIT_REQUIRES_PARAMETER_CHAIN_EMISSION;
            goto RET_CLEANUP;
        }
    } else {
        if (l_emission_hash_str) {
            dap_dl_foreach(l_net->pub.chains, l_chain_emission) {
                ...
                dap_global_db_store_obj_t *l_store_obj = dap_global_db_get_raw_sync(l_mempool_group, l_emission_hash_str);
            ...
            if (!l_emission){
                ...
                l_ret = -DAP_CHAIN_NODE_CLI_COM_TOKEN_EMIT_CANT_FIND_EMI_ERR;
                goto RET_CLEANUP;
            }
        } else {
            ...
            l_ret = -DAP_CHAIN_NODE_CLI_COM_TOKEN_EMIT_REQUIRES_PARAMETER_EMISSION;
            goto RET_CLEANUP;
        }
    }
    ...
    dap_hash_sha3_256(l_emission, l_emission_size, &l_emission_hash);
    // Produce datum
...
int com_chain_ca_pub( int a_argc,  char ** a_argv, dap_json_t *a_json_arr_reply, int a_version)
{
    ...
    s_parse_net_chain_args(a_json_arr_reply, &arg_index,a_argc, a_argv, &l_chain, &l_net, CHAIN_TYPE_CA);
...
/**
 * @brief Initialize chain/token CLI commands
 */
