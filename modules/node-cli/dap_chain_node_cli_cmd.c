    ...
    for (size_t i = l_arr_start; i < l_arr_end; i++) {
        ...
        switch (l_datum->header.type_id) {
            case DAP_CHAIN_DATUM_TX: {
                ...
                if (!json_object_array_length(l_jobj_emit_delegate_list))
                    json_object_object_del(l_jobj_datum, "srv_emit_delegate");
...
dap_chain_node_cli_cmd_t dap_chain_node_cli_cmd_id_from_str(const char *a_cmd_str)
{
    ...
    if (!strcmp(a_cmd_str, "auction")) return DAP_CHAIN_NODE_CLI_CMD_ID_AUCTION;
