        ...
        const char *a_alias, dap_json_t* a_json_arr_reply)
{
    ...
    if ((a_addr && a_addr->uint64) || a_alias) {
        ...
        if(!l_nodes_count || !l_objs) {
            ...
            if (!json_node_list_obj) return dap_dap_json_object_free(json_node_list_obj), DAP_CHAIN_NODE_CLI_COM_NODE_MEMORY_ALLOC_ERR;
            ...
            if (!json_node_list_arr) return dap_dap_json_object_free(json_node_list_obj), DAP_CHAIN_NODE_CLI_COM_NODE_MEMORY_ALLOC_ERR;
            ...
            for (size_t i = 0; i < l_nodes_count; i++) {
                ...
                if (!json_node_obj) return dap_dap_json_object_free(json_node_list_obj), DAP_CHAIN_NODE_CLI_COM_NODE_MEMORY_ALLOC_ERR;
...
int com_global_db(int a_argc, char ** a_argv, void **a_str_reply, int a_version)
{
    ...
    case CMD_FLUSH:
    {
        ...
        case 0:
            ...
            dap_json_object_add_object(json_obj_flush, a_version == 1 ? "command status" : "command_status", json_object_new_string("Commit data base and filesystem caches to disk completed.\n\n"));
    ...
    case CMD_RECORD:
    {
        ...
        json_object* json_obj_rec = dap_json_object_new();
        int l_ret = 0;
        ...
        switch (l_subcmd) {
            case SUMCMD_GET: // Get value
            {
                ...
                if(!l_value_str) {
                    ...
                    DAP_DELETE(l_value);
                    dap_dap_json_object_free(json_obj_rec);
                    return -DAP_CHAIN_NODE_CLI_COM_GLOBAL_DB_MEMORY_ERR;
                }
                dap_json_object_add_object(json_obj_rec, a_version == 1 ? "command status" : "command_status",json_object_new_string("Commit data base and filesystem caches to disk completed."));
                ...
                if (a_version == 1) {
                    ...
                    dap_json_object_add_object(json_obj_rec, "pinned", json_object_new_boolean(l_is_pinned));
                }
                dap_json_object_add_object(json_obj_rec, "value", json_object_new_string(l_value_str));
            ...
            {
                if(l_is_pinned){
                    dap_json_object_add_object(json_obj_rec, a_version == 1 ? "pinned status" : "pinned_status",json_object_new_string("record already pinned"));
                    break;
                }
                if(dap_global_db_pin_sync( l_group, l_key) ==0 ){
                    dap_json_object_add_object(json_obj_rec, a_version == 1 ? "pinned status" : "pinned_status",json_object_new_string("record successfully pinned"));
                }
                else{
            ...
            {
                if(!l_is_pinned) {
                    dap_json_object_add_object(json_obj_rec, a_version == 1 ? "unpinned status" : "unpinned_status",json_object_new_string("record already unpinned"));
                    break;
                }
                if(dap_global_db_unpin_sync(l_group,l_key) == 0 ) {
                    dap_json_object_add_object(json_obj_rec, a_version == 1 ? "unpinned status" : "unpinned_status",json_object_new_string("record successfully unpinned"));
                }
                else {
    ...
    case CMD_WRITE:
    {
        ...
        if (!dap_global_db_set_sync(l_group_str, l_key_str, l_value_str, strlen(l_value_str) +1 , false)) {
            json_object* json_obj_write = dap_json_object_new();
            dap_json_object_add_object(json_obj_write, a_version == 1 ? "write status" : "write_status",json_object_new_string("Data has been successfully written to the database"));
    ...
    case CMD_READ:
    {
        ...
        json_object* json_obj_read = dap_json_object_new();
        if (l_ts) {
            char l_ts_str[80] = { '\0' };
            ...
            if (l_value_hexdump) {
                ...
                dap_json_object_add_object(json_obj_read, "group", json_object_new_string(l_group_str));
                dap_json_object_add_object(json_obj_read, "key", json_object_new_string(l_key_str));
                dap_json_object_add_object(json_obj_read, "time", json_object_new_string(l_ts_str));
                dap_json_object_add_object(json_obj_read, a_version == 1 ? "value len" : "value_len",json_object_new_uint64(l_out_len));
                dap_json_object_add_object(json_obj_read, a_version == 1 ? "value hex" : "value_hex",json_object_new_string(l_value_hexdump_new));
            ...
            if (!l_read_obj || !l_read_obj->value || !l_read_obj->value_len) {
               dap_json_rpc_error_add(*a_json_arr_reply, DAP_CHAIN_NODE_CLI_COM_GLOBAL_DB_TIME_NO_VALUE,
                ...
                dap_json_object_add_object(json_obj_read, "group", json_object_new_string(l_group_str));
                dap_json_object_add_object(json_obj_read, "key", json_object_new_string(l_key_str));
                dap_json_object_add_object(json_obj_read, "error", json_object_new_string((char*)l_read_obj->value));
            }
    ...
    case CMD_DELETE:
    {
        ...
        if(!l_key_str) {
            ...
            json_object* json_obj_del = dap_json_object_new();
            dap_json_object_add_object(json_obj_del, a_version == 1 ? "Removed records" : "records_removed",json_object_new_uint64(j));
            dap_json_object_add_object(json_obj_del, a_version == 1 ? "of records" : "records_total",json_object_new_uint64(i));
            dap_json_object_add_object(json_obj_del, a_version == 1 ? "in table" : "table",json_object_new_string(l_group_str));
        ...
        if (l_del_success) {
            json_object* json_obj_del = dap_json_object_new();
            dap_json_object_add_object(json_obj_del, a_version == 1 ? "Record key" : "record_key",json_object_new_string(l_key_str));
            dap_json_object_add_object(json_obj_del, a_version == 1 ? "Group name" : "group_name",json_object_new_string(l_group_str));
    ...
    case CMD_DROP:
    {
        ...
        {
            json_object* json_obj_drop = dap_json_object_new();
            dap_json_object_add_object(json_obj_drop, a_version == 1 ? "Dropped table" : "table_dropped",json_object_new_string(l_group_str));
    ...
    case CMD_GET_KEYS:
    {
        ...
        json_object* json_arr_keys = dap_json_array_new();
        json_object* json_obj_keys = NULL;
        for(size_t i = 0; i < l_objs_count; i++) {
            char l_ts[64] = { '\0' };
            dap_nanotime_to_str_rfc822(l_ts, sizeof(l_ts), l_objs[i].timestamp);
            json_obj_keys = dap_json_object_new();
            dap_json_object_add_object(json_obj_keys, "key", json_object_new_string(l_objs[i].key));
            dap_json_object_add_object(json_obj_keys, "time", json_object_new_string(l_ts));
            dap_json_object_add_object(json_obj_keys, "type", json_object_new_string(
        ...
        json_object* json_keys_list = dap_json_object_new();
        dap_json_object_add_object(json_keys_list, a_version == 1 ? "group name" : "group_name",json_object_new_string(l_group_str));
    ...
    case CMD_GROUP_LIST: {
        json_object* json_group_list = dap_json_object_new();
        dap_list_t *l_group_list = dap_global_db_driver_get_groups_by_mask("*");
        size_t l_count = 0;
        json_object* json_arr_group = dap_json_array_new();
...
int com_node(int a_argc, char ** a_argv, void **a_str_reply, int a_version)
{
    ...
    case CMD_ADD: {
        ...
        if (l_addr_str || l_hostname) {
            ...
            if (l_res) {
                ...
                if (!json_obj_out) return dap_dap_json_object_free(json_obj_out), DAP_CHAIN_NODE_CLI_COM_NODE_MEMORY_ALLOC_ERR;
                dap_json_object_add_object(json_obj_out, "successfully_added_node", json_object_new_string(l_addr_str));
    ...
    case CMD_DEL: {
        ...
        if (l_addr_str) {
            ...
            if (l_res)
                ...
                json_object* json_obj_out = dap_json_object_new();
                if (!json_obj_out) return dap_dap_json_object_free(json_obj_out), DAP_CHAIN_NODE_CLI_COM_NODE_MEMORY_ALLOC_ERR;
                dap_json_object_add_object(json_obj_out, "successfully_deleted_node", json_object_new_string(l_addr_str));
                dap_json_array_add(*a_json_arr_reply, json_obj_out);
            }
            return l_res;
        }
    ...
    case CMD_DUMP: {
        ...
        dap_json_object_add_object(json_obj_out, "status_dump", json_object_new_string(l_string_reply->str));
        dap_json_array_add(*a_json_arr_reply, json_obj_out);
    ...
    case  CMD_BAN: {
        ...
        json_object* json_obj_out = dap_json_object_new();
        if (!json_obj_out) return dap_dap_json_object_free(json_obj_out), DAP_CHAIN_NODE_CLI_COM_NODE_MEMORY_ALLOC_ERR;
        dap_json_object_add_object(json_obj_out, "datum_placed_status", l_key_str_out ? json_object_new_string(l_key_str_out) :
...
#define DAP_VERSION "0.9-15"
    ...
    dap_json_object_add_object(json_obj_out, "status", json_object_new_string(l_vers));
    DAP_DELETE(l_vers);
    dap_json_array_add(*a_json_arr_reply, json_obj_out);
    return 0;
}
                ...
                if (l_wallet) {
                    if (a_addr) {
                        ...
                        if (l_addr && dap_chain_addr_compare(l_addr, a_addr)) {
                            dap_json_object_add_object(json_obj_wall, "wallet", json_object_new_string(l_file_name));
                            ...
                            dap_json_object_add_object(json_obj_wall, "deprecated", json_object_new_string(
                                                        strlen(dap_chain_wallet_check_sign(l_wallet))!=0 ? "true" : "false"));
                        }
                        else {
                            dap_dap_json_object_free(json_obj_wall);
                    ...
                    dap_json_object_add_object(json_obj_wall, a_version == 1 ? "Wallet" : "wallet", json_object_new_string(l_file_name));
                    if(l_wallet->flags & DAP_WALLET$M_FL_ACTIVE)
                        dap_json_object_add_string(json_obj_wall, "status", "protected-active");
                    else
                        dap_json_object_add_string(json_obj_wall, "status", "unprotected");
                    dap_json_object_add_object(json_obj_wall, "deprecated", json_object_new_string(
                            strlen(dap_chain_wallet_check_sign(l_wallet))!=0 ? "true" : "false"));
                    //if (l_addr_str) {
                    //    dap_json_object_add_object(json_obj_wall, "addr", json_object_new_string(l_addr_str));
                    // }
                    dap_chain_wallet_close(l_wallet);
                } else if (!a_addr){
                    dap_json_object_add_object(json_obj_wall, a_version == 1 ? "Wallet" : "wallet", json_object_new_string(l_file_name));
...
int com_tx_wallet(int a_argc, char **a_argv, void **a_str_reply, int a_version)
{
    ...
    switch (cmd_num) {
        ...
        case CMD_WALLET_INFO: {
            ...
            if(l_wallet)
            {
                dap_json_object_add_object(json_obj_wall, "sign", json_object_new_string(
                                                                  strlen(dap_chain_wallet_check_sign(l_wallet))!=0 ?
                                                                  dap_chain_wallet_check_sign(l_wallet) : "correct"));
                dap_json_object_add_object(json_obj_wall, "wallet", json_object_new_string(l_wallet->name));
            }
            ...
            if (l_addr_tokens_size) {
                ...
                for(size_t i = 0; i < l_addr_tokens_size; i++) {
                    ...
                    dap_json_t *l_jobj_ticker = json_object_new_string(l_addr_tokens[i]);
                    const char *l_description =  dap_ledger_get_description_by_ticker(l_ledger, l_addr_tokens[i]);
                    dap_json_t *l_jobj_description = l_description ? json_object_new_string(l_description)
                    ...
                    dap_json_object_add_object(j_balance_data, "coins", json_object_new_string(l_balance_coins));
                    dap_json_object_add_object(j_balance_data, "datoshi", json_object_new_string(l_balance_datoshi));
            ...
            if (l_locked_outs) {
                ...
                LL_FOREACH_SAFE(l_locked_outs, it, tmp) {
                    dap_json_t *l_jobj_token = dap_json_object_new();
                    dap_json_t *l_jobj_ticker = json_object_new_string(it->ticker);
                    ...
                    dap_json_object_add_object(j_balance_data, "locked_until", json_object_new_string(ts));
        ...
        case CMD_WALLET_OUTPUTS: {
            ...
            dap_json_object_add_object(json_obj_wall, "wallet_addr", json_object_new_string(dap_chain_addr_to_str_static(l_addr)));
            struct dap_json_t *l_json_outs_arr = dap_json_array_new();
            if (!l_json_outs_arr)
                return dap_dap_json_object_free(json_arr_out), DAP_CHAIN_NODE_CLI_COM_TX_WALLET_MEMORY_ERR;
            for (dap_list_t *l_temp = l_outs_list; l_temp; l_temp = l_temp->next) {
                ...
                dap_json_object_add_object(json_obj_item,"item_type", json_object_new_string(l_cond_outs ? "unspent_cond_out" : "unspent_out"));
                dap_json_object_add_object(json_obj_item,"value_coins", json_object_new_string(l_out_value_coins_str));
                dap_json_object_add_object(json_obj_item,a_version == 1 ? "value_datosi" : "value_datoshi", json_object_new_string(l_out_value_str));
                dap_json_object_add_object(json_obj_item,"prev_hash", json_object_new_string(dap_hash_fast_to_str_static(&l_item->tx_hash_fast)));
                dap_json_object_add_object(json_obj_item,"out_prev_idx", json_object_new_int64(l_item->num_idx_out));
            ...
            dap_json_object_add_object(json_obj_wall, "total_value_coins", json_object_new_string(l_out_total_value_coins_str));
            dap_json_object_add_object(json_obj_wall, "total_value_datoshi", json_object_new_string(l_out_total_value_str));
        ...
        default: {
            ...
            switch (cmd_num) {
                ...
                case CMD_WALLET_DEACTIVATE: {
                    ...
                    switch (l_rc) {
                    case 0:
                        dap_json_object_add_object(json_obj_wall, a_version == 1 ? "Wallet name" : "wallet_name", json_object_new_string(l_wallet_name));
                        ...
                        struct dap_json_t *l_json_wallets = wallet_list_json_collect(a_version);
                        dap_notify_server_send(json_object_get_string(l_json_wallets));
                        dap_dap_json_object_free(l_json_wallets);
                        break;
                    case -EBUSY:
                ...
                case CMD_WALLET_CONVERT: {
                    ...
                    dap_json_object_add_object(json_obj_wall, a_version == 1 ? "Sign wallet" : "sig_wallet", json_object_new_string(
                                                                              strlen(dap_chain_wallet_check_sign(l_wallet))!=0 ?
                                                                              dap_chain_wallet_check_sign(l_wallet) : "correct"));
                    dap_json_object_add_object(json_obj_wall, a_version == 1 ? "Wallet name" : "wallet_name", json_object_new_string(l_wallet_name));
                    dap_json_object_add_object(json_obj_wall, a_version == 1 ? "Status" : "status", json_object_new_string( a_version == 1 ? "successfully converted" : "success"));
                    dap_chain_wallet_close(l_wallet);
                    dap_json_array_add(json_arr_out, json_obj_wall);
                    break;
                }
                ...
                case CMD_WALLET_NEW: {
                    ...
                    json_object * json_obj_wall = dap_json_object_new();
                    dap_json_object_add_object(json_obj_wall, a_version == 1 ? "Wallet name" : "wallet_name", json_object_new_string(l_wallet->name));
                    if (l_sign_count > 1) {
                        ...
                        dap_json_object_add_object(json_obj_wall, a_version == 1 ? "Sign type" : "sig_type", json_object_new_string(l_signs_types_str->str));
                        dap_string_free(l_signs_types_str, true);
                    } else
                        dap_json_object_add_object(json_obj_wall, a_version == 1 ? "Sign type" : "sig_type", json_object_new_string(l_sign_type_str));
                    dap_json_object_add_object(json_obj_wall, a_version == 1 ? "Status" : "status", json_object_new_string(a_version == 1 ? "successfully created" : "success"));

                    const char *l_addr_str = NULL;
                    if ( l_net && (l_addr_str = dap_chain_addr_to_str_static(dap_chain_wallet_get_addr(l_wallet,l_net->pub.id))) ) {
                        dap_json_object_add_object(json_obj_wall, a_version == 1 ? "new address" : "new_addr", json_object_new_string(l_addr_str) );
                    }
                    dap_json_array_add(json_arr_out, json_obj_wall);
                    dap_chain_wallet_close(l_wallet);
                    // Notify about wallet
...
void s_com_mempool_list_print_for_chain(json_object* a_json_arr_reply, dap_chain_net_t * a_net, dap_chain_t * a_chain, const char * a_add,
    ...
    if (l_objs_count == 0 || l_objs_count < l_offset) {
        ...
        l_jobj_datums = dap_json_array_new();
        if (!l_jobj_datums) {
            dap_dap_json_object_free(l_obj_chain);
            dap_json_rpc_allocation_error(a_json_arr_reply);
            return;
        ...
        if (l_offset) {
            ...
            dap_json_t *l_jobj_offset = json_object_new_uint64(l_offset);
        ...
        if (a_limit) {
            ...
            dap_json_t *l_jobj_limit = json_object_new_uint64(l_arr_end);
        ...
        for (size_t i = l_arr_start; i < l_arr_end; i++) {
            ...
            dap_json_t *l_jobj_type = json_object_new_string(l_datum_type);
            dap_json_t *l_jobj_hash = json_object_new_string(l_objs[i].key);
            dap_json_t *l_jobj_ts_created = dap_json_object_new();
            dap_json_t *l_jobj_ts_created_time_stamp = json_object_new_uint64(l_ts_create);
            dap_json_t *l_jobj_ts_created_str = json_object_new_string(buff_time);
            if (!l_jobj_type || !l_jobj_hash || !l_jobj_ts_created || !l_jobj_ts_created_str ||
                !l_jobj_ts_created_time_stamp) {
                dap_dap_json_object_free(l_jobj_type);
                dap_dap_json_object_free(l_jobj_hash);
                dap_dap_json_object_free(l_jobj_ts_created);
                dap_dap_json_object_free(l_jobj_ts_created_time_stamp);
                dap_dap_json_object_free(l_jobj_ts_created_str);
                dap_dap_json_object_free(l_jobj_datums);
                dap_dap_json_object_free(l_obj_chain);
            ...
            if (!dap_hash_fast_compare(&l_datum_real_hash, &l_datum_hash_from_key)) {
                ...
                if (!l_wgn) {
                    dap_global_db_objs_delete(l_objs, l_objs_count);
                    dap_dap_json_object_free(l_jobj_datum);
                    dap_dap_json_object_free(l_obj_chain);
                    dap_dap_json_object_free(l_jobj_type);
                    dap_dap_json_object_free(l_jobj_hash);
                    dap_dap_json_object_free(l_jobj_ts_created);
                ...
                dap_json_t *l_jobj_warning = json_object_new_string(l_wgn);
                DAP_DELETE(l_wgn);
                if (!l_jobj_warning) {
                    dap_global_db_objs_delete(l_objs, l_objs_count);
                    dap_dap_json_object_free(l_jobj_datum);
            ...
            if (!a_fast) {
                switch (l_datum->header.type_id) {
                    case DAP_CHAIN_DATUM_TX: {
                        ...
                        dap_json_t *l_jobj_main_ticker = json_object_new_string(
                                l_main_ticker ? l_main_ticker : "UNKNOWN");
                        dap_json_t *l_jobj_ledger_rc = json_object_new_string(l_ledger_rc_str);
                        ...
                        {
                            dap_json_object_add_object(l_jobj_datum, "service", json_object_new_string(service_name));
                            dap_json_object_add_object(l_jobj_datum, "action", json_object_new_string(dap_ledger_tx_action_str(action)));
                        }
                        else
                        {   
                            dap_json_object_add_string(l_jobj_datum, "service", "UNKNOWN");
                            dap_json_object_add_string(l_jobj_datum, "action", "UNKNOWN");
                        }
                        dap_json_object_add_object(l_jobj_datum, "batching", json_object_new_string(!dap_chain_datum_tx_item_get_tsd_by_type(l_tx, DAP_CHAIN_DATUM_TRANSFER_TSD_TYPE_OUT_COUNT) ? "false" : "true"));
                        ...
                        if (!l_sig) {
                            dap_json_t *l_jobj_wgn = json_object_new_string(
                        ...
                        if (l_list_in_reward) {
                            ...
                            for (dap_list_t *it = l_list_in_reward; it; it = it->next) {
                                ...
                                dap_json_t *l_jobj_block_hash = json_object_new_string(l_block_hash);
                                if (!l_jobj_block_hash) {
                                    DAP_DELETE(l_block_hash);
                                    dap_dap_json_object_free(l_obj_in_reward_arary);
                                    dap_list_free(l_list_in_reward);
                                    dap_dap_json_object_free(l_jobj_datum);
                                    dap_dap_json_object_free(l_jobj_datums);
                                    dap_dap_json_object_free(l_obj_chain);
                           ...
                           dap_list_free(l_list_in_reward);
                            ...
                            dap_json_t *l_jobj_addr_from = json_object_new_string(dap_chain_addr_to_str_static(&l_addr_from));
                            if (!l_jobj_addr_from) {
                                dap_dap_json_object_free(l_jobj_datum);
                                dap_dap_json_object_free(l_jobj_datums);
                                dap_dap_json_object_free(l_obj_chain);
                            ...
                            dap_json_object_add_object(l_jobj_datum, "from", l_jobj_addr_from);
                        }
                        ...
                        if (!l_jobj_to_list || !l_jobj_change_list || !l_jobj_fee_list || !l_jobj_stake_lock_list ||
                            ...
                            dap_dap_json_object_free(l_jobj_to_list);
                            dap_dap_json_object_free(l_jobj_change_list);
                            dap_dap_json_object_free(l_jobj_to_from_emi);
                            dap_dap_json_object_free(l_jobj_fee_list);
                            dap_dap_json_object_free(l_jobj_stake_lock_list);
                            dap_dap_json_object_free(l_jobj_xchange_list);
                            dap_dap_json_object_free(l_jobj_stake_pos_delegate_list);
                            dap_dap_json_object_free(l_jobj_emit_delegate_list);
                            dap_dap_json_object_free(l_jobj_pay_list);
                            dap_dap_json_object_free(l_jobj_tx_vote);
                            dap_dap_json_object_free(l_jobj_tx_voting);
                            dap_dap_json_object_free(l_jobj_datum);
                            dap_dap_json_object_free(l_jobj_datums);
                            dap_dap_json_object_free(l_obj_chain);
                            dap_global_db_objs_delete(l_objs, l_objs_count);
                        ...
                        for (dap_list_t *it = l_list_out_items; it; it = it->next) {
                            ...
                            dap_json_object_add_object(l_jobj_money, "value", json_object_new_string(l_value_str));
                            dap_json_object_add_object(l_jobj_money, "coins", json_object_new_string(l_value_coins_str));
                            if (l_dist_token) {
                                dap_json_t *l_jobj_token = json_object_new_string(l_dist_token);
                                if (!l_jobj_token) {
                                    dap_dap_json_object_free(l_jobj_to_list);
                                    dap_dap_json_object_free(l_jobj_change_list);
                                    dap_dap_json_object_free(l_jobj_to_from_emi);
                                    dap_dap_json_object_free(l_jobj_fee_list);
                                    dap_dap_json_object_free(l_jobj_money);
                                    dap_dap_json_object_free(l_jobj_datum);
                                    dap_dap_json_object_free(l_jobj_datums);
                                    dap_dap_json_object_free(l_obj_chain);
                                    dap_global_db_objs_delete(l_objs, l_objs_count);
                                ...
                                if (dap_chain_addr_compare(&l_addr_from, l_dist_addr)) {
                                    ...
                                    for (dap_list_t *it_ems = l_list_in_ems; it_ems; it_ems = it_ems->next) {
                                        ...
                                        if (!dap_strcmp(l_in_ems->header.ticker, l_dist_token)) {
                                            ...
                                            dap_json_t *l_obj_ems_hash = json_object_new_string(l_ems_hash_str);
                                            if (!l_obj_ems_hash) {
                                                dap_dap_json_object_free(l_jobj_to_list);
                                                dap_dap_json_object_free(l_jobj_change_list);
                                                dap_dap_json_object_free(l_jobj_to_from_emi);
                                                ...
                                                dap_dap_json_object_free(l_jobj_datums);
                                                dap_dap_json_object_free(l_obj_chain);
                                                dap_dap_json_object_free(l_jobj_f);
                                                dap_global_db_objs_delete(l_objs, l_objs_count);
                                                dap_json_rpc_allocation_error(a_json_arr_reply);
                                                return;
                                            }
                                    ...
                                    dap_json_object_add_object(l_jobj_f, "addr", json_object_new_string(dap_chain_addr_to_str_static(l_dist_addr)));
                        ...
                        for (dap_list_t *it = l_vote_list; it; it = it->next) {
                            ...
                            dap_json_t *l_jobj_vote = dap_chain_datum_tx_item_vote_to_json(l_vote, a_version);
                            ...
                            dap_json_t *l_answer_text = json_object_new_string(l_answer_text_str ? l_answer_text_str : a_version == 1 ? "{UNDEFINED}" : "UNDEFINED");
                            DAP_DEL_Z(l_answer_text_str);
                            dap_json_object_add_object(l_jobj_vote, "answer_text", l_answer_text);
                        ...
                        dap_json_object_add_object(l_jobj_datum, "srv_pay", l_jobj_pay_list) : dap_dap_json_object_free(
                                l_jobj_pay_list);
                        json_object_array_length(l_jobj_xchange_list) > 0 ?
                        dap_json_object_add_object(l_jobj_datum, "srv_xchange", l_jobj_xchange_list) : dap_dap_json_object_free(
                                l_jobj_xchange_list);
                        ...
                        dap_json_object_add_object(l_jobj_datum, "from_emission", l_jobj_to_from_emi) : dap_dap_json_object_free(
                                l_jobj_to_from_emi);
                        json_object_array_length(l_jobj_tx_vote) > 0 ?
                        dap_json_object_add_object(l_jobj_datum, "vote", l_jobj_tx_vote) : dap_dap_json_object_free(l_jobj_tx_vote);
                        json_object_array_length(l_jobj_tx_voting) > 0 ?
                        dap_json_object_add_object(l_jobj_datum, "voting", l_jobj_tx_voting) : dap_dap_json_object_free(
                                l_jobj_tx_voting);
    ...
    dap_json_t *l_object_total = json_object_new_string(l_nets_str);
...
int _cmd_mempool_delete(dap_chain_net_t *a_net, dap_chain_t *a_chain, const char *a_datum_hash, void **a_str_reply, int a_version)
{
    ...
    dap_json_t *l_jobj_net = json_object_new_string(a_net->pub.name);
    dap_json_t *l_jobj_chain = NULL;
    dap_json_t *l_jobj_datum_hash = json_object_new_string(a_datum_hash);
    if (!a_chain) {
    ...
    dap_json_t *l_jobj_ret_code = json_object_new_int(res);
...
int _cmd_mempool_check(dap_chain_net_t *a_net, dap_chain_t *a_chain, const char *a_datum_hash, const char *a_hash_out_type, void **a_str_reply, int a_version)
{
    ...
    dap_json_t *l_jobj_datum = dap_json_object_new();
    dap_json_t *l_datum_hash = json_object_new_string(a_datum_hash);
    dap_json_t *l_net_obj = json_object_new_string(a_net->pub.name);
    if (!l_jobj_datum || !l_datum_hash || !l_net_obj){
        dap_dap_json_object_free(l_jobj_datum);
        dap_dap_json_object_free(l_datum_hash);
        dap_dap_json_object_free(l_net_obj);
        dap_json_rpc_allocation_error(*a_json_arr_reply);
    ...
    if (l_datum) {
        ...
        dap_json_t *l_find_chain_or_mempool = json_object_new_string(l_found_in_chains ? "chain" : "mempool");
        if (!l_find_chain_or_mempool || !l_find_bool) {
            dap_dap_json_object_free(l_find_chain_or_mempool);
            dap_dap_json_object_free(l_find_bool);
            dap_dap_json_object_free(l_jobj_datum);
            dap_json_rpc_allocation_error(*a_json_arr_reply);
            return DAP_JSON_RPC_ERR_CODE_MEMORY_ALLOCATED;
        }
        ...
        if (l_found_in_chains) {
            ...
            dap_json_t *l_jobj_atom_hash = json_object_new_string(l_atom_hash_str);
            dap_json_t *l_jobj_atom_err = json_object_new_string(dap_ledger_check_error_str(l_ret_code));
            if (!l_obj_atom || !l_jobj_atom_hash || !l_jobj_atom_err) {
                dap_dap_json_object_free(l_jobj_datum);
                dap_dap_json_object_free(l_obj_atom);
                dap_dap_json_object_free(l_jobj_atom_hash);
                dap_dap_json_object_free(l_jobj_atom_err);
                dap_json_rpc_allocation_error(*a_json_arr_reply);
                return DAP_JSON_RPC_ERR_CODE_MEMORY_ALLOCATED;
            }
...
int _cmd_mempool_proc(dap_chain_net_t *a_net, dap_chain_t *a_chain, const char *a_datum_hash, void **a_str_reply, int a_version)
{
    ...
    dap_json_t *l_jobj_datum = dap_json_object_new();
    dap_json_t *l_jobj_hash = json_object_new_string(a_datum_hash);
    dap_json_t *l_jobj_type = json_object_new_string(l_type);
    dap_json_t *l_jobj_ts_created = dap_json_object_new();
    dap_json_t *l_jobj_ts_created_time_stamp = json_object_new_uint64(l_ts_create);
    ...
    if (l_res < 0 || !l_jobj_ts_created || !l_jobj_ts_created_time_stamp || !l_jobj_type ||
        ...
        dap_dap_json_object_free(l_jobj_res);
        dap_dap_json_object_free(l_jobj_datum);
        dap_dap_json_object_free(l_jobj_hash);
        dap_dap_json_object_free(l_jobj_type);
        dap_dap_json_object_free(l_jobj_ts_created);
    ...
    dap_json_t *l_jobj_ts_created_str = json_object_new_string(buf);
    dap_json_t *l_jobj_data_size = json_object_new_uint64(l_datum->header.data_size);
    if (!l_jobj_ts_created_str || !l_jobj_data_size) {
        ...
        dap_dap_json_object_free(l_jobj_ts_created_time_stamp);
        dap_dap_json_object_free(l_jobj_ts_created_str);
        dap_dap_json_object_free(l_jobj_data_size);
        dap_json_rpc_allocation_error(*a_json_arr_reply);
        return DAP_JSON_RPC_ERR_CODE_MEMORY_ALLOCATED;
    }
    dap_json_object_add_object(l_jobj_datum, a_version == 1 ? "hash" : "datum_hash", l_jobj_hash);
    ...
    if (l_verify_datum){
        dap_json_t *l_jobj_verify_err = json_object_new_string(dap_chain_net_verify_datum_err_code_to_str(l_datum, l_verify_datum));
        dap_json_t *l_jobj_verify_status = json_object_new_boolean(FALSE);
        if (!l_jobj_verify_status || !l_jobj_verify_err) {
            dap_dap_json_object_free(l_jobj_verify_status);
            dap_dap_json_object_free(l_jobj_verify_err);
            dap_dap_json_object_free(l_jobj_verify);
            dap_dap_json_object_free(l_jobj_res);
            dap_json_rpc_allocation_error(*a_json_arr_reply);
            return DAP_JSON_RPC_ERR_CODE_MEMORY_ALLOCATED;
        }
        ...
        if (l_chain->callback_add_datums) {
            if (l_chain->callback_add_datums(l_chain, &l_datum, 1) == 0) {
                ...
                dap_json_t *l_jobj_verify_status = json_object_new_boolean(TRUE);
                if (!l_jobj_verify_status) {
                    dap_dap_json_object_free(l_jobj_verify);
                    dap_dap_json_object_free(l_jobj_res);
                    dap_json_rpc_allocation_error(*a_json_arr_reply);
                ...
                if (false) { //dap_global_db_del_sync(l_gdb_group_mempool, a_datum_hash)){
                    dap_json_t *l_jobj_wrn_text = json_object_new_string("Can't delete datum from mempool!");
                    if (!l_jobj_wrn_text) {
                        dap_dap_json_object_free(l_jobj_verify);
                        dap_dap_json_object_free(l_jobj_res);
                        dap_json_rpc_allocation_error(*a_json_arr_reply);
                    ...
                    dap_json_t *l_jobj_text = json_object_new_string("Removed datum from mempool.");
                    if (!l_jobj_text) {
                        dap_dap_json_object_free(l_jobj_verify);
                        dap_dap_json_object_free(l_jobj_res);
                        dap_json_rpc_allocation_error(*a_json_arr_reply);
...
int _cmd_mempool_proc_all(dap_chain_net_t *a_net, dap_chain_t *a_chain, void **a_str_reply)
{
    ...
    if(!dap_chain_net_by_id(a_chain->net_id)) {
        char *l_warn_str = dap_strdup_printf("%s.%s: chain not found\n", a_net->pub.name,
                                             a_chain->name);
        if (!l_warn_str) {
            dap_dap_json_object_free(l_ret);
            dap_json_rpc_allocation_error(*a_json_arr_reply);
            return DAP_JSON_RPC_ERR_CODE_MEMORY_ALLOCATED;
        }
        dap_json_t *l_warn_obj = json_object_new_string(l_warn_str);
   ...
   dap_chain_node_mempool_process_all(a_chain, true);
    ...
    dap_json_t *l_obj_result = json_object_new_string(l_str_result);
...
int com_mempool(int a_argc, char **a_argv, void **a_str_reply, int a_version)
{
    ...
    if (a_argv[1]) {
        if (!dap_strcmp(a_argv[1], "list")) {
            ...
            dap_json_t *l_jobj_str_err = json_object_new_string(l_str_err);
    ...
    switch (l_cmd) {
        case SUBCMD_LIST: {
            ...
            dap_json_t *obj_net = json_object_new_string(l_net->pub.name);
            if (!obj_ret || !obj_net) {
                dap_dap_json_object_free(obj_ret);
                dap_dap_json_object_free(obj_net);
                dap_json_rpc_allocation_error(*a_json_arr_reply);
                return -1;
            }
            ...
            if (dap_cli_server_cmd_find_option_val(a_argv, arg_index, a_argc, "-addr", &l_wallet_addr) && !l_wallet_addr) {
                dap_json_t *l_jobj_err = json_object_new_string("Parameter '-addr' require <addr>");
                if (!l_jobj_err) {
                    dap_json_rpc_allocation_error(*a_json_arr_reply);
                    return -1;
                }
        ...
        case SUBCMD_COUNT: {
            ...
            if(l_chain) {
                ...
                dap_json_t *l_jobj_chain_name = json_object_new_string(l_chain->name);
                dap_json_t *l_jobj_count = json_object_new_uint64(l_objs_count);
                if (!l_jobj_chain || !l_jobj_chain_name || !l_jobj_count) {
                    dap_dap_json_object_free(l_jobj_chains);
                    dap_dap_json_object_free(l_jobj_chain);
                    dap_dap_json_object_free(l_jobj_chain_name);
                    dap_dap_json_object_free(l_jobj_count);
                    dap_dap_json_object_free(obj_ret);
                    dap_json_rpc_allocation_error(*a_json_arr_reply);
                    return DAP_JSON_RPC_ERR_CODE_MEMORY_ALLOCATED;
                }
            ...
            for (l_atom = l_cell->chain->callback_atom_iter_get(l_atom_iter, DAP_CHAIN_ITER_OP_FIRST, &l_atom_size);
                ...
                for (size_t i = 0; i < l_datum_count; i++) {
                    ...
                    if (l_decree->header.sub_type == a_decree_type) {
                        ...
                        dap_json_t *l_jobj_atom_create = json_object_new_string(l_buff_ts);
                        dap_json_t *l_jobj_atom_hash = json_object_new_string(
                        ...
                        dap_json_object_add_object(l_jobj_atom, a_version == 1 ? "hash" : "atom_hash", l_jobj_atom_hash);
                        dap_json_object_add_object(l_jobj_atom, "created", l_jobj_atom_create);
...
int cmd_find(int a_argc, char **a_argv, void **a_reply, int a_version) {
    ...
    switch (l_cmd) {
        ...
        case SUBCMD_ATOM: {
            ...
            dap_json_t *l_obj_atom = dap_json_object_new();
            dap_json_t *l_obj_atom_hash = json_object_new_string(l_atom_hash_str);
            dap_json_object_add_object(l_obj_atom, a_version == 1 ? "hash" : "atom_hash", l_obj_atom_hash);
            ...
            dap_json_t *l_obj_source = NULL;
            dap_json_t *l_jobj_find = NULL;
            if (l_atom_ptr) {
                l_obj_source = dap_json_object_new();
                dap_json_t *l_obj_net = json_object_new_string(l_net->pub.name);
                dap_json_t *l_obj_chain = json_object_new_string(l_chain->name);
                dap_json_object_add_object(l_obj_source, "net", l_obj_net);
        ...
        case SUBCMD_DECREE: {
            ...
            dap_json_object_add_object(l_obj, a_version == 1 ? "type" : "decree_type", json_object_new_string(l_type_decre_str));
...
int _cmd_mempool_add_ca(dap_chain_net_t *a_net, dap_chain_t *a_chain, dap_cert_t *a_cert, void **a_str_reply)
{
    ...
    if (l_hash_str) {
        ...
        dap_json_t *l_obj_message = json_object_new_string(l_msg);
        DAP_DELETE(l_msg);
        ...
        dap_json_t *l_obj_msg = json_object_new_string(l_msg);
        DAP_DELETE(l_msg);
        if (!l_obj_msg) {
            dap_json_rpc_allocation_error(*a_json_arr_reply);
            return DAP_JSON_RPC_ERR_CODE_MEMORY_ALLOCATED;
...
int cmd_decree(int a_argc, char **a_argv, void **a_str_reply, int a_version)
{
    ...
    case CMD_CREATE:{
        ...
        if(l_decree_chain_str) {
            if (!( l_decree_chain = dap_chain_net_get_chain_by_name(l_net, l_decree_chain_str) )) {
                    ...
                    json_object* json_obj_chains = dap_json_array_new();
                    ...
                    DL_FOREACH(l_net->pub.chains, l_chain) {
                        json_object* json_obj_chain = dap_json_object_new();
                        if (!json_obj_chain) return dap_json_rpc_allocation_put_error(json_obj_out);
                        dap_json_object_add_object(json_obj_chain, "chain", json_object_new_string(l_chain->name));
        ...
        if (dap_cli_server_cmd_find_option_val(a_argv, arg_index, a_argc, "-fee", &l_param_value_str)){
            ...
            if (dap_cli_server_cmd_find_option_val(a_argv, arg_index, a_argc, "-addr_pairs", &l_addr_pairs)) {
                ...
                json_object* l_json_arr_addrs = dap_json_object_new();
                for (uint16_t i = 0; l_addrs[i]; i++) {
                    char ** l_addr_pair = dap_strsplit(l_addrs[i], ":", 256);
                    if (!l_addr_pair || !l_addr_pair[0] || !l_addr_pair[1])
                        continue;
                    dap_json_object_add_object(l_json_arr_addrs, l_addr_pair[0], json_object_new_string(l_addr_pair[1]));
                }
        ...
        char *l_key_str_out = dap_chain_mempool_datum_add(l_datum, l_chain, l_hash_out_type);
        DAP_DELETE(l_datum);
        json_object* json_obj_status = dap_json_object_new();
        if (!json_obj_status) return dap_json_rpc_allocation_put_error(json_obj_status);
        dap_json_object_add_object(json_obj_status, "datum_status", l_key_str_out ? json_object_new_string(l_key_str_out) :
    ...
    case CMD_FIND: {
        ...
        json_object* json_obj_status = dap_json_object_new();
        if (!json_obj_status) return dap_json_rpc_allocation_put_error(json_obj_status);
        dap_json_object_add_object(json_obj_status, "find_status", l_decree ? (l_applied ? json_object_new_string("applied") :
    ...
    case CMD_INFO: {
        json_object* json_obj_out = dap_json_object_new();
        if (!json_obj_out) return dap_json_rpc_allocation_put_error(json_obj_out);
        json_object* json_obj_array = dap_json_array_new();
        if (!json_obj_array) return dap_json_rpc_allocation_put_error(json_obj_out);
        dap_json_object_add_object(json_obj_out, "owners", json_obj_array);
        ...
        for (const dap_list_t *it = l_decree_pkeys; it; it = it->next) {
            ...
            dap_json_object_add_object(json_obj_owner, "pkey_hash", json_object_new_string(dap_hash_fast_to_str_static(&l_pkey_hash)));
            i++;
...
int com_stats(int argc, char **a_argv, void **a_str_reply, int a_version)
{
...
#if (defined DAP_OS_UNIX) || (defined __WIN32)
    {
        ...
        dap_usleep(500000);
        json_object* json_arr_cpu_out = dap_json_array_new();
        ...
        for (uint32_t n_cpu_num = 0; n_cpu_num < s_cpu_stats.cpu_cores_count; n_cpu_num++) {
            json_object* json_obj_cpu = dap_json_object_new();
            l_str_cpu_num = dap_strdup_printf("CPU-%d", n_cpu_num);
            l_str_delimiter = dap_strdup_printf("%f%%", s_cpu_stats.cpus[n_cpu_num].load);
            dap_json_object_add_object(json_obj_cpu, l_str_cpu_num, json_object_new_string(l_str_delimiter));
            dap_json_array_add(json_arr_cpu_out, json_obj_cpu);
            DAP_DELETE(l_str_cpu_num);
            DAP_DELETE(l_str_delimiter);
        }
        json_object* json_obj_total = dap_json_object_new();
        l_str_delimiter = dap_strdup_printf("%f%%", s_cpu_stats.cpu_summary.load);
        dap_json_object_add_object(json_obj_total, a_version == 1 ? "Total" : "total", json_object_new_string(l_str_delimiter));
...
int cmd_gdb_export(int a_argc, char **a_argv, void **a_str_reply, int a_version)
{
    ...
    struct dap_json_t *l_json = dap_json_array_new();
    ...
    for (dap_list_t *l_list = l_groups_list; l_list; l_list = dap_list_next(l_list)) {
        ...
        struct dap_json_t *l_json_group = dap_json_array_new();
        struct dap_json_t *l_json_group_inner = dap_json_object_new();
        dap_json_object_add_object(l_json_group_inner, "group", json_object_new_string(l_group_name));
        ...
        for (size_t i = 0; i < l_store_obj_count; ++i) {
            ...
            struct dap_json_t *jobj = dap_json_object_new();
            dap_json_object_add_object(jobj, "key",     json_object_new_string(l_store_obj[i].key));
            dap_json_object_add_object(jobj, "value",   json_object_new_string(l_value_enc_str));
            dap_json_object_add_object(jobj, "value_len", json_object_new_int64((int64_t)l_store_obj[i].value_len));
            dap_json_object_add_object(jobj, "flags", json_object_new_uint64((uint64_t)l_store_obj[i].flags));
            dap_json_object_add_object(jobj, "sign", json_object_new_string(l_sign_str));
...
int cmd_gdb_import(int a_argc, char **a_argv, void **a_str_reply, int a_version)
{
    ...
    char l_path[MAX_PATH + 1];
    snprintf(l_path, sizeof(l_path), "%s/%s.json", l_gdb_path, l_filename);
    struct dap_json_t *l_json = json_object_from_file(l_path);
    if (!l_json) {
#if JSON_C_MINOR_VERSION<15
    ...
    for (size_t i = 0, l_groups_count = json_object_array_length(l_json); i < l_groups_count; ++i) {
        struct dap_json_t *l_group_obj = json_object_array_get_idx(l_json, i);
        if (!l_group_obj) {
            continue;
        }
        struct dap_json_t *l_json_group_name = json_object_object_get(l_group_obj, "group");
        const char *l_group_name = json_object_get_string(l_json_group_name);
        // proc group name
        log_it(L_INFO, "Group %zu: %s", i, l_group_name);
        struct dap_json_t *l_json_records = json_object_object_get(l_group_obj, "records");
        ...
        for (size_t j = 0; j < l_records_count; ++j) {
            ...
            if (l_record_size > 5) {
                dap_json_t *l_jobj_crc = json_object_object_get(l_record, "crc");
                dap_json_t *l_jobj_sign = json_object_object_get(l_record, "sign");
                dap_json_t *l_jobj_flags = json_object_object_get(l_record, "flags");
...
//    }
    ...
    if (error) {
       dap_json_rpc_error_add(*a_json_arr_reply, DAP_CHAIN_NODE_CLI_COM_REMOVE_GENERAL_ERR, "Error when deleting, because:\n%s", return_message);
        ...
        dap_json_object_add_object(json_obj_out, "status", json_object_new_string(l_out_mes));
...
struct json_object *wallets_info_json_collect(int a_version) {
    ...
    struct dap_json_t *l_wallet_list = wallet_list_json_collect(a_version);
    struct dap_json_t *l_wallet_list_arr = json_object_object_get(l_wallet_list, "wallets");
    size_t l_count = json_object_array_length(l_wallet_list_arr);
    for (size_t i = 0; i < l_count; i++) {
        struct dap_json_t *l_json_wallet = json_object_array_get_idx(l_wallet_list_arr, i),
    ...
    dap_json_object_add_object(l_json, "wallets", l_json_wallets);
    dap_dap_json_object_free(l_wallet_list);
    return l_json;
}
...
void dap_notify_new_client_send_info(dap_events_socket_t *a_es, UNUSED_ARG void *a_arg) {
    ...
    struct dap_json_t *l_json_nets = dap_chain_net_list_json_collect(l_version);
    dap_events_socket_write_f(a_es->worker, a_es->uuid, "%s\r\n", json_object_to_json_string(l_json_nets));
    dap_dap_json_object_free(l_json_nets);
    struct dap_json_t *l_json_nets_info = dap_chain_nets_info_json_collect(l_version);
    dap_events_socket_write_f(a_es->worker, a_es->uuid, "%s\r\n", json_object_to_json_string(l_json_nets_info));
    dap_dap_json_object_free(l_json_nets_info);
    struct dap_json_t *l_json_wallets = wallet_list_json_collect(l_version);
    dap_events_socket_write_f(a_es->worker, a_es->uuid, "%s\r\n", json_object_to_json_string(l_json_wallets));
    dap_dap_json_object_free(l_json_wallets);
    struct dap_json_t *l_json_wallets_info = wallets_info_json_collect(l_version);
    dap_events_socket_write_f(a_es->worker, a_es->uuid, "%s\r\n", json_object_to_json_string(l_json_wallets_info));
    dap_dap_json_object_free(l_json_wallets_info);
    for (dap_chain_net_t *l_net = dap_chain_net_iter_start(); l_net; l_net = dap_chain_net_iter_next(l_net)) {
        struct dap_json_t *l_json_net_states = dap_chain_net_states_json_collect(l_net, l_version);
        dap_events_socket_write_f(a_es->worker, a_es->uuid, "%s\r\n", json_object_to_json_string(l_json_net_states));
        dap_dap_json_object_free(l_json_net_states);
    }
}
...
static void s_new_wallet_info_notify(const char *a_wallet_name)
{
    ...
    struct dap_json_t *l_json_wallet_info = dap_json_object_new();
...
int com_file(int a_argc, char ** a_argv, void **a_str_reply, int a_version)
{
    ...
    switch(l_cmd_num) {
        ...
        case CMD_CLEAR_LOG: {
            ...
            switch (res) {
                case 0: {
                    dap_json_array_add(*a_json_arr_reply, json_object_new_string("Log file has been cleared"));
                    break;
                }
                case -1: {
...
int com_policy(int argc, char **argv, void **reply, int a_version) {
    ...
    if (l_cmd == CMD_FIND) {
        ...
        if (l_answer) {
            dap_json_object_add_object(l_answer, "active", json_object_new_string(dap_chain_policy_is_activated(l_net->pub.id, l_policy_num) ? "true" : "false"));
            dap_json_array_add(*a_json_arr_reply, l_answer);
        } else {
            dap_json_array_add(*a_json_arr_reply, json_object_new_string("Detailed information not exist"));
        }
        return 0;
    }
    ...
    if (!l_execute) {
        ...
        dap_json_object_add_object(l_answer, a_version == 1 ? "Current time" : "current_time", json_object_new_string(l_time));
        dap_json_object_add_object(l_answer, a_version == 1 ? "Notification" : "notification", json_object_new_string("It's policy draft, check and use 'execute' command to apply"));
        if (l_answer) {
            dap_json_array_add(*a_json_arr_reply, l_answer);
        } else {
    ...
    dap_json_array_add(*a_json_arr_reply, json_object_new_string(l_approve_str));
