                              ...
                              const char *a_project_name)
{
    ...
    if (!l_project && a_project_name) {
        for (dap_auction_project_cache_item_t *it = NULL; it; it = it->hh.next) {
            if (it->project_name && strcmp(it->project_name, a_project_name) == 0) {
                l_project = it;
                break;
            }
        }
                                              ...
                                              dap_chain_tx_out_cond_t *a_prev_cond, bool a_owner, bool a_check_for_apply)
{
    ...
    if (a_prev_cond->header.subtype != DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_AUCTION_BID) {
        log_it(L_WARNING, "Invalid conditional output subtype (expected auction bid)");
        return -2;
    }

    // Validate project_id 
    uint32_t l_project_id = a_prev_cond->subtype.srv_auction_bid.project_id;
    if (l_project_id == 0) {
    ...
    dap_hash_fast_t l_auction_hash = a_prev_cond->subtype.srv_auction_bid.auction_hash;
    ...
    switch (l_auction->status) {
        ...
        case DAP_AUCTION_STATUS_ENDED: {
            ...
            uint32_t l_bid_project = a_prev_cond->subtype.srv_auction_bid.project_id;
            ...
            if (l_is_winner) { // If project is winner, check if lock period expired
                ...
                dap_time_t l_lock_end_time = l_auction->end_time + a_prev_cond->subtype.srv_auction_bid.lock_time;
    ...
    if (l_cached_auction->projects_count > 0) {
        ...
        if (l_auction->projects) {
            ...
            HASH_ITER(hh, l_cached_auction->projects, l_project, l_tmp_project) {
                ...
                l_auction->projects[l_index].project_name = l_project->project_name ? dap_strdup(l_project->project_name) : NULL;
                                     ...
                                     uint256_t a_amount, dap_time_t a_lock_time, uint32_t a_project_id, uint256_t a_fee, int *a_ret_code)
{
    ...
    dap_chain_srv_uid_t l_srv_uid = {.uint64 = DAP_CHAIN_NET_SRV_AUCTION_ID};
