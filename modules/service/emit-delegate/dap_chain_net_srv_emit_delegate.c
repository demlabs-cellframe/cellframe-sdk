...
static int s_emit_delegate_verificator(dap_ledger_t *a_ledger, dap_chain_tx_out_cond_t *a_cond, dap_chain_datum_tx_t *a_tx_in, bool a_owner)
{
                                                ...
                                                dap_chain_addr_t *a_addr_to, uint256_t a_value, uint256_t a_fee, dap_hash_fast_t *a_tx_in_hash)
{
    ...
    if (!IS_ZERO_256(l_value_back)) {
        ...
        if (!l_out_cond)
            m_tx_fail(ERROR_FUNDS, c_error_memory_alloc);
