...
int dap_chain_net_srv_stake_pos_delegate_init()
{
    ...
    dap_cli_server_cmd_add("srv_stake", s_cli_srv_stake, NULL, "Delegated stake service commands",
    ...
    "srv_stake order list [fee | validator | staker] -net <net_name> [-full] [-h]\n"
    ...
    "srv_stake list keys -net <net_name> [-cert <delegated_cert> | -pkey <pkey_hash_str>] [-full] [-h]\n"
        "\tShow the list of active stake keys (optional delegated with specified cert).\n"
    "srv_stake list tx -net <net_name> [-full] [-h]\n"
    ...
    s_debug_more = g_dap_debug_mode && dap_config_get_item_bool_default(g_config, "stake", "debug_more", s_debug_more);
    return 0;
}
                                         ...
                                         dap_chain_datum_tx_t *a_tx_in, bool a_owner, bool a_check_for_apply)
{
    ...
    if (dap_chain_addr_is_blank(&a_cond->subtype.srv_stake_pos_delegate.signing_addr) ||
        ...
        l_ret = s_stake_cond_out_match(a_cond, l_tx_new_cond);
        if (l_ret != 0)
            return l_ret;
...
static bool s_weights_truncate(dap_chain_net_srv_stake_t *l_srv_stake, const uint256_t a_limit)
{
    ...
    for (dap_chain_net_srv_stake_item_t *it = l_srv_stake->itemlist; it; it = it->hh.next) {
        ...
        if (SUBTRACT_256_256(it->value,
            GET_256_FROM_64(dap_config_get_item_uint64_default(g_config, "stake", "weight_limit_delta", 1000000000000ULL)),
            &l_weight_with_delta)) {
            log_it(L_ERROR, "Weight limit delta calculation overflow in stake weights truncate");
            return true;
        }
                                               ...
                                               dap_chain_datum_tx_t *a_prev_tx, dap_pkey_t *a_pkey)
{
    ...
    if (l_net_fee_used && SUM_256_256(l_fee_total, l_net_fee, &l_fee_total)) {
        log_it(L_ERROR, "Fee calculation overflow");
        return NULL;
    }
    ...
    if (l_order_hash_str) {
        ...
        if (l_order->direction == SERV_DIR_BUY) { // Staker order
...
#if EXTENDED_SRV_DEBUG
            {
                ...
                log_it(L_NOTICE, "Delegation tx params: tax = %s%%, addr = %s", l_tax_str, dap_chain_addr_to_str_static(&l_sovereign_addr));
                DAP_DEL_Z(l_tax_str);
            }
#endif
        } else {
...
static int s_cli_srv_stake_update(int a_argc, char **a_argv, int a_arg_index, void **a_str_reply, const char *a_hash_out_type)
{
    ...
    if (!l_tx) {
        dap_json_rpc_error_add(*a_json_arr_reply, DAP_CHAIN_NODE_CLI_SRV_STAKE_UPDATE_NO_TX_ERR, "Transaction %s not found",
            l_tx_hash_str ? l_tx_hash_str : dap_hash_fast_to_str_static(&l_tx_hash));
        return -21;
    }
    int l_out_num = 0;
    if (!dap_chain_datum_tx_out_cond_get(l_tx, DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_STAKE_POS_DELEGATE, &l_out_num)) {
        dap_json_rpc_error_add(*a_json_arr_reply, DAP_CHAIN_NODE_CLI_SRV_STAKE_UPDATE_INVALID_TX_ERR, "Transaction %s is invalid",
            l_tx_hash_str ? l_tx_hash_str : dap_hash_fast_to_str_static(&l_tx_hash));
        return -22;
    }
    dap_hash_fast_t l_spender_hash = {};
    if (dap_ledger_tx_hash_is_used_out_item(l_net->pub.ledger, &l_tx_hash, l_out_num, &l_spender_hash)) {
        dap_json_rpc_error_add(*a_json_arr_reply, DAP_CHAIN_NODE_CLI_SRV_STAKE_UPDATE_TX_SPENT_ERR, "Transaction %s is spent",
            l_tx_hash_str ? l_tx_hash_str : dap_hash_fast_to_str_static(&l_tx_hash));
        return -23;
    }
    ...
    if (l_tx_new && (l_out_hash_str = s_stake_tx_put(l_tx_new, l_net, a_hash_out_type))) {
        ...
        dap_json_rpc_error_add(*a_json_arr_reply, DAP_CHAIN_NODE_CLI_SRV_STAKE_UPDATE_CANT_COMPOSE_ERR,
            "Can't compose updating transaction %s, examine log files for details", dap_hash_fast_to_str_static(&l_tx_hash));
        DAP_DEL_Z(l_tx_new);
        return -21;
    }
    return 0;
...
static int s_cli_srv_stake_invalidate(int a_argc, char **a_argv, int a_arg_index, void **a_str_reply, const char *a_hash_out_type)
{
    ...
    if (l_wallet_str) {
        dap_chain_datum_tx_t *l_tx = dap_ledger_tx_find_by_hash(l_net->pub.ledger, &l_tx_hash);
        if (!l_tx) {
            dap_json_rpc_error_add(*a_json_arr_reply, DAP_CHAIN_NODE_CLI_SRV_STAKE_INVALIDATE_NO_TX_ERR,
                "Transaction %s not found", l_tx_hash_str ? l_tx_hash_str : dap_hash_fast_to_str_static(&l_tx_hash));
            return DAP_CHAIN_NODE_CLI_SRV_STAKE_INVALIDATE_NO_TX_ERR;
        }

        int l_out_num = 0;
        if (!dap_chain_datum_tx_out_cond_get(l_tx, DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_STAKE_POS_DELEGATE, &l_out_num)) {
            dap_json_rpc_error_add(*a_json_arr_reply, DAP_CHAIN_NODE_CLI_SRV_STAKE_INVALIDATE_INVALID_TX_ERR,
                "Transaction %s is invalid", l_tx_hash_str ? l_tx_hash_str : dap_hash_fast_to_str_static(&l_tx_hash));
        ...
        if (dap_ledger_tx_hash_is_used_out_item(l_net->pub.ledger, &l_tx_hash, l_out_num, &l_spender_hash)) {
            ...
            if (!dap_ledger_tx_find_by_hash(l_net->pub.ledger, &l_tx_hash)) {
                dap_json_rpc_error_add(*a_json_arr_reply, DAP_CHAIN_NODE_CLI_SRV_STAKE_INVALIDATE_NO_PREV_TX_ERR,
                    "Previous transaction %s is not found", l_tx_hash_str ? l_tx_hash_str : dap_hash_fast_to_str_static(&l_tx_hash));
        ...
        if (l_inv_tx && (l_out_hash_str = s_stake_tx_put(l_inv_tx, l_net, a_hash_out_type))) {
            ...
            dap_json_rpc_error_add(*a_json_arr_reply, DAP_CHAIN_NODE_CLI_SRV_STAKE_INVALIDATE_TX_INVALIDATE_ERR,
                "Can't invalidate transaction %s, examine log files for details", l_tx_hash_str ? l_tx_hash_str : dap_hash_fast_to_str_static(&l_tx_hash));
            DAP_DEL_Z(l_inv_tx);
...
static void s_srv_stake_print(dap_chain_net_srv_stake_item_t *a_stake, uint256_t a_total_weight, json_object *a_json_arr)
{
    ...
    if (MULT_256_256(a_stake->value, GET_256_FROM_64(100), &l_tmp)) {
        log_it(L_ERROR, "Related weight calculation multiplication overflow");
        return;
    }
    ...
    json_object_object_add(l_json_obj_stake, "sovereign_addr", json_object_new_string(
        dap_chain_addr_is_blank(&a_stake->sovereign_addr) ? "null" : dap_chain_addr_to_str_static(&a_stake->sovereign_addr)));
...
static int s_cli_srv_stake(int a_argc, char **a_argv, void **a_str_reply, int a_version)
{
    ...
    switch (l_cmd_num) {
        ...
        case CMD_CHECK:
        {
            ...
            json_object_object_add(json_obj_order, a_version == 1 ? "AUTO_ONLINE" : "auto_online", json_object_new_boolean((l_out.header.flags & A_ONLN)));
            json_object_object_add(json_obj_order, a_version == 1 ? "AUTO_UPDATE" : "auto_update", json_object_new_boolean((l_out.header.flags & A_UPDT)));
            json_object_object_add(json_obj_order, a_version == 1 ? "DATA_SIGNED" : "data_signed", json_object_new_boolean((l_out.header.flags & D_SIGN)));
        ...
        case CMD_LIST: {
            ...
            if (dap_cli_server_cmd_find_option_val(a_argv, l_arg_index, a_argc, "keys", NULL)) {
                ...
                for(dap_list_t *tx = l_args->ret; tx; tx = tx->next)
                {
                    ...
                    json_object_object_add(l_json_obj_tx, "signing_addr", json_object_new_string(dap_chain_addr_to_str_static(&l_tx_out_cond->subtype.srv_stake_pos_delegate.signing_addr)));
                    json_object_object_add(l_json_obj_tx, a_version == 1 ? "signing_hash" : "sig_pkey_hash", json_object_new_string(l_pkey_hash_str));
                    if (a_version == 1) {
                        ...
                        json_object_object_add(l_json_obj_tx, "node_addr", json_object_new_string(l_node_address_text_block));
                    }
