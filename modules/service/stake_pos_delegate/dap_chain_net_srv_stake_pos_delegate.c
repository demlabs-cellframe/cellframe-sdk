...
static void s_check_db_callback_fee_collect (dap_global_db_context_t * a_global_db_context, int a_errno, const char * a_group, const char * a_key,
                                             ...
                                             dap_nanotime_t value_ts, bool a_is_pinned, void * a_arg)
{
    ...
    fee_param_t *l_arg = (fee_param_t*)a_arg;
...
static int s_cli_srv_stake(int a_argc, char **a_argv, char **a_str_reply)
{
    ...
    switch (l_cmd_num) {
        case CMD_test:
        {
            ...
            dap_chain_net_t * l_net = NULL;
            uint256_t l_value_out_block = {};
            hash_val_mass_t hash_val1;


            //uint8_t * bute_raw = DAP_MALLOC(sizeof(uint256_t)+2*sizeof(dap_hash_fast_t));
            ...
            hash_val1.l_datum_hash = DAP_NEW_Z_SIZE(dap_hash_fast_t, 2 * sizeof (dap_hash_fast_t));



            const char *hash_1 = "0xB7A2C360683B1475FE1A6E1413EA508E41833C3FB285A8D5A4736EA10EB766E1";
            const char *hash_2 = "0x7418084A6C916636B1EB323CF4E13A6A46F8B3DF20DCC49792C7651DE057885A";
            dap_chain_hash_fast_t   l_hash_block;
            uint256_t vall_fee_1 = {};
            const char *test_value = "test_val";
            dap_hash_fast_t l_datum_hash;
            int val_t = 4;
            int val_t2 = 2;
            int res_1 = 1;
            dap_list_t *l_list_tmp = NULL;
            val_t++;

            dap_chain_hash_fast_from_hex_str(hash_1,&l_hash_block);
            ...
            SUM_256_256(l_value_out_block,vall_fee_1,&hash_val1.fee_vall);
            *hash_val1.l_datum_hash = l_block_cache1->block_hash;

            res_1 = dap_global_db_set("local.block_hash", test_value, &val_t, sizeof(int), false, NULL, NULL);

            dap_chain_hash_fast_from_hex_str(hash_2,&l_hash_block);
            dap_chain_block_cache_t *l_block_cache2 = dap_chain_block_cs_cache_get_by_hash(l_blocks, &l_hash_block);
            dap_chain_block_get_list_tx_cond_outs_with_val(l_chain->ledger,l_block_cache2,&l_value_out_block);
            SUM_256_256(l_value_out_block,vall_fee_1,&hash_val1.fee_vall);
            *(hash_val1.l_datum_hash+1) = l_block_cache2->block_hash;
            hash_val1.count = 2;

            uint8_t * bute_raw = DAP_NEW_Z_SIZE(uint8_t, sizeof(uint256_t) + 2 * sizeof(dap_hash_fast_t));
            *(uint256_t*)bute_raw = hash_val1.fee_vall;
            *(dap_hash_fast_t*)(bute_raw + sizeof(uint256_t))= l_block_cache1->block_hash;
            *(dap_hash_fast_t*)(bute_raw + sizeof(uint256_t)+sizeof(dap_hash_fast_t))= l_block_cache2->block_hash;
            ...
            const char *l_storage_path = dap_config_get_item_str(g_config, "global_db", "path");
             l_storage_path;
             hash_val_mass_t *l_arg3 = DAP_NEW_Z_SIZE(hash_val_mass_t, sizeof(hash_val_mass_t));
            res_1 = dap_global_db_get("local.block_value", test_value,s_check_db_version_callback_get_2, l_arg3);
            l_list_tmp = dap_list_append(l_list_tmp, &val_t2);

        for(dap_list_t * it =l_list_tmp;it;it = it->next)
        {
            int *i=NULL;
            i = (int*)it->data;
            i++;
