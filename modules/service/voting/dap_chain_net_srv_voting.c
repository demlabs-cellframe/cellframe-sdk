...
static int s_callback_purge(dap_chain_net_id_t UNUSED_ARG a_net_id, void *a_service_internal)
{
    ...
    dap_ht_foreach(l_service_internal->ht, it, tmp) {
...
static inline struct voting *s_voting_find(dap_chain_net_id_t a_net_id, dap_hash_sha3_256_t *a_voting_hash)
{
    ...
    if (!votings_ht)
        return NULL;
    dap_ht_find(votings_ht, a_voting_hash, sizeof(dap_hash_sha3_256_t), l_voting);
    return l_voting;
}
...
static inline int s_voting_add(dap_chain_net_id_t a_net_id, struct voting *a_voting)
{
    ...
    dap_ht_add(l_service_internal->ht, hash, a_voting);
...
static inline bool s_voting_delete(dap_chain_net_id_t a_net_id, dap_hash_sha3_256_t *a_voting_hash)
{
    ...
    dap_ht_find(l_service_internal->ht, a_voting_hash, sizeof(dap_hash_sha3_256_t), l_voting);
    if (!l_voting) {
                              ...
                              char **a_hash_tx_out)
{
    ...
    if (l_voting->params->delegate_key_required) {
        ...
        if (dap_cert_get_pkey_hash(a_cert, DAP_HASH_TYPE_SHA3_256, l_pkey_hash.raw, sizeof(dap_hash_sha3_256_t)))
...
static int s_votings_restore(dap_chain_net_id_t a_net_id, byte_t *a_state, uint64_t a_state_size, uint32_t a_states_count)
{
    ...
    for (uint32_t i = 0; i < a_states_count; i++) {
        ...
        struct voting *l_voting = NULL;
        dap_ht_find_by_hashvalue(l_service_internal->ht, &cur->hash, sizeof(dap_hash_sha3_256_t), l_hash_value, l_voting);
        if (!l_voting) {
            ...
            *l_voting->params = (dap_chain_datum_tx_voting_params_t) {
            ...
            dap_ht_add_by_hashvalue(l_service_internal->ht, hash, sizeof(dap_hash_sha3_256_t), l_hash_value, l_voting);
            l_restored_count++;
