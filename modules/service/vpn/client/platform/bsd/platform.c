...
int dap_chain_net_vpn_client_network_platform_get_gateway(char *a_out_gateway, size_t a_gateway_size) {
    ...
    // BSD uses similar route command as macOS
    char *l_result = NULL;
    int l_ret = exec_with_ret(&l_result, "route -n get default | grep gateway | awk '{print $2}'");
    
    if (l_ret != 0 || !l_result) {
        log_it(L_ERROR, "Failed to execute 'route' command (exit code: %d)", l_ret);
        return -2;
    }
...
int dap_chain_net_vpn_client_network_platform_get_interface(char *a_out_interface, size_t a_interface_size) {
    ...
    // BSD uses similar route command as macOS
    char *l_result = NULL;
    int l_ret = exec_with_ret(&l_result, "route -n get default | grep interface | awk '{print $2}'");
...
int dap_chain_net_vpn_client_network_platform_add_host_route(
    ...
    // BSD uses similar route command as macOS
    char l_cmd[512];
    snprintf(l_cmd, sizeof(l_cmd), "route add -host %s %s 2>&1", 
