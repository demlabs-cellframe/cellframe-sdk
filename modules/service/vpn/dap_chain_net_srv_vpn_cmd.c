 ...
 * VPN statistics
 */
int com_vpn_statistics(int a_argc, char ** a_argv, void **a_str_reply, UNUSED_ARG int a_version)
{
    ...
    DL_FOREACH(l_sessions_list, l_item) {
        ...
        if (l_srv_str_session) {
            ...
            l_conn++;
            // time start/length
            uint32_t l_time_len_sec = (uint32_t)(time(NULL) - l_session->time_created);
 ...
 * VPN client control
 */
int com_vpn_client(int a_argc, char ** a_argv, void **a_str_reply, UNUSED_ARG int a_version)
{
#ifndef _WIN32
    enum {
