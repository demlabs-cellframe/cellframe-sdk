...
// External modules
#include "dap_chain_net_srv_vpn_limits.h"  // vpn_srv_limits_update()
 ...
 * @note In unified TUN API, worker routing is handled automatically via channel_info
 */
void vpn_srv_ch_esocket_assigned(dap_events_socket_t *a_es, dap_worker_t *a_worker)
{
    UNUSED(a_es);
    UNUSED(a_worker);
    // No action needed - TUN routing handled by unified API
    // In SERVER mode: routing by destination IP
    // In CLIENT mode: channel_info stored in TUN device
}
...
void vpn_srv_ch_delete(dap_stream_ch_t *a_ch, void *a_arg)
{
    ...
    if (l_ch_vpn->addr_ipv4.s_addr) {
        // Return IP to pool (unified TUN API handles routing cleanup automatically)
        pthread_rwlock_wrlock(&g_vpn_raw_server_rwlock);
        if (g_vpn_raw_server) {
            if (g_vpn_raw_server->ipv4_lease_last.s_addr == l_ch_vpn->addr_ipv4.s_addr) {
                // Revert lease counter
                g_vpn_raw_server->ipv4_lease_last.s_addr = ntohl(ntohl(g_vpn_raw_server->ipv4_lease_last.s_addr) - 1);
            } else {
                l_is_unleased = true;
            }
        }
        pthread_rwlock_unlock(&g_vpn_raw_server_rwlock);
    }
    
    // Remove from clients hash table
    pthread_rwlock_wrlock(&g_vpn_clients_rwlock);
    if (g_vpn_ch_vpn_addrs) {
        HASH_DEL(g_vpn_ch_vpn_addrs, l_ch_vpn);
    }
...
static void s_ch_packet_in_vpn_address_request(dap_stream_ch_t *a_ch, dap_chain_net_srv_usage_t *a_usage)
{
    ...
    if (l_item_ipv4) {
        ...
        pthread_rwlock_unlock(&g_vpn_raw_server_rwlock);
        pthread_rwlock_wrlock(&g_vpn_clients_rwlock);
        HASH_ADD(hh, g_vpn_ch_vpn_addrs, addr_ipv4, sizeof(l_ch_vpn->addr_ipv4), l_ch_vpn);
        pthread_rwlock_unlock(&g_vpn_clients_rwlock);

        // Send VPN_ADDR_REPLY
        dap_stream_ch_vpn_pkt_t *l_pkt_out = DAP_NEW_STACK_SIZE(
            dap_stream_ch_vpn_pkt_t,
            sizeof(l_pkt_out->header) + sizeof(l_ch_vpn->addr_ipv4) + sizeof(g_vpn_raw_server->ipv4_network_addr));
        ...
        l_pkt_out->header.op_data.data_size = sizeof(l_ch_vpn->addr_ipv4) + sizeof(g_vpn_raw_server->ipv4_gw);
        
        memcpy(l_pkt_out->data, &l_ch_vpn->addr_ipv4, sizeof(l_ch_vpn->addr_ipv4));
        memcpy(l_pkt_out->data + sizeof(l_ch_vpn->addr_ipv4), &g_vpn_raw_server->ipv4_gw,
               sizeof(g_vpn_raw_server->ipv4_gw));
        ...
        if (l_data_wrote < l_data_to_write) {
            ...
            inet_ntop(AF_INET, &g_vpn_raw_server->ipv4_gw, l_str_ipv4gw, INET_ADDRSTRLEN);
            inet_ntop(AF_INET, &g_vpn_raw_server->ipv4_network_mask, l_str_ipv4mask, INET_ADDRSTRLEN);
            inet_ntop(AF_INET, &g_vpn_raw_server->ipv4_network_addr, l_str_ipv4netaddr, INET_ADDRSTRLEN);
            inet_ntop(AF_INET, &g_vpn_raw_server->ipv4_lease_last, l_str_ipv4last, INET_ADDRSTRLEN);
            ...
            // IP assigned notification removed - unified TUN API handles routing automatically
        }
    } else {
        // Lease new address
        struct in_addr n_addr = {0}, n_addr_max;
        n_addr.s_addr = ntohl(g_vpn_raw_server->ipv4_lease_last.s_addr);
        n_addr.s_addr++;
        n_addr_max.s_addr = (ntohl(g_vpn_raw_server->ipv4_gw.s_addr) | ~ntohl(g_vpn_raw_server->ipv4_network_mask.s_addr));
        ...
        if (n_addr.s_addr <= n_addr_max.s_addr) {
            ...
            g_vpn_raw_server->ipv4_lease_last.s_addr = n_addr.s_addr;
            ...
            dap_stream_ch_vpn_pkt_t *pkt_out = DAP_NEW_STACK_SIZE(
                dap_stream_ch_vpn_pkt_t,
                sizeof(pkt_out->header) + sizeof(l_ch_vpn->addr_ipv4) + sizeof(g_vpn_raw_server->ipv4_gw));
            
            pkt_out->header.sock_id = 0;  // Unified TUN API: sock_id is deprecated
            pkt_out->header.op_code = VPN_PACKET_OP_CODE_VPN_ADDR_REPLY;
            pkt_out->header.op_data.data_size = sizeof(l_ch_vpn->addr_ipv4) + sizeof(g_vpn_raw_server->ipv4_gw);
            pkt_out->header.usage_id = a_usage->id;
            
            memcpy(pkt_out->data, &l_ch_vpn->addr_ipv4, sizeof(l_ch_vpn->addr_ipv4));
            memcpy(pkt_out->data + sizeof(l_ch_vpn->addr_ipv4), &g_vpn_raw_server->ipv4_gw,
                   sizeof(g_vpn_raw_server->ipv4_gw));
...
bool vpn_srv_ch_packet_in(dap_stream_ch_t *a_ch, void *a_arg)
{
    ...
    debug_if(g_vpn_debug_more, L_INFO, "Got srv_vpn packet with op_code=0x%02x", l_vpn_pkt->header.op_code);
    ...
    if (l_vpn_pkt->header.op_code >= 0xb0) { // Raw packets
        switch (l_vpn_pkt->header.op_code) {
            ...
            case VPN_PACKET_OP_CODE_VPN_SEND: {
                ...
                vpn_srv_limits_update(a_ch, l_srv_session, l_usage, l_ret);
