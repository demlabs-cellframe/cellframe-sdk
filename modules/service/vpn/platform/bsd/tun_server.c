...
{
    // Try to open /dev/tunN devices
    for (int i = 0; i < 16; i++) {
        char l_devname[32];
        snprintf(l_devname, sizeof(l_devname), "/dev/tun%d", i);
        
        int l_tun_fd = open(l_devname, O_RDWR | O_NONBLOCK);
        if (l_tun_fd >= 0) {
            // Success
            if (s_tun_state.device_name[0] == '\0') {
                snprintf(s_tun_state.device_name, IFNAMSIZ, "tun%d", i);
            }
            
            log_it(L_DEBUG, "Opened %s for worker #%u", l_devname, a_worker->id);
            *a_out_fd = l_tun_fd;
            return 0;
        }
    }
    
    log_it(L_ERROR, "Failed to open any TUN device");
    return -1;
}
...
static int s_configure_tun_network(const dap_tun_server_config_t *a_config)
{
    ...
    snprintf(l_buf, sizeof(l_buf), "ifconfig %s %s %s netmask %s up",
             s_tun_state.device_name, l_str_gw, l_str_gw, l_str_mask);
    if (system(l_buf) != 0) {
    ...
    // Add route
    snprintf(l_buf, sizeof(l_buf), "route add -net %s -netmask %s %s",
             l_str_gw, l_str_mask, l_str_gw);
    if (system(l_buf) != 0) {
        log_it(L_WARNING, "Failed to add route (might already exist)");
    }
...
int dap_tun_server_init(dap_config_t *a_config)
{
    ...
    log_it(L_INFO, "Initializing %u TUN sockets", s_tun_state.socket_count);
    ...
    // Configure network (only once for first device)
