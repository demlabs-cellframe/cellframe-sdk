...
static int s_xchange_verificator_callback(dap_ledger_t * a_ledger,
                            dap_chain_datum_tx_t *a_tx_in, dap_hash_sha3_256_t *a_tx_in_hash, dap_chain_tx_out_cond_t *a_cond, bool a_owner, bool a_from_mempool);
...
static int s_cli_srv_xchange(int a_argc, char **a_argv, dap_json_t *a_json_arr_reply, int a_version);
...
static bool s_string_append_tx_cond_info( dap_string_t * a_reply_str, dap_chain_net_t * a_net, 
                                          ...
                                          dap_hash_sha3_256_t *a_tx_hash, tx_opt_status_t a_filter_by_status, bool a_append_prev_hash, 
...
static bool s_string_append_tx_cond_info_json( dap_json_t *a_json_out, dap_chain_net_t *a_net,
                                               dap_chain_addr_t *a_owner_addr, dap_chain_addr_t *a_buyer_addr, dap_chain_datum_tx_t * a_tx, dap_hash_sha3_256_t *a_tx_hash,
...
dap_chain_net_srv_xchange_price_t *s_xchange_price_from_order(dap_chain_net_t *a_net, dap_chain_datum_tx_t *a_order, 
                                                    dap_hash_sha3_256_t *a_order_hash, uint256_t *a_fee, bool a_ret_is_invalid);
static xchange_orders_cache_net_t *s_get_xchange_cache_by_net_id(dap_chain_net_id_t a_net_id);
static void s_ledger_tx_add_notify(dap_ledger_t *a_ledger, dap_chain_datum_tx_t *a_tx, dap_hash_sha3_256_t *a_tx_hash, dap_chain_tx_out_cond_t *a_prev_cond);
static void s_ledger_tx_remove_notify(dap_ledger_t *a_ledger, dap_chain_datum_tx_t *a_tx, dap_hash_sha3_256_t *a_tx_hash,  dap_chain_tx_out_cond_t *a_prev_cond);
static dap_chain_net_srv_xchange_order_status_t s_tx_check_for_open_close(dap_chain_net_t * a_net, dap_chain_datum_tx_t * a_tx);
...
static bool s_extract_order_info(dap_ledger_t *a_ledger, dap_chain_datum_tx_t *a_order_tx, 
                                  dap_hash_sha3_256_t *a_order_hash, xchange_tx_cache_t *a_cache_item)
{
 ...
 */
static bool s_find_order_hash_from_chain(dap_ledger_t *a_ledger, dap_hash_sha3_256_t *a_start_hash, dap_hash_sha3_256_t *a_order_hash)
{
    if (!a_ledger || !a_start_hash || !a_order_hash)
        return false;
    
    dap_hash_sha3_256_t l_current_hash = *a_start_hash;
...
static bool s_string_append_tx_cond_info_json( dap_json_t *a_json_out, dap_chain_net_t *a_net,
                                               ...
                                               tx_opt_status_t a_filter_by_status, bool a_print_prev_hash, bool a_print_status, bool a_print_ts, int a_version)
{
    if (!a_json_out || !a_tx || !a_tx_hash)
        return false;
    
    // Add basic transaction information to JSON
    char *l_tx_hash_str = dap_hash_sha3_256_to_str_new(a_tx_hash);
    if (l_tx_hash_str) {
    ...
    if (a_print_ts) {
        ...
        dap_json_object_add_string(a_json_out, "ts_created", l_time_str);
    }
...
static void s_ledger_tx_add_notify(dap_ledger_t *a_ledger, dap_chain_datum_tx_t *a_tx, dap_hash_sha3_256_t *a_tx_hash, dap_chain_tx_out_cond_t *a_prev_cond)
{
    ...
    dap_ht_find(l_cache->cache, a_tx_hash, sizeof(dap_hash_sha3_256_t), l_existing);
    if (l_existing) {
        log_it(L_DEBUG, "TX %s already in xchange cache", dap_hash_sha3_256_to_str_static(a_tx_hash));
        return;
    }
    ...
    memcpy(&l_cache_item->hash, a_tx_hash, sizeof(dap_hash_sha3_256_t));
    ...
    switch (l_tx_type) {
        case TX_TYPE_ORDER: {
            ...
            memset(&l_cache_item->tx_info.order_info.next_hash, 0, sizeof(dap_hash_sha3_256_t));
            ...
            log_it(L_INFO, "XChange ORDER added to cache: %s (sell %s, buy %s, rate %s)",
                   ...
                   dap_uint256_to_const_char(l_cache_item->rate, NULL));
            break;
        }
        ...
        case TX_TYPE_EXCHANGE: {
            ...
            if (!l_order_tx) {
                log_it(L_WARNING, "Cannot find order TX %s for EXCHANGE", 
                       dap_hash_sha3_256_to_str_static(&l_prev_hash));
                DAP_DELETE(l_cache_item->tx);
                DAP_DELETE(l_cache_item);
                return;
            }
            ...
            memset(&l_cache_item->tx_info.exchange_info.next_hash, 0, sizeof(dap_hash_sha3_256_t));
            
            log_it(L_INFO, "XChange EXCHANGE added to cache: %s (order %s)",
                   dap_hash_sha3_256_to_str_static(a_tx_hash),
                   dap_hash_sha3_256_to_str_static(&l_order_hash));
            break;
        }
    ...
    // Add to hash table
    dap_ht_add(l_cache->cache, hash, l_cache_item);
    
    // Update chain links if needed
    if (l_tx_type != TX_TYPE_ORDER && !dap_hash_sha3_256_is_blank(&l_prev_hash)) {
        // Find previous TX in cache and update its next_hash
        xchange_tx_cache_t *l_prev_item = NULL;
        dap_ht_find(l_cache->cache, &l_prev_hash, sizeof(dap_hash_sha3_256_t), l_prev_item);
        if (l_prev_item) {
...
static void s_ledger_tx_remove_notify(dap_ledger_t *a_ledger, dap_chain_datum_tx_t *a_tx, dap_hash_sha3_256_t *a_tx_hash, dap_chain_tx_out_cond_t *a_prev_cond)
{
    ...
    dap_ht_find(l_cache->cache, a_tx_hash, sizeof(dap_hash_sha3_256_t), l_cache_item);
    if (!l_cache_item) {
    ...
    log_it(L_NOTICE, "XChange TX removed from cache: %s (type: %d)",
           dap_hash_sha3_256_to_str_static(a_tx_hash), l_cache_item->tx_type);
    
    // Remove from hash table
    dap_ht_del(l_cache->cache, l_cache_item);
...
// s_tx_check_for_open_close implementation will be provided later in the file
...
static int s_print_for_srv_xchange_list(dap_json_t *a_json_input, dap_json_t *a_json_output, char **cmd_param, int cmd_cnt)
{
	...
	if (j_obj_headr) {
		if (dap_json_object_get_ex(j_obj_headr, "page", &l_arr_pages) && l_arr_pages) {
			...
			if (l_obj_pages) {
				dap_json_object_get_ex(l_obj_pages, "limit", &limit_obj);
				dap_json_object_get_ex(l_obj_pages, "offset", &offset_obj);
				if (limit_obj)
					l_limit = dap_json_get_int64(limit_obj) ? dap_strdup_printf("%"DAP_INT64_FORMAT,dap_json_get_int64(limit_obj)) : dap_strdup_printf("unlimit");
				if (offset_obj)
					l_offset = dap_json_get_int64(offset_obj) ? dap_strdup_printf("%"DAP_INT64_FORMAT,dap_json_get_int64(offset_obj)) : NULL;
			}
		}
		if(!dap_json_object_get_ex(j_obj_headr, "orders", &l_arr_orders) &&
			!dap_json_object_get_ex(j_obj_headr, "ORDERS", &l_arr_orders)&&
		...
		if (dap_json_get_type(a_json_input) == DAP_JSON_TYPE_ARRAY && l_arr_orders) {
			...
			for (int i = 0; i < result_count; i++) {
				...
				if (dap_json_object_get_ex(json_obj_result, "order_hash", &j_obj_hash) &&
					dap_json_object_get_ex(json_obj_result, "ts_created", &j_obj_create) &&
					dap_json_object_get_ex(json_obj_result, "status", &j_obj_status) &&
					dap_json_object_get_ex(json_obj_result, "proposed_coins", &j_obj_prop_coin) &&
					dap_json_object_get_ex(json_obj_result, "amount_coins", &j_obj_amount_coin) &&
					dap_json_object_get_ex(json_obj_result, "filled_percent", &j_obj_filed_perc) &&
					dap_json_object_get_ex(json_obj_result, "token_buy", &j_obj_token_buy) &&
					dap_json_object_get_ex(json_obj_result, "token_sell", &j_obj_token_sell) &&
					dap_json_object_get_ex(json_obj_result, "rate", &j_obj_rate) &&
					dap_json_object_get_ex(json_obj_result, "owner_addr", &j_obj_owner_addr)) {
					const char *full_hash = dap_json_get_string(j_obj_hash);
					char hash_buffer[16];
					...
					const char *prop_full = dap_json_get_string(j_obj_prop_coin);
					const char *amount_full = dap_json_get_string(j_obj_amount_coin);
					const char *rate_full = dap_json_get_string(j_obj_rate);
					const char *owner_addr_full = j_obj_owner_addr ? dap_json_get_string(j_obj_owner_addr) : NULL;
					char prop_buf[23];
					char amount_buf[23];
					char rate_buf[23];
	...
	if (dap_cli_server_cmd_check_option(cmd_param, 0, cmd_cnt, "tx_list") != -1) {
		...
		for (size_t i = 0; i < (size_t)dap_json_array_length(l_arr); i++) {
			...
			dap_json_object_get_ex(it, "hash", &hash);
			dap_json_object_get_ex(it, "status", &status);
			dap_json_object_get_ex(it, "ticker", &token);
			dap_json_object_get_ex(it, "ts_created", &ts);
			dap_json_object_get_ex(it, "owner_addr", &owner_addr);			
			if (hash && token && ts && status) {
				const char *full_hash = dap_json_get_string(hash);
				const char *owner_addr_full = owner_addr ? dap_json_get_string(owner_addr) : NULL;
				const char *buyer_addr_full = buyer_addr ? dap_json_get_string(buyer_addr) : NULL;
...
int dap_chain_net_srv_xchange_init()
{
    ...
    if (s_xchange_cache_state == XCHANGE_CACHE_ENABLED){
        for(dap_chain_net_t *l_net = dap_chain_net_iter_start(); l_net; l_net=dap_chain_net_iter_next(l_net)) {
            ...
            dap_ledger_verificator_add(DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_XCHANGE, s_xchange_verificator_callback, NULL, s_ledger_tx_add_notify, 
                                            s_ledger_tx_add_notify, s_ledger_tx_remove_notify, s_ledger_tx_remove_notify);
 ...
 * @return
 */
static int s_xchange_verificator_callback(dap_ledger_t *a_ledger, dap_chain_datum_tx_t *a_tx_in, dap_hash_sha3_256_t *a_tx_in_hash,
                                          dap_chain_tx_out_cond_t *a_tx_out_cond, bool a_owner, bool a_from_mempool)
{
    ...
    if (s_debug_more) {
        ...
        dap_uint256_to_const_char(a_tx_out_cond->header.value, &l_value_str);
        log_it(L_NOTICE, "Total sell %s %s from %s", l_value_str, l_sell_ticker, dap_hash_sha3_256_to_str_static(&l_tx_in_cond->header.tx_prev_hash));
        dap_uint256_to_const_char(a_tx_out_cond->subtype.srv_xchange.rate, &l_value_str);
        log_it(L_NOTICE, "Rate is %s", l_value_str);
        dap_uint256_to_const_char(l_sell_again_val, &l_value_str);
        log_it(L_NOTICE, "Resell %s %s", l_value_str, l_sell_ticker);
        dap_uint256_to_const_char(l_buyer_val_expected, &l_value_str);
        log_it(L_NOTICE, "Expect to buy %s %s", l_value_str, l_buy_ticker);
        dap_uint256_to_const_char(l_buy_val, &l_value_str);
        log_it(L_NOTICE, "Buy %s %s", l_value_str, l_buy_ticker);
        dap_uint256_to_const_char(l_fee_val, &l_value_str);
                                                          ...
                                                          uint256_t a_datoshi_fee)
{
    ...
    if (!IS_ZERO_256(a_price->rate)) {
        ...
        if (s_debug_more) {
            const char *l_datoshi_sell_str; dap_uint256_to_const_char(l_datoshi_sell, &l_datoshi_sell_str);
    ...
    if (s_debug_more) {
        const char *l_value_str; dap_uint256_to_const_char(l_tx_out_cond->header.value, &l_value_str);
    ...
    if (compare256(l_tx_out_cond->header.value, l_datoshi_sell) == 1) {
        ...
        if (s_debug_more) {
            const char *l_value_back_str; dap_uint256_to_const_char(l_value_back, &l_value_back_str);
    ...
    if (s_debug_more) {
        const char *l_buy_str; dap_uint256_to_const_char(l_datoshi_buy, &l_buy_str);
    ...
    if (!IS_ZERO_256(a_datoshi_fee)) {
        ...
        if (s_debug_more) {
            const char *l_fee_str; dap_uint256_to_const_char(a_datoshi_fee, &l_fee_str);
    ...
    if (l_net_fee_used) {
        ...
        if (s_debug_more) {
            const char *l_net_fee_str; dap_uint256_to_const_char(l_net_fee, &l_net_fee_str);
    ...
    if (l_service_fee_used) {
        ...
        if (s_debug_more) {
            const char *l_srv_fee_str; dap_uint256_to_const_char(l_service_fee, &l_srv_fee_str);
    ...
    if (s_debug_more) {
        const char *l_value_transfer_str; dap_uint256_to_const_char(l_value_transfer, &l_value_transfer_str);
    ...
    if (!l_pay_with_native && !l_buy_with_native) {
        ...
        if (s_debug_more) {
            const char *l_fee_transfer_str; dap_uint256_to_const_char(l_fee_transfer, &l_fee_transfer_str);
            log_it(L_NOTICE, "l_fee_transfer = %s", l_fee_transfer_str);
            const char *l_val_back_str; dap_uint256_to_const_char(l_value_back, &l_val_back_str);
...
uint64_t dap_chain_net_srv_xchange_get_order_completion_rate(dap_chain_net_t *a_net, dap_hash_sha3_256_t a_order_tx_hash)
{
    ...
    dap_hash_sha3_256_t l_last_tx_hash = dap_ledger_get_final_chain_tx_hash(a_net->pub.ledger, DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_XCHANGE, &l_price->tx_hash, false);
    if ( dap_hash_sha3_256_is_blank(&l_last_tx_hash) ){
 ...
 */
dap_chain_net_srv_xchange_price_t *s_xchange_price_from_order(dap_chain_net_t *a_net, dap_chain_datum_tx_t *a_order, 
                                                            dap_hash_sha3_256_t *a_order_tx_hash, uint256_t *a_fee, bool a_ret_is_invalid)
{
    ...
    dap_strncpy(l_price->token_buy, l_out_cond->subtype.srv_xchange.buy_token, sizeof(l_price->token_buy));

    l_price->order_hash = *a_order_tx_hash;
    ...
    dap_hash_sha3_256_t l_final_hash = dap_ledger_get_final_chain_tx_hash(a_net->pub.ledger,
    ...
    if ( !dap_hash_sha3_256_is_blank(&l_final_hash) ) {
        ...
        if (a_ret_is_invalid) {
            dap_hash_sha3_256_t l_tx_hash_zero = {0};
...
// Wrapper function for CLI command
static int s_cli_srv_xchange(int a_argc, char **a_argv, dap_json_t *a_json_arr_reply, int a_version)
{
    return s_cli_srv_xchange_order(a_argc, a_argv, 1, a_json_arr_reply, a_version);
}
...
static int s_cli_srv_xchange_order(int a_argc, char **a_argv, int a_arg_index, dap_json_t *a_json_arr_reply, int a_version)
{
    ...
    switch (l_cmd_num) {
        ...
        case CMD_HISTORY:{
                ...
                if(l_from_wallet_cache){
                            ...
                            l_datum_tx; l_datum_tx = dap_chain_wallet_cache_iter_get(l_iter, DAP_CHAIN_WALLET_CACHE_GET_NEXT))
                    {
                        ...
                        if(s_xchange_cache_state == XCHANGE_CACHE_ENABLED){
                            ...
                            dap_ht_find(l_cache->cache, l_iter->cur_hash, sizeof(dap_hash_sha3_256_t), l_item); 
                            if (!l_item)
                                continue;
                            ...
                            if (s_string_append_tx_cond_info_json(l_json_obj_order, l_net, NULL, NULL, l_datum_tx, l_iter->cur_hash, 
                                                                                                        TX_STATUS_ALL, true, true, false, a_version))
                                l_total++;
                        }                        
                    }
                    ...
                    if(!l_total)
                        ...
                        if (a_version == 1) {
                            ...
                            dap_json_object_add_string(l_json_obj_order, "number of orders", l_orders);                        
                            DAP_DELETE(l_orders);
                        } else {
                            dap_json_object_add_uint64(l_json_obj_order, "total_orders_count", l_total); 
                        }
                    }
                    dap_json_array_add(a_json_arr_reply, l_json_obj_order);
                } else { 
                    dap_list_t *l_tx_list = dap_chain_net_get_tx_cond_all_for_addr(l_net,l_addr, c_dap_chain_net_srv_xchange_uid );
                    ...
                    if (l_tx_list){
                        ...
                        l_json_obj_order = dap_json_object_new();
                        dap_json_object_add_string(l_json_obj_order, "wallet", l_addr_hash_str);
                        while(l_tx_list_temp ){
                            dap_chain_datum_tx_t * l_tx_cur = (dap_chain_datum_tx_t*) l_tx_list_temp->data;
                            dap_hash_sha3_256_t l_hash = {};
                            dap_hash_sha3_256(l_tx_cur, dap_chain_datum_tx_get_size(l_tx_cur), &l_hash);
            ...
            if(l_order_hash_str){
                dap_hash_sha3_256_t l_order_tx_hash = {};
                dap_hash_sha3_256_from_str(l_order_hash_str, &l_order_tx_hash);
                l_json_obj_order = dap_json_object_new();
                if(s_xchange_cache_state == XCHANGE_CACHE_DISABLED){
                    ...
                    if( l_tx){
                        ...
                        char *l_tx_hash = dap_hash_sha3_256_to_str_new(&l_order_tx_hash);
                        if(l_tx_type != TX_TYPE_ORDER){
                            dap_json_object_add_string(l_json_obj_order, a_version == 1 ? "datum status" : "datum_status", a_version == 1 ? "is not order" : "is_not_order");
                            dap_json_object_add_string(l_json_obj_order, a_version == 1 ? "datum hash" : "datum_hash", l_tx_hash);
                        } else {
                            dap_chain_net_srv_xchange_order_status_t l_rc = s_tx_check_for_open_close(l_net,l_tx);
                            if(l_rc == XCHANGE_ORDER_STATUS_UNKNOWN){
                                dap_json_object_add_string(l_json_obj_order, a_version == 1 ? "WRONG TX" : "wrong_tx", l_tx_hash);
                            }else{
                                dap_json_object_add_string(l_json_obj_order, "history for order", l_order_hash_str);
                                ...
                                while(l_tx_list_temp ){
                                    ...
                                    dap_hash_sha3_256_t l_hash = {};
                                    dap_hash_sha3_256(l_tx_cur, dap_chain_datum_tx_get_size(l_tx_cur), &l_hash);
                                    s_string_append_tx_cond_info_json(l_json_obj_order, l_net, NULL, NULL, l_tx_cur, &l_hash, TX_STATUS_ALL, true, true, false, a_version);
                                    l_tx_list_temp = l_tx_list_temp->next;
                                }
                                dap_list_free(l_tx_list);
                            }
                    ...
                    l_cache = s_get_xchange_cache_by_net_id(l_net->pub.id);
                    xchange_tx_cache_t* l_item = NULL;
                    dap_hash_sha3_256_t l_cur_hash = l_order_tx_hash;
                    l_json_obj_order = dap_json_object_new();                    
                    dap_ht_find(l_cache->cache, &l_cur_hash, sizeof(dap_hash_sha3_256_t), l_item);
                    if (!l_item){
                        dap_json_rpc_error_add(a_json_arr_reply, DAP_CHAIN_NODE_CLI_COM_NET_SRV_XCNGE_ORDRS_HIST_DOES_NO_HISTORY_ERR, "No history");
                    ...
                    while(l_item){
                        ...
                        switch(l_item->tx_type){
                            ...
                            case TX_TYPE_INVALIDATE:{
                                l_cur_hash = (dap_hash_sha3_256_t){0};
                            } break;
                            default:break;
                        }
                        if (dap_hash_sha3_256_is_blank(&l_cur_hash))
                            break;
                        dap_ht_find(l_cache->cache, &l_cur_hash, sizeof(dap_hash_sha3_256_t), l_item);
                    }
                }
        ...
        case CMD_REMOVE:
        {
            ...
            dap_hash_sha3_256_from_str(l_order_hash_str, &l_tx_hash);
            ...
            switch (l_ret_code) {
                case XCHANGE_REMOVE_ERROR_OK:
                    json_obj_order = dap_json_object_new();
                    if (a_version == 1) {
                        dap_json_object_add_string(json_obj_order, "status", "Order successfully removed");
                        dap_json_object_add_string(json_obj_order, "Created inactivate tx with hash", l_tx_hash_ret);
                    } else {
                        dap_json_object_add_string(json_obj_order, "status", "success");
                        dap_json_object_add_string(json_obj_order, "tx_hash", l_tx_hash_ret);
                    }
                    dap_json_array_add(a_json_arr_reply, json_obj_order);
        ...
        case CMD_STATUS: {
            ...
            if (s_xchange_cache_state == XCHANGE_CACHE_ENABLED){
                ...
                l_cache = s_get_xchange_cache_by_net_id(l_net->pub.id);
                xchange_tx_cache_t* l_item = NULL;
                dap_ht_find(l_cache->cache, &l_order_tx_hash, sizeof(dap_hash_sha3_256_t), l_item);
                ...
                dap_hash_sha3_256_t l_last_tx_hash = dap_ledger_get_final_chain_tx_hash(l_ledger, DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_XCHANGE, &l_price->tx_hash, false);
                if( dap_hash_sha3_256_is_blank(&l_last_tx_hash) ){
                ...
                log_it(L_INFO, "Last tx hash %s", dap_hash_sha3_256_to_str_static(&l_last_tx_hash));
                if (!l_last_tx){
            ...
            l_proposed_datoshi_str = dap_uint256_to_const_char(l_proposed, &l_proposed_coins_str);
            dap_json_object_add_string(json_obj_order, "proposed_coins", *l_proposed_coins_str ? l_proposed_coins_str : "0.0");
            dap_json_object_add_string(json_obj_order, "proposed_datoshi", *l_proposed_datoshi_str ? l_proposed_datoshi_str : "0");
            
            l_amount_datoshi_str = dap_uint256_to_const_char(l_amount, &l_amount_coins_str);
            dap_json_object_add_string(json_obj_order, "amount_coins", *l_amount_coins_str ? l_amount_coins_str : "0.0"); 
            dap_json_object_add_string(json_obj_order, "amount_datoshi", *l_amount_datoshi_str ? l_amount_datoshi_str : "0"); 
            dap_json_object_add_uint64(json_obj_order, "filled_percent", l_percent_completed);
            dap_json_object_add_string(json_obj_order, "token_buy", l_token_buy);
            dap_json_object_add_string(json_obj_order, "token_sell", l_token_sell);

            dap_uint256_to_const_char(l_rate, &l_cp_rate);
...
static dap_chain_net_srv_xchange_order_status_t s_tx_check_for_open_close(dap_chain_net_t * a_net, dap_chain_datum_tx_t * a_tx)
{
    ...
    dap_hash_sha3_256_t l_tx_hash = {};
    dap_hash_sha3_256(a_tx, dap_chain_datum_tx_get_size(a_tx), &l_tx_hash);
    dap_hash_sha3_256_t l_last_tx_hash = dap_ledger_get_final_chain_tx_hash(l_ledger, DAP_CHAIN_TX_OUT_COND_SUBTYPE_SRV_XCHANGE, &l_tx_hash, false);
    if ( dap_hash_sha3_256_is_blank(&l_last_tx_hash) ) {
