...
static dap_chain_datum_tx_t *s_xchange_tx_create_request(dap_chain_net_srv_xchange_price_t *a_price, dap_chain_wallet_t *a_wallet)
{
    ...
    if (l_single_channel)
        ...
        l_list_fee_out = dap_chain_wallet_get_list_tx_outs_with_val(l_ledger, l_native_ticker, &l_seller_addr, l_total_fee, &l_fee_transfer);
        if (!l_list_fee_out) {
    ...
    dap_list_t *l_list_used_out = dap_chain_wallet_get_list_tx_outs_with_val(l_ledger, a_price->token_sell, &l_seller_addr, l_value_need, &l_value_transfer);
    if (!l_list_used_out) {
                                                          ...
                                                          uint256_t a_datoshi_fee)
{
    ...
    // list of transaction with 'out' items to sell
    dap_list_t *l_list_used_out = dap_chain_wallet_get_list_tx_outs_with_val(l_ledger, a_price->token_buy, &l_buyer_addr, l_value_need, &l_value_transfer);
    if (!l_list_used_out) {
        log_it(L_WARNING, "Nothing to change from %s (not enough funds in %s (%s))",
    ...
    if (!l_pay_with_native) {
        if (l_buy_with_native)
            ...
            l_list_fee_out = dap_chain_wallet_get_list_tx_outs_with_val(l_ledger, l_native_ticker, &l_buyer_addr, l_total_fee, &l_fee_transfer);
            if (!l_list_fee_out) {
                dap_list_free_full(l_list_used_out, NULL);
                log_it(L_WARNING, "Not enough funds to pay fee");
                return NULL;
...
static char* s_xchange_tx_invalidate(dap_chain_net_srv_xchange_price_t *a_price, dap_chain_wallet_t *a_wallet)
{
    ...
    if (!l_single_channel) {
        ...
        dap_list_t *l_list_used_out = dap_chain_wallet_get_list_tx_outs_with_val(l_ledger, l_native_ticker, &l_seller_addr, l_total_fee, &l_transfer_fee);
        if (!l_list_used_out) {
            dap_chain_datum_tx_delete(l_tx);
...
static int s_cli_srv_xchange_order(int a_argc, char **a_argv, int a_arg_index, void **a_str_reply)
{
    ...
    switch (l_cmd_num) {
        ...
        case CMD_STATUS: {
            ...
            l_cp_rate = dap_chain_balance_coins_print(l_price->rate); // must be free'd
    ...
    switch (l_cmd_num) {
        ...
        case CMD_ORDERS: {
            ...
            for (dap_list_t *it = l_tx_list; it; it = it->next) {
                ...
                char l_tmp_buf[DAP_TIME_STR_SIZE];
                dap_time_to_str_rfc822(l_tmp_buf, DAP_TIME_STR_SIZE, l_tx->header.ts_created);
                l_cp_rate = dap_chain_balance_coins_print(l_price->rate);
