...
/**
 * @brief s_print_for_srv_xchange_list
 * Post-processing callback for srv_xchange command. Formats JSON input into
 ...
 */
static int s_print_for_srv_xchange_list(dap_json_t *a_json_input, dap_json_t *a_json_output, char **a_cmd_param, int a_cmd_cnt)
{
    ...
    if (j_obj_headr) {
        ...
        if (!dap_json_object_get_ex(j_obj_headr, "orders", &l_arr_orders) &&
            ...
            dap_cli_server_cmd_check_option(a_cmd_param, 0, a_cmd_cnt, "tx_list") == -1) {
            DAP_DEL_Z(l_limit);
            DAP_DEL_Z(l_offset);
            return -1;
        }
    ...
    // Branch: orders
    if (dap_cli_server_cmd_check_option(a_cmd_param, 0, a_cmd_cnt, "orders") != -1) {
        if (dap_json_get_type(a_json_input) == DAP_JSON_TYPE_ARRAY && l_arr_orders) {
            ...
            for (int i = 0; i < result_count; i++) {
                ...
                if (dap_json_object_get_ex(json_obj_result, "order_hash", &j_obj_hash) &&
                    ...
                    char prop_buf[23], amount_buf[23], rate_buf[23];
    ...
    // Branch: token_pair
    if (dap_cli_server_cmd_check_option(a_cmd_param, 0, a_cmd_cnt, "token_pair") != -1) {
        if (dap_cli_server_cmd_check_option(a_cmd_param, 0, a_cmd_cnt, "list") != -1) {
            ...
            if (!l_obj_pairs || !l_pairs_arr || dap_json_get_type(l_pairs_arr) != DAP_JSON_TYPE_ARRAY) {
                dap_string_free(l_str, true);
                DAP_DEL_Z(l_limit);
                DAP_DEL_Z(l_offset);
                return -1;
        ...
        if (dap_cli_server_cmd_check_option(a_cmd_param, 0, a_cmd_cnt, "average") != -1) {
            int top_len = dap_json_array_length(a_json_input);
            for (int i = 0; i < top_len; i++) {
                dap_json_t *el = dap_json_array_get_idx(a_json_input, i);
                if (el && dap_json_get_type(el) == DAP_JSON_TYPE_OBJECT) {
        ...
        if (dap_cli_server_cmd_check_option(a_cmd_param, 0, a_cmd_cnt, "history") != -1) {
            dap_json_t *l_arr = NULL, *l_summary = NULL;
            int top_len = dap_json_array_length(a_json_input);
            for (int i = 0; i < top_len; i++) {
                dap_json_t *el = dap_json_array_get_idx(a_json_input, i);
            ...
            if (l_summary) {
                dap_json_t *tx_cnt = NULL, *v1_from = NULL, *v1_to = NULL, *v2_from = NULL, *v2_to = NULL;
    ...
    // Branch: tx_list
    if (dap_cli_server_cmd_check_option(a_cmd_param, 0, a_cmd_cnt, "tx_list") != -1) {
        ...
        for (size_t i = 0; i < (size_t)dap_json_array_length(l_arr); i++) {
            dap_json_t *it = dap_json_array_get_idx(l_arr, i);
            dap_json_t *hash = NULL, *status = NULL, *token = NULL, *ts = NULL, *owner_addr = NULL, *buyer_addr = NULL;
            dap_json_object_get_ex(it, "hash", &hash);
 ...
 */
dap_chain_net_srv_xchange_price_t *s_xchange_price_from_order(dap_chain_net_t *a_net, dap_chain_datum_tx_t *a_order, 
                                                            dap_hash_fast_t *a_order_tx_hash, uint256_t *a_fee, bool a_ret_is_invalid)
{
