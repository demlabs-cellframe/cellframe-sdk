...
static int s_aggregate_fees(dap_chain_cs_blocks_hardfork_fees_t **a_out_list, dap_chain_addr_t *a_addr, uint256_t a_value)
{
    dap_chain_cs_blocks_hardfork_fees_t l_new_fee = { .owner_addr = *a_addr };
            ...
            if (i == 0) {
                for (size_t j = 0; j < l_block_cache->datum_count; j++) {
                    ...
                    if (!IS_ZERO_256(l_value_out)) {
                        ...
                        dap_chain_addr_fill_from_sign(l_sign, &l_owner_addr);
