...
//    a_chain->callback_atom_dump_json =
    ...
    for (uint16_t i = 0; i < l_list_len; i++) {
        ...
        dap_ht_add_hh(hh, l_cs_blocks_pvt->hal, hash, l_hal_item);
    }
...
dap_chain_block_cache_t * dap_chain_block_cache_get_by_hash(dap_chain_type_blocks_t * a_blocks,  dap_hash_sha3_256_t *a_block_hash)
{
    ...
    assert(!err);
    dap_ht_find_hh(hh, PVT(a_blocks)->blocks,a_block_hash, sizeof (*a_block_hash), l_ret );
...
static int s_callback_cs_blocks_purge(dap_chain_t *a_chain)
{
    ...
    for (size_t i = 0; i < PVT(l_blocks)->forked_br_cnt; i++){
        ...
        dap_ht_foreach_hh(hh, PVT(l_blocks)->forked_branches[i]->forked_branch_atoms, l_atom, l_atom_tmp) {
    ...
    dap_ht_foreach_hh(hh, PVT(l_blocks)->blocks, l_block, l_block_tmp) {
        dap_ht_del_hh(hh, PVT(l_blocks)->blocks, l_block);
    ...
    dap_ht_foreach_hh(hh, PVT(l_blocks)->datum_index, l_datum_index, l_datum_index_tmp) {
...
static int s_add_atom_datums(dap_chain_type_blocks_t *a_blocks, dap_chain_block_cache_t *a_block_cache)
{
    ...
    for (size_t i = 0;
        ...
        if (l_datum->header.type_id != DAP_CHAIN_DATUM_TX || l_res != DAP_LEDGER_CHECK_ALREADY_CACHED) { // If this is any datum other than a already cached transaction
            ...
            dap_ht_add_hh(hh, PVT(a_blocks)->datum_index, datum_hash, l_datum_index);
...
static int s_delete_atom_datums(dap_chain_type_blocks_t *a_blocks, dap_chain_block_cache_t *a_block_cache)
{
    ...
    for(size_t i=0; i<a_block_cache->datum_count && l_block_offset +sizeof(a_block_cache->block->hdr) < a_block_cache->block_size;
        ...
        size_t l_datum_data_size = l_datum->header.data_size;
        l_datum_size = l_datum_data_size + sizeof(l_datum->header);
        dap_ht_find_hh(hh, PVT(a_blocks)->datum_index, l_datum_hash, sizeof(dap_hash_sha3_256_t), l_datum_index);
        if (l_datum_index){
    ...
    if (a_main_branch_length < l_longest_branch_length){
        ...
        for (l_atom = a_bcache->hh.next, l_curr_index = 0; 
            a_main_branch_length > l_curr_index && l_atom; l_curr_index++){
                ...
                dap_ht_add_hh(hh, l_new_forked_branch, block_hash, l_new_item);
        ...
        dap_ht_foreach_hh(hh, new_main_branch, l_item, l_temp) {
            dap_chain_block_cache_t *l_curr_atom = l_item->block_cache;
            ++PVT(l_blocks)->blocks_count;
            dap_ht_add_hh(hh, PVT(l_blocks)->blocks, block_hash, l_curr_atom);
    ...
    case ATOM_ACCEPT:{
        ...
        if (!l_block_cache->is_genesis) {
            ...
            if (l_last_block && dap_hash_sha3_256_compare(&l_last_block->block_hash, &l_block_prev_hash)){
                ++PVT(l_blocks)->blocks_count;
                dap_ht_add_hh(hh, PVT(l_blocks)->blocks, block_hash, l_block_cache);
            ...
            for (size_t i = 0; i < PVT(l_blocks)->forked_br_cnt; i++){
                ...
                if (dap_hash_sha3_256_compare(&l_last->block_hash, &l_block_prev_hash)){
                    ...
                    dap_ht_add_hh(hh, l_cur_branch->forked_branch_atoms, block_hash, l_new_item);
...
*/
    ...
    case ATOM_FORK:{
        ...
        dap_ht_find_hh(hh, PVT(l_blocks)->blocks, &l_block_prev_hash, sizeof(dap_hash_sha3_256_t), l_prev_bcache);
        if (l_prev_bcache){
            dap_chain_block_forked_branch_atoms_table_t *l_new_item = DAP_NEW_Z(dap_chain_block_forked_branch_atoms_table_t);
            l_new_item->block_cache = l_block_cache;
            l_new_item->block_hash = l_block_cache->block_hash;
            ...
            dap_ht_add_hh(hh, forked_branch->forked_branch_atoms, block_hash, l_new_item);
...
static dap_chain_atom_verify_res_t s_callback_atom_verify(dap_chain_t *a_chain, dap_chain_atom_ptr_t a_atom, size_t a_atom_size, dap_hash_sha3_256_t *a_atom_hash)
{
    ...
    if ((l_block->hdr.version >= 2 || /* Old bug, crutch for it */ l_block->hdr.meta_n_datum_n_signs_size != l_offset) &&
        ...
        dap_ht_find_hh(hh, l_blocks_pvt->hal, &l_block_hash, sizeof(l_block_hash), l_hash_found);
        if (!l_hash_found) {
    ...
    if (l_is_genesis) {
        ...
        if (l_bcache_last && dap_hash_sha3_256_compare(&l_bcache_last->block_hash, &l_block_prev_hash))
            ...
            for (size_t i = 0; i < PVT(l_blocks)->forked_br_cnt; i++) {
                ...
                dap_ht_find_hh(hh, l_cur_branch->forked_branch_atoms, &l_block_hash, sizeof(dap_hash_sha3_256_t), l_item);
                if (l_item) {
                ...
                dap_ht_find_hh(hh, l_cur_branch->forked_branch_atoms, &l_block_prev_hash, sizeof(dap_hash_sha3_256_t), l_item);
                if (!l_item)
                    continue;
                if (l_item->hh.next) {
                                                        ...
                                                        dap_hash_sha3_256_t *a_block_hash, int *a_ret_code)
{
    ...
    dap_ht_find_hh(hh, PVT(l_cs_blocks)->datum_index, a_datum_hash, sizeof (*a_datum_hash), l_datum_index);
...
static dap_chain_atom_ptr_t s_callback_block_find_by_tx_hash(dap_chain_t * a_chain, dap_hash_sha3_256_t * a_tx_hash, size_t *a_block_size)
{
    ...
    dap_chain_block_datum_index_t *l_datum_index = NULL;
    pthread_rwlock_rdlock(&PVT(l_cs_blocks)->datums_rwlock);
    dap_ht_find_hh(hh, PVT(l_cs_blocks)->datum_index, a_tx_hash, sizeof (*a_tx_hash), l_datum_index);
    pthread_rwlock_unlock(&PVT(l_cs_blocks)->datums_rwlock);
    if (!l_datum_index)
        return NULL;
    if (a_block_size)
...
static dap_chain_block_t *s_block_create(dap_chain_type_blocks_t *a_blocks, size_t *a_new_block_size)
{
    ...
    pthread_rwlock_wrlock(&l_blocks_pvt->rwlock);
        ...
        if (!l_bcache_last)
            l_bcache_last = l_blocks_pvt->blocks;
...
static dap_list_t *s_callback_get_atoms(dap_chain_t *a_chain, size_t a_count, size_t a_page, bool a_reverse)
{
    ...
    if (a_reverse) {
        dap_chain_block_cache_t *l_ptr = dap_ht_last(l_blocks_pvt->blocks);
        if (!l_ptr)
...
static uint256_t s_callback_calc_reward(dap_chain_t *a_chain, dap_hash_sha3_256_t *a_block_hash, dap_pkey_t *a_block_sign_pkey)
{
    ...
    dap_chain_type_blocks_t *l_blocks = DAP_CHAIN_TYPE_BLOCKS(a_chain);
    dap_chain_block_cache_t *l_block_cache = NULL;
    dap_ht_find_hh(hh, PVT(l_blocks)->blocks, a_block_hash, sizeof(*a_block_hash), l_block_cache);
    if (!l_block_cache)
        return l_ret;
    ...
    l_block_cache = NULL;
    dap_ht_find_hh(hh, PVT(l_blocks)->blocks, &l_prev_block_hash, sizeof(l_prev_block_hash), l_block_cache);
    if (!l_block_cache) {
