...
static int s_chain_cs_dag_new(dap_chain_t * a_chain, dap_config_t * a_chain_cfg)
{
    ...
    for (uint16_t i = 0; i < l_list_len; i++) {
        ...
        dap_ht_add(l_dag->hal, hash, l_hal_item);
    }
...
static void s_threshold_free(dap_chain_type_dag_t *a_dag)
{
    ...
    //Free threshold
    dap_ht_foreach(l_pvt->events_treshold, l_current, l_tmp) {
        if (l_current->ts_added < l_time_cut_off) {
            ...
            dap_ht_add(l_pvt->removed_events_from_treshold, hash, l_el);
    ...
    dap_ht_foreach(l_pvt->events_treshold_conflicted, l_current, l_tmp) {
        if (l_current->ts_added < l_time_cut_off) {
            char *l_hash_dag = dap_hash_sha3_256_to_str_new(&l_current->hash);
            if (!a_dag->chain->is_mapped && !l_current->mapped_region)
                DAP_DELETE(l_current->event);
...
static int s_chain_cs_dag_purge(dap_chain_t *a_chain)
{
    ...
    dap_ht_foreach(l_dag_pvt->events_lasts_unlinked, l_event_current, l_event_tmp) {
        dap_ht_del(l_dag_pvt->events_lasts_unlinked, l_event_current);
        //if (!a_chain->is_mapped && !l_event_current->mapped_region)
        //    DAP_DELETE(l_event_current->event);
        DAP_DELETE(l_event_current);
    }
    dap_ht_foreach(l_dag_pvt->events_treshold, l_event_current, l_event_tmp) {
        dap_ht_del(l_dag_pvt->events_treshold, l_event_current);
        if (!a_chain->is_mapped && !l_event_current->mapped_region)
            DAP_DELETE(l_event_current->event);
        DAP_DELETE(l_event_current);
    }
    dap_ht_foreach(l_dag_pvt->events_treshold_conflicted, l_event_current, l_event_tmp) {
...
static bool s_dap_chain_check_if_event_is_present(dap_chain_type_dag_event_item_t * a_hash_table, const dap_hash_sha3_256_t * hash) {
    ...
    dap_ht_find(a_hash_table, hash, sizeof(*hash), l_event_search);
...
static dap_chain_atom_verify_res_t s_chain_callback_atom_read(dap_chain_t *a_chain, dap_chain_atom_ptr_t a_atom, size_t a_atom_size, dap_hash_sha3_256_t *a_atom_hash)
{
    ...
    dap_ht_find_by_hashvalue(PVT(l_dag)->events_prefetched, a_atom_hash, sizeof(*a_atom_hash), hashval, l_event_item);
    if (!l_event_item) {
        ...
        *l_event_item = (dap_chain_type_dag_event_item_t) {
        ...
        dap_ht_add_by_hashvalue(PVT(l_dag)->events_prefetched, hash, sizeof(*a_atom_hash), hashval, l_event_item);
...
static unsigned s_chain_callback_prefetched_atoms_add(dap_chain_t *a_chain) {
    ...
    dap_ht_foreach(PVT(l_dag)->events_prefetched, l_event_item, l_tmp) {
...
static dap_chain_atom_verify_res_t s_chain_callback_atom_add(dap_chain_t * a_chain, dap_chain_atom_ptr_t a_atom, size_t a_atom_size, dap_hash_sha3_256_t *a_atom_hash, bool a_atom_new)
{
    ...
    *l_event_item = (dap_chain_type_dag_event_item_t) {
    ...
    case ATOM_MOVE_TO_THRESHOLD: {
        ...
        dap_ht_find(PVT(l_dag)->removed_events_from_treshold, a_atom_hash, sizeof(dap_hash_sha3_256_t), el);
        if (!el) {
            dap_ht_add(PVT(l_dag)->events_treshold, hash, l_event_item);
    ...
    case ATOM_ACCEPT: {
        ...
        dap_ht_add(PVT(l_dag)->events, hash, l_event_item);
...
static bool s_chain_callback_datums_pool_proc(dap_chain_t *a_chain, dap_chain_datum_t *a_datum)
{
    ...
    if (!dap_ht_count(PVT(l_dag)->events_lasts_unlinked)) {
        ...
        dap_ht_foreach(PVT(l_dag)->events_lasts_unlinked, l_cur_ev, l_tmp_ev) {
            dap_ht_add_hh(hh_select, l_tmp, hash, l_cur_ev);
        }
        pthread_mutex_unlock(&PVT(l_dag)->events_mutex);
...
dap_chain_type_dag_event_t* dap_chain_type_dag_find_event_by_hash(dap_chain_type_dag_t * a_dag, dap_hash_sha3_256_t * a_hash)
{
    dap_chain_type_dag_event_item_t* l_event_item = NULL;
    pthread_mutex_lock(&PVT(a_dag)->events_mutex);
    dap_ht_find(PVT(a_dag)->events, a_hash, sizeof(*a_hash), l_event_item);
    pthread_mutex_unlock(&PVT(a_dag)->events_mutex);
...
static dap_chain_atom_verify_res_t s_chain_callback_atom_verify(dap_chain_t *a_chain, dap_chain_atom_ptr_t a_atom, size_t a_atom_size, dap_hash_sha3_256_t *a_atom_hash)
{
    ...
    if ( dap_chain_net_get_load_mode(dap_chain_net_by_id(a_chain->net_id)) ) {
        if ( l_dag->hal ) {
            ...
            dap_chain_type_dag_hal_item_t *l_hash_search = NULL;
            dap_ht_find(l_dag->hal, a_atom_hash, sizeof(*a_atom_hash), l_hash_search);
            if (l_hash_search)
                return ATOM_ACCEPT;
        }
    } else
    ...
    for (size_t i = 0; i< l_event->header.hash_count; i++) {
        ...
        dap_ht_find(PVT(l_dag)->events, l_hash, sizeof (*l_hash), l_event_search);
...
void s_dag_events_lasts_delete_linked_with_event(dap_chain_type_dag_t * a_dag, dap_chain_type_dag_event_t * a_event)
{
    for (size_t i = 0; i< a_event->header.hash_count; i++) {
        ...
        dap_ht_find(l_dag_pvt->events_lasts_unlinked, l_hash, sizeof (*l_hash), l_event_item);
        if ( l_event_item ){
...
void s_dag_events_lasts_process_new_last_event(dap_chain_type_dag_t *a_dag, dap_chain_type_dag_event_item_t *a_event_item)
{
    ...
    dap_ht_add(PVT(a_dag)->events_lasts_unlinked, hash, l_event_last);
}
...
int dap_chain_type_dag_event_verify_hashes_with_treshold(dap_chain_type_dag_t * a_dag, dap_chain_type_dag_event_t * a_event)
{
    ...
    for (size_t i = 0; i< a_event->header.hash_count; i++) {
        dap_hash_sha3_256_t * l_hash =  ((dap_hash_sha3_256_t *) a_event->hashes_n_datum_n_signs) + i;
        dap_chain_type_dag_event_item_t * l_event_search = NULL;
        dap_ht_find(PVT(a_dag)->events_treshold_conflicted, l_hash, sizeof (*l_hash), l_event_search);
        if ( l_event_search ){
          //event is linked to event we consider conflicting
          ret = DAP_THRESHOLD_CONFLICTING;
          break;
        }
        dap_ht_find(PVT(a_dag)->events, l_hash, sizeof (*l_hash), l_event_search);
        if ( l_event_search == NULL ){ // If not found in events - search in treshhold
            l_is_events_main_hashes = false;
            dap_ht_find(PVT(a_dag)->events_treshold, l_hash, sizeof (*l_hash), l_event_search);
...
dap_chain_type_dag_event_item_t* s_dag_proc_treshold(dap_chain_type_dag_t * a_dag)
{
    ...
    dap_ht_foreach(PVT(a_dag)->events_treshold, l_event_item, l_event_item_tmp) {
        ...
        if (ret == DAP_THRESHOLD_OK) {
            ...
            if (!l_add_res) {
                dap_ht_add(PVT(a_dag)->events, hash, l_event_item);
            ...
            dap_ht_del(PVT(a_dag)->events_treshold, l_event_item);
            dap_ht_add(PVT(a_dag)->events_treshold_conflicted, hash, l_event_item);
        }
    }
    pthread_mutex_unlock(&PVT(a_dag)->events_mutex);
        ...
        if ( l_event->header.hash_count > 0){
            ...
            *a_links_size_array = DAP_NEW_Z_SIZE(size_t, l_event->header.hash_count*sizeof (size_t));
            ...
            for (uint16_t i = 0; i < l_event->header.hash_count; i++){
                ...
                pthread_mutex_lock(&PVT(l_dag)->events_mutex);
                dap_ht_find(PVT(l_dag)->events, l_link_hash, sizeof(*l_link_hash), l_link_item);
                pthread_mutex_unlock(&PVT(l_dag)->events_mutex);
                if ( l_link_item ){
                                                                       ...
                                                                       dap_hash_sha3_256_t * a_atom_hash, size_t *a_atom_size)
{
    ...
    dap_chain_type_dag_event_item_t *l_event_item = NULL;
    pthread_mutex_lock(&PVT(l_dag)->events_mutex);
    dap_ht_find(PVT(l_dag)->events, a_atom_hash, sizeof(*a_atom_hash), l_event_item);
    if (l_event_item) {
        ...
        case SUBCMD_EVENT_DUMP: {
            ...
            size_t l_event_size = 0;
                if (l_from_events_str && strcmp(l_from_events_str,"round.new") == 0) {
                    ...
                    dap_chain_type_dag_event_item_t * l_event_item = NULL;
                    pthread_mutex_lock(&PVT(l_dag)->events_mutex);
                    dap_ht_find(PVT(l_dag)->events_lasts_unlinked, &l_event_hash, sizeof(l_event_hash), l_event_item);
                    pthread_mutex_unlock(&PVT(l_dag)->events_mutex);
                    if ( l_event_item )
                    ...
                    dap_chain_type_dag_event_item_t * l_event_item = NULL;
                    pthread_mutex_lock(&PVT(l_dag)->events_mutex);
                    dap_ht_find(PVT(l_dag)->events, &l_event_hash, sizeof(l_event_hash), l_event_item);
                    pthread_mutex_unlock(&PVT(l_dag)->events_mutex);
                    if ( l_event_item ) {
                        l_event = l_event_item->event;
                    ...
                    dap_chain_type_dag_event_item_t * l_event_item = NULL;
                    pthread_mutex_lock(&PVT(l_dag)->events_mutex);
                    dap_ht_find(PVT(l_dag)->events_treshold, &l_event_hash, sizeof(l_event_hash), l_event_item);
                    pthread_mutex_unlock(&PVT(l_dag)->events_mutex);
                    if (l_event_item)
                        l_event = l_event_item->event;
                    else {
            ...
            case SUBCMD_EVENT_LIST: {
                ...
                *l_from_hash_str = NULL, *l_to_hash_str = NULL;
                ...
                if (l_to_hash_str && l_from_hash_str) {
                    ...
                    dap_ht_find(PVT(l_dag)->events, &l_to_hash, sizeof(l_to_hash), l_event_item_to);
                    if (!l_event_item_to) {
                        dap_ht_find(PVT(l_dag)->events_treshold, &l_to_hash, sizeof(l_to_hash), l_event_item_to);
                        if (!l_event_item_to) {
                    ...
                    dap_ht_find(PVT(l_dag)->events, &l_from_hash, sizeof(l_from_hash), l_event_item_from);
                    if (!l_event_item_from) {
                        dap_ht_find(PVT(l_dag)->events_treshold, &l_from_hash, sizeof(l_from_hash), l_event_item_from);
                        if (!l_event_item_from) {
                            dap_json_rpc_error_add(a_json_arr_reply, DAP_CHAIN_NODE_CLI_COM_DAG_FIND_ERR,
                ...
                if (l_from_events_str && strcmp(l_from_events_str,"round.new") == 0) {
                    ...
                    dap_chain_type_dag_event_item_t * l_event_item = NULL,*l_event_item_tmp = NULL;
                    if (l_head){
                        dap_ht_foreach(PVT(l_dag)->events, l_event_item, l_event_item_tmp) {
                    ...
                    size_t i_tmp = 0;
                    if (l_head){
                        dap_ht_foreach(PVT(l_dag)->events_treshold, l_event_item, l_event_item_tmp) {
                            if (s_filter_event(l_event_item, &l_to_hash, &l_from_hash, &i_tmp, l_arr_end, l_arr_start, (char *)l_to_hash_str, (char *)l_from_hash_str, 
                                            l_from_time, l_to_time, json_arr_obj_event, a_version, l_head, &l_hash_flag)) 
                                break;
                        }
            ...
            case SUBCMD_EVENT_COUNT: {
                ...
                if (l_gdb_group_events) {
                    size_t l_objs_count = dap_global_db_group_count(l_gdb_group_events, false);
            ...
            case SUBCMD_EVENT_FIND:{
                ...
                dap_ht_foreach(PVT(l_dag)->events, l_curr_event, l_temp){                                        
