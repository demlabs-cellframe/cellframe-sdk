...
typedef struct dap_nonconsensus_datum_hash_item {
    char key[DAP_HASH_SHA3_256_STR_SIZE];
    dap_hash_sha3_256_t datum_data_hash;
...
static dap_chain_atom_verify_res_t s_nonconsensus_callback_atom_add(dap_chain_t * a_chain, dap_chain_atom_ptr_t, size_t, dap_hash_sha3_256_t *a_atom_hash, bool a_atom_new); //    Accept new event in gdb
static dap_chain_atom_verify_res_t s_nonconsensus_callback_atom_verify(dap_chain_t * a_chain, dap_chain_atom_ptr_t, size_t, dap_hash_sha3_256_t *a_atom_hash); //    Verify new event in gdb
static size_t s_nonconsensus_callback_atom_get_static_hdr_size(void); //    Get gdb event header size

static dap_chain_atom_iter_t* s_nonconsensus_callback_atom_iter_create(dap_chain_t * a_chain, dap_chain_cell_id_t a_cell_id, dap_hash_sha3_256_t *a_hash_from);
...
static dap_chain_datum_t *s_nonconsensus_callback_datum_find_by_hash(dap_chain_t *a_chain, dap_hash_sha3_256_t *a_datum_hash,
                                                                   dap_hash_sha3_256_t *a_atom_hash, int *a_ret_code);
...
static void s_nonconsensus_callback_mempool_notify(dap_global_db_store_obj_t *a_obj, void *a_arg)
{
    if (dap_global_db_store_obj_get_type(a_obj) == DAP_GLOBAL_DB_OPTYPE_ADD)
        dap_chain_node_mempool_process_all(a_arg, false);
}

static void s_changes_callback_notify(dap_global_db_store_obj_t *a_obj, void *a_arg)
{
    ...
    dap_hash_sha3_256_from_hex_str(a_obj->key, &l_hash);
...
static void s_nonconsensus_ledger_load(dap_chain_t *a_chain)
{
    ...
    for (size_t i = 0; l_values && i < l_values_count; i++) {
        ...
        // load ledger
        dap_hash_sha3_256_t l_hash = {};
        dap_hash_sha3_256_from_hex_str(it->key, &l_hash);
...
static size_t s_nonconsensus_callback_datums_pool_proc(dap_chain_t * a_chain, dap_chain_datum_t ** a_datums, size_t a_datums_count)
{
    for(size_t i = 0; i < a_datums_count; i++) {
        ...
        char l_db_key[DAP_HASH_SHA3_256_STR_SIZE];
        dap_chain_datum_calc_hash(l_datum, &l_datum_hash);
        dap_hash_sha3_256_to_str(&l_datum_hash, l_db_key, sizeof(l_db_key));
 ...
 */
static dap_chain_atom_verify_res_t s_nonconsensus_callback_atom_add(dap_chain_t * a_chain, dap_chain_atom_ptr_t a_atom, size_t a_atom_size, dap_hash_sha3_256_t *a_atom_hash, bool UNUSED_ARG a_atom_new)
{
    if (NULL == a_chain) {
    ...
    dap_hash_sha3_256_t l_datum_hash = *a_atom_hash;
    ...
    dap_hash_sha3_256_to_str(&l_hash_item->datum_data_hash, l_hash_item->key, sizeof(l_hash_item->key));
    dap_dl_append(l_nochain_priv->hash_items, l_hash_item);
    if (!l_nochain_priv->is_load_mode && a_chain->atom_notifiers) {
        dap_list_t *l_iter;
        dap_dl_foreach(a_chain->atom_notifiers, l_iter) {
 ...
 * @return dap_chain_atom_verify_res_t
 */
static dap_chain_atom_verify_res_t s_nonconsensus_callback_atom_verify(dap_chain_t * a_chain, dap_chain_atom_ptr_t a_atom, size_t a_atom_size, dap_hash_sha3_256_t *a_atom_hash)
{
    (void) a_chain;
    (void) a_atom;
    (void) a_atom_size;
 ...
 * @param a_atom_hash dap_hash_sha3_256_t atom hash
...
static dap_chain_atom_ptr_t s_nonconsensus_callback_atom_iter_find_by_hash(dap_chain_atom_iter_t * a_atom_iter,
        dap_hash_sha3_256_t * a_atom_hash, size_t *a_atom_size)
{
    char l_key[DAP_HASH_SHA3_256_STR_SIZE];
    dap_hash_sha3_256_to_str(a_atom_hash, l_key, sizeof(l_key));
    size_t l_ret_size;
...
static dap_chain_atom_ptr_t s_nonconsensus_callback_atom_iter_get(dap_chain_atom_iter_t *a_atom_iter, dap_chain_iter_op_t a_operation, size_t *a_atom_size)
{
    ...
    if (a_atom_iter->cur_item) {
        ...
        a_atom_iter->cur_hash = DAP_NEW_Z(dap_hash_sha3_256_t);
        dap_hash_sha3_256_from_str(l_item->key, a_atom_iter->cur_hash);
    } else
...
static dap_chain_datum_t *s_nonconsensus_callback_datum_iter_get_first(dap_chain_datum_iter_t *a_datum_iter)
{
    ...
    if (a_datum_iter->cur_item) {
        ...
        a_datum_iter->cur_hash = DAP_NEW_Z(dap_hash_sha3_256_t);
        dap_hash_sha3_256_from_str(l_item->key, a_datum_iter->cur_hash);
                                                                   ...
                                                                   dap_hash_sha3_256_t *a_atom_hash, int *a_ret_code)
{
    ...
    dap_dl_foreach(PVT(DAP_NONCONSENSUS(a_chain))->hash_items, l_item) {
        if (dap_hash_sha3_256_compare(a_datum_hash, &l_item->datum_data_hash)) {
            if (a_atom_hash)
