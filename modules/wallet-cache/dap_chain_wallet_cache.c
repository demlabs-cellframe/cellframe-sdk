...
typedef struct dap_wallet_cache_unspent_outs {
    ...
    char token_ticker[DAP_CHAIN_TICKER_SIZE_MAX];
    uint256_t anon_known_value;
    bool has_anon_known_value;
...
typedef struct anon_out_seed_item {
    unspent_cache_hh_key key;
    uint256_t value;
    dap_chain_addr_t addr;
    char token_ticker[DAP_CHAIN_TICKER_SIZE_MAX];
    dap_ht_handle_t hh;
} anon_out_seed_item_t;
...
static anon_out_seed_item_t *s_anon_out_seeds = NULL;
...
static int s_save_tx_cache_for_addr(dap_chain_t *a_chain, dap_chain_addr_t *a_addr, dap_chain_datum_tx_t *a_tx,
                                    dap_hash_sha3_256_t *a_tx_hash, dap_hash_sha3_256_t *a_atom_hash, int a_ret_code, char* a_main_token_ticker,
...
static void s_callback_datum_notify(void *a_arg, dap_hash_sha3_256_t *a_datum_hash, dap_hash_sha3_256_t *a_atom_hash, void *a_datum, 
                                    size_t a_datum_size, int a_ret_code, uint32_t a_action, 
                                    dap_chain_srv_uid_t a_uid);
static void s_callback_datum_removed_notify(void *a_arg, dap_hash_sha3_256_t *a_datum_hash, dap_chain_datum_t *a_datum);
...
int dap_chain_wallet_cache_init()
{
    ...
    for(dap_list_t *it = l_local_addr_list; it; it=it->next){
        ...
        if (!l_wallet_item){
            ...
            dap_ht_add(s_wallets_cache, wallet_addr, l_wallet_item);
...
int dap_chain_wallet_cache_tx_find(dap_chain_addr_t *a_addr, char *a_token, dap_chain_datum_tx_t **a_datum, dap_hash_sha3_256_t *a_tx_hash_curr, int* a_ret_code)
{
    dap_wallet_cache_t *l_wallet_item = NULL;
    ...
    dap_ht_find(s_wallets_cache, a_addr, sizeof(dap_chain_addr_t), l_wallet_item);
    if ( l_wallet_item ) {
    ...
    if (!dap_hash_sha3_256_is_blank(a_tx_hash_curr)) {
        // find start transaction
        dap_ht_find(l_wallet_item->wallet_txs, a_tx_hash_curr, sizeof(dap_hash_sha3_256_t), l_current_wallet_tx);
        if (!l_current_wallet_tx){
            debug_if(s_debug_more, L_DEBUG, "Can't find tx %s for address %s", dap_hash_sha3_256_to_str_static(a_tx_hash_curr), dap_chain_addr_to_str_static(a_addr));
            pthread_rwlock_unlock(&s_wallet_cache_rwlock);
            return 0;
        }
    ...
    dap_ht_foreach(l_current_wallet_tx, l_current_wallet_tx_iter, l_tmp) {
        ...
        if (a_token){
                ...
                !dap_strcmp(l_current_wallet_tx_iter->token_ticker, a_token))
            {
                ...
                for (dap_list_t *l_temp = l_current_wallet_tx_iter->tx_wallet_outputs; l_temp; l_temp=l_temp->next){
                    ...
                    if (l_out_type == TX_ITEM_TYPE_OUT_ANON && !dap_strcmp(a_token, ((dap_chain_tx_out_anon_t *)l_cur_out_cache->tx_out)->token_ticker)) {
                        skip = false;
                        break;
                    }
                }
...
int dap_chain_wallet_cache_tx_find_in_history(dap_chain_addr_t *a_addr, char **a_token, int* a_ret_code, dap_chain_tx_tag_action_type_t *a_action,
                                    dap_chain_srv_uid_t *a_uid, dap_chain_datum_tx_t **a_datum, dap_hash_sha3_256_t *a_tx_hash_curr)
{
    dap_wallet_cache_t *l_wallet_item = NULL;
                                                    ...
                                                    dap_list_t **a_outs_list, uint256_t *a_value_transfer, bool a_mempool_check)
{
    ...
    dap_ht_foreach(l_wallet_item->unspent_outputs, l_item_cur, l_tmp) {
        if (dap_strcmp(l_item_cur->token_ticker, a_token_ticker))
            ...
            case TX_ITEM_TYPE_OUT_ANON: {
                dap_chain_tx_out_anon_t *l_out_anon = (dap_chain_tx_out_anon_t *)l_out_cur->tx_out;
                if (dap_strcmp(l_out_anon->token_ticker, a_token_ticker))
                    continue;
                /* Amount is hidden in Pedersen commitment; caller supplies needed value. */
                l_value = uint256_0;
            } break;
            default:
                continue;
                                                    ...
                                                    dap_list_t **a_outs_list, uint256_t a_value_need, uint256_t *a_value_transfer, bool a_mempool_check)
{
    ...
    dap_ht_foreach(l_wallet_item->unspent_outputs, l_item_cur, l_tmp) {
        if (dap_strcmp(l_item_cur->token_ticker, a_token_ticker))
            ...
            case TX_ITEM_TYPE_OUT_ANON: {
                ...
                if (dap_strcmp(l_out_anon->token_ticker, a_token_ticker))
                    continue;
                if (l_item_cur->has_anon_known_value)
                    l_value = l_item_cur->anon_known_value;
                else
                    l_value = a_value_need;
            } break;
            default:
                continue;
            }
    ...
    /* Provisional OUT_ANON from CLI seed (mempool TX not yet indexed into unspent_outputs) */
    if (s_anon_out_seeds) {
        anon_out_seed_item_t *l_seed = NULL, *l_seed_tmp = NULL;
        pthread_rwlock_rdlock(&s_wallet_cache_rwlock);
        dap_ht_foreach(s_anon_out_seeds, l_seed, l_seed_tmp) {
            if (dap_chain_addr_is_blank(&l_seed->addr) || dap_strcmp(l_seed->token_ticker, a_token_ticker))
                continue;
            if (!dap_chain_addr_compare(&l_seed->addr, a_addr))
                continue;
            if (a_mempool_check && dap_chain_mempool_out_is_used(a_net, &l_seed->key.tx_hash, l_seed->key.out_idx))
                continue;
            bool l_already = false;
            for (dap_list_t *l_it = l_list_used_out; l_it; l_it = l_it->next) {
                dap_chain_tx_used_out_item_t *l_u = (dap_chain_tx_used_out_item_t *)l_it->data;
                if (l_u && !memcmp(&l_u->tx_hash_fast, &l_seed->key.tx_hash, sizeof(l_u->tx_hash_fast)) &&
                        l_u->num_idx_out == (uint32_t)l_seed->key.out_idx) {
                    l_already = true;
                    break;
                }
            }
            if (l_already)
                continue;
            dap_chain_tx_used_out_item_t *l_item = DAP_NEW_Z(dap_chain_tx_used_out_item_t);
            *l_item = (dap_chain_tx_used_out_item_t) {
                l_seed->key.tx_hash, (uint32_t)l_seed->key.out_idx, l_seed->value };
            l_list_used_out = dap_list_append(l_list_used_out, l_item);
            SUM_256_256(l_value_transfer, l_item->value, &l_value_transfer);
        }
        pthread_rwlock_unlock(&s_wallet_cache_rwlock);
    ...
    TX_ITEM_ITER_TX(l_tx_item, l_size, a_tx) {
        ...
        case TX_ITEM_TYPE_IN_ANON: {
            if (*l_tx_item == TX_ITEM_TYPE_IN) {
                ...
                const dap_chain_tx_in_anon_t *l_in_anon = (const dap_chain_tx_in_anon_t *)l_tx_item;
                l_prev_tx_hash = l_in_anon->prev_hash;
                l_prev_idx = (int)l_in_anon->prev_out_idx;
            }
            if ( dap_hash_sha3_256_is_blank(&l_prev_tx_hash) )
                continue;
            ...
            if (!l_tx_prev) {
                log_it(L_ERROR, "Can't find previous transaction by hash \"%s\"", dap_hash_sha3_256_to_str_static(&l_prev_tx_hash));
                continue;
            }
            l_prev_item = dap_chain_datum_tx_item_get_nth(l_tx_prev, TX_ITEM_TYPE_OUT_ALL, l_prev_idx);
            if (!l_prev_item) {
                log_it(L_ERROR, "Can't find output %d in tx \"%s\"", l_prev_idx, dap_hash_sha3_256_to_str_static(&l_prev_tx_hash));
                continue;
            }
            switch (*l_prev_item) {
            case TX_ITEM_TYPE_OUT_OLD:
            ...
            case TX_ITEM_TYPE_OUT_ANON:
                l_value = uint256_0;
                l_addr = ((dap_chain_tx_out_anon_t *)l_prev_item)->addr;
                break;
            default:
                continue;
            }
        ...
        case TX_ITEM_TYPE_OUT_STD:
            ...
            break;
        case TX_ITEM_TYPE_OUT_ANON:
            l_addr = ((dap_chain_tx_out_anon_t *)l_tx_item)->addr;
            l_multichannel = true;
            break;
        ...
        case 'a':
            if (!l_wallet_item) {
                ...
                dap_ht_find(l_wallet_item->wallet_txs, a_tx_hash, sizeof(dap_hash_sha3_256_t), l_wallet_tx_item);
            ...
            if (!l_wallet_tx_item) {
                ...
                *l_wallet_tx_item = (dap_wallet_tx_cache_t){ .tx_hash = *a_tx_hash, .atom_hash = *a_atom_hash, .tx = a_tx,
                ...
                dap_ht_add(l_wallet_item->wallet_txs, tx_hash, l_wallet_tx_item);
            }
            break;
        case 'd': {
            if (!l_wallet_item)
            ...
            dap_ht_find(l_wallet_item->wallet_txs, a_tx_hash, sizeof(dap_hash_sha3_256_t), l_wallet_tx_item);
            if (l_wallet_tx_item){
                dap_ht_del(l_wallet_item->wallet_txs, l_wallet_tx_item);
        ...
        case TX_ITEM_TYPE_IN_ANON:
            ...
            case 'a': {
                ...
                *l_tx_in = (dap_wallet_tx_cache_input_t) { .tx_prev_hash = l_prev_tx_hash, .tx_out_prev_idx = l_prev_idx, .value = l_value };
                ...
                if (!a_ret_code) {
                    ...
                    dap_ht_find(l_wallet_item->unspent_outputs, &key, sizeof(unspent_cache_hh_key), l_item);
                    if (l_item) {
                        dap_ht_del(l_wallet_item->unspent_outputs, l_item);
                        DAP_DELETE(l_item);
                    }
                    if (s_anon_out_seeds) {
                        anon_out_seed_item_t *l_seed = NULL;
                        dap_ht_find(s_anon_out_seeds, &key, sizeof(unspent_cache_hh_key), l_seed);
                        if (l_seed) {
                            dap_ht_del(s_anon_out_seeds, l_seed);
                            DAP_DELETE(l_seed);
                        }
                    }
                }
            ...
            case 'd': {
                ...
                dap_ht_find(l_wallet_item->wallet_txs, &l_prev_tx_hash, sizeof(dap_hash_sha3_256_t), l_wallet_prev_tx_item);
                if ( l_wallet_prev_tx_item && !l_wallet_prev_tx_item->ret_code ) {
                    ...
                    if (l_out_item) {
                        ...
                        dap_ht_find(l_wallet_item->unspent_outputs, &l_key, sizeof(unspent_cache_hh_key), l_item);
                        if ( !l_item ) {
                            ...
                            *l_item = (dap_wallet_cache_unspent_outs_t) { .key = l_key, .output = l_out_item->data };
                            dap_strncpy(l_item->token_ticker, *l_prev_item == TX_ITEM_TYPE_OUT_EXT ? ((dap_chain_tx_out_ext_t*)l_prev_item)->token
                                        ...
                                        : *l_prev_item == TX_ITEM_TYPE_OUT_ANON ? ((dap_chain_tx_out_anon_t *)l_prev_item)->token_ticker
                                        : l_wallet_prev_tx_item->token_ticker, DAP_CHAIN_TICKER_SIZE_MAX);
                            dap_ht_add(l_wallet_item->unspent_outputs, key, l_item);
                        }
                    }
                }
            } break;
        ...
        default:
            switch (a_cache_op) {
            case 'a': {
                int l_out_all_idx = dap_chain_datum_tx_out_all_index(a_tx, l_tx_item);
                if (l_out_all_idx < 0)
                    break;
                dap_wallet_tx_cache_output_t *l_out = NULL;
                dap_wallet_tx_cache_output_t l_sought_out = { .tx_out_idx = l_out_all_idx };
                ...
                if ( !l_out_item ) {
                    ...
                    *l_out = (dap_wallet_tx_cache_output_t){ .tx_out = l_tx_item, .tx_out_idx = l_out_all_idx };
                ...
                if (!a_ret_code) {
                    dap_wallet_cache_unspent_outs_t *l_item = NULL;
                    unspent_cache_hh_key l_key = { .tx_hash = *a_tx_hash, .out_idx = l_out_all_idx };
                    dap_ht_find(l_wallet_item->unspent_outputs, &l_key, sizeof(unspent_cache_hh_key), l_item);
                    if ( !l_item ) {
                        l_item = DAP_NEW(dap_wallet_cache_unspent_outs_t);
                        *l_item = (dap_wallet_cache_unspent_outs_t) { .key = l_key, .output = l_out };
                        dap_strncpy(l_item->token_ticker, *l_tx_item == TX_ITEM_TYPE_OUT_EXT ? ((dap_chain_tx_out_ext_t*)l_tx_item)->token
                                ...
                                : *l_tx_item == TX_ITEM_TYPE_OUT_ANON ? ((dap_chain_tx_out_anon_t *)l_tx_item)->token_ticker
                                : a_main_token_ticker ? a_main_token_ticker : "0", DAP_CHAIN_TICKER_SIZE_MAX);
                        {
                            anon_out_seed_item_t *l_seed = NULL;
                            if (s_anon_out_seeds) {
                                dap_ht_find(s_anon_out_seeds, &l_key, sizeof(unspent_cache_hh_key), l_seed);
                                if (l_seed) {
                                    l_item->anon_known_value = l_seed->value;
                                    l_item->has_anon_known_value = true;
                                }
                            }
                        }
                        dap_ht_add(l_wallet_item->unspent_outputs, key, l_item);
            ...
            case 'd': {
                ...
                if ( !l_wallet_item->wallet_txs ) {
                    dap_ht_del(s_wallets_cache, l_wallet_item);
                    DAP_DELETE(l_wallet_item);
                }            
                unspent_cache_hh_key key = { .tx_hash = *a_tx_hash, .out_idx = l_out_all_idx };
                dap_wallet_cache_unspent_outs_t *l_item = NULL;
                dap_ht_find(l_wallet_item->unspent_outputs, &key, sizeof(unspent_cache_hh_key), l_item);
                if (l_item) {
                    dap_ht_del(l_wallet_item->unspent_outputs, l_item);
...
dap_chain_datum_tx_t *dap_chain_wallet_cache_iter_get(dap_chain_wallet_cache_iter_t *a_iter, dap_chain_wallet_getting_type_t a_type)
{
    ...
    switch (a_type){
        ...
        case DAP_CHAIN_WALLET_CACHE_GET_LAST:{
            ...
            if (l_wallet_cache)
                l_tx_cache = dap_ht_last(l_wallet_cache->wallet_txs);
...
{
    dap_chain_wallet_cache_seed_anon_out_for_addr(NULL, NULL, a_tx_hash, a_out_idx, a_value);
}
...
void dap_chain_wallet_cache_seed_anon_out_for_addr(const dap_chain_addr_t *a_addr, const char *a_token_ticker,
                                                   ...
                                                   uint256_t a_value)
{
    if (!a_tx_hash || IS_ZERO_256(a_value))
        return;

    pthread_rwlock_wrlock(&s_wallet_cache_rwlock);
    unspent_cache_hh_key l_key = { .tx_hash = *a_tx_hash, .out_idx = (int)a_out_idx };
    anon_out_seed_item_t *l_item = NULL;
    dap_ht_find(s_anon_out_seeds, &l_key, sizeof(l_key), l_item);
    if (!l_item) {
        l_item = DAP_NEW(anon_out_seed_item_t);
        *l_item = (anon_out_seed_item_t) { .key = l_key, .value = a_value };
        dap_ht_add(s_anon_out_seeds, key, l_item);
    } else {
    ...
    if (a_addr && !dap_chain_addr_is_blank(a_addr))
        l_item->addr = *a_addr;
    if (a_token_ticker)
        dap_strncpy(l_item->token_ticker, a_token_ticker, DAP_CHAIN_TICKER_SIZE_MAX);
    pthread_rwlock_unlock(&s_wallet_cache_rwlock);
}
