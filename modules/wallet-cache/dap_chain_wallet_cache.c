...
int dap_chain_wallet_cache_init()
{
    ...
    for(dap_list_t *it = l_local_addr_list; it; it=it->next){
        ...
        dap_ht_find_hh(hh, s_wallets_cache, l_addr, sizeof(dap_chain_addr_t), l_wallet_item);
        if (!l_wallet_item){
            l_wallet_item = DAP_NEW_Z(dap_wallet_cache_t);
            memcpy (&l_wallet_item->wallet_addr, l_addr, sizeof(dap_chain_addr_t));
            dap_ht_add_hh(hh, s_wallets_cache, wallet_addr, l_wallet_item);
...
int dap_chain_wallet_cache_tx_find(dap_chain_addr_t *a_addr, char *a_token, dap_chain_datum_tx_t **a_datum, dap_hash_sha3_256_t *a_tx_hash_curr, int* a_ret_code)
{
    ...
    dap_ht_find_hh(hh, s_wallets_cache, a_addr, sizeof(dap_chain_addr_t), l_wallet_item);
    if ( l_wallet_item ) {
    ...
    if (!dap_hash_sha3_256_is_blank(a_tx_hash_curr)) {
        ...
        dap_ht_find_hh(hh, l_wallet_item->wallet_txs, a_tx_hash_curr, sizeof(dap_hash_sha3_256_t), l_current_wallet_tx);
    ...
    dap_ht_foreach_hh(hh, l_current_wallet_tx, l_current_wallet_tx_iter, l_tmp) {
                                                    ...
                                                    dap_list_t **a_outs_list, uint256_t *a_value_transfer, bool a_mempool_check)
{
    ...
    dap_ht_foreach_hh(hh, l_wallet_item->unspent_outputs, l_item_cur, l_tmp) {
    ...
    TX_ITEM_ITER_TX(l_tx_item, l_size, a_tx) {
        ...
        case 'a':
            if (!l_wallet_item) {
                ...
                dap_ht_find_hh(hh, l_wallet_item->wallet_txs, a_tx_hash, sizeof(dap_hash_sha3_256_t), l_wallet_tx_item);
            ...
            if (!l_wallet_tx_item) {
                ...
                *l_wallet_tx_item = (dap_wallet_tx_cache_t){ .tx_hash = *a_tx_hash, .atom_hash = *a_atom_hash, .tx = a_tx,
                ...
                dap_ht_add_hh(hh, l_wallet_item->wallet_txs, tx_hash, l_wallet_tx_item);
            }
            break;
        case 'd': {
            if (!l_wallet_item)
        ...
        case TX_ITEM_TYPE_IN:
            ...
            case 'a': {
                ...
                *l_tx_in = (dap_wallet_tx_cache_input_t) { .tx_prev_hash = l_prev_tx_hash, .tx_out_prev_idx = l_prev_idx, .value = l_value };
                ...
                if (!a_ret_code) {
                    ...
                    dap_ht_find_hh(hh, l_wallet_item->unspent_outputs, &key, sizeof(unspent_cache_hh_key), l_item);
                    if (l_item) {
            ...
            case 'd': {
                ...
                dap_ht_find_hh(hh, l_wallet_item->wallet_txs, &l_prev_tx_hash, sizeof(dap_hash_sha3_256_t), l_wallet_prev_tx_item);
                if ( l_wallet_prev_tx_item && !l_wallet_prev_tx_item->ret_code ) {
                    ...
                    if (l_out_item) {
                        ...
                        dap_ht_find_hh(hh, l_wallet_item->unspent_outputs, &l_key, sizeof(unspent_cache_hh_key), l_item);
                        if ( !l_item ) {
                            ...
                            *l_item = (dap_wallet_cache_unspent_outs_t) { .key = l_key, .output = l_out_item->data };
                            ...
                            dap_ht_add_hh(hh, l_wallet_item->unspent_outputs, key, l_item);
                        }
                    }
                }
            } break;
