...
#include "dap_global_db.h"  // For GDB read (includes dap_global_db_store_obj_t)
#include "dap_json.h"
...
static int s_wallet_shared_verificator(dap_ledger_t *a_ledger, dap_chain_datum_tx_t *a_tx_in, dap_hash_sha3_256_t *a_tx_in_hash,
                                       dap_chain_tx_out_cond_t *a_cond, bool UNUSED_ARG a_owner, bool UNUSED_ARG a_from_mempool)
{
    size_t l_tsd_hashes_count = a_cond->tsd_size / (sizeof(dap_tsd_t) + sizeof(dap_hash_sha3_256_t));
    ...
    TX_ITEM_ITER_TX(l_item, l_tx_item_size, a_tx_in) {
        ...
        case TX_ITEM_TYPE_OUT_COND:
            if (a_cond->header.subtype == ((dap_chain_tx_out_cond_t *)l_item)->header.subtype) {
                if (l_cond_out) {
                    log_it(L_ERROR, "Tx %s verificator error: Only the condional output allowed for target subtype", dap_hash_sha3_256_to_str_static(a_tx_in_hash));
                    return -3;
                }
        ...
        case TX_ITEM_TYPE_TSD: {
            ...
            if (l_tsd->size != sizeof(uint256_t)) {
                log_it(L_ERROR, "Tx %s verificator error: TSD section size control error", dap_hash_sha3_256_to_str_static(a_tx_in_hash));
                return -4;
            }
            if (!IS_ZERO_256(l_writeoff_value)) {
                log_it(L_ERROR, "Tx %s verificator error: More than one TSD section is forbidden", dap_hash_sha3_256_to_str_static(a_tx_in_hash));
                return -5;
            }
        ...
        case TX_ITEM_TYPE_SIG: {
            ...
            if (l_signs_counter >= l_tsd_hashes_count * 2) {
                log_it(L_WARNING, "Tx %s verificator error: Too many signs, can't process more than %zu", dap_hash_sha3_256_to_str_static(a_tx_in_hash), l_tsd_hashes_count * 2);
                return -1;
            }
            ...
            dap_tsd_iter(l_tsd, l_tsd_size, a_cond->tsd, a_cond->tsd_size) {
                if (l_tsd->type == DAP_CHAIN_TX_OUT_COND_TSD_HASH && l_tsd->size == sizeof(dap_hash_sha3_256_t) &&
                        dap_hash_sha3_256_compare(&l_pkey_hash, (dap_hash_sha3_256_t *)l_tsd->data) &&
    ...
    if (IS_ZERO_256(l_writeoff_value)) {
        log_it(L_ERROR, "Tx %s verificator error: Write-off value not found, can't process", dap_hash_sha3_256_to_str_static(a_tx_in_hash));
        return -6;
    }
    ...
    if (l_change_type == DAP_CHAIN_WALLET_SHARED_TSD_WRITEOFF && l_signs_verified < a_cond->subtype.wallet_shared.signers_minimum) {
        log_it(L_WARNING, "Tx %s verificator error: Not enough valid signs (%u from %u) for shared funds tx",
                            dap_hash_sha3_256_to_str_static(a_tx_in_hash), l_signs_verified, a_cond->subtype.wallet_shared.signers_minimum);
...
static int s_collect_cert_pkey_hashes()
{
    ...
    for (dap_list_t *l_item = l_certs_list; l_item; l_item = l_item->next) {
        ...
        dap_hash_sha3_256_t l_pkey_hash;
        if (dap_cert_get_pkey_hash(l_cert, DAP_HASH_TYPE_SHA3_256, l_pkey_hash.raw, sizeof(dap_hash_sha3_256_t))) {
...
dap_chain_datum_tx_t *dap_chain_wallet_shared_refilling_tx_create(dap_json_t *a_json_arr_reply, dap_chain_net_t *a_net, dap_enc_key_t *a_enc_key,
    uint256_t a_value, uint256_t a_fee, dap_hash_sha3_256_t *a_tx_in_hash, dap_list_t* a_tsd_items)
{
    ...
    dap_hash_sha3_256_t l_final_tx_hash = dap_ledger_get_final_chain_tx_hash(l_ledger, DAP_CHAIN_TX_OUT_COND_SUBTYPE_WALLET_SHARED, a_tx_in_hash, false);
    if (dap_hash_sha3_256_is_blank(&l_final_tx_hash))
        m_tx_fail(ERROR_FUNDS, "Nothing to refill, can't find tx");

    log_it(L_NOTICE, "Actual TX hash %s will be used for refill TX composing", dap_hash_sha3_256_to_str_static(&l_final_tx_hash));
...
static bool s_is_key_present(dap_chain_tx_out_cond_t *a_cond, dap_enc_key_t *a_enc_key)
{
    if (!a_cond->tsd_size || !a_enc_key->pub_key_data_size)
        return false;
    dap_hash_sha3_256_t l_pub_key_hash;
    if (dap_enc_key_get_pkey_hash(a_enc_key, DAP_HASH_TYPE_SHA3_256, l_pub_key_hash.raw, sizeof(dap_hash_sha3_256_t)))
        return false;
    dap_tsd_t *l_tsd; size_t l_tsd_size;
    dap_tsd_iter(l_tsd, l_tsd_size, a_cond->tsd, a_cond->tsd_size)
        if (l_tsd->type == DAP_CHAIN_TX_OUT_COND_TSD_HASH && l_tsd->size == sizeof(dap_hash_sha3_256_t) &&
                dap_hash_sha3_256_compare(&l_pub_key_hash, (dap_hash_sha3_256_t *)l_tsd->data))
            return true;
    return false;
}
...
dap_chain_datum_tx_t *dap_chain_wallet_shared_taking_tx_create(dap_json_t *a_json_arr_reply, dap_chain_net_t *a_net, dap_enc_key_t *a_enc_key,
    dap_chain_addr_t *a_to_addr, uint256_t *a_value, uint32_t a_addr_count /*!not change type!*/, uint256_t a_fee, dap_hash_sha3_256_t *a_tx_in_hash, dap_list_t* a_tsd_items)
{
    ...
    dap_hash_sha3_256_t l_final_tx_hash = dap_ledger_get_final_chain_tx_hash(l_ledger, DAP_CHAIN_TX_OUT_COND_SUBTYPE_WALLET_SHARED, a_tx_in_hash, true);
    if (dap_hash_sha3_256_is_blank(&l_final_tx_hash))
        m_tx_fail(ERROR_FUNDS, "Nothing to emit (not enough funds)");

    log_it(L_NOTICE, "Actual TX hash with unspent output %s will be used for taking TX composing", dap_hash_sha3_256_to_str_static(&l_final_tx_hash));
    dap_chain_datum_tx_t *l_tx_in = dap_ledger_tx_find_by_hash(l_ledger, &l_final_tx_hash);
    assert(l_tx_in);
    int l_prev_cond_idx = 0;
    dap_chain_tx_out_cond_t *l_cond_prev = dap_chain_datum_tx_out_cond_get(l_tx_in, DAP_CHAIN_TX_OUT_COND_SUBTYPE_WALLET_SHARED, &l_prev_cond_idx);
...
dap_chain_datum_tx_t *dap_chain_wallet_shared_taking_tx_sign(dap_json_t *a_json_arr_reply, dap_chain_net_t *a_net, dap_enc_key_t *a_enc_key, dap_chain_datum_tx_t *a_tx_in)
{
    ...
    size_t l_tsd_hashes_count = l_cond->tsd_size / (sizeof(dap_tsd_t) + sizeof(dap_hash_sha3_256_t));
...
static int s_cli_hold(int a_argc, char **a_argv, int a_arg_index, dap_json_t *a_json_arr_reply, dap_chain_net_t *a_net, dap_chain_t *a_chain, const char *a_hash_out_type)
{
    ...
    size_t l_hashes_count_max = l_pkeys_str_size / DAP_ENC_BASE58_ENCODE_SIZE(sizeof(dap_hash_sha3_256_t)),
           l_hashes_count = 0;
    dap_hash_sha3_256_t *l_pkey_hashes = DAP_NEW_Z_COUNT(dap_hash_sha3_256_t, l_hashes_count_max);
    if (!l_pkey_hashes) {
        dap_json_rpc_error_add(a_json_arr_reply, ERROR_MEMORY, c_error_memory_alloc);
        DAP_DELETE(l_enc_key);
        return ERROR_MEMORY;
    }
    char l_hash_str_buf[DAP_HASH_SHA3_256_STR_SIZE+1];
    ...
    for (size_t i = 0; i < l_hashes_count_max; i++) {
        ...
        dap_strncpy(l_hash_str_buf, l_token_ptr, dap_min(DAP_HASH_SHA3_256_STR_SIZE, l_cur_ptr - l_token_ptr + 1));
        if (dap_hash_sha3_256_from_str(l_hash_str_buf, l_pkey_hashes + i)) {
        ...
        for (size_t j = 0; j < i; ++j) {
            if (!memcmp(l_pkey_hashes + j, l_pkey_hashes + i, sizeof(dap_hash_sha3_256_t))){
...
static int s_cli_refill(int a_argc, char **a_argv, int a_arg_index, dap_json_t *a_json_arr_reply, dap_chain_net_t *a_net, dap_chain_t *a_chain, const char *a_hash_out_type)
{
    ...
    dap_hash_sha3_256_t l_tx_in_hash;
    if (dap_hash_sha3_256_from_str(l_tx_in_hash_str, &l_tx_in_hash)) {
...
static int s_cli_sign(int a_argc, char **a_argv, int a_arg_index, dap_json_t *a_json_arr_reply, dap_chain_net_t *a_net, dap_chain_t *a_chain, const char *a_hash_out_type)
{
    ...
    if (dap_hash_sha3_256_from_str(l_tx_in_hash_str, &l_tx_hash)) {
        dap_json_rpc_error_add(a_json_arr_reply, ERROR_VALUE, "Can't recognize %s as a hex or base58 format hash", l_tx_in_hash_str);
        return ERROR_VALUE;
    }
    ...
    dap_global_db_store_obj_t *l_store_obj = dap_global_db_get_raw_sync(l_mempool_group, l_tx_in_hash_str);
...
int dap_chain_shared_tx_find_in_mempool(dap_chain_t *a_chain, dap_hash_sha3_256_t *a_final_tx_hash, dap_json_t *a_jobj_waiting_operations_hashes)
{
    ...
    for (size_t i = 0; i < l_objs_count; ++i) {
        ...
        TX_ITEM_ITER_TX(l_item, l_item_size, l_tx_mempool) {
            if (*l_item == TX_ITEM_TYPE_IN_COND) {
                ...
                if (
                    dap_hash_sha3_256_compare(&l_in_cond->header.tx_prev_hash, a_final_tx_hash) &&
...
static int s_cli_info(int a_argc, char **a_argv, int a_arg_index, dap_json_t *a_json_arr_reply, dap_chain_net_t *a_net, dap_chain_t *a_chain, const char *a_hash_out_type)
{
    ...
    dap_hash_sha3_256_t l_final_tx_hash = dap_ledger_get_final_chain_tx_hash(a_net->pub.ledger, DAP_CHAIN_TX_OUT_COND_SUBTYPE_WALLET_SHARED, &l_tx_hash, false);
    dap_chain_datum_tx_t *l_tx = dap_ledger_tx_find_by_hash(a_net->pub.ledger, &l_final_tx_hash);
    if (!l_tx) {
        dap_json_rpc_error_add(a_json_arr_reply, ERROR_TX_MISMATCH, "Can't find final datum %s", dap_hash_sha3_256_to_str_static(&l_final_tx_hash));
    ...
    // verify block
    dap_json_object_add_object(l_jobj_take_verify, "signs_minimum", dap_json_object_new_uint64(l_cond->subtype.wallet_shared.signers_minimum));
    dap_tsd_t *l_tsd = NULL; size_t l_tsd_size = 0;
    dap_tsd_iter(l_tsd, l_tsd_size, l_cond->tsd, l_cond->tsd_size) {
        if (l_tsd->type == DAP_CHAIN_TX_OUT_COND_TSD_HASH && l_tsd->size == sizeof(dap_hash_sha3_256_t)) {
            dap_json_array_add(l_jobj_pkey_hashes, dap_json_object_new_string(l_is_base_hash_type ? dap_enc_base58_encode_hash_to_str_static((const dap_hash_sha3_256_t *)l_tsd->data) : dap_hash_sha3_256_to_str_static((const dap_hash_sha3_256_t *)l_tsd->data)));
        }
        if (l_tsd->type == DAP_CHAIN_TX_OUT_COND_TSD_STR) {
            dap_json_array_add(l_jobj_tags, dap_json_object_new_string((char*)(l_tsd->data)));
        }
    }
    dap_json_object_add_object(l_jobj_take_verify, "owner_hashes", l_jobj_pkey_hashes);
    // result block
    dap_hash_sha3_256_t l_creator_hash = {0};
    dap_sign_t *l_sig = dap_chain_datum_tx_get_sign(l_tx, 0);
    dap_sign_get_pkey_hash(l_sig, &l_creator_hash);
    dap_json_object_add_object(l_json_jobj_info, "tx_hash", dap_json_object_new_string(l_is_base_hash_type ? dap_enc_base58_encode_hash_to_str_static(&l_tx_hash) : dap_hash_sha3_256_to_str_static(&l_tx_hash)));
    dap_json_object_add_object(l_json_jobj_info, "tx_hash_final", dap_json_object_new_string(l_is_base_hash_type ? dap_enc_base58_encode_hash_to_str_static(&l_final_tx_hash) : dap_hash_sha3_256_to_str_static(&l_final_tx_hash)));
    dap_json_object_add_object(l_json_jobj_info, "tags", l_jobj_tags);
    dap_json_object_add_object(l_json_jobj_info, "balance", l_jobj_balance);
    dap_json_object_add_object(l_json_jobj_info, "token", l_jobj_token);
    dap_json_object_add_object(l_json_jobj_info, "creator", dap_json_object_new_string(l_is_base_hash_type ? dap_enc_base58_encode_hash_to_str_static(&l_creator_hash) : dap_hash_sha3_256_to_str_static(&l_creator_hash)));
...
static int s_cli_list(int a_argc, char **a_argv, int a_arg_index, dap_json_t *a_json_arr_reply, dap_chain_net_t *a_net, dap_chain_t *a_chain, const char *a_hash_out_type)
{
    ...
    if (l_pkey_hash_str) {
        ...
        memcpy(&l_pkey_hash, &l_addr->data.hash, sizeof(dap_hash_sha3_256_t));
        DAP_DELETE(l_addr);
        ...
        if (dap_enc_key_get_pkey_hash(l_cert->enc_key, DAP_HASH_TYPE_SHA3_256, l_pkey_hash.raw, sizeof(dap_hash_sha3_256_t)) != 0) {
            dap_json_rpc_error_add(a_json_arr_reply, ERROR_VALUE, 
    ...
    dap_global_db_store_obj_t *l_values = NULL;
    if (l_filter_count) {
        l_values_count = 1;
    } else if (l_local) {
    ...
    dap_list_t *l_groups_list = dap_global_db_get_groups_by_mask(s_wallet_shared_gdb_group);
    if (!l_groups_list) {
    ...
    for (size_t i = 0; i < l_values_count; i++) {
        ...
        if (!l_filter_count) {
            ...
            dap_json_object_add_string(l_jobj_item, "pkey_hash", dap_hash_sha3_256_to_str_static(&l_pkey_hash));
        }
        ...
        for (dap_list_t *l_item = l_groups_list; l_item; l_item = l_item->next) {
            ...
            if (l_net && dap_strcmp((char *)l_item->data + sizeof(s_wallet_shared_gdb_group), l_net_name))
                continue;
            hold_tx_hashes_t *l_hold_hashes_by_name = (hold_tx_hashes_t *)dap_global_db_get_sync(l_item->data, l_filter_count ? dap_hash_sha3_256_to_str_static(&l_pkey_hash) : l_values[i].key, NULL, NULL, NULL);
            if (l_hold_hashes_by_name) {
                ...
                for (size_t j = 0; j < l_hold_hashes_by_name->tx_count; j++) {
                    if (l_hold_hashes_by_name->tx[j].role == TX_ROLE_OWNER)
                        dap_json_array_add(l_jobj_owned_tx, dap_json_object_new_string(dap_hash_sha3_256_to_str_static(&l_hold_hashes_by_name->tx[j].hash)));
                }
    ...
    dap_global_db_store_obj_free(l_values, l_values_count);
...
static void s_hold_tx_add(dap_chain_datum_tx_t *a_tx, const char *a_group, dap_hash_sha3_256_t *a_pkey_hash, tx_role_t a_role)
{
    size_t l_tx_hashes_count = 0;
    size_t l_shared_hashes_size = 0;
    char *l_pkey_hash_str = dap_hash_sha3_256_to_str_new(a_pkey_hash);
    ...
    log_it(L_DEBUG, "Added pkey hash %s as %s to shared hashes: %s", l_pkey_hash_str,a_role == TX_ROLE_CREATOR ? "creator" : "owner", dap_hash_sha3_256_to_str_static(&l_shared_hashes->tx[l_shared_hashes->tx_count - 1].hash));
...
int dap_chain_wallet_shared_hold_tx_add(dap_chain_datum_tx_t *a_tx, const char *a_net_name)
{
    ...
    dap_tsd_iter(l_tsd, l_tsd_size, l_cond->tsd, l_cond->tsd_size) {
        if (l_tsd->type == DAP_CHAIN_TX_OUT_COND_TSD_HASH && l_tsd->size == sizeof(dap_hash_sha3_256_t)) {
            s_hold_tx_add(a_tx, l_group, (dap_hash_sha3_256_t *)l_tsd->data, TX_ROLE_OWNER);
            dap_global_db_set_sync(s_wallet_shared_gdb_pkeys, dap_hash_sha3_256_to_str_static((dap_hash_sha3_256_t *)l_tsd->data), NULL, 0, false);
        }
    }
    DAP_DELETE(l_group);
    return 0;
...
dap_json_t *dap_chain_wallet_shared_get_tx_hashes_json(dap_hash_sha3_256_t *a_pkey_hash, const char *a_net_name)
{
    dap_json_t *l_json_ret = dap_json_array_new();
    char *l_group = dap_strdup_printf("%s.%s", s_wallet_shared_gdb_group, a_net_name);
    hold_tx_hashes_t *l_item = (hold_tx_hashes_t *)dap_global_db_get_sync(l_group, dap_hash_sha3_256_to_str_static(a_pkey_hash), NULL, NULL, false);
    DAP_DELETE(l_group);
    if (!l_item) {
        return NULL;
    }
    for (size_t i = 0; i < l_item->tx_count; i++) {
        if (l_item->tx[i].role == TX_ROLE_OWNER) {
            dap_json_array_add(l_json_ret, dap_json_object_new_string(dap_hash_sha3_256_to_str_static(&l_item->tx[i].hash)));
        }
    }
    DAP_DELETE(l_item);
    return l_json_ret;
...
static void s_shared_tx_mempool_notify(dap_global_db_store_obj_t *a_obj, void *a_arg)
{
    dap_return_if_fail(a_obj && a_arg);
    if (dap_global_db_store_obj_get_type(a_obj) != DAP_GLOBAL_DB_OPTYPE_ADD || !a_obj->value)
        return;
    ...
    dap_hash_sha3_256_t l_in_cond_hash = {0};
    byte_t *l_item; size_t l_tx_item_size;
    uint16_t l_change_type = 0;
