            ...
            switch (cmd_num) {
                ...
                case CMD_WALLET_DEACTIVATE: {
                    ...
                    l_rc = cmd_num == CMD_WALLET_ACTIVATE
                            ? dap_chain_wallet_activate(l_wallet_name, strlen(l_wallet_name), NULL, l_pass_str, strlen(l_pass_str), l_rc, NULL)
                ...
                case CMD_WALLET_CONVERT: {
                    ...
                    snprintf(l_file_name->file_name, sizeof(l_file_name->file_name), "%s/%s_%012llu%s", c_wallets_path, l_wallet_name, (unsigned long long)time(NULL),".backup");
