...
void s_wallet_addr_cache_add(dap_chain_addr_t *a_addr, const char *a_wallet_name){
    ...
    l_cache->addr = *a_addr;
    dap_ht_add_hh(hh, s_wallet_addr_cache, addr, l_cache);
}
const char *dap_chain_wallet_addr_cache_get_name(dap_chain_addr_t *a_addr){
    struct wallet_addr_cache *l_tmp = NULL;
    dap_ht_find_hh(hh, s_wallet_addr_cache, a_addr, sizeof(dap_chain_addr_t), l_tmp);
...
dap_list_t* dap_chain_wallet_get_local_addr(){
    ...
    dap_ht_foreach_hh(hh, s_wallet_addr_cache, l_item, l_tmp){
...
int dap_chain_wallet_get_pkey_hash(dap_chain_wallet_t *a_wallet, dap_hash_sha3_256_t *a_out_hash)
{
    ...
    int ret = dap_enc_key_get_pkey_hash(l_key, DAP_HASH_TYPE_SHA3_256, (byte_t *)a_out_hash, sizeof(*a_out_hash));
    dap_enc_key_delete(l_key);
    return ret;
}
