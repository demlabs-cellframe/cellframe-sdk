...
size_t l_len;
    ...
    {
        if ( l_dir_entry->d_type !=  DT_REG )
            continue;

        l_len = strlen(l_dir_entry->d_name);
...
char *l_cp;
    ...
    return  strncpy(s_wallets_path, l_cp, sizeof(s_wallets_path) );         /* Make local copy , return it to caller */
}
        ...
        const char *a_pass)
{
    ...
    if ( dap_chain_wallet_save(l_wallet, a_pass) == 0 )
        return l_wallet;
    else {
...
uint32_t    csum = CRC32C_INIT;
    ...
    l_iov[0].iov_base = &l_file_hdr;
    l_len = l_iov[0].iov_len = sizeof(l_file_hdr);

    l_iov[1].iov_base = l_cp;
    l_len += l_iov[1].iov_len = l_file_hdr.wallet_len;

    l_rc = writev (l_fd, l_iov, 2 );                                        /* Performs writting vectorized buffer */
    if ( l_len != l_rc )
    {
        close(l_fd);
    ...
    csum = s_crc32c (csum, l_iov[0].iov_base, l_iov[0].iov_len);            /* CRC for file header part */
    csum = s_crc32c (csum, l_iov[1].iov_base, l_iov[1].iov_len);
    ...
    for ( size_t i = 0; i < l_wallet_internal->certs_count ; i++)
    {
        ...
        */
        ...
        l_iov[0].iov_base  = &l_wallet_cert_hdr;                            /* Cert's record header */
        l_len += l_iov[0].iov_len  = sizeof(l_wallet_cert_hdr);

        l_iov[1].iov_base  = l_enc_key ? l_buf : l_cert_raw;                /* Cert itself or buffer with has been encrypted cert */
        l_len += l_iov[1].iov_len  = l_wallet_cert_hdr.cert_raw_size;

        l_rc = writev (l_fd, l_iov, 2 );                                    /* Perform writting vectorized buffer */
    ...
    if ( l_file_hdr.version == DAP_WALLET$K_VER_2 )
    {
        ...
        l_iov[0].iov_base  = &l_wallet_cert_hdr;
        l_len += l_iov[0].iov_len  = sizeof(l_wallet_cert_hdr);

        l_iov[1].iov_base  = &csum;
        l_len += l_iov[1].iov_len  = sizeof(csum);
...
uint32_t    l_csum = CRC32C_INIT, l_csum2 = CRC32C_INIT;
    ...
    if ( (l_file_hdr.version == DAP_WALLET$K_VER_2) && (!a_pass) )
    ...
    if ( (l_file_hdr.version == DAP_WALLET$K_VER_2) && a_pass )             /* Generate encryptor context  */
        if ( !(l_enc_key = dap_enc_key_new_generate(DAP_ENC_KEY_TYPE_GOST_OFB, NULL, 0, a_pass, strlen(a_pass), 0)) )
...
char l_file_name [MAX_PATH];
    ...
    dap_snprintf(l_file_name, sizeof(l_file_name) - 1, "%s/%s.dwallet", a_wallets_path, a_wallet_name);

    return  dap_chain_wallet_open_file(l_file_name, NULL);
}


dap_chain_wallet_t *dap_chain_wallet_open_ext (const char *a_wallet_name,
        const char *a_wallets_path,
        const char *pass)
{
char l_file_name [MAX_PATH];
        ...
        const char *a_pass_str)
{
    ...
    log_it(L_ERROR,"Can't save the new wallet in disk, errno:%d ", errno);
    dap_chain_wallet_close(l_wallet);
    return NULL;
