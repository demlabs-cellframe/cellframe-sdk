...
int dap_chain_wallet_cache_tx_find(dap_chain_addr_t *a_addr, char *a_token, dap_chain_datum_tx_t **a_datum, dap_hash_fast_t *a_tx_hash_curr, int* a_ret_code)
{
    ...
    if ((l_wallet_item && l_wallet_item->is_loading) || (!l_wallet_item && s_wallets_cache_type != DAP_WALLET_CACHE_TYPE_ALL)){
        log_it(L_INFO, "Can't find wallet with address %s", dap_chain_addr_to_str_static(a_addr));
        pthread_rwlock_unlock(&s_wallet_cache_rwlock);
        return -101;
    } else if(!l_wallet_item) {
        log_it(L_INFO, "Can't find wallet with address %s. All wallets cache is enabled, so the wallet is empty.", dap_chain_addr_to_str_static(a_addr));
        pthread_rwlock_unlock(&s_wallet_cache_rwlock);
        return 0;
    }
