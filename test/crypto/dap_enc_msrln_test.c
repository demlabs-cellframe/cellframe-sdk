...
void dap_enc_msrln_tests_run(void)
{
    ...
    alice_msg = malloc(test_k->priv_key_data_size);
    memcpy(alice_msg, (uint8_t *) test_k->priv_key_data, test_k->priv_key_data_size * sizeof(uint8_t));

    /* Bob's response */
    rc = dap_enc_msrln_encode(test_k, (unsigned char *) alice_msg, &alice_msg_len, (unsigned char *) &bob_msg);
    bob_key = malloc(test_k->priv_key_data_size);
    memcpy(bob_key, (uint8_t *) test_k->priv_key_data, test_k->priv_key_data_size * sizeof(uint8_t));

    dap_enc_key_t * kk = dap_enc_key_new(DAP_ENC_KEY_TYPE_MSRLN);

    kk->priv_key_data_size = test_k->priv_key_data_size;
    kk->priv_key_data = (uint8_t *)malloc(kk->priv_key_data_size);
    memcpy(kk->priv_key_data, test_k->priv_key_data, kk->priv_key_data_size);

    /* Alice processes Bob's response */
    rc = dap_enc_msrln_decode(test_k, alice_priv, &alice_msg_len, bob_msg);
    alice_key = malloc(test_k->priv_key_data_size);
    memcpy(alice_key, (uint8_t *) test_k->priv_key_data, test_k->priv_key_data_size * sizeof(uint8_t));

    /* compare session key values */
    rc = memcmp(alice_key, bob_key, test_k->priv_key_data_size);
    if (rc != 0) {
