    ...
    dap_chain_hash_fast_t *a_emission_hash_out)
{
    ...
    // Step 1: Create token fixture signed by provided cert (same as token owner)
    // This ensures the token's auth_pkeys match the provided cert, which is critical
    // for arbitrage signature validation and other owner-based checks.
    test_token_fixture_t *l_token_fixture = DAP_NEW_Z(test_token_fixture_t);
    if (!l_token_fixture) {
        log_it(L_CRITICAL, "%s", c_error_memory_alloc);
        return NULL;
    }
    dap_strncpy(l_token_fixture->ticker, a_ticker, DAP_CHAIN_TICKER_SIZE_MAX);
    l_token_fixture->token_ticker = dap_strdup(a_ticker);
    l_token_fixture->flags = 0;
    l_token_fixture->owner_cert = a_emission_cert;

    uint256_t l_total_supply_val = dap_chain_balance_scan(a_total_supply_str);
    dap_chain_datum_token_t *l_token = DAP_NEW_Z(dap_chain_datum_token_t);
    if (!l_token) {
        log_it(L_CRITICAL, "%s", c_error_memory_alloc);
        DAP_DELETE(l_token_fixture->token_ticker);
        DAP_DELETE(l_token_fixture);
        return NULL;
    }
    l_token->version = 2;
    l_token->type = DAP_CHAIN_DATUM_TOKEN_TYPE_DECL;
    ...
    l_token->total_supply = l_total_supply_val;
    l_token->header_native_decl.decimals = 18;
    l_token->header_native_decl.flags = 0;
    l_token->header_native_decl.tsd_total_size = 0;
    l_token->signs_total = 0;

    dap_sign_t *l_sign = dap_cert_sign(a_emission_cert, l_token, sizeof(dap_chain_datum_token_t));
    if (!l_sign) {
        log_it(L_ERROR, "Failed to sign token with emission cert");
        DAP_DELETE(l_token);
        DAP_DELETE(l_token_fixture->token_ticker);
        DAP_DELETE(l_token_fixture);
        return NULL;
    ...
    l_token_fixture->token = l_token;
    l_token_fixture->token_size = sizeof(dap_chain_datum_token_t) + l_sign_size;

    int l_res = dap_ledger_token_add(a_ledger, (byte_t *)l_token_fixture->token,
                                      l_token_fixture->token_size, dap_time_now());
    if (l_res != DAP_LEDGER_CHECK_OK) {
        log_it(L_ERROR, "Failed to add token to ledger: %s", dap_ledger_check_error_str(l_res));
        DAP_DELETE(l_token);
        DAP_DELETE(l_token_fixture->token_ticker);
        DAP_DELETE(l_token_fixture);
        return NULL;
...
void test_token_fixture_destroy(test_token_fixture_t *a_fixture)
{
    ...
    if (a_fixture->owner_cert && a_fixture->owns_cert)
