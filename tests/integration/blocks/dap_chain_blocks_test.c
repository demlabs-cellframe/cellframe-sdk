...
typedef struct {
    dap_hash_sha3_256_t block_before_fork_hash;
    dap_list_t *reverted_blocks;
    uint64_t reverted_blocks_cnt;
    uint64_t main_blocks_cnt;
} last_fork_resolved_notification_data_t;

static dap_hash_sha3_256_t s_last_confirmed_block_hash = {};
static dap_hash_sha3_256_t s_last_notified_block_hash = {};
...
typedef struct {
    dap_hash_sha3_256_t *last_notified_hash;
    int *cnt;
} notify_arg_t;
...
dap_hash_sha3_256_t dap_chain_block_test_add_new_block (dap_hash_sha3_256_t *a_prev_block_hash, dap_chain_t *a_chain, dap_chain_block_t **a_block, size_t *a_block_size)
{
    ...
    dap_hash_sha3_256(l_block, l_block_size, &l_block_hash);
...
bool dap_chain_block_test_compare_chain_hash_lists(dap_chain_t* a_chain, dap_list_t* a_atoms_hash_list)
{
    ...
    for (dap_list_t *l_branch_temp = a_atoms_hash_list; l_branch_temp && l_atom;
        ...
        dap_test_msg("Check block %s : num %" DAP_UINT64_FORMAT_U " and %s", dap_hash_sha3_256_to_str_static(l_iter->cur_hash), l_iter->cur_num, l_branch_hash_str);
        if (!dap_hash_sha3_256_compare(l_iter->cur_hash, (dap_hash_sha3_256_t*)l_branch_temp->data)){
...
void dap_chain_blocks_test()
{
    ...
    dap_hash_sha3_256_t l_forked_block_hash = {};
    dap_hash_sha3_256_t l_block_hash = {};
    dap_hash_sha3_256_t l_genesis_block_hash = {};
    ...
    dap_hash_sha3_256_t l_block_repeat_first_forked_hash = {};
    dap_hash_sha3_256_t l_block_double_main_branch_hash = {};
    dap_hash_sha3_256_t l_block_repeat_middle_forked_hash = {};
    dap_hash_sha3_256_t l_block_middle_prev_forked_hash = {};
    ...
    dap_hash_sha3_256_t *l_block_hash_copy = DAP_DUP(&l_block_hash);
    ...
    dap_assert_PIF((s_custom_notify_counter == 2 && dap_hash_sha3_256_compare(&s_last_notified_block_hash, &l_forked_block_hash)), "Check custom notify: ");
    ...
    l_block_hash = dap_chain_block_test_add_new_block ((dap_hash_sha3_256_t*)dap_list_last(l_first_branch_atoms_list)->data, l_chain, NULL, NULL);
    l_block_hash_copy = DAP_DUP(&l_block_hash);
    l_first_branch_atoms_list = dap_list_append(l_first_branch_atoms_list, l_block_hash_copy);
    ...
    dap_hash_sha3_256_t l_last_former_main_branch_hash = l_block_hash;

    // genesis block must be confirmed, check counter and hash of confirmed block
    dap_assert_PIF((s_confirmed_blocks_counter == 1 && dap_hash_sha3_256_compare(&s_last_confirmed_block_hash, &l_genesis_block_hash)), "Check confirmed block: ");
    ...
    dap_hash_sha3_256_t l_third_confirmed_block = l_block_hash;
    l_block_hash_copy = DAP_DUP(&l_block_hash);
    l_third_branch_atoms_list = dap_list_append(l_third_branch_atoms_list, l_block_hash_copy);
    ...
    dap_assert_PIF((s_confirmed_blocks_counter == 2 && dap_hash_sha3_256_compare(&s_last_confirmed_block_hash, &l_forked_block_hash)), "Check confirmed block: ");
    ...
    dap_assert_PIF((s_confirmed_blocks_counter == 3 && dap_hash_sha3_256_compare(&s_last_confirmed_block_hash, &l_third_confirmed_block)), "Check confirmed block: ");
